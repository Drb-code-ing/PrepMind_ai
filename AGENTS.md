...
PrepMind AI 是移动端优先的 Web + PWA 智能备考助手。Phase 7 核心工程化已完成；Phase 7.8.5 RAG runtime parity 已完成真实 Docker 验收。当前 Phase 6.9.7 TutorAgent / WrongQuestionOrganizerAgent 唯一 V3 controlled-Live 已以 `quality_gate_failed` 封存；V4 R0--R4 已完成零 Provider 复盘、bounded diagnostics、Tutor/Organizer 单一语义 policy、独立 robustness 与 crash-safe evidence lineage，下一步仅为 R5 static/Mock checkpoint 与独立终审。Phase 6.9.7 仍未完成；V4 Live、产品 Docker/API/浏览器、Task 13/main 与 Phase 6.10 均不得提前开始。R5 通过后仍须新的精确一次性 V4 controlled-Live 授权。完成 Phase 6.9 全部真实模型 Agent 架构、通信、权限、可执行 LangGraph 与生产验收后，才进入 Phase 6.10 分层记忆补强；随后进入 Phase 8 性能与 PWA、Phase 9 MCP Tool 体系。
...
| Phase 6.9.7 V4 R4           | 已完成     | 独立 held-out/metamorphic/schema-negative robustness、实际 prompt 防泄漏、authority/reorder/abort/预算/写隔离，以及 V4 marker/journal/recovery/evidence lineage 已完成；Live CLI 在 R6 前硬拒绝，下一步仅 R5 static/Mock checkpoint           |
...
3. Phase 6.9.6 的唯一 V2 Live、R7 产品 acceptance、可见 `/knowledge`、精确清理、main default-off 回放与远程推送已经完成。Phase 6.9.7 Task 0--11 已完成；Task 12 V1、V2 R7 与 V3 R5 三条唯一 Live 均已分别以 `quality_gate_failed` 封存且不得重跑。V3 run `ff2e1a54...` 在 `organizer-runtime-14` 命中 `subject_authority_violation` 后熔断，最终 `27/48` strict runtime、Tutor/Organizer semantic `0.5280555556/0.4376201923`。V4 R0 已完成 bounded 复盘：Tutor 已执行 14 例中有 3 个具体 intent 被降级为 `general_follow_up`；Organizer topic/evidence 指标弱，首错仍只能确认是本地 subject-authority contract。当前只继续 R1--R5 零网络/static/Mock 任务；不得修改 V1/V2/V3 history、提前调用 V4 Provider、开始产品验收/Task 13/main 或进入记忆注入/Episodic Memory。
...
6. V3 R5 已再次核对 V1/V2 四个历史 SHA 与专用 validator；V3 marker/journal/evidence 已按 run `ff2e1a54...` durable seal，recovery claim 为 0。禁止删除 marker、重跑 V1/V2/V3 或把部分成功拼接成通过。V4 已冻结独立 runner/prompt/marker/journal/evidence/validator identity，但 R5 static/Mock checkpoint 通过并取得新的 V4 精确授权前，不得执行任何 V4 controlled-Live。
