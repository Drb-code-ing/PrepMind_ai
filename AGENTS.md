...
| Phase 6.9.4.3 | 验收未完成 | JSON-mode 完整 Live 已完成；28/28、72/72 通过但 Router P95 延迟失败，terminal deterministic fallback |
...
| Phase 7.8.5  | 实施完成，待真实 Docker 验收 | RAG runtime parity、Qwen `text-embedding-v4` / 1536、provider-aware fail-closed、queue/hybrid smoke 证据加固 |
...
RAG queue / Docker 验收必须在宿主环境显式设置 `KNOWLEDGE_PROCESSING_MODE=queue`；真实 embedding 使用 `RAG_EMBEDDING_PROVIDER=qwen`、`RAG_EMBEDDING_MODEL=text-embedding-v4`、`RAG_EMBEDDING_DIMENSIONS=1536`、无凭据的 HTTPS `RAG_EMBEDDING_BASE_URL` 与规范 `QWEN_API_KEY`。Compose 静态校验只运行 `docker compose -f docker/docker-compose.dev.yml --profile worker config --quiet`，不输出可能含凭据的完整解析配置。
...
1. Phase 6.9.4.3 JSON-mode resolution 已完成唯一完整 Live：28/28 strict success、72/72 zero-call，但 Router additional P95 4264ms 超门槛；记录 terminal deterministic fallback，不得补跑或拼接历史 evidence。
2. 下一步先提交本次 Live evidence 与终局文档，将当前分支 `--no-ff` 合并 main，在 main 复验并推送；然后从新 main 开 Phase 6.9.5，当前 production 继续 deterministic。
3. 下一会话可直接问：`请收尾 Phase 6.9.4.3 JSON-mode Live evidence 与终局 fallback 文档，合并 main、复验并推送；然后从新 main 开 Phase 6.9.5。`
