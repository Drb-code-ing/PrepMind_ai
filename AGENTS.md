...
| Phase 6.9.4.3 | 验收未完成 | Attempt D 达到 Router 15/16 strict success 后仍 `structured_output`；候选保持关闭 |
...
Phase 6.9.4.3 的同 case paired eval 工程、Mock 验收、共享 provider diagnostics、400-token structured-output headroom 与四次不可拼接 Live 证据均已完成，但阶段验收未完成。Fresh Mock 为 complete：28 条 eligible case 全部 strict success，72 条 ineligible/safety case 保持零 provider 调用；deterministic baseline 仍为 74/100、critical=2。2026-07-14 Attempt D 是当前 canonical `incomplete / structured_output` evidence：从最新 main 通过零调用 preflight 和 250 条 diagnostics/paired 精确测试后，只执行一次 Live；`observed/notRun=52/48`、`providerAttempts/strictSuccesses=16/15`，固定失败 case 为 `router_ambiguous_mixed_chat_16`。15 条成功 usage 为 4446 input / 2185 output，输出范围 59~341，部分可审计成本 USD 0.001297004654663；最后失败 entry usage 不可验证，Router/Verifier 均为 `enabled=false / usage_unverifiable`。400-token 修复显著改善了成功深度，但成功 output 未触及 400，不能继续把根因简单归为 token cap，也不能盲目重跑或自动重试。历史 A/B/C/D 不改写，共享 diagnostics/provenance/sanitizer、USD 0.10、28 calls、72 zero-call、strict schema 与 10 秒 timeout 均保持。candidate 未接生产 Chat，production 继续 deterministic。下一任务先对固定失败 case 做零网络 prompt/schema/provider compatibility 韧性设计；只有独立修复后新整轮达到 28 次 strict success 并通过全部门槛，才能标记阶段完成。证据见 `docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md`。
...
1. Phase 6.9.4.3 后续：Attempt D 已在 400/11,200 合同下达到 Router 15/16 strict success，但固定最后一条 `router_ambiguous_mixed_chat_16` 仍为 `structured_output`；不得直接重跑 Live 或继续盲目提高 cap。
2. 下一任务从最新 main 做零网络 prompt/schema/provider compatibility 韧性设计与 TDD，保留 strict schema、72 zero-call、无自动重试、无 raw output 和成本边界；独立修复后才允许新的 100-case 整轮 Live。当前 production 继续 deterministic。
3. 下一会话可直接问：`请从最新 main 分析 Attempt D 的 router_ambiguous_mixed_chat_16 structured_output；先做零网络 prompt/schema/provider compatibility 设计，不重跑 Live、不自动重试。`
