...
2026-07-21 Phase 6.9.6 设计已通过书面审阅，实施计划已完成。选定方案复用当前用户已经持久化的 Qwen `text-embedding-v4` / 1536 安全 Chunk embedding 形成最多 12 个语义候选，再由 DeepSeek V4 Pro 只裁决本地 ordinal 和受限关系；exact hash 保持 provider 前零调用。本阶段继续只返回建议，不写 Document / Chunk / 分类表，不自动删除、替换、合并、改名或分类。设计固定 72-case paired eval、两个独立 default-off server gate、并行共享预算、Docker/API/可见 `/knowledge` 与精确清理标准。实施计划拆成 13 个一任务一提交的 TDD 单元，Task 12 在静态/Mock 分支验收后必须暂停并重新取得 controlled-Live 授权，Task 13 才执行真实模型、Docker、可见浏览器、精确清理、main 回放与推送；当前已完成 baseline、strict schema/安全投影、Dedup/Organizer 两个 candidate 与本地权威 merger、单一 owner snapshot/双 stale fence、owner-scoped pgvector semantic shortlist、default-off DeepSeek runtime/价格/共享预算 composition、Service/API/Trace 并行编排、`/knowledge` local/hybrid/degraded 只读来源状态，以及 strict Mock paired runner/CLI/evidence validator。真实模型与 Docker/浏览器验收仍未开始。权威设计见 `docs/superpowers/specs/2026-07-21-phase-6-9-6-knowledge-agents-design.md`，执行计划见 `docs/superpowers/plans/2026-07-21-phase-6-9-6-knowledge-agents.md`。
