...
2026-07-20 更正：上段关于“不能称为 Review/Planner 真实模型可用”的历史结论已被 Phase 6.9.5 完整验收更新。当前 default-off 时产品确实返回确定性建议，但受控 DeepSeek V4 Pro API 与可见 `/plan` 已证明真实模型 candidate 可用；main replay 进一步证明 default-off 回滚，本地只读权限和事实权威未变。
...
下一会话可以问：“请开始 Phase 6.9.6，为 KnowledgeDedupAgent / KnowledgeOrganizerAgent 制定真实模型、只读权限、评测与验收方案。”或“为什么 Review/Planner 已通过真实模型 API/浏览器验收却仍保持 default-off？”
