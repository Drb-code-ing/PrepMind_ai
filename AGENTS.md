...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选；v1--v6 都是独立终态；V7 Task 1--7 已完成离线工程、success seal 与两轮复审，尚未运行或授权 controlled-Live |
...
2026-07-17 的 V7 usage recovery 已完成 Task 1--7 离线工程。V6 将 `estimatedInputTokens=96` 错当 provider actual usage 上限的问题由 `97/4` fixture 修复；raw response audit 只保留冻结的 `missing/invalid/positive` usage shape，不保留 token 数值。V7 使用独立 profile/schema/marker/evidence/CLI，one-shot CLI 只接受精确 confirmation，V1--V6 immutable snapshot 为 `treeHash=9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba` / `18 entries`。production parity 只公开六字段 sanitized identity；V7 eval gate 不进入 Docker、Web、worker 或 server config allowlist。直接 Mock 与 strict-fake 的 48/26/22/48/48/0 仅为离线证据。Task 7 增加私有 `success_candidate` 与 exclusive、hash/history-bound success seal，standalone/mismatched candidate 只能被公开 reader 投影为不含 token/cost 的 `evidence_io`。focused V7 为 AI 190、Server 86、native evidence 15/130 assertions；全量离线门为 AI 190、Agent 406、Server 980 passed/30 skipped、Web 409。两轮最终复审均 PASS，Critical/Important/Minor 均为 0。当前没有真实 V7 marker/evidence，controlled-Live 未运行且未授权，两个业务 gate 继续默认 `false`；下一步只能是新的单独 V7 Live 授权。
...
下一会话可以问：“我明确授权执行唯一一次 Phase 6.9.5 V7 controlled-Live；不启用产品 gate，严格按 success seal、费用和历史不可变边界收口。”
...
2. 当前 `codex/phase-6-9-5-review-planner-live-diagnostics` 分支的 V6 已以 `usage_unverifiable` 终态关闭且不可重跑；V7 Task 1--7 已完成离线实现、composition parity、success seal、全量门禁、两轮复审与文档同步。下一步只能在用户新的单独授权后执行唯一 V7 Live；业务 gate 默认关闭。
...
6. 下一会话可直接问：`我明确授权执行唯一一次 Phase 6.9.5 V7 controlled-Live；不启用产品 gate，完成后按 success seal 和授权边界停止。`
