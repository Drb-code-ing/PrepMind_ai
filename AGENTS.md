...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选与诊断工程已完成；DeepSeek V4 Pro v5 CLI/Mock 已离线就绪但尚未 Live；隔离的 v1/v2/v3/v4 controlled-Live 均为 `invalid_attempted / structured_output`，各 1 次 provider 尝试、usage 不可验证、gate 关闭且不得重跑；v3/v4 仅在安全 evidence 中记录 `provider_json_parse` |
...
2026-07-17 的 DeepSeek V4 Pro v5 CLI 已完成离线/Mock 工程收口，但未执行任何真实模型、Docker 或浏览器步骤。它只接受 `--confirm-controlled-live-v5-deepseek-v4-pro`，依序完成 preflight、v1--v4 历史树快照、独立 v5 evidence reserve、历史复核、attempt marker、一个 JSON-object canary，再仅在 `complete + usageKnown + 1 attempt` 时运行冻结的 48-case paired eval。最终只有 `quality_gate_passed`、48 entries/26 verified zero-call/22 runtime、23 total attempts、P95 不超过 4500ms、正数 usage，以及受控 `deepseek-v4-pro-non-cached-cny-v1` 聚合费用不超过 CNY 1 时才输出 `open`；其余状态关闭。CLI stdout 仅含严格白名单摘要，绝不序列化 prompt、candidate、credential、endpoint 或 raw provider 输出。`bun --filter @repo/agent eval:review-planner -- --mode mock --out <new .tmp path>` 已离线得到 48/26/22/48/0 与 `mock_quality_not_evidence`，不构成 Live 结论，也未消费 v5 marker；两条业务 gate 继续默认 `false`。
...
2. 从最新 main 新开 Phase 6.9.5 分支：完成 ReviewAgent / PlannerAgent 真实模型只读建议路径与独立验收，保持 Agent 权限边界、预算、超时、schema 与 deterministic fallback。
...
6. 下一会话可直接问：`请从最新 main 开始 Phase 6.9.5：为 ReviewAgent / PlannerAgent 设计并实现真实模型只读建议路径，明确通信、权限、评测、预算、超时、降级与验收；不要提前进入分层记忆系统。`
