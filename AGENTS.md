...
PrepMind AI 是移动端优先的 Web + PWA 智能备考助手。项目按 Phase 0 ~ Phase 10 推进，当前 Phase 7.23.2 审计证据包 contract、Prisma 持久化模型与 SYSTEM 后台任务边界已完成；可靠投递、ZIP Worker、保留维护、API、Admin UI 和 Docker 运行验收尚未实现，后续继续 Phase 7 后台管理产品化边界、更多后台任务生产化和生产观测增强。
...
| Phase 7.23.3 ~ 7.23.8 | 已计划 | 可靠投递、ZIP Worker、保留维护、下载 API、Admin UI、Docker 验收与面试博客 |
...
- Phase 7 已落地知识库文档处理队列地基、RAG SafetyGuard、事件可观测、Swagger、API/worker 拆分、Worker Observability、Durable Outbox、Outbox Ops、Worker Readiness、Docker 全栈、Operator Audit 和独立 Admin Console；Phase 7.23.2 已完成导出 contract、Prisma 模型、SYSTEM job 与 fail-safe config，但事务型 Outbox 可靠投递、Worker、维护任务、API 和 Admin UI 尚未实现。
- Phase 7.23.2 的为什么 / 怎么做：证据包执行会跨越请求人生命周期，不能复用会随用户级联删除的账号任务；因此数据库 CHECK 强制 ACCOUNT/SYSTEM 所有权，`SET NULL` 保留请求人删除后的导出事实，并让账号 service 每次查询显式限定 ACCOUNT。验证覆盖 strict DTO、配置时序、ACCOUNT cascade、SYSTEM survival 与账号查询隔离。
- 回顾时可以问：“为什么 SYSTEM BackgroundJob 不直接关联 OperatorAuditExport？”、“为什么 ACCOUNT 隔离既要 userId 又要 scope？”、“为什么 production 任一审计数据路径开启时都必须先提供至少 32 字符的 fingerprint secret？”
...
1. Phase 7.23.3：从最新 `main` 独立实现导出申请事务与 Outbox-only 可靠投递，保持 Phase 7.23.2 的 gates 默认关闭；之后按 7.23.4 ~ 7.23.8 完成 Worker、维护、API、Admin UI 和 Docker 验收。
