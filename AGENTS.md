...
| Phase 6.9.4.4 | 进行中 | Router/Verifier 混合生产接入；Task 9 已有 evidence/清理，浏览器 Router timeout、Verifier 未执行，分支验收未通过 |
...
Phase 6.9.4.4 Task 9 当前已有分支 gates、Mock、5/5 controlled-Live harness、Docker 全栈、可见浏览器、失败 fallback、精确清理与 evidence 记录，但分支验收尚未通过。Harness 的 Router/Verifier 5/5 均为 `candidate_applied`；生产浏览器唯一 Router eligible 请求为 `attempted=true / fallback_timeout / 5002ms`，HTTP 200、Mock 最终回答和 Trace 继续，按 stop-on-first-failure 未再发送 Verifier eligible 浏览器请求；RAG conflict/injection 的完整 UI 查询也未形成 hit。Docker 已恢复 `AI_PROVIDER_MODE=mock`、全局 live 关闭、Router/Verifier gate 默认关闭；synthetic user/document/chunk/Trace/job/conversation/outbox 与浏览器 storage 均清理为 0。不得把 harness applied 与 browser timeout 拼接或改写，也不得声称 Browser Verifier Live 已通过。证据见 `docs/acceptance/2026-07-14-phase-6-9-4-4-router-verifier-production.md`。

下一会话可以问：“请继续收口 Phase 6.9.4.4 Task 9 的分支验收缺口：先分析生产浏览器 Router 5002ms timeout 与 RAG UI 0-hit，不重复已有 100-case 或盲目追加 Live；完成 Router/Verifier 可见浏览器验收后再进入 Task 10。不要开始 Phase 6.9.5 或记忆系统。”
...
1. Phase 6.9.4.4 Task 9 当前已有分支完整 gates、Mock、controlled-Live harness、Docker、限制性 fallback、合成数据精确清理和 evidence/current-doc，但生产可见浏览器 Router timeout、Verifier 未执行、RAG UI 0-hit，分支验收未通过；生产 gate 已恢复默认关闭。
2. 当前分支下一步是先分析并收口 Task 9 浏览器验收缺口；通过后才进入 Task 10 的最终 spec/质量复核、完整分支 gates、`--no-ff` 合并 main、main 静态/controlled-Live/Docker/可见浏览器复验、推送远程和 SHA 核对。
...
6. 下一会话可直接问：`请继续收口 Phase 6.9.4.4 Task 9 的分支验收缺口：先分析生产浏览器 Router 5002ms timeout 与 RAG UI 0-hit，不重复已有 100-case 或盲目追加 Live；完成 Router/Verifier 可见浏览器验收后再进入 Task 10。不要开始 Phase 6.9.5 或记忆系统。`
