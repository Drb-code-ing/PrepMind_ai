...
| Phase 6.9.6.1 | 已完成 | 72-case Knowledge 数据集、固定指标、deterministic baseline `12/48`、semantic `0.2322452551`；未实现 candidate 或调用模型 |
| Phase 6.9.6 Task 2 | 已完成 | Dedup/Organizer strict schema、动态关联校验、`knowledge-model-projection-v1`、完整字段先扫描与 ordinal-only 深冻结投影；candidate/runtime 尚未实现 |
...
| Phase 6.9.6 Task 5 | 已完成 | owner-scoped `REPEATABLE READ` + `READ ONLY` 不可变快照、域分离 HMAC owner、完整 fingerprint 与 provider 前 stale fence；尚未接 shortlist/runtime |
| Phase 6.9.6 Task 6 | 已完成 | owner-scoped Qwen pgvector shortlist、每资料 6 个稳定采样 Chunk、top-3 mean、最多 12 pair、provenance/safety/漂移 fail-closed；尚未接 provider/gate |
...
2026-07-21 Phase 6.9.6 设计已通过书面审阅，实施计划已完成。选定方案复用当前用户已经持久化的 Qwen `text-embedding-v4` / 1536 安全 Chunk embedding 形成最多 12 个语义候选，再由 DeepSeek V4 Pro 只裁决本地 ordinal 和受限关系；exact hash 保持 provider 前零调用。本阶段继续只返回建议，不写 Document / Chunk / 分类表，不自动删除、替换、合并、改名或分类。设计固定 72-case paired eval、两个独立 default-off server gate、并行共享预算、Docker/API/可见 `/knowledge` 与精确清理标准。实施计划拆成 13 个一任务一提交的 TDD 单元，Task 12 在静态/Mock 分支验收后必须暂停并重新取得 controlled-Live 授权，Task 13 才执行真实模型、Docker、可见浏览器、精确清理、main 回放与推送；当前已完成 baseline、strict schema/安全投影、Dedup/Organizer 两个 candidate 与本地权威 merger、单一 owner snapshot/stale fence，以及 owner-scoped pgvector semantic shortlist；server provider/gate、paired eval 和真实模型仍未完成。权威设计见 `docs/superpowers/specs/2026-07-21-phase-6-9-6-knowledge-agents-design.md`，执行计划见 `docs/superpowers/plans/2026-07-21-phase-6-9-6-knowledge-agents.md`。

Phase 6.9.6.1 已固定 `phase-6.9-knowledge-agents-v1`：Dedup 40 条、Organizer 32 条，其中 24 条 zero-call contract 与 48 条 runtime quality case 按 `pairedRunIndex=0..23` 配对。未经修饰的 deterministic baseline 为 `12/48` 完整命中、critical `0`、Dedup macro-F1 `0.3343653251`、revision recall `0`、Organizer subject/tag/collection `0.25/0/0.4347826087`、weighted semantic `0.2322452551`，provider/token/cost 均为 0。24 条 zero-call 尚未穿过未实现的 candidate guard，不得写成 24/24 已验证；当前仍未实现模型 candidate、shortlist、gate 或生产接入。证据见 `docs/acceptance/phase-6-9-6-1-knowledge-agent-baseline.md`，下一任务是 strict schema 与 `knowledge-model-projection-v1`。
...
下一会话可以问：“继续按 Phase 6.9.6 计划执行 Task 7：default-off gates、DeepSeek runtime、价格表与不可变共享预算。”或“Task 6 为什么要同时约束 owner、Qwen embedding provenance、安全 metadata、top-3 mean 与 provider 前 fingerprint 重验？”
...
- KnowledgeDedupAgent / KnowledgeOrganizerAgent：`GET /knowledge-agent/suggestions` 经过 `JwtAuthGuard`，按当前 `userId` 读取 `Document` 与每份资料最多少量 `Chunk` 摘要，生成重复资料、疑似新版、互补资料、集合和标签建议；当前产品仍返回 deterministic 在线只读建议。Phase 6.9.6 已完成 Dedup/Organizer 两个受治理 candidate、本地 merger、owner snapshot/stale fence 与 owner-scoped pgvector shortlist，但尚未接入真实 provider 或生产 gate。全阶段仍不写 Document / Chunk / 分类表，不自动合并、删除、替换、重命名或分类资料，不进入 Dexie `mutationQueue`。
