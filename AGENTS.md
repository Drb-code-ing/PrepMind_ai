...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选与诊断工程已完成；v1--v6 都是独立终态，V6 唯一 canary 为 `invalid_attempted / usage_unverifiable`；gate 关闭且不得重跑任何历史 profile |
...
下一会话可以问：“请说明 V6 为什么以 `usage_unverifiable` 终态关闭、为什么不能重跑，以及若要继续需要怎样的新零网络根因设计和独立复审；保持两个业务 gate 为 `false`。”
...
2. 当前 `codex/phase-6-9-5-review-planner-live-diagnostics` 分支的 Phase 6.9.5 V6 已完成离线 resolver/runtime、factory、evidence、CLI 与 Mock/复审；唯一 controlled-Live 已以 `usage_unverifiable` 终态关闭。不得重跑或进入 48-case/Docker/浏览器；继续保持 Agent 权限边界、预算、超时、schema、deterministic fallback 和业务 gate 默认关闭，先做新根因设计与复审。
...
6. 下一会话可直接问：`请说明当前 Phase 6.9.5 V6 的终态证据、为什么不能重跑，以及新的隔离 profile 在什么前提下才可被讨论；不要启动 Docker 或浏览器。`
