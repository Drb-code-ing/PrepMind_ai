...
Phase 6.9.3.1 已完成会话记忆的 strict 公共 contract 与 PostgreSQL/Prisma 地基：公开 `ConversationState` 仅包含 active goal/question 与版本/时间字段，不暴露内部 action proposal、tool names、summary hash 或模型元数据；child 通过 `(conversationId, userId) -> Conversation(id, userId)` 复合外键锁定会话 ownership。摘要校验长度与 `sha256:` source hash，state TTL 约束为 `expiresAt > updatedAt`。本 slice 尚未实现 prepare API、Redis、摘要模型调用或 Chat 注入；下一任务是 Phase 6.9.3.2 state + prepare API。
...
1. Phase 6.9.3.2：实现 ConversationState、Redis 降级缓存与 prepare API；不在 6.9.3.1 contract/database slice 中提前接入。
