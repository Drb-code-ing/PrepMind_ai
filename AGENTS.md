...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选与诊断工程已完成；v1--v6 都是独立终态；V7 已确认 preview/actual usage validator 缺陷并完成 parity 设计，尚未实现或授权 Live |
...
2. 当前 `codex/phase-6-9-5-review-planner-live-diagnostics` 分支的 V6 已以 `usage_unverifiable` 终态关闭且不可重跑；V7 已零网络复现 96-token preview/actual validator 缺陷并完成 parity + safe usage-shape 设计，下一步是用户审阅后编写 TDD 实施计划。不得运行 Live、Docker 或浏览器，业务 gate 默认关闭。
...
6. 下一会话可直接问：`请审阅 Phase 6.9.5 V7 preview/actual usage parity 设计；确认后编写实施计划并逐任务推进，不运行 Live、Docker 或浏览器。`
