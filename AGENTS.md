...
Phase 6.9.3.2 已实现 `POST /conversation-context/prepare` 的 state 路径：服务端先用 `(conversationId, userId)` 验证 ownership，再读取/更新 PostgreSQL 权威 `ConversationState`；只允许客户端 patch `activeGoal` / `activeQuestionId`，显式 `null` 可清空，状态变化或过期恢复才推进 `stateVersion` 与 24 小时 TTL。Redis key 使用 `sha256(userId + NUL + conversationId)`，只缓存 strict public state，读取/解析/写入/删除失败均 fail-open 回源 PostgreSQL。`GET /chat-messages` 与 sync 响应可选返回未过期 sanitized state，清会话后 best-effort 删除缓存。prepare 当前只复用已有 summary，不生成摘要、不调用模型，也尚未注入 `/api/chat`；下一任务是 Phase 6.9.3.3 rolling summary + source hash/CAS。
...
1. Phase 6.9.3.3：实现滚动会话摘要、ModelAgentRuntime composition root、source hash 与并发 CAS；不在 state slice 中提前接入 Chat。
