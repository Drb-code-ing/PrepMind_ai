...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选；v1--v6 都是独立终态；V7 Task 1--6 已完成离线工程与全量门禁，尚未运行或授权 controlled-Live |
...
2026-07-17 的 V7 usage recovery 已完成 Task 1--6 离线工程。V6 将 `estimatedInputTokens=96` 错当 provider actual usage 上限的问题由 `97/4` fixture 修复；raw response audit 只保留冻结的 `missing/invalid/positive` usage shape，不保留 token 数值。V7 使用独立 profile/schema/marker/evidence/CLI，one-shot CLI 只接受精确 confirmation，V1--V6 immutable snapshot 当前为 `treeHash=9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba` / `18 entries`。production parity 只公开六字段 sanitized identity；V7 eval gate 不进入 Docker、Web、worker 或 server config allowlist。直接 Mock report 固定为 `mock_quality_not_evidence`，strict-fake evaluator 回归另标 `mock_quality_not_live_evidence`；两者均为 48/26/22/48/48/0 的离线证据，不是 provider 质量结论。focused V7 为 AI 190、Server 86、native evidence 9/40 assertions；全量离线门为 AI 190、Agent 406、Server 980 passed/30 skipped、Web 409，相关 lint/typecheck/build、Compose `config --quiet` 与 diff check 均通过。当前没有真实 V7 marker/evidence，controlled-Live 未运行且未授权，两个业务 gate 继续默认 `false`。下一步只做 Task 7 两轮最终离线复审，然后停在新的单独 Live 授权边界。
...
下一会话可以问：“请完成 Phase 6.9.5 V7 Task 7 两轮最终离线复审；复审通过后停止，不运行 controlled-Live、Docker 或浏览器，并向我申请新的单独 V7 Live 授权。”
...
2. 当前 `codex/phase-6-9-5-review-planner-live-diagnostics` 分支的 V6 已以 `usage_unverifiable` 终态关闭且不可重跑；V7 Task 1--6 已完成离线实现、composition parity、证据隔离、全量门禁与文档同步。下一步是 Task 7 两轮最终离线复审；不得运行 Live、Docker 或浏览器，业务 gate 默认关闭。
...
6. 下一会话可直接问：`请完成 Phase 6.9.5 V7 Task 7 两轮最终离线复审；通过后停止并申请新的单独 V7 controlled-Live 授权，不运行 Docker 或浏览器。`
