...
3. Phase 6.9.6 的唯一 V2 Live、R7 产品 acceptance、可见 `/knowledge`、精确清理、main default-off 回放与远程推送已经完成。当前在普通分支推进 Phase 6.9.7 Tutor/WrongQuestionOrganizer；Task 0--5 已完成，Tutor 已接入 Web server-only default-off composition，WrongQuestionOrganizer 仍待 owner/write 与 NestJS composition，两条真实 provider 验收均未执行。下一任务是 Task 6 Organizer owner snapshot、双 stale fence 与授权写 command。不得提前进入记忆注入或 Episodic Memory。
...
6. 下一会话可直接问：`请继续 Phase 6.9.7 Task 6：建立 WrongQuestionOrganizer owner-scoped 不可变快照、双 stale fence 与 model-free 授权写 command；provider 不得位于事务内，不调用真实 provider。`
