...
| Phase 6.9.6 | 设计中 | KnowledgeDedup/Organizer 的 Qwen embedding shortlist + DeepSeek V4 Pro 受限语义判断、只读 merger、72-case paired eval 与 default-off 验收设计 |
...
2026-07-21 已进入 Phase 6.9.6 设计检查点。选定方案复用当前用户已经持久化的 Qwen `text-embedding-v4` / 1536 安全 Chunk embedding 形成最多 12 个语义候选，再由 DeepSeek V4 Pro 只裁决本地 ordinal 和受限关系；exact hash 保持 provider 前零调用。本阶段继续只返回建议，不写 Document / Chunk / 分类表，不自动删除、替换、合并、改名或分类。设计固定 72-case paired eval、两个独立 default-off server gate、并行共享预算、Docker/API/可见 `/knowledge` 与精确清理标准；当前尚未实现 candidate、运行评测或调用真实模型。权威设计见 `docs/superpowers/specs/2026-07-21-phase-6-9-6-knowledge-agents-design.md`。
...
下一会话可以问：“请审阅 Phase 6.9.6 Knowledge Agent 设计，重点检查 embedding shortlist、72-case 门槛、只读权限和成本预算。”或“KnowledgeDedupAgent 与 KnowledgeOrganizerAgent 分别做什么，为什么不能自动整理资料？”
