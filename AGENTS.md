...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选；V8 离线工程与产品验收 runner 已完成，待唯一 Live、branch/main 产品验收与推送 |
...
下一会话可以问：“请从 `faa97a8` 的 V8 离线 checkpoint 继续 Phase 6.9.5：只执行唯一 V8 controlled-Live；成功后用 executable runner 完成 branch/main 产品验收、默认关闭恢复、精确清理和推送。不得重跑 V1--V7。”
...
2. 当前 `codex/phase-6-9-5-review-planner-live-diagnostics` 分支的 V6 与 V7 都已终态关闭且不可重跑；V8 离线实现、最终门和双复审已完成。下一步只执行唯一 V8 controlled-Live；成功后用 executable runner 完成 branch/main 产品验收，业务 gate 默认关闭。
...
6. 下一会话可直接问：`请从 faa97a8 的 V8 离线 checkpoint 继续：只执行唯一 V8 controlled-Live；成功后完成 branch/main 产品验收、默认关闭恢复、精确清理与推送。不得重跑 V1--V7。`
