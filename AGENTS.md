...
3. Phase 6.9.6 的唯一 V2 Live、R7 产品 acceptance、可见 `/knowledge`、精确清理、main default-off 回放与远程推送已经完成。当前在普通分支推进 Phase 6.9.7 Tutor/WrongQuestionOrganizer；Task 0--4 已完成，两个 package candidate/merger 均尚未接入产品或真实 provider，下一任务是 Task 5 Tutor Web server-only default-off runtime、Chat 编排与安全 Trace。不得提前进入记忆注入或 Episodic Memory。
...
6. 下一会话可直接问：`请继续 Phase 6.9.7 Task 5：把 Tutor candidate 接入 Web server-only default-off runtime、Chat 编排与安全 Trace；保持独立 credential/预算，不调用真实 provider。`
