...
## Current checkpoint: SR5 v6 live-contract recovery (2026-08-13)

The previous v5 tag was documentation-only: the active Live CLI still bound the historical v2 tag and unversioned SR5 authorization strings. This branch updates the active runtime contract to `main`, the new immutable `phase-6-9-8-retriever-final-response-schema-recovery-sr5-live-v6-approved` tag, and V6-bound data-boundary/authorization strings. No credential or provider call has been made. The v5 tag remains immutable and its authorization is not consumed. After merge, push, merged-main zero-provider replay, and a new v6 tag, a fresh V6 authorization is required before the single controlled-Live.
