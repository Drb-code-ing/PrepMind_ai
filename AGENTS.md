...
PrepMind AI 是移动端优先的 Web + PWA 智能备考助手。项目按 Phase 0 ~ Phase 10 推进，当前 Phase 5.1 已完成，下一步进入 Phase 5.2。
...
- RAG 当前处于 Phase 5.1 地基完成状态：`Document` / `Chunk` 以 PostgreSQL + pgvector 为权威来源，`Chunk.embedding` 固定为 `vector(1536)`；RAG 只增强 Chat 回答，未上传资料、未命中或检索失败时必须降级为普通 AI 回答。
