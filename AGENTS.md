...
Phase 6.9.5 的 V12 已 `recovered`，V13 的 Bun 1.3.14 segmentation fault reservation 和 V14 的 root-absent `default_off` preflight 都是不可重跑、不可恢复、不可改写的历史；V10 controlled-Live 仍是唯一语义质量 authority。V14 的根因是普通 Compose 的安全 `mock/default-off` Chat 使用 `AI_MODEL=deepseek-v4-flash`，而历史 receipt 固定要求 Pro。新 V15 只修复该关闭态 comparison：仅 Flash/Pro 被允许，重复受控 Docker 环境键 fail-closed，其余 live、gate、credential、capability 与 max-request 字段仍严格关闭。V15 有独立 confirmation、ledger/recovery/execution/browser roots、Node runner 和 V11--V14 native sentinel；若 reservation 后、diagnostics 前初始化失败，只有严格证明零资源时才回滚，否则 fail-closed。V15 product/recovery/Docker/browser/API/provider 尚未运行，两个业务 gate 仍为 false。只有 V15 branch `passed` 才可合并 main、执行 main replay 并推送；详细边界见 `docs/superpowers/specs/2026-07-20-phase-6-9-5-v15-product-lineage-design.md`。
...
| Phase 6.9.5 | 验收未完成 | V10 是唯一语义质量 authority；V11/V12 recovered、V13/V14 不可重跑；V15 已完成独立离线安全收口，gate 默认关闭，待唯一 branch product 验收 |
