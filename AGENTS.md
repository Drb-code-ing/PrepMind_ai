...
3. Phase 6.9.6 的唯一 V2 Live、R7 产品 acceptance、可见 `/knowledge`、精确清理、main default-off 回放与远程推送已经完成。当前在普通分支推进 Phase 6.9.7 Tutor/WrongQuestionOrganizer；Task 0--7 已完成，Tutor 已接入 Web server-only default-off composition，WrongQuestionOrganizer 已完成 owner snapshot、三阶段 stale fence、model-free command、server-only default-off runtime、single/batch 单次 dispatch、两阶段 Trace 与 HTTP abort；两条真实 provider 验收均未执行。下一任务是 Task 8 strict API runtime metadata 与 `/error-book` 来源状态。不得提前进入记忆注入或 Episodic Memory。
...
6. 下一会话可直接问：`请继续 Phase 6.9.7 Task 8：为 WrongQuestionOrganizer 增加 strict API runtime metadata 与 /error-book local/hybrid/degraded 来源状态；保持 Task 7 gate 默认关闭，不调用真实 provider。`
