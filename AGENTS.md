...
| Phase 6.9.6 Task 13 | 进行中 | 产品 R1--R6 历史均封存；R6 暴露 Prisma `ntile(bigint)` 导致 shortlist fail-closed，已用 `::integer` TDD 修复并完成真实 PostgreSQL 诊断；等待新提交/镜像产品验收 |
...
2026-07-21 Phase 6.9.6 设计已通过书面审阅，实施计划已完成。选定方案复用当前用户已经持久化的 Qwen `text-embedding-v4` / 1536 安全 Chunk embedding 形成最多 12 个语义候选，再由 DeepSeek V4 Pro 只裁决本地 ordinal 和受限关系；exact hash 保持 provider 前零调用。本阶段继续只返回建议，不写 Document / Chunk / 分类表，不自动删除、替换、合并、改名或分类。设计固定 72-case paired eval、两个独立 default-off server gate、并行共享预算、Docker/API/可见 `/knowledge` 与精确清理标准。实施计划拆成 13 个一任务一提交的 TDD 单元，Task 12 在静态/Mock 分支验收后必须暂停并重新取得 controlled-Live 授权，Task 13 才执行真实模型、Docker、可见浏览器、精确清理、main 回放与推送；当前已完成 baseline、strict schema/安全投影、Dedup/Organizer 两个 candidate 与本地权威 merger、单一 owner snapshot/双 stale fence、owner-scoped pgvector semantic shortlist、default-off DeepSeek runtime/价格/共享预算 composition、Service/API/Trace 并行编排、`/knowledge` local/hybrid/degraded 只读来源状态、strict Mock paired runner/CLI/evidence validator，以及 API-only Docker gate/timeout/独立 `KNOWLEDGE_AGENT_DEEPSEEK_API_KEY` 运维边界。worker/web/admin 不接收 Knowledge credential/gate/timeout，Review/Planner 产品验收也拒绝 Knowledge 能力同时开启。V1 controlled-Live 已执行并因固定语义质量门失败而封存；V2 R4 静态/Mock 已完成，V2 controlled-Live 与产品 Docker/浏览器验收尚未开始。权威设计见 `docs/superpowers/specs/2026-07-21-phase-6-9-6-knowledge-agents-design.md`，执行计划见 `docs/superpowers/plans/2026-07-21-phase-6-9-6-knowledge-agents.md`。
...
下一会话可以问：“Phase 6.9.6 V2 controlled-Live 为什么已经通过，但 R1--R6 产品验收仍不能宣告阶段完成？”或“Prisma 参数化 `ntile` 为什么会在真实 PostgreSQL 上变成 provider 前零调用？”
...
3. 当前先在包含 `ntile(?::integer)` 修复的新提交/镜像上完成 Phase 6.9.6 独立产品 acceptance lineage、可见 `/knowledge`、精确清理、main default-off 回放与 push；随后再按 Phase 6.9.7～6.9.10 完成 Tutor/WrongQuestionOrganizer、Retriever/FinalResponse、MemoryAgent 候选提取和 MCP-ready Orchestrator。不得提前进入记忆注入或 Episodic Memory。
...
6. 下一会话可直接问：`请从 Phase 6.9.6 Task 13 的 ntile PostgreSQL 修复提交继续产品验收；保留 R1--R6 与唯一 V2 Live，不得重跑或改写。`
