...
| Phase 6.9.4.3 | 验收未完成 | Attempt C 定位 `structured_output`；待 output headroom 修复后再跑 Live，候选保持关闭 |
...
Phase 6.9.4.3 的同 case paired eval 工程、Mock 验收、三次失败证据检查点与共享 provider diagnostics contract 已完成，但阶段验收未完成。Fresh Mock 为 complete：28 条 eligible case 全部 strict success，72 条 ineligible/safety case 保持零 provider 调用；deterministic baseline 仍为 74/100、critical=2。Attempt A/B/C 分别在第 3/1/1 次 provider attempt 后 fail-closed 停止；A 仍因 filename identity mismatch 被 strict validator 拒绝，B 为合法的历史 incomplete evidence，2026-07-14 Attempt C 为当前 canonical `incomplete` evidence：`observed/notRun=37/63`、`providerAttempts/strictSuccesses=1/0`，并首次把固定 `PROVIDER_ERROR` 安全分类为 `structured_output`。历史两条 Router strict success 的 output usage 为 `61/120` 与 `108/120`，后者已占上限 90%，支持“当前 structured-output token headroom 不稳定”的高置信假设，但不等同于保存 raw output 或已精确证明截断。共享 `@repo/ai` 的八类 diagnostics、default dependency identity + invocation scope + one-shot provenance、Error/Trace 双边一致、candidate/evidence strict sanitizer 均保持不变；分类不改变 `usage_unverifiable`、不授权自动重试。Router/Verifier 仍为 `enabled=false`，candidate 未接生产 Chat，production 继续 deterministic。下一任务从最新 main 按 TDD 同步 Router/Verifier 400-token 输出上限及全局预算/成本合同，修复后才允许新 controlled-Live；只有 28 次 strict success 与全部质量、安全、延迟、成本门槛通过后才能标记阶段完成。证据见 `docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md`。
...
1. Phase 6.9.4.3 后续：从最新已推送 `main` 开始 structured-output headroom 修复；按 TDD 把 Router/Verifier 单次输出上限统一提高到 400，并同步 per-agent budget、Live ceiling、global output cap、pricing 与 strict evidence contract，不得直接盲目重跑 Live。
2. headroom 修复合并、main 复验并推送后，才允许从新的 main 发起一次完整 controlled-Live；只有达到 28 次 strict success 且质量、安全、延迟、成本门槛全部通过，才能标记 Phase 6.9.4.3 完成并讨论 Router / Verifier enablement。当前 production 继续 deterministic。
3. 下一会话可直接问：`请从最新 main 开始 Phase 6.9.4.3 structured-output headroom 修复；先按 TDD 同步 Router/Verifier 400-token 上限和全局成本合同，不要直接重跑 Live。`
...
5. 回顾时可以问：Phase 6.9.4.3 为什么保留三次 incomplete evidence、`structured_output` 排除了什么、为什么 `108/120` 只支持 headroom 假设而非绝对证明，以及为什么不能拼接不同 run？
