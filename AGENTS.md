...
PrepMind AI 是移动端优先的 Web + PWA 智能备考助手。Phase 7 核心工程化已完成；Phase 7.8.5 RAG runtime parity 已完成真实 Docker 验收。当前 Phase 6.9.7 TutorAgent / WrongQuestionOrganizerAgent 的 V1、V2、V3 与唯一 V4 R6 controlled-Live 均已以 `quality_gate_failed` 独立封存且不得重跑。V4 run `0fb47591...` 保持 `24/24` guard zero-call，在第 6 对 Tutor `invalid_evidence_association` 后熔断，最终 `10/48` strict runtime；同对 Organizer aborted，后续 36 个 runtime 未启动。Phase 6.9.7 仍未完成；R7--R9 产品 Docker/API/浏览器、Task 13/main 与 Phase 6.10 均不得开始。完成 Phase 6.9 全部真实模型 Agent 架构、通信、权限、可执行 LangGraph 与生产验收后，才进入 Phase 6.10 分层记忆补强；随后进入 Phase 8 性能与 PWA、Phase 9 MCP Tool 体系。
...
3. Phase 6.9.6 的唯一 V2 Live、R7 产品 acceptance、可见 `/knowledge`、精确清理、main default-off 回放与远程推送已经完成。Phase 6.9.7 Task 0--11 已完成；V1、V2 R7、V3 R5 与唯一 V4 R6 Live 均已分别以 `quality_gate_failed` 封存且不得重跑。V4 run `0fb47591...` 为 `24/24` zero-call、`10/48` strict runtime；Tutor `tutor-runtime-06` 的 raw schema 通过但命中本地 `dynamic_contract / invalid_evidence_association`，Organizer sibling abort，后续 36 runtime 为 breaker 未执行。当前不得开始 R7--R9 产品验收、Task 13/main 或记忆注入；若继续只能先建立与 V1--V4 双向隔离的新版本零 Provider remediation。
...
6. V4 R6 evidence/journal/marker 已按 run `0fb47591...` durable seal，file/bundle validator 均通过；V1--V3 七个历史 SHA 与三版 validator 保持不变。禁止删除任何 marker、重跑 V1--V4 或把不同版本/部分成功拼接成通过。完整证据见 `docs/acceptance/2026-07-26-phase-6-9-7-tutor-organizer-v4-controlled-live-failure.md`。
