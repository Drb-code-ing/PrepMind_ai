...
Phase 6.9.5 的 V12 已在 `review_api_trace_canonicalize` 因 Trace 总耗时与候选步骤耗时错误关联而 `operation_failed`，其唯一 recovery 已完成，V12 为不可重试、不可改写的 `recovered` 历史。根因已由真实 Trace DTO 回归测试修复。V13 唯一 branch product 随后被 Bun 1.3.14 segmentation fault 中断：只留下 reservation/owner-lock，没有 execution manifest、checkpoint、failure terminal、Docker/API/browser/provider 或合成资源，故不满足 recovery preflight，不能重试或恢复。V10 controlled-Live 仍是唯一语义质量 authority。V14 已完成独立 Node runner、schema、ledger/recovery/execution/browser root、native sentinel 与离线 Docker/default-off 收口；其唯一 branch CLI 随后在 preflight 返回 `default_off`，没有 owner/ledger/Docker mutation/browser/API/provider/合成资源，三类 V14 root 仍不存在，故没有 recovery-admissible terminal。根因是普通 Compose 的 mock/default-off server 保留 Chat 的 `AI_MODEL=deepseek-v4-flash`，而历史 strict receipt 仍要求 `deepseek-v4-pro`。V14 不得重试；必须取得新的用户决定后，才可选择修复 strict default-off contract 并建立新 lineage，或采取其他路线。V14 runner 仍严格 allowlist 两个 entry、scripts/review-agent roots 和 database/agent-diagnostics 两个精确 bridge，V7/V8/V9 evidence 通过窄 diagnostics subpath 避免 Agent barrel，保留模块级 `__dirname`，不生成 bundle。只有未来 branch `passed` 才可合并 main、执行 main replay 并推送。
...
| Phase 6.9.5 | 验收未完成 | V10 是唯一语义质量 authority；V11/V12 recovered、V13 interrupted reservation 均不可重跑；V14 CLI 已 preflight `default_off` 且 root-absent，不可 retry/recovery，待新用户决定 |
