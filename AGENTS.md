...
PrepMind AI 是移动端优先的 Web + PWA 智能备考助手。项目按 Phase 0 ~ Phase 10 推进，当前 Phase 7.12 已完成，后续继续 Phase 7 工程化增强。
...
- Phase 7.12 起 Docker Compose `worker` service 接入容器级 healthcheck，容器内使用构建产物命令 `node dist/scripts/worker-readiness.js`，不依赖本机 Bun workspace。`WORKER_READINESS_CLI_TIMEOUT_MS` 默认 `5000`，healthcheck 默认 `interval=30s`、`timeout=10s`、`retries=3`、`start_period=30s`。本地可用 `docker compose -f docker/docker-compose.dev.yml --profile worker ps` 查看 `healthy / unhealthy`。
