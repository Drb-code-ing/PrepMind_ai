...
3. Phase 6.9.6 的唯一 V2 Live、R7 产品 acceptance、可见 `/knowledge`、精确清理、main default-off 回放与远程推送已经完成。当前在普通分支推进 Phase 6.9.7 Tutor/WrongQuestionOrganizer；Task 0--3 已完成，Tutor package candidate 尚未接入产品，下一任务是 Task 4 WrongQuestionOrganizer candidate 与本地 merger。不得提前进入记忆注入或 Episodic Memory。
...
6. 下一会话可直接问：`请继续 Phase 6.9.7 Task 4：实现 WrongQuestionOrganizer candidate 与本地 merger；保持 owner、真实 ID 和写权限本地权威，不读取密钥或调用真实 provider。`
