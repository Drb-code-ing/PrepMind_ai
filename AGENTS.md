...
| Phase 6.9.4.3 | 验收未完成 | 400-token output headroom 已完成；待新 controlled-Live，候选保持关闭 |
...
Phase 6.9.4.3 的同 case paired eval 工程、Mock 验收、三次失败证据检查点、共享 provider diagnostics 与 structured-output headroom 修复已完成，但阶段验收未完成。Fresh Mock 为 complete：28 条 eligible case 全部 strict success，72 条 ineligible/safety case 保持零 provider 调用；deterministic baseline 仍为 74/100、critical=2。Attempt A/B/C 分别在第 3/1/1 次 provider attempt 后 fail-closed 停止；A 仍因 filename identity mismatch 被 strict validator 拒绝，B 为合法的历史 incomplete evidence，2026-07-14 Attempt C 为当前 canonical `incomplete / structured_output` evidence：`observed/notRun=37/63`、`providerAttempts/strictSuccesses=1/0`。历史两条 Router strict success 的 output usage 为 `61/120` 与 `108/120`，后者占旧上限 90%；基于该证据，Router/Verifier candidate 单次 output 已统一提高到 400，local/provider global output cap 为 11,200，旧定价 worst-case 为 USD 0.017418937304，仍受 USD 0.10、28 calls、72 zero-call、strict schema、10 秒 timeout 与零重试约束。历史 A/B/C 不改写，共享 diagnostics/provenance/sanitizer 不变。Router/Verifier 仍为 `enabled=false`，candidate 未接生产 Chat，production 继续 deterministic。下一任务从修复后最新 main 发起新的 controlled-Live；只有 28 次 strict success 与全部质量、安全、延迟、成本门槛通过后才能标记阶段完成。证据见 `docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md`。
...
1. Phase 6.9.4.3 后续：400-token structured-output headroom 与 11,200 global output cap 已完成 TDD；从合并并推送后的最新 `main` 发起一次完整 controlled-Live，先执行 zero-call preflight、diagnostics 与 pricing 核对。
2. 新 run 必须从 100 条 case 开头串行执行，不重试、不拼接；只有达到 28 次 strict success 且质量、安全、延迟、token、成本门槛全部通过，才能标记 Phase 6.9.4.3 完成并讨论 Router / Verifier enablement。当前 production 继续 deterministic。
3. 下一会话可直接问：`请从最新 main 开始新的 Phase 6.9.4.3 controlled-Live；先核对 400/11,200 headroom、diagnostics、双开关与 pricing，不自动重试。`
...
5. 回顾时可以问：为什么保留三次 incomplete evidence、`structured_output` 排除了什么、400/11,200 如何推导、为什么提高上限没有放宽 strict schema，以及为什么不能拼接不同 run？
