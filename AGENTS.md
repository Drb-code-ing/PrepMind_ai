...
| Phase 6.9.5 | 验收未完成 | Review/Planner 受限只读模型候选；V7 已关闭，V8 零正文 stage-diagnostics 与完整产品验收设计已冻结 |
...
下一会话可以问：“请按已冻结的 V8 设计与实施计划继续 Phase 6.9.5；严格 TDD，不得重跑或改写 V7。”
...
2. 当前 `codex/phase-6-9-5-review-planner-live-diagnostics` 分支的 V6 与 V7 都已终态关闭且不可重跑；V8 stage-diagnostics 与完整产品验收设计已冻结，下一步按 TDD 实施，业务 gate 默认关闭。
...
6. 下一会话可直接问：`请按 V8 stage-diagnostics completion 设计与计划继续实施 Phase 6.9.5，不得重跑或改写 V7。`
