...
- Phase 5.6：`/knowledge` 学习资料工作台，支持上传、列表、处理、删除和检索测试。
...
- `/knowledge` 页面已接入 RAG 文档管理与检索测试：支持资料上传、列表、处理/重新处理、删除内联确认、状态摘要和手动检索预览；该页面在线直连 knowledge API，不进入 Dexie `mutationQueue`。
