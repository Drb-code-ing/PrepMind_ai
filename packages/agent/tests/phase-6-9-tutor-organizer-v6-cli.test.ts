...
import {
  executePhase697TutorOrganizerV6Cli,
  parsePhase697TutorOrganizerV6Cli,
  sealPhase697TutorOrganizerV6Orphan,
} from '../scripts/phase-6-9-7-tutor-wrong-question-v6-cli.ts';
import { readPhase697V6Journal } from '../scripts/phase-6-9-7-tutor-wrong-question-v6-durability.ts';
...
async function temporaryRoot() {
  const root = await mkdtemp(join(tmpdir(), 'prepmind-v6-r4-cli-'));
  roots.push(root);
  return root;
}
...
function authorizedLiveEnv(overrides?: Readonly<Record<string, string | undefined>>) {
  return {
    [PHASE_6_9_7_V6_APPROVAL_ENV]: 'true',
...
describe('Phase 6.9.7 V6 R3 CLI', () => {
  test('requires exact one-time authorization and rejects hostile environment accessors', () => {
    ...
    expect(
      parsePhase697TutorOrganizerV6Cli({
        argv: ['live', PHASE_6_9_7_V6_CONFIRMATION],
        env: { [PHASE_6_9_7_V6_APPROVAL_ENV]: 'true' },
      }),
    ).toEqual({ ok: true, mode: 'live', runScope: 'branch' });
    const hostile = Object.create(null) as Record<string, string | undefined>;
    let reads = 0;
    Object.defineProperty(hostile, PHASE_6_9_7_V6_APPROVAL_ENV, {
      enumerable: true,
      get() {
        reads += 1;
        return 'true';
  ...
  test('publishes isolated Mock evidence without creating a Live marker', async () => {
    ...
    const result = await executePhase697TutorOrganizerV6Cli({
      ...
      harnessFactory: ({ mode, runId, runScope }) =>
        createPhase697V6SyntheticHarness({ mode, runId, runScope }),
    });
  ...
  test('fsyncs marker and journal before invoking a synthetic Live factory', async () => {
    ...
    const result = await executePhase697TutorOrganizerV6Cli({
      ...
      harnessFactory: async ({ mode, runId, runScope }) => {
        markerExistedAtFactory = await Bun.file(resolve(root, PHASE_6_9_7_V6_MARKER_PATH)).exists();
        return createPhase697V6SyntheticHarness({ mode, runId, runScope });
      },
    });
    ...
    const replay = await executePhase697TutorOrganizerV6Cli({
      argv: ['live', PHASE_6_9_7_V6_CONFIRMATION],
      env: authorizedLiveEnv(),
      repositoryRoot: root,
      harnessFactory: ({ mode, runId, runScope }) =>
  ...
  test('fails closed when an injected Live harness disagrees with marker provenance', async () => {
    ...
    expect(await Bun.file(resolve(root, PHASE_6_9_7_V6_MARKER_PATH)).exists()).toBe(true);
  });
  ...
  test('rejects evidence whose report provenance drifts from the durable marker', async () => {
    ...
    envelope.reportSha256 = sha256Phase697V6Stable(envelope.report);
  ...
  test('consumes the marker when the injected factory crashes and seals only after owner death', async () => {
    ...
    expect(await sealPhase697TutorOrganizerV6Orphan({ root, processAlive: () => true })).toEqual({
      ok: false,
      code: 'live_attempt_in_progress',
    });
    const sealed = await sealPhase697TutorOrganizerV6Orphan({
      root,
      processAlive: () => false,
    });
    ...
    const secondSeal = await sealPhase697TutorOrganizerV6Orphan({
      root,
      processAlive: () => false,
    });
  ...
  test('keeps the one-shot marker after evidence publication failure and recovers without replay', async () => {
    ...
    const result = await executePhase697TutorOrganizerV6Cli({
      ...
      harnessFactory: ({ mode, runId: harnessRunId, runScope }) =>
        createPhase697V6SyntheticHarness({ mode, runId: harnessRunId, runScope }),
      durabilityOverrides: {
        async link() {
    ...
    expect(
      await executePhase697TutorOrganizerV6Cli({
        argv: ['live', PHASE_6_9_7_V6_CONFIRMATION],
        env: authorizedLiveEnv(),
        repositoryRoot: root,
        harnessFactory: ({ mode, runId: harnessRunId, runScope }) =>
    ...
    expect(
      await sealPhase697TutorOrganizerV6Orphan({ root, processAlive: () => false }),
  ...
  test('rejects journal-tail drift after recovery claim acquisition', async () => {
    ...
    const failed = await executePhase697TutorOrganizerV6Cli({
      argv: ['live', PHASE_6_9_7_V6_CONFIRMATION],
      env: authorizedLiveEnv(),
      repositoryRoot: root,
      runId,
    ...
    const guardProjection = projectPhase697V6TerminalEntry(guardTerminal);
    if (!guardProjection) throw new Error('guard projection unavailable');
    const drifted = await sealPhase697TutorOrganizerV6Orphan({
      root,
      processAlive: () => false,
