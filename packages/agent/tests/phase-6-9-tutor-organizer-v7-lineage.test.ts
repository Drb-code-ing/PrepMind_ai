...
import { assertPhase697V7PathIdentity } from '../src/evals/phase-6-9-tutor-wrong-question-v7-durability-contract.ts';
...
import {
  hasPriorPhase697V7ArtifactLineage,
  validatePhase697TutorOrganizerV7EvidenceValue,
} from '../scripts/validate-phase-6-9-7-tutor-wrong-question-v7-evidence.ts';
import { createPhase697V7SyntheticHarness } from './fixtures/phase-6-9-tutor-organizer-v7-runner.ts';

describe('Phase 6.9.7 V7 R2 lineage isolation', () => {
  test('keeps the frozen V2 dataset and V6 semantic authority hashes unchanged', () => {
    ...
    expect(PHASE_6_9_7_V7_SOURCE_MANIFEST_SHA256).toMatch(/^sha256:[0-9a-f]{64}$/);
    assertPhase697V7PathIdentity();
  });

  test('rejects every V1-V6 artifact namespace while permitting frozen semantic-source identities', async () => {
    const report = await runPhase697TutorOrganizerPairedEvalV7(
      createPhase697V7SyntheticHarness({ runId: '00000000-0000-4000-8000-000000000741' }),
    );
    ...
    if (!envelope) throw new Error('V7 envelope unavailable');
    expect(validatePhase697TutorOrganizerV7EvidenceValue(envelope)).toEqual({ ok: true });
    expect(hasPriorPhase697V7ArtifactLineage(envelope)).toBe(false);
    ...
    for (let version = 1; version <= 6; version += 1) {
      ...
      expect(validatePhase697TutorOrganizerV7EvidenceValue(legacy)).toEqual({
        ok: false,
      ...
      for (const token of artifactTokens) {
        expect(hasPriorPhase697V7ArtifactLineage({ nested: [{ token }] }), token).toBe(true);
      }
      ...
      expect(validatePhase697TutorOrganizerV7EvidenceValue(nestedLegacy)).toEqual({
        ok: false,
        code: 'prior_lineage_detected',
      });
    }
  ...
  test('is rejected by all V1-V6 validators and keeps the new fixed counter allowlist narrow', async () => {
    const report = await runPhase697TutorOrganizerPairedEvalV7(
      createPhase697V7SyntheticHarness({ runId: '00000000-0000-4000-8000-000000000742' }),
    );
    const envelope = buildPhase697V7EvidenceEnvelope({
      report,
      disposition: 'mock_direct',
