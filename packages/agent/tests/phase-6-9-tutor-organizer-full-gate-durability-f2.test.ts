...
describe('Phase 6.9.7 full-gate F2 durability', () => {
  test('persists a canonical complete journal and recomputes the published bundle', async () => {
    const root = await temporaryRoot();
    const { report, published } = await completedRun(root, F2_RUN_ID, true);
    if (!published) throw new Error('F2 artifact was not published');
    ...
    const marker = await readFile(
      resolve(root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH),
      'utf8',
    );
    const journal = await readFile(resolve(root, journalRelativePath(F2_RUN_ID)), 'utf8');
  ...
  test('allows only one concurrent reservation of the fixed marker', async () => {
    ...
    const results = await Promise.allSettled([
      reserveF2SyntheticAttempt(root, randomUUID()),
      reserveF2SyntheticAttempt(root, randomUUID()),
    ]);
    ...
    expect(
      files.filter((file) => file === PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH.split('/').at(-1)),
    ).toHaveLength(1);
  ...
  test('rejects truncated, CRLF, hash-tampered, and extra formal bundle files', async () => {
    const root = await temporaryRoot();
    await completedRun(root, F2_RUN_ID, true);
    const journalPath = resolve(root, journalRelativePath(F2_RUN_ID));
    const original = await readFile(journalPath, 'utf8');

    await writeFile(journalPath, original.trimEnd(), 'utf8');
    expect((await validatePhase697FullGateBundle({ root })).ok).toBe(false);
    ...
    await writeFile(
      join(
        root,
        '.tmp',
        `phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live-${randomUUID()}.journal.jsonl`,
      ),
      '{}\n',
      'utf8',
    );
  ...
  test('crash-seals reserved lanes, conservatively closes the denominator, and validates recovery', async () => {
    ...
    const runtime = PHASE_6_9_7_FULL_GATE_EXPECTED_ENTRIES.filter(
    ...
    const result = await sealPhase697FullGateInterruptedAttemptForTest({
      root,
      processAlive: () => false,
    });
    ...
    const validation = await validatePhase697FullGateBundle({ root });
    expect(validation).toMatchObject({ ok: true, runId: F2_RUN_ID, qualityAuthority: 'none' });
    const artifact = await readArtifact(root, F2_RUN_ID);
  ...
  test('refuses to seal a live owner and creates neither claim nor artifact', async () => {
    const root = await temporaryRoot();
    await reserveF2SyntheticAttempt(root);
    ...
    expect(await exists(resolve(root, recoveryClaimRelativePath(F2_RUN_ID)))).toBe(false);
    expect(await exists(resolve(root, artifactRelativePath('branch', F2_RUN_ID)))).toBe(false);
  });

  test('recovers publication after a durable runtime terminal without rewriting the report', async () => {
    const root = await temporaryRoot();
    const { report } = await completedRun(root, F2_RUN_ID, false);
  ...
  test('makes publication_started permanently fail-closed after a hard-link conflict', async () => {
    const root = await temporaryRoot();
    const { reservation, report } = await completedRun(root, F2_RUN_ID, false);
    const artifactPath = resolve(root, artifactRelativePath('branch', F2_RUN_ID));
    await mkdir(artifactPath);

    await expect(reservation.publishArtifact(report)).rejects.toThrow();
    expect(
      await sealPhase697FullGateInterruptedAttemptForTest({
        root,
        processAlive: () => false,
      }),
  ...
  test('elects one crash-seal winner across recovery claim takeover races', async () => {
    ...
    if (!firstTutor) throw new Error('F2 first Tutor lane missing');
    await reservation.lifecycle.reserveLane(runtimeIdentity(firstTutor));
    await rewriteMarkerOwner(root, F2_RUN_ID, 999_972);
    ...
    const records = journalRecords(
      await readFile(resolve(root, journalRelativePath(F2_RUN_ID)), 'utf8'),
    );
  ...
  test('rejects a fully rehashed duplicate recovery claim and hidden guard completion mode', async () => {
    ...
    const reservation = await reserveF2SyntheticAttempt(duplicateRoot);
    await appendPassingGuards(reservation.lifecycle);
    await sealPhase697FullGateInterruptedAttemptForTest({
      root: duplicateRoot,
      processAlive: () => false,
    });
    const duplicateJournalPath = resolve(duplicateRoot, journalRelativePath(F2_RUN_ID));
    ...
    if (recoveryIndex < 0) throw new Error('F2 recovery record missing');
    ...
    expect((await validatePhase697FullGateBundle({ root: duplicateRoot })).ok).toBe(false);

    const completionRoot = await temporaryRoot();
    await completedRun(completionRoot, F2_RUN_ID, true);
    const completionJournalPath = resolve(completionRoot, journalRelativePath(F2_RUN_ID));
    ...
    if (!guard) throw new Error('F2 guard record missing');
    ...
    expect((await validatePhase697FullGateBundle({ root: completionRoot })).ok).toBe(false);
  });

  test('refuses a pre-existing non-file marker without replacing it', async () => {
    const root = await temporaryRoot();
    const markerPath = resolve(root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH);
    await mkdir(join(root, '.tmp'));
    await mkdir(markerPath);

    await expect(reserveF2SyntheticAttempt(root)).rejects.toThrow();
    expect(
...
async function completedRun(root: string, runId: string, publish: boolean) {
  const reservation = await reserveF2SyntheticAttempt(root, runId);
...
async function appendPassingGuards(
  lifecycle: Awaited<ReturnType<typeof reserveF2SyntheticAttempt>>['lifecycle'],
) {
...
async function temporaryRoot() {
  const root = await mkdtemp(join(tmpdir(), 'prepmind-full-gate-f2-'));
  roots.push(root);
  return root;
}
...
function assertHashChain(records: readonly Record<string, unknown>[]) {
  ...
  for (const [index, record] of records.entries()) {
    ...
    expect(recordHash).toBe(computePhase697FullGateCanonicalSha256(base));
