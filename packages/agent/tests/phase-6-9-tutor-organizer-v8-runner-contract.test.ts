...
describe('Phase 6.9.7 V8 R3 runner and report contract', () => {
  test('keeps fixed denominators and derives four independent wire counters', async () => {
    const recordedStages: string[] = [];
    const report = await runPhase697TutorOrganizerPairedEvalV8(createPhase697V8SyntheticHarness(), {
      lifecycle: {
    ...
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(report).success).toBe(true);
    ...
    expect(PHASE_6_9_7_V8_EVAL_POLICY.wire).toMatchObject({
      executorInvocations: 48,
      providerDispatches: 48,
      providerResponses: 48,
      verifiedUsages: 48,
  ...
  test('fails before the delegate when durable dispatch evidence cannot be appended', async () => {
    let delegateCalls = 0;
    const lifecycle: Phase697V8RunnerLifecycle = {
  ...
  test('turns a semantic success without wire completion into a fail-closed breaker terminal', async () => {
    ...
    const harness: Phase697V8Harness = Object.freeze({
      runId: RUN_ID,
      runScope: 'branch',
      mode: 'mock',
      provider: 'mock',
    ...
    const report = await runPhase697TutorOrganizerPairedEvalV8(harness);
    expect(report.scheduler).toMatchObject({
      breakerState: 'quality_gate_impossible',
      dispatchedPairs: 1,
      completedPairs: 1,
  ...
  test('carries a bounded Organizer schema diagnostic into the breaker report and rejects drift', async () => {
    ...
    if (!organizer?.boundedSchemaDiagnostic) {
      throw new Error('V8 bounded Organizer diagnostic unavailable');
    }
    ...
    if (!missingEntry) throw new Error('V8 diagnostic entry unavailable');
    missingEntry.boundedSchemaDiagnostic = null;
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(missingDiagnostic).success).toBe(
      false,
    );
    ...
    for (const { label, mutate } of tamperCases) {
      ...
      if (!diagnostic) throw new Error('V8 tamper diagnostic unavailable');
      mutate(diagnostic);
      expect(validatePhase697TutorOrganizerV8EvidenceValue(tampered), label).toEqual({
        ok: false,
  ...
  test('recomputes V8 identities, wire aggregates, entry linkage, and synthetic Live gate', async () => {
    const report = await runPhase697TutorOrganizerPairedEvalV8(createPhase697V8SyntheticHarness());
    ...
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(wireTamper).success).toBe(false);

    const identityTamper = clone();
    (identityTamper.identities as Record<string, unknown>).semanticAuthoritySha256 =
      `sha256:${'0'.repeat(64)}`;
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(identityTamper).success).toBe(
      false,
    );
    ...
    if (!firstRuntime) throw new Error('V8 runtime entry unavailable');
    firstRuntime.wireEvidence = { disposition: 'missing_after_attempt', snapshot: null };
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(entryTamper).success).toBe(false);

    const liveReport = await runPhase697TutorOrganizerPairedEvalV8(
      createPhase697V8SyntheticHarness({ mode: 'live' }),
    );
    ...
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(liveGateTamper).success).toBe(
      false,
    );
  });
  ...
  test('rejects every derived wire counter and formal aggregate when report bytes drift', async () => {
    ...
    for (const { label, mutate } of tamperCases) {
      ...
      mutate(tampered);
      expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(tampered).success, label).toBe(
        false,
      );
    }
    ...
    if (!runtimeEntry) throw new Error('V8 runtime entry unavailable');
    ...
    expect(
      PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(runtimeCounterTamper).success,
    ).toBe(false);
  });

  test('keeps V6 and V8 report/artifact identities mutually isolated', async () => {
    const v6Report = await runPhase697TutorOrganizerPairedEvalV6(
      createPhase697V6SyntheticHarness({ runId: RUN_ID }),
    );
    const v8Report = await runPhase697TutorOrganizerPairedEvalV8(
      createPhase697V8SyntheticHarness(),
    );
    expect(parsePhase697TutorOrganizerV8Report(v6Report)).toBeNull();
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V6_REPORT_SCHEMA.safeParse(v8Report).success).toBe(false);
    expect(JSON.stringify(v8Report)).not.toContain('phase-6.9.7-tutor-organizer-runner-v6');
    expect(JSON.stringify(v8Report)).not.toContain('phase-6.9.7-v6-runtime-evidence');
    ...
    expect(PHASE_6_9_7_TUTOR_ORGANIZER_V8_REPORT_SCHEMA.safeParse(oldRunner).success).toBe(false);
  });
});
