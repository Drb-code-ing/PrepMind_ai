...
import {
  PHASE_6_9_7_TUTOR_ORGANIZER_RUNNER_VERSION_V7,
  PHASE_6_9_7_V7_CASE_ENTRY_SCHEMA,
  PHASE_6_9_7_V7_EVAL_POLICY_SHA256,
  PHASE_6_9_7_V7_EVIDENCE_ENVELOPE_SCHEMA,
  PHASE_6_9_7_V7_EVIDENCE_PREFIX,
  PHASE_6_9_7_V7_JOURNAL_VERSION,
  PHASE_6_9_7_V7_MARKER_PATH,
  PHASE_6_9_7_V7_MARKER_SCHEMA,
  PHASE_6_9_7_V7_MARKER_VERSION,
  PHASE_6_9_7_V7_RECOVERY_CLAIM_SCHEMA,
  PHASE_6_9_7_V7_RECOVERY_CLAIM_VERSION,
  PHASE_6_9_7_V7_RUNTIME_EVIDENCE_VERSION,
  PHASE_6_9_7_V7_SOURCE_MANIFEST_SHA256,
  buildPhase697TutorOrganizerV7Report,
  buildPhase697V7CaseEntry,
  phase697V7DispatchKeySha256,
  phase697V7EvidencePath as phase697V7EvidencePathNullable,
  phase697V7JournalPath as phase697V7JournalPathNullable,
  phase697V7RecoveryClaimPath as phase697V7RecoveryClaimPathNullable,
  projectPhase697V7EntryToV6,
  sha256Phase697V7Stable,
  type Phase697TutorOrganizerV7Report,
  type Phase697V7CaseEntry,
  type Phase697V7EvidenceEnvelope,
  type Phase697V7Marker,
  type Phase697V7RecoveryClaimRecord,
...
export type { Phase697V7EvidenceEnvelope, Phase697V7Marker, Phase697V7RecoveryClaimRecord };
...
export const PHASE_6_9_7_V7_TERMINAL_PROJECTION_SCHEMA = z
  .object({
    runtimeEvidenceVersion: z.literal(PHASE_6_9_7_V7_RUNTIME_EVIDENCE_VERSION),
    entry: PHASE_6_9_7_V7_CASE_ENTRY_SCHEMA,
    terminalEntrySha256: sha256Schema,
  })
  .strict();

export type Phase697V7TerminalProjection = z.infer<
  typeof PHASE_6_9_7_V7_TERMINAL_PROJECTION_SCHEMA
>;

export const PHASE_6_9_7_V7_JOURNAL_PAYLOAD_SCHEMA = z.discriminatedUnion('kind', [
  z
    .object({
  ...
  z
    .object({
      ...
      terminal: PHASE_6_9_7_V7_TERMINAL_PROJECTION_SCHEMA,
    })
    .strict(),
  z
    .object({
      ...
      wireCapabilityVersion: z.literal(PHASE_6_9_7_V7_WIRE_CAPABILITY_VERSION),
    })
    .strict(),
  z
    .object({
      ...
      dispatchKeySha256: sha256Schema,
      stage: z.enum(PHASE_6_9_7_V7_WIRE_STAGES),
    })
    .strict(),
  z
    .object({
...
export type Phase697V7JournalPayload = z.infer<typeof PHASE_6_9_7_V7_JOURNAL_PAYLOAD_SCHEMA>;

export const PHASE_6_9_7_V7_JOURNAL_RECORD_SCHEMA = z
  .object({
    journalVersion: z.literal(PHASE_6_9_7_V7_JOURNAL_VERSION),
    ...
    payload: PHASE_6_9_7_V7_JOURNAL_PAYLOAD_SCHEMA,
    recordSha256: sha256Schema,
  })
  .strict();

export type Phase697V7JournalRecord = z.infer<typeof PHASE_6_9_7_V7_JOURNAL_RECORD_SCHEMA>;
...
export type Phase697V7ValidatedJournal = Readonly<{
  records: readonly Phase697V7JournalRecord[];
  runId: string;
  runScope: 'branch' | 'main';
  markerSha256: string;
  tailSha256: string;
  lastSequence: number;
  guardTerminals: ReadonlyMap<string, Phase697V7CaseEntry>;
  lanes: ReadonlyMap<string, ValidatedLane>;
  runtimeTerminals: ReadonlyMap<string, Phase697V7CaseEntry>;
  pairedDurations: ReadonlyMap<
    number,
    Extract<Phase697V7JournalPayload, { kind: 'pair_terminal' }>['pairedDurationEvidence']
  >;
  breaker: Extract<Phase697V7JournalPayload, { kind: 'breaker_opened' }> | null;
  runCompleted: Readonly<{
    reportSha256: string;
    gate: 'quality_gate_passed' | 'quality_gate_failed';
  }> | null;
  sealed: Extract<Phase697V7JournalPayload, { kind: 'evidence_sealed' }> | null;
}>;

export function stablePhase697V7JsonStringify(value: unknown): string {
  return JSON.stringify(sortStableValue(value));
}

export function sha256Phase697V7Bytes(value: string | Uint8Array): `sha256:${string}` {
  return `sha256:${createHash('sha256').update(value).digest('hex')}`;
}

export function phase697V7JournalPath(runId: string): string {
  const path = phase697V7JournalPathNullable(runId);
  if (path === null) throw new Error('PHASE_6_9_7_V7_RUN_ID_INVALID');
  return path;
}

export function phase697V7RecoveryClaimPath(runId: string): string {
  const path = phase697V7RecoveryClaimPathNullable(runId);
  if (path === null) throw new Error('PHASE_6_9_7_V7_RUN_ID_INVALID');
  return path;
}
...
export function phase697V7EvidencePath(input: {
  ...
  const path = phase697V7EvidencePathNullable(input);
  if (path === null) throw new Error('PHASE_6_9_7_V7_RUN_ID_INVALID');
  return path;
}

export function projectPhase697V7TerminalEntry(
  entry: Readonly<Phase697V7CaseEntry>,
): Readonly<Phase697V7TerminalProjection> | null {
  const parsed = PHASE_6_9_7_V7_TERMINAL_PROJECTION_SCHEMA.safeParse({
    runtimeEvidenceVersion: PHASE_6_9_7_V7_RUNTIME_EVIDENCE_VERSION,
    entry,
    terminalEntrySha256: sha256Phase697V7Stable(entry),
  });
  return parsed.success ? Object.freeze(parsed.data) : null;
}

export function restorePhase697V7TerminalEntry(
  projection: unknown,
): Readonly<Phase697V7CaseEntry> | null {
  const parsed = PHASE_6_9_7_V7_TERMINAL_PROJECTION_SCHEMA.safeParse(projection);
  if (!parsed.success) return null;
  return sha256Phase697V7Stable(parsed.data.entry) === parsed.data.terminalEntrySha256
    ? Object.freeze(parsed.data.entry)
    : null;
}

export function buildPhase697V7JournalRecord(input: {
  runId: string;
  sequence: number;
  previousRecordSha256: string | null;
  payload: Phase697V7JournalPayload;
}): Phase697V7JournalRecord {
  const withoutHash = {
    journalVersion: PHASE_6_9_7_V7_JOURNAL_VERSION,
  ...
  return PHASE_6_9_7_V7_JOURNAL_RECORD_SCHEMA.parse({
    ...withoutHash,
    recordSha256: sha256Phase697V7Stable(withoutHash),
  });
}

export function parseAndValidatePhase697V7Journal(text: string): Phase697V7ValidatedJournal | null {
  ...
  const records: Phase697V7JournalRecord[] = [];
  try {
    for (const line of lines) {
      const parsed = PHASE_6_9_7_V7_JOURNAL_RECORD_SCHEMA.parse(JSON.parse(line) as unknown);
      const { recordSha256, ...withoutHash } = parsed;
      if (sha256Phase697V7Stable(withoutHash) !== recordSha256) return null;
      records.push(parsed);
    }
  } catch {
    return null;
  ...
  const guards = new Map<string, Phase697V7CaseEntry>();
  ...
  >();
  const terminals = new Map<string, Phase697V7CaseEntry>();
  const pairDurations = new Map<
    number,
    Extract<Phase697V7JournalPayload, { kind: 'pair_terminal' }>['pairedDurationEvidence']
  >();
  let breaker: Extract<Phase697V7JournalPayload, { kind: 'breaker_opened' }> | null = null;
  let runCompleted: Phase697V7ValidatedJournal['runCompleted'] = null;
  let sealed: Phase697V7ValidatedJournal['sealed'] = null;
  ...
  for (const [index, record] of records.entries()) {
    ...
    if (payload.kind === 'guard_terminal') {
      const terminal = restorePhase697V7TerminalEntry(payload.terminal);
      if (
        mutableLanes.size > 0 ||
        breaker !== null ||
        runCompleted !== null ||
    ...
    if (payload.kind === 'lane_reserved') {
      const expectedKey = phase697V7DispatchKeySha256({
        runId,
        agent: payload.agent,
    ...
    if (payload.kind === 'wire_stage') {
      ...
      if (
        ...
        payload.stage !== PHASE_6_9_7_V7_WIRE_STAGES[lane.stages.length]
      ) {
        return null;
      }
    ...
    if (payload.kind === 'breaker_opened') {
      ...
      if (payload.breakerState === 'guard_failed') {
        ...
        if (
          [...guards.values()].some((entry) => !entry.zeroCallVerified) ||
          !trigger ||
          runtimeContractSuccessV7(trigger) ||
          trigger.agent !== payload.triggerAgent ||
    ...
    if (payload.kind === 'run_completed') {
      ...
      const runtimeFailure = [...terminals.values()].some(
        (entry) => !runtimeContractSuccessV7(entry),
      );
      if (
        runCompleted !== null ||
        guards.size !== 24 ||
...
export function buildPhase697V7SealedReport(input: {
  marker: Readonly<Phase697V7Marker>;
  markerSha256: string;
  journal: Readonly<Phase697V7ValidatedJournal> | null;
  ...
  const expectedTerminals = new Map<string, Phase697V7CaseEntry>();
  for (const testCase of PHASE_6_9_TUTOR_WRONG_QUESTION_V2_CASES) {
    let v7Entry: Phase697V7CaseEntry;
    if (testCase.expectedRuntimeInvocations === 0) {
      v7Entry =
        input.journal?.guardTerminals.get(testCase.id) ??
        buildPhase697V7CaseEntry(buildMissingGuardEntry(testCase), null);
    } else {
      const dispatchKey = phase697V7DispatchKeySha256({
      ...
      if (terminal) {
        ...
        if (lane) {
          ...
          v7Entry = buildPhase697V7CaseEntry(
      ...
      if (v7Entry.wireEvidence.snapshot !== null) {
        wireSnapshots.set(testCase.id, v7Entry.wireEvidence.snapshot);
      }
    }
    const projected = projectPhase697V7EntryToV6(v7Entry);
  ...
  try {
    ...
    for (const [caseId, terminal] of expectedTerminals) {
      ...
      if (!rebuilt || sha256Phase697V7Stable(rebuilt) !== sha256Phase697V7Stable(terminal)) {
        return null;
      }
    }
    if (
      completed !== null &&
      (completed.reportSha256 !== sha256Phase697V7Stable(report) || completed.gate !== report.gate)
    ) {
      return null;
    }
    return report;
...
export function assertPhase697V7PathIdentity(): void {
  if (
    !PHASE_6_9_7_V7_MARKER_PATH.includes('-v7-') ||
    !PHASE_6_9_7_V7_EVIDENCE_PREFIX.endsWith('-v7') ||
    PHASE_6_9_7_V7_RECOVERY_CLAIM_VERSION.includes('-v6-') ||
    PHASE_6_9_7_V7_MARKER_VERSION.includes('-v6-')
  ) {
    throw new Error('PHASE_6_9_7_V7_DURABILITY_IDENTITY_INVALID');
  }
}

function runtimeContractSuccessV7(entry: Readonly<Phase697V7CaseEntry>): boolean {
  ...
  return (
    ...
    snapshot.version === PHASE_6_9_7_V7_WIRE_DIAGNOSTICS_VERSION &&
...
function buildActiveSnapshot(stages: readonly Phase697V7WireStage[]): Phase697V7WireSnapshot {
  return Object.freeze({
