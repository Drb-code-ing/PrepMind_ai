...
import {
  ...
  type Phase697FullGateProxyRecord,
...
export const PHASE_6_9_7_FULL_GATE_DURABILITY_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-durability-v1' as const;
export const PHASE_6_9_7_FULL_GATE_MARKER_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-marker-v1' as const;
export const PHASE_6_9_7_FULL_GATE_JOURNAL_RECORD_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-journal-record-v1' as const;
export const PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-recovery-claim-v1' as const;
export const PHASE_6_9_7_FULL_GATE_ARTIFACT_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-artifact-v1' as const;
export const PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH =
  '.tmp/phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live.marker' as const;

const FORMAL_JOURNAL_FILE =
  /^phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live-[0-9a-f-]{36}\.journal\.jsonl$/u;
const FORMAL_CLAIM_FILE =
  /^phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live-[0-9a-f-]{36}\.recovery\.claim$/u;
const FORMAL_ARTIFACT_FILE =
  /^phase-6-9-7-tutor-organizer-full-gate-l3-(branch|main)-controlled-live-[0-9a-f-]{36}\.json$/u;
...
const proxyRecordSchema = z
  .object({
    version: z.literal('phase-6.9.7-tutor-organizer-full-gate-proxy-attestation-v1'),
    status: z.enum(['direct_ready', 'loopback_proxy_ready']),
    providerCalls: z.literal(0),
  })
  .strict();

export const PHASE_6_9_7_FULL_GATE_MARKER_SCHEMA = z
  .object({
    markerVersion: z.literal(PHASE_6_9_7_FULL_GATE_MARKER_VERSION),
    durabilityVersion: z.literal(PHASE_6_9_7_FULL_GATE_DURABILITY_VERSION),
    ...
    ownerToken: UUID,
    source: PHASE_6_9_7_FULL_GATE_SOURCE_SCHEMA,
...
export type Phase697FullGateMarker = z.infer<typeof PHASE_6_9_7_FULL_GATE_MARKER_SCHEMA>;

export const PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA = z
  .object({
    claimVersion: z.literal(PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_VERSION),
    lineage: z.literal(PHASE_6_9_7_FULL_GATE_LINEAGE),
    runId: UUID,
    claimedAt: ISO_DATE,
    ownerProcessId: z.number().int().safe().positive(),
...
const journalRecordSchema = z
  .object({
    recordVersion: z.literal(PHASE_6_9_7_FULL_GATE_JOURNAL_RECORD_VERSION),
    lineage: z.literal(PHASE_6_9_7_FULL_GATE_LINEAGE),
    runId: UUID,
    ...
    caseEntry: PHASE_6_9_7_FULL_GATE_CASE_ENTRY_SCHEMA.nullable(),
    report: PHASE_6_9_7_FULL_GATE_REPORT_SCHEMA.nullable(),
...
export type Phase697FullGateJournalRecord = z.infer<typeof journalRecordSchema>;

export const PHASE_6_9_7_FULL_GATE_ARTIFACT_SCHEMA = z
  .object({
    artifactVersion: z.literal(PHASE_6_9_7_FULL_GATE_ARTIFACT_VERSION),
    lineage: z.literal(PHASE_6_9_7_FULL_GATE_LINEAGE),
    authority: z.enum(['controlled_live', 'synthetic_test']),
    ...
    report: PHASE_6_9_7_FULL_GATE_REPORT_SCHEMA,
    durability: z
      .object({
        version: z.literal(PHASE_6_9_7_FULL_GATE_DURABILITY_VERSION),
  ...
  .superRefine((value, context) => {
    ...
    const quality =
      value.authority === 'controlled_live' && value.report.gate === 'full_gate_quality_gate_passed'
...
export type Phase697FullGateArtifact = z.infer<typeof PHASE_6_9_7_FULL_GATE_ARTIFACT_SCHEMA>;

type LaneState = {
  identity: Phase697FullGateLaneIdentity;
  reserved: boolean;
  wireStages: Phase697V7WireStage[];
  terminal: Phase697FullGateCaseEntry | null;
  notStarted: boolean;
};

type ReservationState = {
  root: string;
  marker: Phase697FullGateMarker;
  markerBytes: string;
  markerSha256: string;
  journalPath: string;
  sequence: number;
  previousHash: string | null;
  records: Phase697FullGateJournalRecord[];
  guards: Map<string, Phase697FullGateCaseEntry>;
  lanes: Map<string, LaneState>;
  pairs: Set<number>;
  terminal: Phase697FullGateJournalRecord | null;
  report: Phase697FullGateReport | null;
...
export type Phase697FullGateReservation = Readonly<{
  runId: string;
  markerRelativePath: typeof PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH;
  journalRelativePath: string;
  lifecycle: Phase697FullGateLifecycle;
  publishArtifact(report: Phase697FullGateReport): Promise<
    Readonly<{
      relativePath: string;
...
export type Phase697FullGateCrashSealResult =
  | Readonly<{
      ok: true;
      runId: string;
      disposition: 'crash_only_sealed' | 'terminal_publication_recovered';
      gate: Phase697FullGateReport['gate'];
      evidenceSha256: string;
    }>
  | Readonly<{
      ok: false;
...
const DURABILITY_ERROR = 'PHASE_6_9_7_FULL_GATE_DURABILITY_REJECTED';

export async function reservePhase697FullGateAttempt(input: {
  root: string;
  runId: string;
  ...
  let proxy: Phase697FullGateProxyRecord;
  try {
    proxy = claimPhase697FullGateConsumedProxyAttestation(input.proxyAttestation, input.authority);
  } catch {
  ...
  try {
    ...
    const marker = deepFreeze(
      PHASE_6_9_7_FULL_GATE_MARKER_SCHEMA.parse({
    ...
    const markerPath = resolveRelative(root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH);
 ...
 * Provider transport.
 */
export function sealPhase697FullGateInterruptedAttempt(input: {
  root: string;
}): Promise<Phase697FullGateCrashSealResult> {
...
export function sealPhase697FullGateInterruptedAttemptForTest(input: {
  root: string;
  processAlive?: (processId: number) => boolean;
}): Promise<Phase697FullGateCrashSealResult> {
  return sealInterruptedAttempt(
...
async function sealInterruptedAttempt(
  ...
  let marker: Phase697FullGateMarker;
  let markerSha256: string;
  let records: readonly Phase697FullGateJournalRecord[];
  try {
    root = await requireRoot(input.root);
    markerBytes = await readRegularFile(
      resolveRelative(root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH),
    );
    marker = PHASE_6_9_7_FULL_GATE_MARKER_SCHEMA.parse(JSON.parse(markerBytes));
    if (
  ...
  try {
    const [rereadMarkerBytes, rereadJournalBytes] = await Promise.all([
      readRegularFile(resolveRelative(root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH)),
...
function recoveryState(input: {
  ...
  markerBytes: string;
  markerSha256: string;
  records: readonly Phase697FullGateJournalRecord[];
  replay: ReturnType<typeof replayRecords>;
  recoveryClaimSha256: string;
}): ReservationState {
  const guards = new Map<string, Phase697FullGateCaseEntry>();
...
async function acquireRecoveryClaim(
  ...
  | Readonly<{
      ok: true;
      claim: z.infer<typeof PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA>;
      claimBytes: string;
      claimSha256: string;
      assertOwned(): Promise<boolean>;
      release(): Promise<void>;
    }>
  | Readonly<{ ok: false; result: Phase697FullGateCrashSealResult }>
> {
  let claim: z.infer<typeof PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA>;
  let claimBytes: string;
  try {
    claim = PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA.parse({
      claimVersion: PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_VERSION,
      lineage: PHASE_6_9_7_FULL_GATE_LINEAGE,
      runId: input.runId,
  ...
  try {
    ...
    let existing: z.infer<typeof PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA>;
    try {
      const existingBytes = await readRegularFile(claimPath);
      existing = PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA.parse(JSON.parse(existingBytes));
...
async function assertRecoveryFence(input: {
  ...
  const [markerBytes, claimBytes, journalBytes] = await Promise.all([
    readRegularFile(resolveRelative(input.root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH)),
...
function createReservation(state: ReservationState): Phase697FullGateReservation {
  ...
  const reservation = Object.freeze({
    ...
    lifecycle,
    publishArtifact: (report: Phase697FullGateReport) =>
...
async function appendGuardTerminal(state: ReservationState, entry: Phase697FullGateCaseEntry) {
  const parsed = PHASE_6_9_7_FULL_GATE_CASE_ENTRY_SCHEMA.parse(entry);
  const expected = PHASE_6_9_7_FULL_GATE_EXPECTED_ENTRIES[state.guards.size];
  if (
    state.terminal ||
    expected?.kind !== 'guard' ||
...
async function appendLaneNotStarted(state: ReservationState, entry: Phase697FullGateCaseEntry) {
  const parsed = PHASE_6_9_7_FULL_GATE_CASE_ENTRY_SCHEMA.parse(entry);
...
async function appendRunTerminal(state: ReservationState, report: Phase697FullGateReport) {
  return appendRunTerminalWithMode(state, report, 'runtime');
}

async function appendRunTerminalWithMode(
  state: ReservationState,
  report: Phase697FullGateReport,
  completionMode: 'runtime' | 'recovery',
) {
  if (
    state.terminal ||
  ...
  const parsed = parsePhase697FullGateReport(report);
...
async function publishArtifact(state: ReservationState, report: Phase697FullGateReport) {
  if (
...
function buildArtifact(state: ReservationState): Phase697FullGateArtifact {
  const terminal = state.terminal;
  const report = state.report;
  if (!terminal || !report) throw new Error(DURABILITY_ERROR);
  return deepFreeze(
    PHASE_6_9_7_FULL_GATE_ARTIFACT_SCHEMA.parse({
      ...
      qualityAuthority:
        ...
        report.gate === 'full_gate_quality_gate_passed'
          ? 'full_gate_semantic_gate'
          : 'none',
      runId: state.marker.runId,
...
export async function validatePhase697FullGateBundle(input: { root: string }) {
  try {
    const root = await requireRoot(input.root);
    const markerBytes = await readRegularFile(
      resolveRelative(root, PHASE_6_9_7_FULL_GATE_MARKER_RELATIVE_PATH),
    );
    const marker = PHASE_6_9_7_FULL_GATE_MARKER_SCHEMA.parse(JSON.parse(markerBytes));
    ...
    const artifact = PHASE_6_9_7_FULL_GATE_ARTIFACT_SCHEMA.parse(JSON.parse(evidenceBytes));
    ...
    const recomputed = buildPhase697FullGateReport({
      runId: marker.runId,
...
async function appendRecord(
  state: ReservationState,
  input: Partial<Phase697FullGateJournalRecord> & Pick<Phase697FullGateJournalRecord, 'event'>,
) {
...
function nextRecord(
  ...
  const base = {
    recordVersion: PHASE_6_9_7_FULL_GATE_JOURNAL_RECORD_VERSION,
    lineage: PHASE_6_9_7_FULL_GATE_LINEAGE,
    runId: state.marker.runId,
...
function commitRecord(state: ReservationState, record: Phase697FullGateJournalRecord) {
...
function replayRecords(
  ...
  let terminalRecord: Phase697FullGateJournalRecord | null = null;
  let terminalReport: Phase697FullGateReport | null = null;
...
function replayLane(lanes: Map<string, LaneState>, record: Phase697FullGateJournalRecord) {
...
async function validateRecoveryClaim(
  ...
  markerSha256: string,
  records: readonly Phase697FullGateJournalRecord[],
  artifact: Phase697FullGateArtifact,
) {
  ...
  const claim = PHASE_6_9_7_FULL_GATE_RECOVERY_CLAIM_SCHEMA.parse(JSON.parse(bytes));
...
export function journalRelativePath(runId: string) {
  ...
  return `.tmp/phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live-${runId}.journal.jsonl`;
}

export function recoveryClaimRelativePath(runId: string) {
  if (!isUuid(runId)) throw new Error(DURABILITY_ERROR);
  return `.tmp/phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live-${runId}.recovery.claim`;
}

export function artifactRelativePath(runScope: 'branch' | 'main', runId: string) {
  if (!isUuid(runId)) throw new Error(DURABILITY_ERROR);
  return `.tmp/phase-6-9-7-tutor-organizer-full-gate-l3-${runScope}-controlled-live-${runId}.json`;
}
