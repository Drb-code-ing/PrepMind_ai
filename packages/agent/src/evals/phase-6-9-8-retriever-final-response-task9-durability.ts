...
import {
  ...
  type Phase698Task9ReservationAdmissionCapability,
} from './phase-6-9-8-retriever-final-response-task9-source-admission.ts';

export const PHASE_6_9_8_TASK9_DURABILITY_VERSION =
  'phase-6.9.8-retriever-final-response-task9-durability-v1' as const;
export const PHASE_6_9_8_TASK9_MARKER_VERSION =
  'phase-6.9.8-retriever-final-response-task9-marker-v1' as const;
export const PHASE_6_9_8_TASK9_JOURNAL_RECORD_VERSION =
  'phase-6.9.8-retriever-final-response-task9-journal-record-v1' as const;
export const PHASE_6_9_8_TASK9_RECOVERY_CLAIM_VERSION =
  'phase-6.9.8-retriever-final-response-task9-recovery-claim-v1' as const;
export const PHASE_6_9_8_TASK9_ARTIFACT_VERSION =
  'phase-6.9.8-retriever-final-response-task9-artifact-v1' as const;
export const PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH =
  '.tmp/phase-6-9-8-retriever-final-response-task9c-controlled-live.marker' as const;

const UUID = z.string().uuid();
const SHA256 = z.string().regex(/^[0-9a-f]{64}$/u);
const DATETIME = z.string().datetime({ offset: true });
const MAX_FILE_BYTES = 64 * 1024 * 1024;
const DURABILITY_ERROR = 'PHASE_6_9_8_TASK9_DURABILITY_INVALID';
const FORMAL_FILE =
...
export const PHASE_6_9_8_TASK9_MARKER_SCHEMA = z
  .object({
    markerVersion: z.literal(PHASE_6_9_8_TASK9_MARKER_VERSION),
    durabilityVersion: z.literal(PHASE_6_9_8_TASK9_DURABILITY_VERSION),
    ...
    source: PHASE_6_9_8_TASK9_SOURCE_SCHEMA,
  })
  .strict()
...
export type Phase698Task9Marker = z.infer<typeof PHASE_6_9_8_TASK9_MARKER_SCHEMA>;

export const PHASE_6_9_8_TASK9_RECOVERY_CLAIM_SCHEMA = z
  .object({
    version: z.literal(PHASE_6_9_8_TASK9_RECOVERY_CLAIM_VERSION),
    lineage: z.literal(PHASE_6_9_8_TASK9_LINEAGE),
    runId: UUID,
    markerSha256: SHA256,
...
const RECORD_BASE = {
  recordVersion: z.literal(PHASE_6_9_8_TASK9_JOURNAL_RECORD_VERSION),
  lineage: z.literal(PHASE_6_9_8_TASK9_LINEAGE),
  runId: UUID,
...
const JOURNAL_RECORD_SCHEMA = z.discriminatedUnion('event', [
  ...
  z
    .object({
      ...
      entry: PHASE_6_9_8_TASK9_GUARD_ENTRY_SCHEMA,
    })
    .strict(),
  z
  ...
  z
    .object({
      ...
      stage: z.enum(PHASE_6_9_8_TASK9_WIRE_STAGES),
      preparedSuccess: PHASE_6_9_8_TASK9_CALL_ENTRY_SCHEMA.nullable(),
    })
    .strict(),
  z
    .object({
      ...
      entry: PHASE_6_9_8_TASK9_CALL_ENTRY_SCHEMA,
    })
    .strict(),
  z
    .object({
      ...
      entry: PHASE_6_9_8_TASK9_REWRITE_ENTRY_SCHEMA,
    })
    .strict(),
  z
    .object({
      ...
      entry: PHASE_6_9_8_TASK9_FINAL_ENTRY_SCHEMA,
    })
    .strict(),
  ...
  z
    .object({
      ...
      report: PHASE_6_9_8_TASK9_REPORT_SCHEMA,
...
export type Phase698Task9JournalRecord = z.infer<typeof JOURNAL_RECORD_SCHEMA>;

export const PHASE_6_9_8_TASK9_ARTIFACT_SCHEMA = z
  .object({
    artifactVersion: z.literal(PHASE_6_9_8_TASK9_ARTIFACT_VERSION),
    lineage: z.literal(PHASE_6_9_8_TASK9_LINEAGE),
    authority: z.enum(['synthetic_test', 'controlled_live']),
    ...
    durability: z
      .object({
        version: z.literal(PHASE_6_9_8_TASK9_DURABILITY_VERSION),
        completionMode: z.enum(['runtime', 'recovery']),
  ...
  .superRefine((artifact, context) => {
    const expectedQuality =
      artifact.authority === 'controlled_live' && artifact.report.gate.passed
        ? 'retriever_final_response_semantic_gate'
        : 'none';
    if (
...
export type Phase698Task9Artifact = z.infer<typeof PHASE_6_9_8_TASK9_ARTIFACT_SCHEMA>;

type ActiveCall = {
  identity: Phase698Task9CallIdentity;
  stages: Phase698Task9WireStage[];
  preparedSuccess: Phase698Task9CallEntry | null;
};

type MutableReplay = {
  marker: Phase698Task9Marker;
  markerSha256: string;
  records: Phase698Task9JournalRecord[];
  guards: Phase698Task9GuardEntry[];
  calls: Phase698Task9CallEntry[];
  rewrites: Phase698Task9RewriteEntry[];
  finals: Phase698Task9FinalEntry[];
  activeCall: ActiveCall | null;
  recoveryClaimSha256: string | null;
  terminal: Extract<Phase698Task9JournalRecord, { event: 'run_terminal' }> | null;
  publicationStarted: Extract<Phase698Task9JournalRecord, { event: 'publication_started' }> | null;
  published: Extract<Phase698Task9JournalRecord, { event: 'evidence_published' }> | null;
};
...
export type Phase698Task9Reservation = Readonly<{
  runId: string;
  markerRelativePath: typeof PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH;
...
export type Phase698Task9CrashSealResult =
  | Readonly<{
      ok: true;
      runId: string;
      disposition: 'crash_only_sealed' | 'terminal_publication_recovered';
      artifactSha256: string;
      gate: Phase698Task9Report['gate'];
    }>
  | Readonly<{
      ok: false;
      code:
...
export async function reservePhase698Task9Attempt(input: {
  root: string;
  runId: string;
  createdAt: string;
  ...
  const admission = consumePhase698Task9ReservationAdmissionCapability(
    input.reservationCapability,
    root,
  );
  const credentialReads = admission.authority === 'controlled_live' ? 3 : 0;
  const marker = PHASE_6_9_8_TASK9_MARKER_SCHEMA.parse({
    markerVersion: PHASE_6_9_8_TASK9_MARKER_VERSION,
    durabilityVersion: PHASE_6_9_8_TASK9_DURABILITY_VERSION,
  ...
  const markerPath = resolveRelative(root, PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH);
...
export async function validatePhase698Task9Bundle(input: { root: string }) {
  try {
    const root = await requireRoot(input.root);
    const markerBytes = await readRegular(
      resolveRelative(root, PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH),
    );
    const marker = PHASE_6_9_8_TASK9_MARKER_SCHEMA.parse(JSON.parse(markerBytes));
    ...
    const artifact = PHASE_6_9_8_TASK9_ARTIFACT_SCHEMA.parse(JSON.parse(artifactBytes));
    if (artifactBytes !== `${JSON.stringify(artifact)}\n`) throw new Error(DURABILITY_ERROR);
    const expected = buildArtifact(replay);
    const reportSha = sha256Phase698Task9(canonicalPhase698Task9Json(report));
    const artifactSha = sha256(artifactBytes);
    if (
      canonicalPhase698Task9Json(artifact) !== canonicalPhase698Task9Json(expected) ||
      canonicalPhase698Task9Json(artifact.report) !== canonicalPhase698Task9Json(report) ||
...
export function sealPhase698Task9InterruptedAttempt(input: {
  root: string;
}): Promise<Phase698Task9CrashSealResult> {
  return sealInterrupted(input.root, isProcessAlive);
}

/** Synthetic-only process-liveness seam for isolated temp-root fault tests. */
export function sealPhase698Task9InterruptedAttemptForTest(input: {
  root: string;
...
async function sealInterrupted(
  ...
  let root: string;
  let marker: Phase698Task9Marker;
  let markerBytes: string;
  try {
    root = await requireRoot(rootInput);
    markerBytes = await readRegular(resolveRelative(root, PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH));
    marker = PHASE_6_9_8_TASK9_MARKER_SCHEMA.parse(JSON.parse(markerBytes));
    if (markerBytes !== `${JSON.stringify(marker)}\n`) throw new Error();
  } catch {
...
function reservationFromState(
  ...
  const lifecycle: Phase698Task9Lifecycle = Object.freeze({
    ...
    reserveCall: (identity) =>
      enqueueResult(state, async () => {
        ...
        const callLifecycle: Phase698Task9CallLifecycle = Object.freeze({
  ...
  return Object.freeze({
    runId: state.marker.runId,
    markerRelativePath: PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH,
...
async function appendGuard(state: ReservationState, entry: Phase698Task9GuardEntry) {
  await appendRecord(state, {
    event: 'guard_terminal',
    entry: PHASE_6_9_8_TASK9_GUARD_ENTRY_SCHEMA.parse(entry),
  });
}

async function reserveCall(state: ReservationState, identity: Phase698Task9CallIdentity) {
...
async function appendCall(state: ReservationState, entry: Phase698Task9CallEntry) {
  await appendRecord(state, {
    event: 'call_terminal',
    entry: PHASE_6_9_8_TASK9_CALL_ENTRY_SCHEMA.parse(entry),
  });
}

async function appendRewrite(state: ReservationState, entry: Phase698Task9RewriteEntry) {
  await appendRecord(state, {
    event: 'rewrite_terminal',
    entry: PHASE_6_9_8_TASK9_REWRITE_ENTRY_SCHEMA.parse(entry),
  });
}

async function appendFinal(state: ReservationState, entry: Phase698Task9FinalEntry) {
  await appendRecord(state, {
    event: 'final_terminal',
    entry: PHASE_6_9_8_TASK9_FINAL_ENTRY_SCHEMA.parse(entry),
  });
}
...
async function appendRunTerminal(
  ...
  const parsed = parsePhase698Task9Report(report);
  const expected = recomputeReport(state, completionMode);
  if (!parsed || canonicalPhase698Task9Json(parsed) !== canonicalPhase698Task9Json(expected)) {
    throw new Error(DURABILITY_ERROR);
  }
  await appendRecord(state, {
    event: 'run_terminal',
    report: parsed,
    reportLogicalSha256: sha256Phase698Task9(canonicalPhase698Task9Json(parsed)),
    completionMode,
  });
}

async function publishArtifact(state: ReservationState, report: Phase698Task9Report) {
  if (
    !state.terminal ||
    canonicalPhase698Task9Json(report) !== canonicalPhase698Task9Json(state.terminal.report) ||
    state.published
  ) {
    throw new Error(DURABILITY_ERROR);
  }
...
function buildArtifact(state: MutableReplay): Phase698Task9Artifact {
  ...
  return PHASE_6_9_8_TASK9_ARTIFACT_SCHEMA.parse({
    artifactVersion: PHASE_6_9_8_TASK9_ARTIFACT_VERSION,
    lineage: PHASE_6_9_8_TASK9_LINEAGE,
    ...
    durability: {
      version: PHASE_6_9_8_TASK9_DURABILITY_VERSION,
...
async function completeRecovery(state: ReservationState) {
  while (state.guards.length < 16) {
    ...
    await appendGuard(
      state,
      PHASE_6_9_8_TASK9_GUARD_ENTRY_SCHEMA.parse({
        kind: 'guard',
...
async function recoverNextCall(state: ReservationState, guardFailed: boolean) {
  const identity = expectedPhase698Task9CallSchedule()[state.calls.length];
  ...
  if (state.activeCall) {
    ...
    await appendCall(
      state,
      PHASE_6_9_8_TASK9_CALL_ENTRY_SCHEMA.parse({
        kind: 'provider_call',
        ...identity,
        transportAuthority:
...
function recomputeReport(state: MutableReplay, completionMode: 'runtime' | 'recovery') {
  return buildPhase698Task9Report({
    runId: state.marker.runId,
    authority: state.marker.authority,
    completionMode,
    source: state.marker.source,
...
function notStarted(
  identity: Phase698Task9CallIdentity,
  authority: Phase698Task9Marker['authority'],
  guardFailed: boolean,
) {
...
function nextRecord(state: MutableReplay, event: Record<string, unknown>) {
  const base = {
    recordVersion: PHASE_6_9_8_TASK9_JOURNAL_RECORD_VERSION,
    lineage: PHASE_6_9_8_TASK9_LINEAGE,
    runId: state.marker.runId,
  ...
  return JOURNAL_RECORD_SCHEMA.parse({
    ...base,
    recordHash: sha256Phase698Task9(canonicalPhase698Task9Json(base)),
  });
}

function applyRecordPreview(state: MutableReplay, record: Phase698Task9JournalRecord) {
  const clone = cloneReplay(state);
  applyRecord(clone, record);
}

function applyRecord(state: MutableReplay, record: Phase698Task9JournalRecord) {
  ...
  switch (record.event) {
    ...
    case 'call_reserved': {
      ...
      const expected = expectedPhase698Task9CallSchedule()[state.calls.length];
      if (
        !expected ||
        canonicalPhase698Task9Json(expected) !== canonicalPhase698Task9Json(record.identity)
      ) {
        throw new Error(DURABILITY_ERROR);
      }
    ...
    case 'wire_stage': {
      ...
      const expectedStage = PHASE_6_9_8_TASK9_WIRE_STAGES[active?.stages.length ?? 99];
      if (
        !active ||
    ...
    case 'call_terminal': {
      ...
      if (!record.entry.disposition.startsWith('not_started_')) {
        ...
        if (canonicalPhase698Task9Json(wire) !== canonicalPhase698Task9Json(record.entry.wire)) {
          throw new Error(DURABILITY_ERROR);
        }
        if (
          active.preparedSuccess !== null &&
          canonicalPhase698Task9Json(active.preparedSuccess) !==
            canonicalPhase698Task9Json(record.entry)
        ) {
          throw new Error(DURABILITY_ERROR);
        }
      }
    ...
    case 'rewrite_terminal': {
      if (state.activeCall || state.rewrites.length >= 16 || state.finals.length > 0) {
        throw new Error(DURABILITY_ERROR);
      }
    ...
    case 'final_terminal': {
      ...
      if (
        !expected ||
        expected.caseId !== record.entry.caseId ||
        state.calls.length !== 48 + state.finals.length + 1
      ) {
        throw new Error(DURABILITY_ERROR);
      }
    ...
    case 'run_terminal': {
      ...
      if (
        canonicalPhase698Task9Json(recomputed) !== canonicalPhase698Task9Json(record.report) ||
        record.reportLogicalSha256 !== sha256Phase698Task9(canonicalPhase698Task9Json(recomputed))
      ) {
        throw new Error(DURABILITY_ERROR);
      }
...
function assertCallScheduleBoundary(state: MutableReplay) {
  if (state.guards.length !== 16 || state.terminal || state.publicationStarted || state.published) {
    throw new Error(DURABILITY_ERROR);
  }
  const index = state.calls.length;
  if (
    (index < 48 && Math.floor(index / 3) !== state.rewrites.length) ||
    (index >= 48 && (state.rewrites.length !== 16 || index - 48 !== state.finals.length))
  ) {
    throw new Error(DURABILITY_ERROR);
  }
}
...
function assertCanTerminateCall(state: MutableReplay, entry: Phase698Task9CallEntry) {
  assertCallScheduleBoundary(state);
...
function wireFromStages(stages: readonly Phase698Task9WireStage[]) {
  return {
    attempts: 1,
    dispatches: stages.includes('dispatch_started') ? 1 : 0,
    responses: stages.includes('response_received') ? 1 : 0,
...
function parseJournal(bytes: string, marker: Phase698Task9Marker, markerSha256: string) {
...
function replayRecords(
  marker: Phase698Task9Marker,
  markerSha256: string,
  records: readonly Phase698Task9JournalRecord[],
) {
...
function createReplay(marker: Phase698Task9Marker, markerSha256: string): MutableReplay {
  return {
    marker,
    markerSha256,
    records: [],
...
async function acquireRecoveryClaim(
  ...
  const claim = PHASE_6_9_8_TASK9_RECOVERY_CLAIM_SCHEMA.parse({
    version: PHASE_6_9_8_TASK9_RECOVERY_CLAIM_VERSION,
    lineage: PHASE_6_9_8_TASK9_LINEAGE,
  ...
  try {
    ...
    try {
      ...
      const parsed = PHASE_6_9_8_TASK9_RECOVERY_CLAIM_SCHEMA.parse(JSON.parse(bytes));
      if (
...
async function validateClaim(root: string, replay: MutableReplay) {
  ...
  const claim = PHASE_6_9_8_TASK9_RECOVERY_CLAIM_SCHEMA.parse(JSON.parse(bytes));
  if (
    bytes !== `${JSON.stringify(claim)}\n` ||
...
async function requireOnlyExpectedFiles(root: string, replay: MutableReplay) {
  ...
  const expected = new Set([
    PHASE_6_9_8_TASK9_MARKER_RELATIVE_PATH.split('/').at(-1)!,
...
async function formalFiles(root: string) {
  try {
    ...
    return entries.filter((entry) => entry.isFile() && FORMAL_FILE.test(entry.name));
  } catch (error) {
...
export function journalRelativePath(runId: string) {
  UUID.parse(runId);
  return `.tmp/phase-6-9-8-retriever-final-response-task9c-controlled-live-${runId}.journal.jsonl`;
}

export function recoveryClaimRelativePath(runId: string) {
  UUID.parse(runId);
  return `.tmp/phase-6-9-8-retriever-final-response-task9c-controlled-live-${runId}.recovery.claim`;
}

export function artifactRelativePath(runId: string) {
  UUID.parse(runId);
  return `.tmp/phase-6-9-8-retriever-final-response-task9c-branch-controlled-live-${runId}.json`;
}
