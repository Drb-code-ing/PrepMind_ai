...
import {
  type Phase697FullGateGuardCase,
  type Phase697FullGateHarness,
  type Phase697FullGateRuntimeResult,
...
export const PHASE_6_9_7_FULL_GATE_DEEPSEEK_BASE_URL = 'https://api.deepseek.com/v1' as const;
export const PHASE_6_9_7_FULL_GATE_MODEL = 'deepseek-v4-pro' as const;
 ...
 */
export function createPhase697FullGateLiveHarness(input: {
  runId: string;
  apiKey: string;
}): Readonly<Phase697FullGateHarness> {
  if (!isUuid(input.runId) || !isCredential(input.apiKey)) {
    throw new Error('PHASE_6_9_7_FULL_GATE_LIVE_CONFIGURATION_INVALID');
  }
...
function runGuard(entry: Phase697FullGateGuardCase) {
...
async function runTutor(
  ...
  try {
    ...
    const semantic: Phase697FullGateRuntimeResult['semantic'] = strict
      ? {
          agent: 'tutor',
          observation: {
            caseId: entry.id,
    ...
    return Object.freeze({
      ...
      orchestrationDurationMs: strict
        ? Math.max(trace.durationMs, performance.now() - orchestrationStartedAt)
        : null,
...
async function runOrganizer(
  ...
  try {
    ...
    const candidate = await runWrongQuestionOrganizerV9ModelCandidate({
      ...
      budget: createModelAgentBudget({
        maxCalls: 1,
        maxInputTokens: PHASE_6_9_7_FULL_GATE_EVAL_POLICY.budget.organizerPerLane.inputTokensMax,
        maxOutputTokens: PHASE_6_9_7_FULL_GATE_EVAL_POLICY.budget.organizerPerLane.outputTokensMax,
      }),
...
function createTrackedRuntime(
  ...
  if (adapter.provenance !== 'first_party_deepseek_v4_pro_direct') {
    throw new Error('PHASE_6_9_7_FULL_GATE_ADAPTER_PROVENANCE_INVALID');
  }
  let invocations = 0;
...
function buildOrganizerObservations(
  ...
  NonNullable<Phase697FullGateCaseEntry['semantic']>,
...
function verifiedUsage(agent: Agent, usage: { inputTokens: number; outputTokens: number }) {
  ...
  if (estimatedCostCny > lane.costCnyMax) return null;
  return Object.freeze({
...
function runtimeFailure(signal: AbortSignal, invocations: number): Phase697FullGateRuntimeResult {
