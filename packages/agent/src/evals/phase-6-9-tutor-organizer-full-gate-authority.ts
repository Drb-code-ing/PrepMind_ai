...
import { PHASE_6_9_7_FULL_GATE_SOURCE_HASHES } from './phase-6-9-tutor-organizer-full-gate-contract.ts';

export const PHASE_6_9_7_FULL_GATE_CONTROLLED_LIVE_BRANCH =
  'codex/phase-6-9-7-tutor-wrong-question-agents' as const;
export const PHASE_6_9_7_FULL_GATE_APPROVAL_ENV =
  'PHASE_6_9_7_TUTOR_ORGANIZER_FULL_GATE_L3_APPROVED' as const;
export const PHASE_6_9_7_FULL_GATE_CREDENTIAL_ENV =
  'PHASE_6_9_7_TUTOR_ORGANIZER_FULL_GATE_L3_DEEPSEEK_API_KEY' as const;
export const PHASE_6_9_7_FULL_GATE_EXACT_CONFIRMATION =
  'I_AUTHORIZE_PHASE_6_9_7_TUTOR_ORGANIZER_FULL_GATE_L3_CONTROLLED_LIVE_ONCE' as const;
export const PHASE_6_9_7_FULL_GATE_PROXY_ATTESTATION_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-proxy-attestation-v1' as const;
export const PHASE_6_9_7_FULL_GATE_SOURCE_VERSION =
  'phase-6.9.7-tutor-organizer-full-gate-source-v1' as const;
export const PHASE_6_9_7_FULL_GATE_APPROVED_SOURCE_REF =
  'refs/tags/phase-6-9-7-tutor-organizer-full-gate-s3-approved' as const;
...
export const PHASE_6_9_7_FULL_GATE_SOURCE_SCHEMA = z
  .object({
    sourceVersion: z.literal(PHASE_6_9_7_FULL_GATE_SOURCE_VERSION),
    branch: z.literal(PHASE_6_9_7_FULL_GATE_CONTROLLED_LIVE_BRANCH),
    commit: COMMIT,
    trackingCommit: COMMIT,
    remoteCommit: COMMIT,
    approvedRunnableSourceRef: z.literal(PHASE_6_9_7_FULL_GATE_APPROVED_SOURCE_REF),
...
export type Phase697FullGateSource = z.infer<typeof PHASE_6_9_7_FULL_GATE_SOURCE_SCHEMA>;

export type Phase697FullGateProxyAttestation = Readonly<{
  version: typeof PHASE_6_9_7_FULL_GATE_PROXY_ATTESTATION_VERSION;
}>;

export type Phase697FullGateConsumedProxyAttestation = Readonly<{
  version: typeof PHASE_6_9_7_FULL_GATE_PROXY_ATTESTATION_VERSION;
}>;

export type Phase697FullGateProxyRecord = Readonly<{
  version: typeof PHASE_6_9_7_FULL_GATE_PROXY_ATTESTATION_VERSION;
...
type AttestationState = {
  ...
  status: Phase697FullGateProxyRecord['status'];
  consumed: boolean;
};
...
const FORMAL_MARKER = 'phase-6-9-7-tutor-organizer-full-gate-l3-controlled-live.marker';
...
export async function runPhase697FullGateProductionProxyPreflight(
  signal: AbortSignal,
): Promise<Phase697FullGateProxyAttestation> {
...
export function createPhase697FullGateSyntheticProxyAttestationForTest(
  status: Phase697FullGateProxyRecord['status'] = 'direct_ready',
): Phase697FullGateProxyAttestation {
  return mintAttestation('synthetic_test', status);
}

export function consumePhase697FullGateProxyAttestation(
  value: unknown,
  authority: 'controlled_live' | 'synthetic_test',
): Phase697FullGateConsumedProxyAttestation {
  try {
    ...
    const consumed = Object.freeze({
      version: PHASE_6_9_7_FULL_GATE_PROXY_ATTESTATION_VERSION,
    });
 ...
 */
export function claimPhase697FullGateConsumedProxyAttestation(
  value: unknown,
  authority: 'controlled_live' | 'synthetic_test',
): Phase697FullGateProxyRecord {
  try {
    if (typeof value !== 'object' || value === null) throw authorityError();
 ...
 */
export async function readPhase697FullGateSource(
  rawRoot: string,
): Promise<Readonly<Phase697FullGateSource>> {
  try {
    const root = await requireRoot(rawRoot);
    const branch = gitOutput(root, ['branch', '--show-current']);
    if (branch !== PHASE_6_9_7_FULL_GATE_CONTROLLED_LIVE_BRANCH) throw authorityError();
    ...
    const approvedRunnableSourceCommit = gitOutput(root, [
      'rev-parse',
      `${PHASE_6_9_7_FULL_GATE_APPROVED_SOURCE_REF}^{commit}`,
    ]);
    ...
    return deepFreeze(
      PHASE_6_9_7_FULL_GATE_SOURCE_SCHEMA.parse({
        sourceVersion: PHASE_6_9_7_FULL_GATE_SOURCE_VERSION,
        branch,
        commit,
        trackingCommit,
        remoteCommit,
    ...
    throw new Error('PHASE_6_9_7_FULL_GATE_SOURCE_INVALID');
  }
}

export function readPhase697FullGateApproval(
  env: Readonly<Record<string, string | undefined>>,
): true {
  if (
    readEnv(env, PHASE_6_9_7_FULL_GATE_APPROVAL_ENV) !== PHASE_6_9_7_FULL_GATE_EXACT_CONFIRMATION
  ) {
    throw authorityError();
  }
  return true;
...
export function readPhase697FullGateCredential(
  env: Readonly<Record<string, string | undefined>>,
): string {
  const credential = readEnv(env, PHASE_6_9_7_FULL_GATE_CREDENTIAL_ENV);
...
async function hashRunnableSources(
  root: string,
): Promise<Readonly<Phase697FullGateSource['sourceHashes']>> {
  const paths = {
...
function authorityError() {
  return new Error('PHASE_6_9_7_FULL_GATE_AUTHORITY_REJECTED');
}
