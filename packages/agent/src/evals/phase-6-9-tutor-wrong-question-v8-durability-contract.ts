...
import {
  PHASE_6_9_7_TUTOR_ORGANIZER_RUNNER_VERSION_V8,
  PHASE_6_9_7_V8_CASE_ENTRY_SCHEMA,
  PHASE_6_9_7_V8_EVAL_POLICY_SHA256,
  PHASE_6_9_7_V8_EVIDENCE_ENVELOPE_SCHEMA,
  PHASE_6_9_7_V8_EVIDENCE_PREFIX,
  PHASE_6_9_7_V8_JOURNAL_VERSION,
  PHASE_6_9_7_V8_MARKER_PATH,
  PHASE_6_9_7_V8_MARKER_SCHEMA,
  PHASE_6_9_7_V8_MARKER_VERSION,
  PHASE_6_9_7_V8_RECOVERY_CLAIM_SCHEMA,
  PHASE_6_9_7_V8_RECOVERY_CLAIM_VERSION,
  PHASE_6_9_7_V8_RUNTIME_EVIDENCE_VERSION,
  ...
  phase697V8EvidencePath as phase697V8EvidencePathNullable,
  phase697V8JournalPath as phase697V8JournalPathNullable,
  phase697V8RecoveryClaimPath as phase697V8RecoveryClaimPathNullable,
  projectPhase697V8EntryToV6,
...
export type { Phase697V8EvidenceEnvelope, Phase697V8Marker, Phase697V8RecoveryClaimRecord };
...
export const PHASE_6_9_7_V8_TERMINAL_PROJECTION_SCHEMA = z
  .object({
    runtimeEvidenceVersion: z.literal(PHASE_6_9_7_V8_RUNTIME_EVIDENCE_VERSION),
    entry: PHASE_6_9_7_V8_CASE_ENTRY_SCHEMA,
    terminalEntrySha256: sha256Schema,
  })
  .strict();

export type Phase697V8TerminalProjection = z.infer<
  typeof PHASE_6_9_7_V8_TERMINAL_PROJECTION_SCHEMA
>;

export const PHASE_6_9_7_V8_JOURNAL_PAYLOAD_SCHEMA = z.discriminatedUnion('kind', [
  z
    .object({
  ...
  z
    .object({
      ...
      terminal: PHASE_6_9_7_V8_TERMINAL_PROJECTION_SCHEMA,
    })
    .strict(),
  z
    .object({
      ...
      wireCapabilityVersion: z.literal(PHASE_6_9_7_V8_WIRE_CAPABILITY_VERSION),
    })
    .strict(),
  z
    .object({
      ...
      dispatchKeySha256: sha256Schema,
      stage: z.enum(PHASE_6_9_7_V8_WIRE_STAGES),
    })
    .strict(),
  z
    .object({
...
export type Phase697V8JournalPayload = z.infer<typeof PHASE_6_9_7_V8_JOURNAL_PAYLOAD_SCHEMA>;

export const PHASE_6_9_7_V8_JOURNAL_RECORD_SCHEMA = z
  .object({
    journalVersion: z.literal(PHASE_6_9_7_V8_JOURNAL_VERSION),
    ...
    payload: PHASE_6_9_7_V8_JOURNAL_PAYLOAD_SCHEMA,
    recordSha256: sha256Schema,
  })
  .strict();

export type Phase697V8JournalRecord = z.infer<typeof PHASE_6_9_7_V8_JOURNAL_RECORD_SCHEMA>;

type ValidatedLane = Readonly<{
  caseId: string;
  agent: 'tutor' | 'wrong_question_organizer';
  pairedRunIndex: number;
  stages: readonly Phase697V8WireStage[];
}>;

export type Phase697V8ValidatedJournal = Readonly<{
  records: readonly Phase697V8JournalRecord[];
  runId: string;
  runScope: 'branch' | 'main';
  markerSha256: string;
  tailSha256: string;
  lastSequence: number;
  guardTerminals: ReadonlyMap<string, Phase697V8CaseEntry>;
  lanes: ReadonlyMap<string, ValidatedLane>;
  runtimeTerminals: ReadonlyMap<string, Phase697V8CaseEntry>;
  pairedDurations: ReadonlyMap<
    number,
    Extract<Phase697V8JournalPayload, { kind: 'pair_terminal' }>['pairedDurationEvidence']
  >;
  breaker: Extract<Phase697V8JournalPayload, { kind: 'breaker_opened' }> | null;
  runCompleted: Readonly<{
    reportSha256: string;
    gate: 'quality_gate_passed' | 'quality_gate_failed';
  }> | null;
  sealed: Extract<Phase697V8JournalPayload, { kind: 'evidence_sealed' }> | null;
}>;

export function stablePhase697V8JsonStringify(value: unknown): string {
  return JSON.stringify(sortStableValue(value));
}

export function sha256Phase697V8Bytes(value: string | Uint8Array): `sha256:${string}` {
  return `sha256:${createHash('sha256').update(value).digest('hex')}`;
}

export function phase697V8JournalPath(runId: string): string {
  const path = phase697V8JournalPathNullable(runId);
  if (path === null) throw new Error('PHASE_6_9_7_V8_RUN_ID_INVALID');
  return path;
}

export function phase697V8RecoveryClaimPath(runId: string): string {
  const path = phase697V8RecoveryClaimPathNullable(runId);
  if (path === null) throw new Error('PHASE_6_9_7_V8_RUN_ID_INVALID');
  return path;
}
...
export function phase697V8EvidencePath(input: {
  ...
  const path = phase697V8EvidencePathNullable(input);
  if (path === null) throw new Error('PHASE_6_9_7_V8_RUN_ID_INVALID');
  return path;
}

export function projectPhase697V8TerminalEntry(
  entry: Readonly<Phase697V8CaseEntry>,
): Readonly<Phase697V8TerminalProjection> | null {
  const parsed = PHASE_6_9_7_V8_TERMINAL_PROJECTION_SCHEMA.safeParse({
    runtimeEvidenceVersion: PHASE_6_9_7_V8_RUNTIME_EVIDENCE_VERSION,
    entry,
    terminalEntrySha256: sha256Phase697V8Stable(entry),
  });
  return parsed.success ? Object.freeze(parsed.data) : null;
}

export function restorePhase697V8TerminalEntry(
  projection: unknown,
): Readonly<Phase697V8CaseEntry> | null {
  const parsed = PHASE_6_9_7_V8_TERMINAL_PROJECTION_SCHEMA.safeParse(projection);
  if (!parsed.success) return null;
  return sha256Phase697V8Stable(parsed.data.entry) === parsed.data.terminalEntrySha256
    ? Object.freeze(parsed.data.entry)
    : null;
}

export function buildPhase697V8JournalRecord(input: {
  runId: string;
  sequence: number;
  previousRecordSha256: string | null;
  payload: Phase697V8JournalPayload;
}): Phase697V8JournalRecord {
  const withoutHash = {
    journalVersion: PHASE_6_9_7_V8_JOURNAL_VERSION,
  ...
  return PHASE_6_9_7_V8_JOURNAL_RECORD_SCHEMA.parse({
    ...withoutHash,
    recordSha256: sha256Phase697V8Stable(withoutHash),
  });
}

export function parseAndValidatePhase697V8Journal(text: string): Phase697V8ValidatedJournal | null {
  ...
  const records: Phase697V8JournalRecord[] = [];
  try {
    for (const line of lines) {
      const parsed = PHASE_6_9_7_V8_JOURNAL_RECORD_SCHEMA.parse(JSON.parse(line) as unknown);
      const { recordSha256, ...withoutHash } = parsed;
      if (sha256Phase697V8Stable(withoutHash) !== recordSha256) return null;
      records.push(parsed);
    }
  } catch {
    return null;
  ...
  const guards = new Map<string, Phase697V8CaseEntry>();
  const mutableLanes = new Map<
    string,
    {
      ...
      agent: 'tutor' | 'wrong_question_organizer';
      pairedRunIndex: number;
      stages: Phase697V8WireStage[];
    }
  >();
  const terminals = new Map<string, Phase697V8CaseEntry>();
  const pairDurations = new Map<
    number,
    Extract<Phase697V8JournalPayload, { kind: 'pair_terminal' }>['pairedDurationEvidence']
  >();
  let breaker: Extract<Phase697V8JournalPayload, { kind: 'breaker_opened' }> | null = null;
  let runCompleted: Phase697V8ValidatedJournal['runCompleted'] = null;
  let sealed: Phase697V8ValidatedJournal['sealed'] = null;
  ...
  for (const [index, record] of records.entries()) {
    ...
    if (payload.kind === 'guard_terminal') {
      const terminal = restorePhase697V8TerminalEntry(payload.terminal);
      if (
        mutableLanes.size > 0 ||
        breaker !== null ||
        runCompleted !== null ||
    ...
    if (payload.kind === 'lane_reserved') {
      const expectedKey = phase697V8DispatchKeySha256({
        runId,
        agent: payload.agent,
    ...
    if (payload.kind === 'wire_stage') {
      ...
      if (
        ...
        payload.stage !== PHASE_6_9_7_V8_WIRE_STAGES[lane.stages.length]
      ) {
        return null;
      }
    ...
    if (payload.kind === 'breaker_opened') {
      ...
      if (payload.breakerState === 'guard_failed') {
        ...
        if (
          [...guards.values()].some((entry) => !entry.zeroCallVerified) ||
          !trigger ||
          runtimeContractSuccessV8(trigger) ||
          trigger.agent !== payload.triggerAgent ||
    ...
    if (payload.kind === 'run_completed') {
      ...
      const runtimeFailure = [...terminals.values()].some(
        (entry) => !runtimeContractSuccessV8(entry),
      );
      if (
        runCompleted !== null ||
        guards.size !== 24 ||
...
export function buildPhase697V8SealedReport(input: {
  marker: Readonly<Phase697V8Marker>;
  markerSha256: string;
  journal: Readonly<Phase697V8ValidatedJournal> | null;
  ...
  const boundedSchemaDiagnostics = new Map<
    string,
    NonNullable<Phase697V8CaseEntry['boundedSchemaDiagnostic']>
  >();
  const v6Entries: Phase697V6CaseEntry[] = [];
  const expectedTerminals = new Map<string, Phase697V8CaseEntry>();
  ...
  for (const testCase of PHASE_6_9_TUTOR_WRONG_QUESTION_V2_CASES) {
    let v8Entry: Phase697V8CaseEntry;
    if (testCase.expectedRuntimeInvocations === 0) {
      v8Entry =
        input.journal?.guardTerminals.get(testCase.id) ??
        buildPhase697V8CaseEntry(buildMissingGuardEntry(testCase), null);
    } else {
      const dispatchKey = phase697V8DispatchKeySha256({
      ...
      if (terminal) {
        ...
        if (lane) {
          ...
          v8Entry = buildPhase697V8CaseEntry(
      ...
      if (v8Entry.wireEvidence.snapshot !== null) {
        wireSnapshots.set(testCase.id, v8Entry.wireEvidence.snapshot);
      }
    }
    const projected = projectPhase697V8EntryToV6(v8Entry);
    if (projected === null) return null;
    if (v8Entry.boundedSchemaDiagnostic !== null) {
      boundedSchemaDiagnostics.set(testCase.id, v8Entry.boundedSchemaDiagnostic);
    }
  ...
  try {
    ...
    for (const [caseId, terminal] of expectedTerminals) {
      ...
      if (!rebuilt || sha256Phase697V8Stable(rebuilt) !== sha256Phase697V8Stable(terminal)) {
        return null;
      }
    }
    if (
      completed !== null &&
      (completed.reportSha256 !== sha256Phase697V8Stable(report) || completed.gate !== report.gate)
    ) {
      return null;
    }
    return report;
...
export function assertPhase697V8PathIdentity(): void {
  const identities = [
    PHASE_6_9_7_V8_MARKER_PATH,
    PHASE_6_9_7_V8_EVIDENCE_PREFIX,
    PHASE_6_9_7_V8_RECOVERY_CLAIM_VERSION,
  ...
  if (
    !PHASE_6_9_7_V8_MARKER_PATH.includes('-v8-') ||
    !PHASE_6_9_7_V8_EVIDENCE_PREFIX.endsWith('-v8') ||
    !PHASE_6_9_7_V8_RECOVERY_CLAIM_VERSION.includes('phase-6.9.7-v8-') ||
    !PHASE_6_9_7_V8_MARKER_VERSION.includes('phase-6.9.7-v8-') ||
    identities.some((identity) =>
      /(?:phase-6[.-]9[.-]7|tutor-organizer)-v[1-7](?:[./-]|$)/u.test(identity),
    )
  ) {
    throw new Error('PHASE_6_9_7_V8_DURABILITY_IDENTITY_INVALID');
  }
}

function runtimeContractSuccessV8(entry: Readonly<Phase697V8CaseEntry>): boolean {
  ...
  return (
    ...
    snapshot.version === PHASE_6_9_7_V8_WIRE_DIAGNOSTICS_VERSION &&
...
function buildActiveSnapshot(stages: readonly Phase697V8WireStage[]): Phase697V8WireSnapshot {
  return Object.freeze({
    version: PHASE_6_9_7_V8_WIRE_DIAGNOSTICS_VERSION,
    state: 'active',
