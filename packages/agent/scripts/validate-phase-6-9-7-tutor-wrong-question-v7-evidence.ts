...
import { PHASE_6_9_7_V7_MARKER_SCHEMA } from '../src/evals/phase-6-9-tutor-wrong-question-v7-durability-contract.ts';
import {
  readPhase697V7EvidenceBytes,
...
const PRIOR_ARTIFACT_TOKENS = Object.freeze([
  ...
  ...Array.from({ length: 6 }, (_, index) => `phase-6.9.7-v${index + 1}-runtime-evidence`),
  ...Array.from({ length: 6 }, (_, index) => `phase-6.9.7-v${index + 1}-live-marker`),
  ...Array.from({ length: 6 }, (_, index) => `phase-6.9.7-v${index + 1}-journal`),
  ...Array.from({ length: 6 }, (_, index) => `phase-6.9.7-v${index + 1}-evidence-envelope`),
  ...Array.from({ length: 6 }, (_, index) => `phase-6.9.7-v${index + 1}-recovery-claim`),
]);

const PRIOR_ARTIFACT_PATH = /(?:^|[\\/.-])phase-6-9-7-tutor-organizer-v[1-6](?:[\\/.-]|$)/u;
const PRIOR_ARTIFACT_KEY = /^(?:sourceV[1-6].*|partialMetrics|partialUsage|partialCost|legacy.*)$/u;

export type Phase697V7EvidenceValidationResult =
  | Readonly<{ ok: true }>
  | Readonly<{
      ok: false;
      code:
...
export function validatePhase697TutorOrganizerV7EvidenceValue(
  value: unknown,
): Phase697V7EvidenceValidationResult {
  if (hasPriorPhase697V7ArtifactLineage(value)) {
    return { ok: false, code: 'prior_lineage_detected' };
  }
  if (hasSensitivePhase697Evidence(value)) return { ok: false, code: 'sensitive_evidence' };
  return PHASE_6_9_7_V7_EVIDENCE_ENVELOPE_SCHEMA.safeParse(value).success
    ? { ok: true }
    : { ok: false, code: 'report_contract_invalid' };
}

export function hasPriorPhase697V7ArtifactLineage(value: unknown): boolean {
...
export async function validatePhase697TutorOrganizerV7EvidenceFile(input: {
  path: string;
}): Promise<Phase697V7EvidenceValidationResult> {
  const read = await readPhase697V7Evidence(input.path);
...
export async function validatePhase697TutorOrganizerV7EvidenceBundle(input: {
  root: string;
  evidencePath: string;
}): Promise<Phase697V7EvidenceValidationResult> {
  const filename = await validatePhase697TutorOrganizerV7EvidenceFile({ path: input.evidencePath });
  if (!filename.ok) return filename;
  const evidenceFile = await readPhase697V7EvidenceBytes({ path: input.evidencePath });
  if (!evidenceFile.ok) {
    return {
      ok: false,
      code:
  ...
  const valueValidation = validatePhase697TutorOrganizerV7EvidenceValue(envelope);
  ...
  const marker = await readPhase697V7Marker({ root: input.root });
  ...
  if (
    ...
    !PHASE_6_9_7_V7_MARKER_SCHEMA.safeParse(marker.marker).success
  ) {
    return { ok: false, code: 'durability_identity_invalid' };
  }

  const journal = await readPhase697V7Journal({ root: input.root, runId: envelope.runId });
  ...
  if (
    ...
    (completed !== null &&
      (completed.reportSha256 !== sha256Phase697V7Stable(envelope.report) ||
...
export async function validatePhase697TutorOrganizerV7EvidenceFiles(
  paths: readonly string[],
): Promise<Phase697V7EvidenceValidationResult> {
  if (paths.length === 0) return { ok: false, code: 'evidence_read_failed' };
  const runIds: string[] = [];
  for (const path of paths) {
    const valid = await validatePhase697TutorOrganizerV7EvidenceFile({ path });
    if (!valid.ok) return valid;
    const read = await readPhase697V7Evidence(path);
    if (!read.ok) return read;
...
async function readPhase697V7Evidence(
  path: string,
): Promise<
  | Readonly<{ ok: true; envelope: Phase697V7EvidenceEnvelope }>
  | Exclude<Phase697V7EvidenceValidationResult, Readonly<{ ok: true }>>
> {
  try {
    ...
    const valid = validatePhase697TutorOrganizerV7EvidenceValue(value);
    if (!valid.ok) return valid;
    const parsed = PHASE_6_9_7_V7_EVIDENCE_ENVELOPE_SCHEMA.safeParse(value);
...
if (import.meta.main) {
  ...
  const result =
    paths.length === 0
      ...
      : await validatePhase697TutorOrganizerV7EvidenceFiles(paths);
