...
import {
  ...
  PHASE_6_9_7_V7_JOURNAL_RECORD_SCHEMA,
...
export type Phase697V7DurabilityFsOverrides = Readonly<
  Partial<{
    open: typeof fsOpen;
...
export type Phase697V7JournalWriter = Readonly<{
  path: string;
  append(payload: Phase697V7JournalPayload): Promise<Phase697V7JournalRecord>;
  snapshot(): Promise<Readonly<{ tailSha256: string; lastSequence: number }>>;
  close(): Promise<void>;
}>;

export type Phase697V7RecoveryClaim = Readonly<{
  path: string;
  runId: string;
  ownerToken: string;
...
export type Phase697V7RecoveryClaimResult =
  | Readonly<{ ok: true; claim: Phase697V7RecoveryClaim }>
  | Readonly<{
      ok: false;
      code: 'live_attempt_in_progress' | 'recovery_claim_io_failed' | 'recovery_claim_path_invalid';
    }>;

export type Phase697V7MarkerReservationResult =
  | Readonly<{
      ok: true;
      marker: Phase697V7Marker;
      markerSha256: string;
      markerPath: string;
    }>
  | Readonly<{
...
export async function reservePhase697V7Marker(input: {
  root: string;
  marker: Readonly<Phase697V7Marker>;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<Phase697V7MarkerReservationResult> {
  const fs = resolveFs(input.overrides);
  const markerPath = resolve(input.root, PHASE_6_9_7_V7_MARKER_PATH);
  if (!(await ensurePlainDirectory(dirname(markerPath), fs))) {
    return { ok: false, code: 'marker_path_invalid' };
  }
  const bytes = `${stablePhase697V7JsonStringify(input.marker)}\n`;
  ...
  try {
    ...
    return {
      ok: true,
      marker: PHASE_6_9_7_V7_MARKER_SCHEMA.parse(input.marker),
      markerSha256: sha256Phase697V7Bytes(bytes),
      markerPath,
    };
  } catch (error) {
...
export async function createPhase697V7Journal(input: {
  root: string;
  marker: Readonly<Phase697V7Marker>;
  markerSha256: string;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V7JournalWriter; journalPath: string }>
  | Readonly<{
      ok: false;
...
> {
  const fs = resolveFs(input.overrides);
  const journalPath = resolve(input.root, phase697V7JournalPath(input.marker.runId));
  ...
  try {
    ...
    const initial = buildPhase697V7JournalRecord({
    ...
    await handle.writeFile(`${stablePhase697V7JsonStringify(initial)}\n`, 'utf8');
    await handle.sync();
    return {
      ok: true,
...
export async function acquirePhase697V7RecoveryClaim(input: {
  root: string;
  marker: Readonly<Phase697V7Marker>;
  markerSha256: string;
  journalTailSha256: string | null;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<Phase697V7RecoveryClaimResult> {
  const fs = resolveFs(input.overrides);
  if (isProcessAlive(input.marker.ownerProcessId, input.overrides?.processAlive)) {
    return { ok: false, code: 'live_attempt_in_progress' };
  }
  const claimPath = resolve(input.root, phase697V7RecoveryClaimPath(input.marker.runId));
  ...
  const record = PHASE_6_9_7_V7_RECOVERY_CLAIM_SCHEMA.safeParse({
    claimVersion: PHASE_6_9_7_V7_RECOVERY_CLAIM_VERSION,
  ...
  const bytes = `${stablePhase697V7JsonStringify(record.data)}\n`;
...
export async function openPhase697V7JournalAppender(input: {
  root: string;
  journal: Readonly<Phase697V7ValidatedJournal>;
  claim: Phase697V7RecoveryClaim;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V7JournalWriter }>
  | Readonly<{
      ok: false;
      code:
...
> {
  const fs = resolveFs(input.overrides);
  const path = resolve(input.root, phase697V7JournalPath(input.journal.runId));
      ...
      writer: createJournalWriter({
        ...
        beforeAppend: async (expected) => {
          if (!(await input.claim.assertOwned())) {
            throw new Error('PHASE_6_9_7_V7_RECOVERY_CLAIM_LOST');
          }
          const currentTail = await verifyJournalTail(path, expected, fs);
          if (currentTail !== 'match') {
            throw new Error('PHASE_6_9_7_V7_JOURNAL_TAIL_DRIFT');
          }
        },
      }),
    };
...
export async function readPhase697V7Marker(input: {
  root: string;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<
  | Readonly<{
...
> {
  ...
  try {
    ...
    const marker = PHASE_6_9_7_V7_MARKER_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
    return marker.success
      ? { ok: true, marker: marker.data, markerSha256: sha256Phase697V7Bytes(bytes), markerPath }
...
export async function readPhase697V7Journal(input: {
  root: string;
  runId: string;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; journal: Phase697V7ValidatedJournal; journalPath: string }>
  | Readonly<{
      ok: false;
      code:
        | 'journal_missing'
...
> {
  const fs = resolveFs(input.overrides);
  const journalPath = resolve(input.root, phase697V7JournalPath(input.runId));
  try {
    ...
    const journal = parseAndValidatePhase697V7Journal(text);
    return journal
...
export async function publishPhase697V7Evidence(input: {
  root: string;
  evidencePath: string;
...
> {
  const validated = PHASE_6_9_7_V7_EVIDENCE_ENVELOPE_SCHEMA.safeParse(input.envelope);
  if (!validated.success || hasSensitivePhase697Evidence(validated.data)) {
    return { ok: false, code: 'evidence_contract_invalid' };
  }
  const fs = resolveFs(input.overrides);
  const expectedPath = phase697V7EvidencePath({
  ...
  const evidenceSha256 = sha256Phase697V7Bytes(bytes);
...
export async function readPhase697V7EvidenceBytes(input: {
  path: string;
  overrides?: Phase697V7DurabilityFsOverrides;
}): Promise<
  | Readonly<{
      ...
      envelope: Phase697V7EvidenceEnvelope;
    }>
  | Readonly<{
      ok: false;
...
> {
  ...
  try {
    ...
    const parsed = PHASE_6_9_7_V7_EVIDENCE_ENVELOPE_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
    return parsed.success && !hasSensitivePhase697Evidence(parsed.data)
      ? { ok: true, bytes, evidenceSha256: sha256Phase697V7Bytes(bytes), envelope: parsed.data }
...
function createJournalWriter(input: {
  ...
  const append = (payload: Phase697V7JournalPayload): Promise<Phase697V7JournalRecord> => {
    if (!accepting || closed) {
      return Promise.reject(new Error('PHASE_6_9_7_V7_JOURNAL_UNAVAILABLE'));
    }
    let result: Phase697V7JournalRecord | null = null;
    const operation = queue.then(async () => {
      if (failed) throw new Error('PHASE_6_9_7_V7_JOURNAL_UNAVAILABLE');
      await input.beforeAppend?.({ tailSha256, lastSequence: sequence });
      result = buildPhase697V7JournalRecord({
        runId: input.runId,
        sequence: sequence + 1,
      ...
      try {
        await input.handle.writeFile(`${stablePhase697V7JsonStringify(result)}\n`, 'utf8');
  ...
  return Object.freeze({
    ...
    async snapshot() {
      await queue;
      if (failed) throw new Error('PHASE_6_9_7_V7_JOURNAL_FAILED');
...
function assertSafeJournalRecord(record: Phase697V7JournalRecord): void {
  if (
    !PHASE_6_9_7_V7_JOURNAL_RECORD_SCHEMA.safeParse(record).success ||
    hasSensitivePhase697Evidence(record)
  ) {
    throw new Error('PHASE_6_9_7_V7_JOURNAL_CONTRACT_INVALID');
  }
}
...
async function readRecoveryClaimFile(input: {
  ...
  | Readonly<{ ok: true; bytes: string; record: Phase697V7RecoveryClaimRecord }>
...
> {
  try {
    ...
    const parsed = PHASE_6_9_7_V7_RECOVERY_CLAIM_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
async function createRecoveryClaimLease(input: {
  ...
  bytes: string;
  fs: ResolvedFs;
}): Promise<Phase697V7RecoveryClaim | null> {
...
function resolveFs(overrides: Phase697V7DurabilityFsOverrides | undefined) {
  return {
