...
import {
  ...
  PHASE_6_9_7_V6_JOURNAL_RECORD_SCHEMA,
  ...
  phase697V6RecoveryClaimPath,
  sha256Bytes,
  stableJsonStringify,
  type Phase697V6EvidenceEnvelope,
  type Phase697V6JournalPayload,
  type Phase697V6JournalRecord,
  type Phase697V6Marker,
  type Phase697V6RecoveryClaimRecord,
  type Phase697V6ValidatedJournal,
...
export type Phase697V6DurabilityFsOverrides = Readonly<
  Partial<{
    open: typeof fsOpen;
...
export type Phase697V6JournalWriter = Readonly<{
  path: string;
  append(payload: Phase697V6JournalPayload): Promise<Phase697V6JournalRecord>;
  snapshot(): Promise<Readonly<{ tailSha256: string; lastSequence: number }>>;
  close(): Promise<void>;
}>;

export type Phase697V6RecoveryClaim = Readonly<{
  path: string;
  runId: string;
  ownerToken: string;
...
export type Phase697V6RecoveryClaimResult =
  | Readonly<{ ok: true; claim: Phase697V6RecoveryClaim }>
  | Readonly<{
      ok: false;
      code: 'live_attempt_in_progress' | 'recovery_claim_io_failed' | 'recovery_claim_path_invalid';
    }>;

export type Phase697V6MarkerReservationResult =
  | Readonly<{
      ok: true;
      marker: Phase697V6Marker;
      markerSha256: string;
      markerPath: string;
    }>
  | Readonly<{
...
export async function reservePhase697V6Marker(input: {
  root: string;
  marker: Readonly<Phase697V6Marker>;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<Phase697V6MarkerReservationResult> {
  const fs = resolveFs(input.overrides);
  const markerPath = resolve(input.root, PHASE_6_9_7_V6_MARKER_PATH);
  ...
  try {
    ...
    return {
      ok: true,
      marker: PHASE_6_9_7_V6_MARKER_SCHEMA.parse(input.marker),
...
export async function createPhase697V6Journal(input: {
  root: string;
  marker: Readonly<Phase697V6Marker>;
  markerSha256: string;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V6JournalWriter; journalPath: string }>
  | Readonly<{
      ok: false;
...
> {
  const fs = resolveFs(input.overrides);
  const relativePath = phase697V6JournalPath(input.marker.runId);
  ...
  try {
    ...
    const initial = buildPhase697V6JournalRecord({
      ...
      payload: {
        ...
        mode: 'live',
        datasetBindingVersion: input.marker.datasetBindingVersion,
        datasetBindingSha256: input.marker.datasetBindingSha256,
...
export async function acquirePhase697V6RecoveryClaim(input: {
  root: string;
  marker: Readonly<Phase697V6Marker>;
  markerSha256: string;
  journalTailSha256: string | null;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<Phase697V6RecoveryClaimResult> {
  const fs = resolveFs(input.overrides);
  if (isProcessAlive(input.marker.ownerProcessId, input.overrides?.processAlive)) {
    return { ok: false, code: 'live_attempt_in_progress' };
  }
  const claimPath = resolve(input.root, phase697V6RecoveryClaimPath(input.marker.runId));
  ...
  const claimRecord = PHASE_6_9_7_V6_RECOVERY_CLAIM_SCHEMA.safeParse({
...
export async function openPhase697V6JournalAppender(input: {
  root: string;
  journal: Readonly<Phase697V6ValidatedJournal>;
  claim: Phase697V6RecoveryClaim;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V6JournalWriter }>
  | Readonly<{
      ok: false;
      code:
        | 'journal_io_failed'
...
> {
  const fs = resolveFs(input.overrides);
  const path = resolve(input.root, phase697V6JournalPath(input.journal.runId));
      ...
      writer: createJournalWriter({
        ...
        beforeAppend: async (expected) => {
          if (!(await input.claim.assertOwned())) {
            throw new Error('PHASE_6_9_7_V6_RECOVERY_CLAIM_LOST');
          }
          const currentTail = await verifyJournalTail(path, expected, fs);
          if (currentTail !== 'match') {
            throw new Error('PHASE_6_9_7_V6_JOURNAL_TAIL_DRIFT');
          }
        },
      }),
    };
...
export async function readPhase697V6Marker(input: {
  root: string;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; marker: Phase697V6Marker; markerSha256: string; markerPath: string }>
  | Readonly<{
      ok: false;
      code:
        | 'marker_missing'
...
> {
  ...
  try {
    ...
    const marker = PHASE_6_9_7_V6_MARKER_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
export async function readPhase697V6Journal(input: {
  root: string;
  runId: string;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; journal: Phase697V6ValidatedJournal; journalPath: string }>
  | Readonly<{
      ok: false;
      code:
        | 'journal_missing'
...
> {
  const fs = resolveFs(input.overrides);
  const journalPath = resolve(input.root, phase697V6JournalPath(input.runId));
  try {
    ...
    const journal = parseAndValidatePhase697V6Journal(text);
    return journal
...
export async function publishPhase697V6Evidence(input: {
  root: string;
  evidencePath: string;
...
> {
  const validated = PHASE_6_9_7_V6_EVIDENCE_ENVELOPE_SCHEMA.safeParse(input.envelope);
  if (!validated.success || hasSensitivePhase697Evidence(validated.data)) {
    return { ok: false, code: 'evidence_contract_invalid' };
  }
  const fs = resolveFs(input.overrides);
  const expectedEvidencePath = phase697V6EvidencePath({
...
export async function readPhase697V6EvidenceBytes(input: {
  path: string;
  overrides?: Phase697V6DurabilityFsOverrides;
}): Promise<
  | Readonly<{
      ...
      envelope: Phase697V6EvidenceEnvelope;
    }>
  | Readonly<{
      ok: false;
...
> {
  ...
  try {
    ...
    const parsed = PHASE_6_9_7_V6_EVIDENCE_ENVELOPE_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
function createJournalWriter(input: {
  ...
  const append = (payload: Phase697V6JournalPayload): Promise<Phase697V6JournalRecord> => {
    if (!accepting || closed) {
      return Promise.reject(new Error('PHASE_6_9_7_V6_JOURNAL_UNAVAILABLE'));
    }
    let result: Phase697V6JournalRecord | null = null;
    const operation = queue.then(async () => {
      if (failed) throw new Error('PHASE_6_9_7_V6_JOURNAL_UNAVAILABLE');
      await input.beforeAppend?.({ tailSha256, lastSequence: sequence });
      result = buildPhase697V6JournalRecord({
        runId: input.runId,
        sequence: sequence + 1,
  ...
  return Object.freeze({
    ...
    async snapshot() {
      await queue;
      if (failed) throw new Error('PHASE_6_9_7_V6_JOURNAL_FAILED');
...
function assertSafeJournalRecord(record: Phase697V6JournalRecord): void {
  if (
    !PHASE_6_9_7_V6_JOURNAL_RECORD_SCHEMA.safeParse(record).success ||
    hasSensitivePhase697Evidence(record)
  ) {
    throw new Error('PHASE_6_9_7_V6_JOURNAL_CONTRACT_INVALID');
  }
}

type Phase697V6ResolvedFs = ReturnType<typeof resolveFs>;

async function createRecoveryClaimFile(input: {
  path: string;
  bytes: string;
  fs: Phase697V6ResolvedFs;
...
async function readRecoveryClaimFile(input: {
  ...
  | Readonly<{
      ok: true;
      bytes: string;
      record: Phase697V6RecoveryClaimRecord;
    }>
...
> {
  try {
    ...
    const parsed = PHASE_6_9_7_V6_RECOVERY_CLAIM_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
async function createRecoveryClaimLease(input: {
  ...
  bytes: string;
  fs: Phase697V6ResolvedFs;
}): Promise<Phase697V6RecoveryClaim | null> {
...
async function readFileIdentity(path: string, fs: Phase697V6ResolvedFs): Promise<string | null> {
  try {
    const stat = await fs.lstat(path);
...
function resolveFs(overrides: Phase697V6DurabilityFsOverrides | undefined) {
  return {
