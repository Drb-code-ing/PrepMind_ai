...
import { PHASE_6_9_7_V9_MARKER_SCHEMA } from '../src/evals/phase-6-9-tutor-wrong-question-v9-durability-contract.ts';
import {
  readPhase697V9EvidenceBytes,
...
const PRIOR_ARTIFACT_TOKENS = Object.freeze([
  ...
  ...Array.from({ length: 8 }, (_, index) => `phase-6.9.7-v${index + 1}-runtime-evidence`),
  ...Array.from({ length: 8 }, (_, index) => `phase-6.9.7-v${index + 1}-live-marker`),
  ...Array.from({ length: 8 }, (_, index) => `phase-6.9.7-v${index + 1}-journal`),
  ...Array.from({ length: 8 }, (_, index) => `phase-6.9.7-v${index + 1}-evidence-envelope`),
  ...Array.from({ length: 8 }, (_, index) => `phase-6.9.7-v${index + 1}-recovery-claim`),
]);

const PRIOR_ARTIFACT_PATH = /(?:^|[\\/.-])phase-6-9-7-tutor-organizer-v[1-8](?:[\\/.-]|$)/u;
const PRIOR_ARTIFACT_KEY = /^(?:sourceV[1-8].*|partialMetrics|partialUsage|partialCost|legacy.*)$/u;

export type Phase697V9EvidenceValidationResult =
  | Readonly<{ ok: true }>
  | Readonly<{
      ok: false;
      code:
...
export function validatePhase697TutorOrganizerV9EvidenceValue(
  value: unknown,
): Phase697V9EvidenceValidationResult {
  if (hasPriorPhase697V9ArtifactLineage(value)) {
    return { ok: false, code: 'prior_lineage_detected' };
  }
  if (hasSensitivePhase697Evidence(value)) return { ok: false, code: 'sensitive_evidence' };
  return PHASE_6_9_7_V9_EVIDENCE_ENVELOPE_SCHEMA.safeParse(value).success
    ? { ok: true }
    : { ok: false, code: 'report_contract_invalid' };
}

export function hasPriorPhase697V9ArtifactLineage(value: unknown): boolean {
...
export async function validatePhase697TutorOrganizerV9EvidenceFile(input: {
  path: string;
}): Promise<Phase697V9EvidenceValidationResult> {
  const read = await readPhase697V9Evidence(input.path);
...
export async function validatePhase697TutorOrganizerV9EvidenceBundle(input: {
  root: string;
  evidencePath: string;
}): Promise<Phase697V9EvidenceValidationResult> {
  const filename = await validatePhase697TutorOrganizerV9EvidenceFile({ path: input.evidencePath });
  if (!filename.ok) return filename;
  const evidenceFile = await readPhase697V9EvidenceBytes({ path: input.evidencePath });
  if (!evidenceFile.ok) {
    return {
      ok: false,
      code:
  ...
  const valueValidation = validatePhase697TutorOrganizerV9EvidenceValue(envelope);
  ...
  const marker = await readPhase697V9Marker({ root: input.root });
  ...
  if (
    ...
    !PHASE_6_9_7_V9_MARKER_SCHEMA.safeParse(marker.marker).success
  ) {
    return { ok: false, code: 'durability_identity_invalid' };
  }

  const journal = await readPhase697V9Journal({ root: input.root, runId: envelope.runId });
  ...
  if (
    ...
    (completed !== null &&
      (completed.reportSha256 !== sha256Phase697V9Stable(envelope.report) ||
...
export async function validatePhase697TutorOrganizerV9EvidenceFiles(
  paths: readonly string[],
): Promise<Phase697V9EvidenceValidationResult> {
  if (paths.length === 0) return { ok: false, code: 'evidence_read_failed' };
  const runIds: string[] = [];
  for (const path of paths) {
    const valid = await validatePhase697TutorOrganizerV9EvidenceFile({ path });
    if (!valid.ok) return valid;
    const read = await readPhase697V9Evidence(path);
    if (!read.ok) return read;
...
async function readPhase697V9Evidence(
  path: string,
): Promise<
  | Readonly<{ ok: true; envelope: Phase697V9EvidenceEnvelope }>
  | Exclude<Phase697V9EvidenceValidationResult, Readonly<{ ok: true }>>
> {
  try {
    ...
    const valid = validatePhase697TutorOrganizerV9EvidenceValue(value);
    if (!valid.ok) return valid;
    const parsed = PHASE_6_9_7_V9_EVIDENCE_ENVELOPE_SCHEMA.safeParse(value);
...
if (import.meta.main) {
  ...
  const result =
    paths.length === 0
      ...
      : await validatePhase697TutorOrganizerV9EvidenceFiles(paths);
