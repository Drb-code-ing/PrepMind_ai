...
import {
  ...
  PHASE_6_9_7_V9_JOURNAL_RECORD_SCHEMA,
...
export type Phase697V9DurabilityFsOverrides = Readonly<
  Partial<{
    open: typeof fsOpen;
...
export type Phase697V9JournalWriter = Readonly<{
  path: string;
  append(payload: Phase697V9JournalPayload): Promise<Phase697V9JournalRecord>;
  snapshot(): Promise<Readonly<{ tailSha256: string; lastSequence: number }>>;
  close(): Promise<void>;
}>;

export type Phase697V9RecoveryClaim = Readonly<{
  path: string;
  runId: string;
  ownerToken: string;
...
export type Phase697V9RecoveryClaimResult =
  | Readonly<{ ok: true; claim: Phase697V9RecoveryClaim }>
  | Readonly<{
      ok: false;
      code: 'live_attempt_in_progress' | 'recovery_claim_io_failed' | 'recovery_claim_path_invalid';
    }>;

export type Phase697V9MarkerReservationResult =
  | Readonly<{
      ok: true;
      marker: Phase697V9Marker;
      markerSha256: string;
      markerPath: string;
    }>
  | Readonly<{
...
export async function reservePhase697V9Marker(input: {
  root: string;
  marker: Readonly<Phase697V9Marker>;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<Phase697V9MarkerReservationResult> {
  const fs = resolveFs(input.overrides);
  const markerPath = resolve(input.root, PHASE_6_9_7_V9_MARKER_PATH);
  if (!(await ensurePlainDirectory(dirname(markerPath), fs))) {
    return { ok: false, code: 'marker_path_invalid' };
  }
  const bytes = `${stablePhase697V9JsonStringify(input.marker)}\n`;
  ...
  try {
    ...
    return {
      ok: true,
      marker: PHASE_6_9_7_V9_MARKER_SCHEMA.parse(input.marker),
      markerSha256: sha256Phase697V9Bytes(bytes),
      markerPath,
    };
  } catch (error) {
...
export async function createPhase697V9Journal(input: {
  root: string;
  marker: Readonly<Phase697V9Marker>;
  markerSha256: string;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V9JournalWriter; journalPath: string }>
  | Readonly<{
      ok: false;
...
> {
  const fs = resolveFs(input.overrides);
  const journalPath = resolve(input.root, phase697V9JournalPath(input.marker.runId));
  ...
  try {
    ...
    const initial = buildPhase697V9JournalRecord({
    ...
    await handle.writeFile(`${stablePhase697V9JsonStringify(initial)}\n`, 'utf8');
    await handle.sync();
    return {
      ok: true,
...
export async function acquirePhase697V9RecoveryClaim(input: {
  root: string;
  marker: Readonly<Phase697V9Marker>;
  markerSha256: string;
  journalTailSha256: string | null;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<Phase697V9RecoveryClaimResult> {
  const fs = resolveFs(input.overrides);
  if (isProcessAlive(input.marker.ownerProcessId, input.overrides?.processAlive)) {
    return { ok: false, code: 'live_attempt_in_progress' };
  }
  const claimPath = resolve(input.root, phase697V9RecoveryClaimPath(input.marker.runId));
  ...
  const record = PHASE_6_9_7_V9_RECOVERY_CLAIM_SCHEMA.safeParse({
    claimVersion: PHASE_6_9_7_V9_RECOVERY_CLAIM_VERSION,
  ...
  const bytes = `${stablePhase697V9JsonStringify(record.data)}\n`;
...
export async function openPhase697V9JournalAppender(input: {
  root: string;
  journal: Readonly<Phase697V9ValidatedJournal>;
  claim: Phase697V9RecoveryClaim;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V9JournalWriter }>
  | Readonly<{
      ok: false;
      code:
        | 'journal_io_failed'
...
> {
  const fs = resolveFs(input.overrides);
  const path = resolve(input.root, phase697V9JournalPath(input.journal.runId));
      ...
      writer: createJournalWriter({
        ...
        beforeAppend: async (expected) => {
          if (!(await input.claim.assertOwned())) {
            throw new Error('PHASE_6_9_7_V9_RECOVERY_CLAIM_LOST');
          }
          const currentTail = await verifyJournalTail(path, expected, fs);
          if (currentTail !== 'match') {
            throw new Error('PHASE_6_9_7_V9_JOURNAL_TAIL_DRIFT');
          }
        },
      }),
    };
...
export async function readPhase697V9Marker(input: {
  root: string;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<
  | Readonly<{
...
> {
  ...
  try {
    ...
    const marker = PHASE_6_9_7_V9_MARKER_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
    return marker.success
      ? { ok: true, marker: marker.data, markerSha256: sha256Phase697V9Bytes(bytes), markerPath }
...
export async function readPhase697V9Journal(input: {
  root: string;
  runId: string;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; journal: Phase697V9ValidatedJournal; journalPath: string }>
  | Readonly<{
      ok: false;
      code:
        | 'journal_missing'
...
> {
  const fs = resolveFs(input.overrides);
  const journalPath = resolve(input.root, phase697V9JournalPath(input.runId));
  try {
    ...
    const journal = parseAndValidatePhase697V9Journal(text);
    return journal
...
export async function publishPhase697V9Evidence(input: {
  root: string;
  evidencePath: string;
...
> {
  const validated = PHASE_6_9_7_V9_EVIDENCE_ENVELOPE_SCHEMA.safeParse(input.envelope);
  if (!validated.success || hasSensitivePhase697Evidence(validated.data)) {
    return { ok: false, code: 'evidence_contract_invalid' };
  }
  const fs = resolveFs(input.overrides);
  const expectedPath = phase697V9EvidencePath({
  ...
  const evidenceSha256 = sha256Phase697V9Bytes(bytes);
...
export async function readPhase697V9EvidenceBytes(input: {
  path: string;
  overrides?: Phase697V9DurabilityFsOverrides;
}): Promise<
  | Readonly<{
      ...
      envelope: Phase697V9EvidenceEnvelope;
    }>
  | Readonly<{
      ok: false;
...
> {
  ...
  try {
    ...
    const parsed = PHASE_6_9_7_V9_EVIDENCE_ENVELOPE_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
    return parsed.success && !hasSensitivePhase697Evidence(parsed.data)
      ? { ok: true, bytes, evidenceSha256: sha256Phase697V9Bytes(bytes), envelope: parsed.data }
...
function createJournalWriter(input: {
  ...
  const append = (payload: Phase697V9JournalPayload): Promise<Phase697V9JournalRecord> => {
    if (!accepting || closed) {
      return Promise.reject(new Error('PHASE_6_9_7_V9_JOURNAL_UNAVAILABLE'));
    }
    let result: Phase697V9JournalRecord | null = null;
    const operation = queue.then(async () => {
      if (failed) throw new Error('PHASE_6_9_7_V9_JOURNAL_UNAVAILABLE');
      await input.beforeAppend?.({ tailSha256, lastSequence: sequence });
      result = buildPhase697V9JournalRecord({
        runId: input.runId,
        sequence: sequence + 1,
      ...
      try {
        await input.handle.writeFile(`${stablePhase697V9JsonStringify(result)}\n`, 'utf8');
  ...
  return Object.freeze({
    ...
    async snapshot() {
      await queue;
      if (failed) throw new Error('PHASE_6_9_7_V9_JOURNAL_FAILED');
...
function assertSafeJournalRecord(record: Phase697V9JournalRecord): void {
  if (
    !PHASE_6_9_7_V9_JOURNAL_RECORD_SCHEMA.safeParse(record).success ||
    hasSensitivePhase697Evidence(record)
  ) {
    throw new Error('PHASE_6_9_7_V9_JOURNAL_CONTRACT_INVALID');
  }
}
...
async function readRecoveryClaimFile(input: {
  ...
  | Readonly<{ ok: true; bytes: string; record: Phase697V9RecoveryClaimRecord }>
...
> {
  try {
    ...
    const parsed = PHASE_6_9_7_V9_RECOVERY_CLAIM_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
async function createRecoveryClaimLease(input: {
  ...
  bytes: string;
  fs: ResolvedFs;
}): Promise<Phase697V9RecoveryClaim | null> {
...
function resolveFs(overrides: Phase697V9DurabilityFsOverrides | undefined) {
  return {
