...
import {
  ...
  PHASE_6_9_7_V3_JOURNAL_RECORD_SCHEMA,
  PHASE_6_9_7_V3_MARKER_PATH,
  PHASE_6_9_7_V3_MARKER_SCHEMA,
  PHASE_6_9_7_V3_RECOVERY_CLAIM_SCHEMA,
  PHASE_6_9_7_V3_RECOVERY_CLAIM_VERSION,
  ...
  type Phase697V3JournalRecord,
  type Phase697V3Marker,
  type Phase697V3RecoveryClaimRecord,
...
export type Phase697V3DurabilityFsOverrides = Readonly<
  Partial<{
    open: typeof fsOpen;
...
export type Phase697V3JournalWriter = Readonly<{
  path: string;
  append(payload: Phase697V3JournalPayload): Promise<Phase697V3JournalRecord>;
  snapshot(): Promise<Readonly<{ tailSha256: string; lastSequence: number }>>;
  close(): Promise<void>;
}>;

export type Phase697V3RecoveryClaim = Readonly<{
  path: string;
  runId: string;
  ownerToken: string;
...
export type Phase697V3RecoveryClaimResult =
  | Readonly<{ ok: true; claim: Phase697V3RecoveryClaim }>
  | Readonly<{
      ok: false;
      code: 'live_attempt_in_progress' | 'recovery_claim_io_failed' | 'recovery_claim_path_invalid';
    }>;

export type Phase697V3MarkerReservationResult =
  | Readonly<{
      ok: true;
      marker: Phase697V3Marker;
      markerSha256: string;
      markerPath: string;
    }>
  | Readonly<{
...
export async function reservePhase697V3Marker(input: {
  root: string;
  marker: Readonly<Phase697V3Marker>;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<Phase697V3MarkerReservationResult> {
  const fs = resolveFs(input.overrides);
  const markerPath = resolve(input.root, PHASE_6_9_7_V3_MARKER_PATH);
  ...
  try {
    ...
    return {
      ok: true,
      marker: PHASE_6_9_7_V3_MARKER_SCHEMA.parse(input.marker),
...
export async function createPhase697V3Journal(input: {
  root: string;
  marker: Readonly<Phase697V3Marker>;
  markerSha256: string;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V3JournalWriter; journalPath: string }>
  | Readonly<{
      ok: false;
...
> {
  const fs = resolveFs(input.overrides);
  const relativePath = phase697V3JournalPath(input.marker.runId);
  ...
  try {
    ...
    const initial = buildPhase697V3JournalRecord({
...
export async function acquirePhase697V3RecoveryClaim(input: {
  root: string;
  marker: Readonly<Phase697V3Marker>;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<Phase697V3RecoveryClaimResult> {
  const fs = resolveFs(input.overrides);
  if (isProcessAlive(input.marker.ownerProcessId, input.overrides?.processAlive)) {
    return { ok: false, code: 'live_attempt_in_progress' };
  }
  const claimPath = resolve(input.root, phase697V3RecoveryClaimPath(input.marker.runId));
  ...
  const claimRecord = PHASE_6_9_7_V3_RECOVERY_CLAIM_SCHEMA.safeParse({
    claimVersion: PHASE_6_9_7_V3_RECOVERY_CLAIM_VERSION,
...
export async function openPhase697V3JournalAppender(input: {
  root: string;
  journal: Readonly<Phase697V3ValidatedJournal>;
  claim: Phase697V3RecoveryClaim;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; writer: Phase697V3JournalWriter }>
  | Readonly<{
      ok: false;
...
> {
  const fs = resolveFs(input.overrides);
  const path = resolve(input.root, phase697V3JournalPath(input.journal.runId));
      ...
      writer: createJournalWriter({
        ...
        beforeAppend: async () => {
          if (!(await input.claim.assertOwned())) {
            throw new Error('PHASE_6_9_7_V3_RECOVERY_CLAIM_LOST');
          }
        },
      }),
    };
...
export async function readPhase697V3Marker(input: {
  root: string;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; marker: Phase697V3Marker; markerSha256: string; markerPath: string }>
  | Readonly<{
      ok: false;
      code:
        | 'marker_missing'
...
> {
  ...
  try {
    ...
    const marker = PHASE_6_9_7_V3_MARKER_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
export async function readPhase697V3Journal(input: {
  root: string;
  runId: string;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<
  | Readonly<{ ok: true; journal: Phase697V3ValidatedJournal; journalPath: string }>
  | Readonly<{
      ok: false;
      code:
        | 'journal_missing'
...
> {
  const fs = resolveFs(input.overrides);
  const journalPath = resolve(input.root, phase697V3JournalPath(input.runId));
  try {
    ...
    const journal = parseAndValidatePhase697V3Journal(text);
    return journal
...
export async function publishPhase697V3Evidence(input: {
  root: string;
  evidencePath: string;
...
> {
  const validated = PHASE_6_9_7_V3_EVIDENCE_ENVELOPE_SCHEMA.safeParse(input.envelope);
  if (!validated.success || hasSensitivePhase697Evidence(validated.data)) {
    return { ok: false, code: 'evidence_contract_invalid' };
  }
  const fs = resolveFs(input.overrides);
  const expectedEvidencePath = phase697V3EvidencePath({
...
export async function readPhase697V3EvidenceBytes(input: {
  path: string;
  overrides?: Phase697V3DurabilityFsOverrides;
}): Promise<
  | Readonly<{
      ...
      envelope: Phase697V3EvidenceEnvelope;
    }>
  | Readonly<{
      ok: false;
...
> {
  ...
  try {
    ...
    const parsed = PHASE_6_9_7_V3_EVIDENCE_ENVELOPE_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
function createJournalWriter(input: {
  ...
  const append = (payload: Phase697V3JournalPayload): Promise<Phase697V3JournalRecord> => {
    if (!accepting || closed) {
      return Promise.reject(new Error('PHASE_6_9_7_V3_JOURNAL_UNAVAILABLE'));
    }
    let result: Phase697V3JournalRecord | null = null;
    const operation = queue.then(async () => {
      if (failed) throw new Error('PHASE_6_9_7_V3_JOURNAL_UNAVAILABLE');
      await input.beforeAppend?.();
      result = buildPhase697V3JournalRecord({
        runId: input.runId,
        sequence: sequence + 1,
  ...
  return Object.freeze({
    ...
    async snapshot() {
      await queue;
      if (failed) throw new Error('PHASE_6_9_7_V3_JOURNAL_FAILED');
...
function assertSafeJournalRecord(record: Phase697V3JournalRecord): void {
  if (
    !PHASE_6_9_7_V3_JOURNAL_RECORD_SCHEMA.safeParse(record).success ||
    hasSensitivePhase697Evidence(record)
  ) {
    throw new Error('PHASE_6_9_7_V3_JOURNAL_CONTRACT_INVALID');
  }
}

type Phase697V3ResolvedFs = ReturnType<typeof resolveFs>;

async function createRecoveryClaimFile(input: {
  path: string;
  bytes: string;
  fs: Phase697V3ResolvedFs;
...
async function readRecoveryClaimFile(input: {
  ...
  | Readonly<{
      ok: true;
      bytes: string;
      record: Phase697V3RecoveryClaimRecord;
    }>
...
> {
  try {
    ...
    const parsed = PHASE_6_9_7_V3_RECOVERY_CLAIM_SCHEMA.safeParse(JSON.parse(bytes) as unknown);
...
function resolveFs(overrides: Phase697V3DurabilityFsOverrides | undefined) {
  return {
