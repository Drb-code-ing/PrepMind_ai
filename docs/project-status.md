...
| ChatRunBudget 合同          | `@repo/types`、Prisma schema/migration（已部署到本地 PostgreSQL）、owner-scoped repository、Worker reservation/settlement/terminal reconcile、显式 UNCERTAIN recovery 与 `@repo/agent` typed budget port 已实现 | 隔离数据库并发证据、产品 Agent stage 注入、Trace 对账和真实模型结算未完成                |
...
2. 在隔离数据库补齐 ChatRunBudget Serializable/CAS 并发、crash/recovery 证据，补齐其他 Agent stage 和 Trace 对账（ticket 05）。
