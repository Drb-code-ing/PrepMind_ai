...
| ChatRunBudget 合同          | `@repo/types`、Prisma schema/migration、owner-scoped repository 与 Worker reservation/settlement 已实现       | 隔离数据库并发证据、其他 Agent stage、Trace 对账和真实模型结算未完成                |
...
2. 在隔离数据库完成 ChatRunBudget migration 与 Serializable/CAS 并发验收，补齐其他 Agent stage 和 Trace 对账（ticket 05）。
