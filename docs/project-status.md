...
| ChatRunBudget 合同          | `@repo/types` 与 Prisma schema/migration 已冻结 policy、run ledger、reservation、usage 和 bounded event 边界  | runtime repository、CAS、跨节点上限、Worker/Trace 接入和真实模型结算未完成          |
...
2. 实现 ChatRunBudget runtime repository、Serializable/CAS 跨节点上限与 Trace 对账（ticket 05）；共享类型和 Prisma 结构已完成但尚未接入运行时。
