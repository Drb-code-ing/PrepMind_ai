...
deterministic Worker durable baseline 上补齐 Chat Stream contract、Redis bounded replay、owner-bound 状态查询、认证
`POST /chat-turns` 入队 seam 和 Web bounded enqueue adapter；它仍未切换 `/api/chat` 产品路径，也不是真实模型 Worker。
...
| ChatTurn enqueue API/Web adapter | JWT owner-bound `POST /chat-turns`；Web 生成稳定 hash/request id，只发送 bounded facts，严格要求 `202`       | API 与 adapter 仅为 implemented + mock/static；`/api/chat` bridge、浏览器 replay、Worker 产品切换和真实模型仍未完成                   |
