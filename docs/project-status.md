...
| ChatRunBudget 合同          | `@repo/types`、Prisma schema/migration（已部署到本地 PostgreSQL）、owner-scoped repository、Worker reservation/settlement/terminal reconcile、显式 UNCERTAIN recovery、真实单 ledger 并发验收与 `@repo/agent` typed budget port 已实现 | 跨节点/crash 证据、产品 Agent stage 注入、Trace 对账和真实模型结算未完成                |
...
2. 补齐 ChatRunBudget 跨节点/crash recovery 证据，补齐其他 Agent stage 和 Trace 对账（ticket 05）；单 ledger PostgreSQL 竞争已验收。
