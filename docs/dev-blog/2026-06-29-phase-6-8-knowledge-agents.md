...
```ts
exact_duplicate | possible_revision | complementary | insufficient_signal
```

对应建议动作也被限制在固定枚举里：

```ts
use_existing | replace_old | keep_both | review_manually
```
...
第二层是疑似新版：文件名归一化后高度相似，文件类型一致，`contentHash` 不同，并且更新时间有先后，就认为可能是 `possible_revision`。

第三层是互补资料：主题关键词有重合，但文件名不像同一份资料的版本关系，就认为是 `complementary`。比如“考研数学 极限讲义”和“考研数学 极限练习题”更适合提示“可以都保留”。

如果资料太少、没有处理完成、chunk 摘要不足，就输出 `insufficient_signal`，避免硬猜。
...
```ts
const target = query.documentId
  ? await prisma.document.findFirst({
      where: { id: query.documentId, userId },
...
const documents = target
  ? [target, ...recentDocuments.filter((doc) => doc.id !== target.id)]
  : recentDocuments;
```
