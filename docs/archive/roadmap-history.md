...
> 归档说明：本文保留 2026-08-28 之前的长篇路线与阶段回执，仅用于历史回顾。当前路线以 [`docs/roadmap.md`](../roadmap.md) 和
> [`docs/project-status.md`](../project-status.md) 为准；历史段落中的授权、分支和命令不可直接重跑。
...
统一的 proxy failure；这不是代理 listener、Bun dotenv、cwd、账号或 Provider 根因。

当前分支 `drb/phase-6-9-8-sr5-proxy-port-recovery` 已修复 port 组装，并新增 ready/not-ready 双向 zero-provider 回归：
新的 source contract 预留待创建的 immutable tag identity
`phase-6-9-8-retriever-final-response-schema-recovery-sr5-live-v2-approved`，保留 `live-v1` tag 不动；source manifest=
`sha256:61afe007...fa2829`，Live manifest=`372abb46...df67a4`。focused `16/16`（63 assertions），
SR5 + Task 9B boundary `54/54`（191 assertions），typecheck/lint/diff check 通过，`providerCalls=0 / credentialReads=0 /
formalEvidence=0 / businessWrites=0`；Agent full `1529/1529`（25224 expect()，196 files）。详见
`docs/acceptance/phase-6-9-8-retriever-final-response-schema-recovery-sr5-live-proxy-port-recovery-zero-provider.md`。

该 v2 Git tag 当前尚未创建。源码变化后，历史 SR5 tag 与旧授权不可复用；此前 proxy fail-closed 没有消费一次性名额，formal namespace 仍为 `0`。
固定下一顺序：推送分支 → `--no-ff` 合并并推送 `main` → main 二次 zero-provider → 最终 parity commit 创建新的
annotated tag 并核对 source bundle → 用户重新接受 DeepSeek/Qwen 数据边界并提供绑定新 source 的两行 exact authorization
→ 唯一一次 controlled-Live。成功只形成语义 authority，失败 durable seal 后停止；禁止 retry/replay/curl/单 case/追加探测。

controlled-Live 语义门通过后，另行授权进入 SR6 产品验收（现有 Docker、真实 `/api/chat`、Trace/usage/cost、错误回退、权限
隔离、可见浏览器/PWA，并保留窗口和精确清理证据）。当前尚未做 Docker/API/浏览器产品验收，因此不能宣称产品链路已可用。

## 历史 checkpoint：Phase 6.9.8 Schema Recovery SR5 Live tag compatibility recovery（2026-08-10）
...
That diagnosis is now complete: PowerShell and profile-free Git Bash report `direct_ready`, while the login shell used for v9 injected four proxy variables for unavailable `127.0.0.1:7897` and deterministically returned `loopback_proxy_unavailable`. No source defect exists in the shared fail-closed preflight. The next atomic task is a new-lineage design/implementation decision that binds a native/no-profile launcher and bounded preflight reason before any new tag or authorization; v9 remains non-rerunnable and SR6 remains blocked.

The v10 implementation checkpoint is merged and pushed as `main 95ea523a`. It allocates a new tag/authorization/evidence namespace and carries only strict bounded preflight metadata through CLI v2; malformed or URL-bearing results are not reflected. Merged-main focused `128/128`, Agent full `1658/1658`, typecheck/lint/format/diff, profile-free `direct_ready`, and v10 namespace=0 pass with zero credential/Provider/evidence/business writes. Next: an independent v10 annotated-tag operation and final Git verifier. Fresh authorization and any Live remain later gates; SR6 stays blocked.

The v10 tag/verifier and fresh authorization subsequently passed, but the unique controlled-Live is now sealed as a quality failure. Qwen original retrieval succeeded once; DeepSeek v4-pro candidate terminated as `schema_invalid` after dispatch with no verified response/usage, so the breaker left 22 slots unstarted. `qualityAuthority=none`; retry/recover/appended Provider probes and SR6 product acceptance are prohibited. Next: a new zero-provider postmortem for the DeepSeek schema/adapter boundary, without reconstructing raw Provider content.
