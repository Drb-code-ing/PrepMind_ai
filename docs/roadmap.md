...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition、受控诊断和前端降级状态的工程准备；本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v4 分别以 `invalid_attempted / structured_output` 关闭，v3/v4 为私有 `structuredOutputStage=provider_json_parse`；v5 使用与生产候选对齐的 `deepseek-v4-pro` JSON-object executor，但唯一 canary 仍关闭为 `invalid_attempted / closed / providerAttemptCount=1 / usageKnown=false / structured_output`。V6 不是重试：Task 1--6 已完成独立根因设计、typed non-thinking transport、resolver/factory/evidence/CLI/Mock/复审的离线闭环；只对精确 DeepSeek V4 Pro `/v1` candidate request 写入固定 `thinking:{type:'disabled'}`，并本地拒绝 tool/schema drift 与 reasoning response。用户授权后的唯一 V6 canary 已封存为 `finalized / invalid_attempted / closed / providerAttemptCount=1 / usageKnown=false / usage_unverifiable`。v1--v6 evidence/marker 均不可重跑或拼接，且不构成 quality pass、zero-call、零成本或可用性结论。V6 的 48-case、Docker、浏览器、main 复验或推送均不得执行；最大 23 次 CNY `0.18726` reservation（hard cap CNY `1.00`）不代表实际账单或 Live passed，两个业务 gate 默认保持 `false`。若继续，必须先有新的零网络根因设计与独立复审，再由用户决定是否批准新的隔离 profile。证据见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`。
...
- 当前实现事实：Router/Verifier 已完成混合模型生产接入且默认 gate 已恢复关闭；Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup 与 KnowledgeOrganizer 仍是 deterministic policy。Review/Planner 已有受限只读 candidate，但两个业务 gate 默认关闭；v1--v6 都是关闭且不可重跑的 controlled-Live evidence，最新 V6 为 `usage_unverifiable`，因此当前项目仍返回 deterministic 建议，不能称为模型可用。FinalResponse 由既有 `/api/chat` mock/live 模型链路承担；Retriever 由 Qwen embedding + pgvector/关键词混合检索承担。下一步只能先做新的零网络根因设计与独立复审，不能从 Mock、Docker 或既有 provider profile 推断下一次 Live 授权。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1--v6 controlled-Live 均已独立关闭，V6 唯一 canary 为 `usage_unverifiable`，当前项目保持 deterministic、两个业务 gate 默认 `false`；下一步必须先完成新的零网络根因设计与独立复审，不得重跑 V6。（验收未完成）
...
下一会话可以复制：“请说明 Phase 6.9.5 V6 为什么以 `usage_unverifiable` 终态关闭，并基于现有 evidence 设计新的零网络根因调查和独立复审；不要重跑 V6、不要启动 Docker 或浏览器、不要提前进入分层记忆系统。”
