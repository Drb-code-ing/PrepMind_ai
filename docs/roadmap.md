...
| Phase 6    | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.6 已完成；Phase 6.9.7 V3 R0--R3 已完成，下一步 R4 static/Mock |
...
R7 已失败封存，V3 R0--R3 已完成。下一任务只允许 R4 分支 static/Mock checkpoint 与独立复审；R4 通过后仍必须停止并重新申请精确 V3 branch controlled-Live 授权。V1/V2 均不得重跑，产品验收、Task 13/main 合并和 Phase 6.10 均不得开始。任何未来网络运行都需要独立 marker/journal/evidence 与新的精确授权。
