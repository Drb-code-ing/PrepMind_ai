...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition、受控诊断与 success-seal 工程准备；本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v6 均为不可重跑的关闭终态。用户授权的唯一 V7 Live 已以 `finalized / invalid_attempted / closed / 23 / false / evidence_io` 关闭；once marker 已消费，无 success seal、token/cost 或质量 counters。V1--V6 snapshot 运行后仍为 `9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba` / 18 entries。最窄可证边界是：全部 23 个允许的 provider attempts 被安全计数后，paired-result/orchestration failure 或 evidence finalization/history I/O failure 被折叠为 `evidence_io`；现有脱敏证据不能唯一定位或反推 provider 质量/费用。两个业务 gate 保持 `false`，V7 不可重跑；下一步只能为新 lineage 设计受治理的 stage diagnostics。详细证据见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`。
...
- ReviewAgent / PlannerAgent 当前边界：只读建议，不创建 `ReviewTask(source=PLANNER)`，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck，不进入 Dexie `mutationQueue`。V7 Live 已终态关闭且产品启用未完成；FSRS、容量事实、写操作和用户确认始终由后端控制。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V7 唯一 controlled-Live 已以 `23 / false / evidence_io` 关闭，无质量/费用结论且不可重跑。当前项目保持 deterministic、两个业务 gate 默认 `false`；新 lineage stage diagnostics 待设计。（验收未完成）
...
下一会话可以复制：“请为 Phase 6.9.5 新 lineage 设计零网络 stage diagnostics，解决 V7 `23 / false / evidence_io` 不可唯一定位问题；不得重跑或改写 V7，也不要提前进入分层记忆系统。”
