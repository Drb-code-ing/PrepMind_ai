...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, Memory, Orchestrator, Agent Eval, Layered Memory                                                 | Phase 6.9.4.3 验收未完成；diagnostics contract 已完成，下一步新 controlled-Live |
...
- Phase 6.9.4.3 的 deterministic / Mock 工程、两次 controlled-Live 失败证据检查点与共享 provider diagnostics contract 已完成，但阶段验收未完成。Mock 为 complete，deterministic 仍为 74/100、critical=2；canonical Live 仍为 `incomplete`，`observed/notRun=37/63`、`providerAttempts/strictSuccesses=1/0`，Router/Verifier 均为 `enabled=false / usage_unverifiable`。diagnostics 在 `@repo/ai` 固定八类低基数枚举，默认 dependency identity + 当前 invocation scope + one-shot consume 建立可信 provenance，Error/Trace、strict candidate sanitizer 与 attempted Live `PROVIDER_ERROR` evidence 已接通；raw provider error 被丢弃，custom/injected 只得 `unknown`，无自动重试，历史 A/B 未改写。分类尚未接入生产 Trace API/UI，candidate 尚未接入生产 Chat，production 继续 deterministic。下一任务是在 Phase 6.9.4.3 内从最新 main 发起新的 controlled-Live paired eval；只有 28 次 strict success 与质量、安全、延迟、成本门槛全部通过后才能标记阶段完成。证据见 `docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md`。
...
- Phase 6.9.4.3 后续：核对 diagnostics、Live 双开关与 pricing 后，执行新的完整 controlled-Live paired eval；失败只记录固定分类，不记录 raw error。（下一任务）
