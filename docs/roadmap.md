...
- Phase 6.9.7 Full-gate Schema Recovery SR6：只回放 SR5 sealed authority，完成 Tutor Chat、Organizer single/batch、default-off/forced failure、Trace、owner/locked-name/write isolation、headed browser 与合成数据精确清理；不重跑 Provider。（下一任务）
- Phase 6.9.7 Full-gate Schema Recovery SR7：仅 SR6 完成、提交并推送后，才执行 main 合并、远程推送与 default-off 回放。（阻断）
