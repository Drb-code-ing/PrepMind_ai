...
| Phase 6    | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.7 SR6 分支产品验收已完成；待 SR7/main |
...
- Phase 6.9.7 Full-gate Schema Recovery SR7：仅 SR6 原子提交并推送当前功能分支后，才执行 main 合并、远程推送与 default-off 回放；不重跑 SR5 或再次启用 SR6 replay。（阻断，待 SR6 提交/推送）
