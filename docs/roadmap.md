...
> 当前状态：项目按 Phase 0 ~ Phase 10 顺序推进，当前 Phase 7.4 已完成，后续继续 Phase 7 worker 拆分、durable outbox 和生产化工程增强。
...
| Phase 7 | 工程化增强 | BullMQ, BackgroundJob, RAG SafetyGuard, EventBus, Swagger, Docker | 已启动，Phase 7.4 已完成 |
...
- Swagger / OpenAPI 已完成第一版 debug docs：`/api-docs` 和 `/api-docs-json` 默认在非 production 开启，production 默认关闭；`SWAGGER_ENABLED=true` 只适合受控环境、内网或临时诊断，不放宽 `JwtAuthGuard`。
