...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition、受控诊断和前端降级状态的工程准备；本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1 发现本地 probe 与 canonical Review candidate schema 不匹配后，已在零网络 schema-contract 修复与复审后创建完全隔离的 v2 profile；v3 又在独立的零网络阶段归因设计与复审后创建，且只在 v3 evidence 写入受信内部阶段；v4 在独立的零网络封闭式 JSON 归一化与 stage-provenance 边界复审后创建。v1/v2/v3/v4 均以 `invalid_attempted / structured_output` 关闭，各自 `providerAttemptCount=1`、`usageKnown=false`、`gate=closed`；v3/v4 为 `structuredOutputStage=provider_json_parse`。这不是 quality pass、zero-call、零成本或可用性结论，四组计数不得拼接；v4 不得重试。48-case Live、Docker authenticated suggestions/plan、可见浏览器、合成数据清理、main 复验和推送均未执行。四个原生 once marker 已消耗，当前不得重跑任一 profile 或继续上述验收；两条业务 gate 默认保持 `false`，项目继续使用确定性只读建议。证据见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`。
...
- 独立 Qwen Chat v5 目前只有零网络设计文档：`docs/superpowers/specs/2026-07-17-phase-6-9-5-qwen-controlled-live-v5-design.md`。它不重试或替代 v1--v4；在实现或一次 provider 调用前，仍需受审计的精确 Qwen Chat 价格 profile、来源日期/计量依据与独立总费用上限。
- 当前实现事实：Router/Verifier 已完成混合模型生产接入且默认 gate 已恢复关闭；Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup 与 KnowledgeOrganizer 仍是 deterministic policy。Review/Planner 已有受限只读 candidate，但默认 gate 关闭，v1/v2/v3/v4 controlled-Live 均为 `invalid_attempted / structured_output`（v3/v4 私有 evidence 阶段为 `provider_json_parse`），故当前项目仍返回 deterministic 建议，不能称为模型可用。四个 profile 均不可重跑；两个业务 gate 默认保持 `false`。FinalResponse 由既有 `/api/chat` mock/live 模型链路承担；Retriever 由 Qwen embedding + pgvector/关键词混合检索承担。后续必须先做新的零网络根因设计与复审。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1/v2/v3/v4 controlled-Live 均关闭，v3/v4 的安全 evidence 阶段为 `provider_json_parse`，当前项目保持 deterministic、两个业务 gate 默认 `false`，后续只可先做新的零网络根因设计与复审。（验收未完成）
...
下一会话可以复制：“请基于 Phase 6.9.5 v1/v2/v3/v4 `invalid_attempted / structured_output` 关闭记录（v3/v4 为 `provider_json_parse`），先完成新的零网络根因设计与复审；不要重跑现有 profile、48-case、Docker 或浏览器验收，也不要提前进入分层记忆系统。”
