...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, Memory, Orchestrator, Agent Eval, Layered Memory                                                 | Phase 6.9.4.3 验收未完成；Attempt C=`structured_output`，下一步 output headroom 修复 |
...
- Phase 6.9.4.3 的 deterministic / Mock 工程、三次 controlled-Live 失败证据检查点与共享 provider diagnostics contract 已完成，但阶段验收未完成。Mock 为 complete，deterministic 仍为 74/100、critical=2；2026-07-14 Attempt C 是当前 canonical `incomplete` evidence，`observed/notRun=37/63`、`providerAttempts/strictSuccesses=1/0`、`providerFailureCategory=structured_output`，Router/Verifier 均为 `enabled=false / usage_unverifiable`。历史成功 Router entry 为 `61/120` 与 `108/120` output tokens，显示当前上限缺少稳定余量；raw provider error 仍在 adapter 边界丢弃，custom/injected 只得 `unknown`，无自动重试，历史 A/B 未改写。下一任务从最新 main 按 TDD 同步 Router/Verifier 400-token headroom、全局 output cap 与 pricing/strict evidence contract；修复合并后才允许新的完整 controlled-Live。candidate 尚未接入生产 Chat，production 继续 deterministic；只有 28 次 strict success 与质量、安全、延迟、成本门槛全部通过后才能标记阶段完成。证据见 `docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md`。
...
- Phase 6.9.4.3 后续：按 TDD 修复 Router/Verifier structured-output headroom，同步 400-token 单次上限、全局预算、pricing 与 strict evidence contract；合并后再跑 Live。（下一任务）
