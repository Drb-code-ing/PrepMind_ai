...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.6 已完成；Phase 6.9.7 V1 失败封存，V2 R0--R6 已完成，等待 R7 新授权 |
...
- Phase 6.4 已完成 WrongQuestionOrganizerAgent 的确定性产品能力：`@repo/agent/wrong-question-organizer` 根据错题结构化字段和已有 deck 摘要推荐学科组与专题 deck；Phase 6.9.7 Task 4 又完成 package 级受治理 candidate/merger，Task 6/7 已接入 NestJS owner/write fencing 与 server-only default-off runtime/Trace/HTTP abort。Task 12 V1 已调用真实 provider，但语义质量门失败，尚未进入产品 Docker/API/浏览器验收；V2 R1--R5 只完成 bounded diagnostics、共享规则源、robustness suite 与独立 runner/evidence lineage，不改变该产品可用性结论。
...
- Phase 6.9.7 V2 R6 边界说明：Organizer 当前是同步 API，不冒充 BullMQ/Outbox durable task；R6 证明失败可见、最终写入唯一、未写题可补偿，但不宣称并发多实例 provider exactly-once。contract/security/concurrency/routing 与 operations/acceptance/history 两路终审均 `APPROVED`，无未关闭 Critical/Important。下一步必须先取得新的 V2 branch controlled-Live 精确授权。（已完成）
...
R6 已完成，下一会话不得直接执行真实模型。继续 R7 前需要用户重新接受 DeepSeek 当前账号的数据保留/训练边界，并明确授权唯一一次 `Phase 6.9.7 Tutor/Organizer V2 branch controlled-Live`。V1/V2 均不得擅自重跑；没有该新授权，不读取 credential、不创建 V2 marker/evidence、不调用 provider 或启动产品 Docker/API/浏览器。
