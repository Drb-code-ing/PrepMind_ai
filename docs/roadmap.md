...
> 当前日期：2026-06-14。项目按 Phase 0 ~ Phase 10 顺序推进，当前准备进入 Phase 3。
...
| Phase 3 | AI 讲题系统 | OCR structured output, Prompt, Tool Calling | 下一步 |
...
目标：把当前“OCR Markdown 结果 + 前端解析”升级为更稳定的题目结构化识别和可扩展讲题链路。

优先顺序：

1. 定义 OCR structured output schema。
2. 标准化题目字段：题干、选项、答案、解析、知识点、难度、题型、可保存状态。
3. 设计 AI 讲题 prompt：分步讲解、公式规范、非题目输入门禁、多题输入处理。
4. 规划 tool calling 边界：
   - `createWrongQuestion`
   - `searchKnowledge`
   - `createReviewTask`
5. 明确前端保存错题策略：单题直接保存，多题让用户选择保存单题或批量保存。
