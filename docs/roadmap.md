...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1--v6 controlled-Live 均已独立关闭；V7 已复现 preview/actual usage validator 缺陷并完成 parity + safe usage-shape 设计，尚未实现、Mock 或授权 Live。当前项目保持 deterministic、两个业务 gate 默认 `false`。（验收未完成）
...
下一会话可以复制：“请审阅 Phase 6.9.5 V7 preview/actual usage parity 设计；确认后按 TDD 编写实施计划，不运行 Live、Docker 或浏览器，也不要提前进入分层记忆系统。”
