...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, Memory, Orchestrator, Agent Eval, Layered Memory                                                 | Phase 6.9.3.3 已完成，继续补强 |
...
- Phase 6.9.3.3 已完成 rolling summary/CAS：prepare 按 12 条未覆盖消息或 70% token pressure 触发，只覆盖完整 assistant 轮次；ModelAgentRuntime 调用在事务外，Serializable 事务内复核 source hash 并用 summaryVersion CAS 推进。输入/输出凭据、usage 上限、first-create/update/stale race 均 fail-closed；当前仍未接入 Chat prompt。
...
- Phase 6.9.3.4：Web prepare、分层 context assembler 与 Dexie 恢复。（下一任务）
