...
> 当前状态：Phase 7 核心工程化里程碑已推进至 7.23.8；Phase 7.8.5 RAG runtime parity 补强已完成真实 Docker 验收。Phase 6.9.7 V5 R0 已完成零 Provider 根因取证，下一原子任务为 V5 R1 新建独立 V2 dataset/coherence authority。完成 Phase 6.9 全部 Agent 架构后再进入 Phase 6.10 分层记忆；随后依次进入 Phase 8 性能/PWA、Phase 9 MCP Tool 体系。
...
| Phase 6    | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.6 已完成；Phase 6.9.7 V5 R0 已完成，阶段未完成 |
...
- Phase 6.4 已完成 WrongQuestionOrganizerAgent 的确定性产品能力：`@repo/agent/wrong-question-organizer` 根据错题结构化字段和已有 deck 摘要推荐学科组与专题 deck；Phase 6.9.7 Task 4 又完成 package 级受治理 candidate/merger，Task 6/7 已接入 NestJS owner/write fencing 与 server-only default-off runtime/Trace/HTTP abort。Task 12 V1 与 V2 R7 两条唯一 Live 均质量失败；V2 虽已完成 bounded diagnostics、共享规则源、robustness、独立 lineage、并发/恢复/路由 checkpoint，但 24 个 Organizer runtime 全在结构化对象前失败，仍未进入产品 Docker/API/浏览器验收。
...
- Phase 6.9.7 V5 R0 已完成零 Provider 根因取证。冻结 V1 `tutor-runtime-06` 把中文代数 latest text 与英文微积分 active context 拼接，并因数组奇偶误标为 `en`；但 exact input 差分回归同时证明合法 evidence 在产品 candidate 应用、缺 primary/错误 evidence 才被产品 candidate 拒绝，V4 diagnostic 只是如实投影，不能把 V4 failure 翻案为脚本误判。V4 前 5 对还保留 3 个中文 hint -> general、Organizer topic `2/5` 与一次 `major -> computer` 的真实语义证据。已冻结 V5 R1--R8：独立 V2 dataset/coherence、Tutor 本地 evidence authority + 双语有界选择、Organizer local shortlist + ordinal-only、独立 runner/evidence/static/Mock/新 Live 授权门。下一步仅 R1，仍为 zero-provider。
...
- Phase 6.9.7 V5 R1：显式绑定 Tutor language/exercise family/latest/context，收敛 Organizer structured subject/taxonomy/topic candidate authority，冻结新 SHA/baseline/quality gate。（下一步）
- Phase 6.9.7 Task 13：V3 R5 与 V4 R6 均已失败封存；V4 一次性名额已消费且没有 Live quality authority，V5 只完成 R0。没有新的独立版本 Live quality authority 与产品验收前，不得执行分支收尾、`--no-ff` 合并 main、main default-off 回放或 main 远程推送。（不得开始）
...
V2 R7、V3 R5 与 V4 R6 均已失败封存，各自一次性授权已经消费且不得重跑。V5 R0 已区分 fixture 缺陷、product candidate rejection 与真实语义偏差；产品验收、Task 13/main 合并、Phase 6.10 与博客收尾均不得开始。下一步仅 V5 R1 独立 V2 dataset/coherence authority；当前没有任何新的网络授权。
