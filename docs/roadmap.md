...
下一会话可以复制：“请继续 Phase 6.9.5 V14：V10 是唯一语义质量 authority；V11/V12 是不可重跑 recovery 终态，V13 是 Bun crash 后不可重试且不满足 recovery 的 reservation。Trace duration correlation 已修复，V14 有独立 roots，native sentinel 已证明不写 V11/V12/V13。先跑最终静态与 Docker image 门禁；随后在同目录构建 V14 CJS bundle 并用 Node 执行唯一 branch product，保留可见浏览器。只有 branch `passed` 才能 main replay、合并与 push。”
