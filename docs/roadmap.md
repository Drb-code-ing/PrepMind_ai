...
- Phase 6.9.7 Small-sample S2：reviewed Mock/static、fresh baseline、candidate/adapter/validator/merger 全链、全量静态/历史 parity、三路终审与 Reader Testing。（下一原子任务，zero-provider）
- Phase 6.9.7 Small-sample L2/P2：未来 exact-authorized 唯一 L2 与 sealed 终态后的路线决策；当前均被 S2 阻断。L2 即使通过也只解锁 P2 zero-provider full-gate 设计，不直接授权产品/main。（规划中/被阻断）
