...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition、受控诊断和前端降级状态的工程准备；本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v4 分别以 `invalid_attempted / structured_output` 关闭，v3/v4 为私有 `structuredOutputStage=provider_json_parse`；v5 使用与生产候选对齐的 `deepseek-v4-pro` JSON-object executor，但唯一 canary 仍关闭为 `invalid_attempted / closed / providerAttemptCount=1 / usageKnown=false / structured_output`。五条 evidence/marker 均不可重跑或拼接，且不构成 quality pass、zero-call、零成本或可用性结论。V6 不是重试：Task 1--6 已完成独立根因设计、typed non-thinking transport、resolver/factory/evidence/CLI/Mock/复审的离线闭环；只对精确 DeepSeek V4 Pro `/v1` candidate request 写入固定 `thinking:{type:'disabled'}`，并本地拒绝 tool/schema drift 与 reasoning response。complete evidence 只接受 `not_reported`/`reported_zero` 的安全 reasoning projection，却按完整 completion aggregate 记 CNY；v1--v5 immutable no-reparse snapshot 与 V6 private provisional/seal 防止证据重解析或伪造。V6 尚未创建 evidence/marker、未调用 provider、未运行 48-case、Docker、浏览器、main 复验或推送；最大 23 次 CNY `0.18726` reservation（hard cap CNY `1.00`）不代表实际账单或 Live passed，两个业务 gate 默认保持 `false`。证据见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`。
...
- 当前实现事实：Router/Verifier 已完成混合模型生产接入且默认 gate 已恢复关闭；Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup 与 KnowledgeOrganizer 仍是 deterministic policy。Review/Planner 已有受限只读 candidate，但两个业务 gate 默认关闭；v1--v5 都是关闭的不可重跑 controlled-Live 证据，V6 Task 1--6 虽已完成离线 transport/resolver/factory/evidence/CLI/Mock/复审，仍没有 V6 provider/evidence/Live，因此当前项目仍返回 deterministic 建议，不能称为模型可用。FinalResponse 由既有 `/api/chat` mock/live 模型链路承担；Retriever 由 Qwen embedding + pgvector/关键词混合检索承担。下一步只能由用户另行授权一次 V6 controlled-Live，不能从 Mock、Docker 或既有 provider profile 推断授权。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1--v5 controlled-Live 均已独立关闭，V6 Task 1--6 已完成 non-thinking transport/resolver/factory/evidence/CLI/Mock/复审的离线验证，当前项目保持 deterministic、两个业务 gate 默认 `false`；下一步只能先取得一次独立 Live 授权。（验收未完成）
...
下一会话可以复制：“请说明 Phase 6.9.5 V6 已完成的离线验收、23 次 CNY `0.18726 / 1.00` reservation 和 gate 状态；若我另行明确批准，只执行一次 `bun --filter @repo/server eval:review-planner:live:v6:deepseek-nonthinking -- --confirm-controlled-live-v6-deepseek-v4-pro-nonthinking`，期间不要启动 Docker 或浏览器，也不要提前进入分层记忆系统。”
