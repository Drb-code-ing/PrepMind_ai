...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition 与 V10 最小语义修复。本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限；模型只返回产品实际合并的 `focusIndexes` / `blockOrder`。V1--V9 都是不可重跑的历史，V9 唯一 Live 为 `quality_gate_failed`。V10 唯一 Live 已通过 `23/22`、`48/48` strict/quality、critical `0`、P95 `1465ms`、usage `5764/232` 和 CNY `0.018684/1.00`；V1--V9 manifest 未变，V10 evidence/success seal immutable。两个业务 gate 保持 `false`，下一步是分支 Docker/headed-browser 验收；main 合并/replay 与推送尚未执行。完整结果见 `docs/acceptance/phase-6-9-5-review-planner-v10-offline-checkpoint.md`。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V9 唯一 Live 已 `quality_gate_failed` 封存。V10 唯一 controlled-Live 已通过，分支产品验收、main replay 与 push 尚未执行，产品 gate 默认关闭。（验收未完成）
