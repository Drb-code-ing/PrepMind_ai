...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.4.4 Task 9 已有 evidence/清理但分支验收未通过；先收口浏览器缺口，其后完成全部 Agent |
...
- Phase 6.9.4.4 Task 9 当前已有 evidence、环境恢复与精确清理，但分支验收尚未通过。分支 gates 为 Agent 374/374、AI 151/151、Web 404/404、Server 735 passed / 2 skipped，Mock 定向门禁为 114/114 + 35/35，注入式 fallback 为 11/11。最小 controlled-Live harness 为 5/5 strict success、5 provider attempts、9.8s、Router/Verifier 均 `candidate_applied`；生产可见浏览器唯一 Router eligible 请求则在 5002ms 得到 `fallback_timeout`，HTTP 200、Mock 最终回复与 Trace 继续，按 stop-on-first-failure 没有再发送 Verifier eligible 浏览器请求，RAG conflict/injection 完整 UI 查询也为 0-hit。Docker 已恢复 Mock/default-off，synthetic PostgreSQL/Redis/浏览器数据清理为 0。二者必须并列保留，不能把 harness applied 改写成 browser applied。证据见 `docs/acceptance/2026-07-14-phase-6-9-4-4-router-verifier-production.md`。
...
- Phase 6.9.4.4 Task 9：在分支完成完整 gates、Mock、controlled-Live、Docker、可见浏览器验收、合成数据精确清理和 evidence/current-doc 提交。（evidence/清理已完成；Router timeout、Verifier 未执行、RAG UI 0-hit 待收口）
- Phase 6.9.4.4 Task 10：最终 spec/质量复核、完整分支 gates、`--no-ff` 合并 main、main 静态/controlled-Live/Docker/可见浏览器复验、推送和 SHA 核对。（待 Task 9 分支验收通过）
...
下一会话可以复制：“请继续收口 Phase 6.9.4.4 Task 9 的分支验收缺口：先分析生产浏览器 Router 5002ms timeout 与 RAG UI 0-hit，不重复已有 100-case 或盲目追加 Live；完成 Router/Verifier 可见浏览器验收后再进入 Task 10。不要开始 Phase 6.9.5 或记忆系统。”
