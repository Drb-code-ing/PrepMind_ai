...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V10 唯一 controlled-Live 是唯一语义质量 authority。V11/V12 recovered、V13 interruption reservation、V14/V15/V16 root-absent preflight、V17 wrapper/parser pre-confirmation stop 与 V18 Node-runner root-absent preflight 都不可重跑或恢复。V18 已确认 confirmation argv 正确，但 direct Bun host 与 Node product preflight 的 parity 未被证明。V19 以新 confirmation、Node runner、ledger/recovery/execution/browser/public-evidence roots 隔离，保留 repository-root CWD、allowlist、bridge、resolver、root-or-v1 URL、Flash/Pro、receipt 和 recovery 边界；先使用严格 parser 的只读 Node preflight（owner 前、零资源）验证真实 host path。V19 product/recovery/Docker/browser/API/provider 尚未运行；两个 gate 默认为 false。只有 V19 preflight `ready` 后的 V19 branch `passed` 才能 main replay、合并与 push。（验收未完成）
...
下一会话可以复制：“请继续 Phase 6.9.5 V19：V10 是唯一语义质量 authority；V11--V18 都不可重跑。V19 已离线收口：先在同一 Node runner/strict parser/default host 中执行零资源 read-only preflight，确认 ready 后才执行已经授权的唯一 V19 branch command，保留可见浏览器。只有 branch `passed` 才能 main replay、合并与 push。”
