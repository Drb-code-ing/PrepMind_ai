...
| Phase 6    | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.6 已完成；Phase 6.9.7 V4 R0--R5 已完成，阶段未完成 |
...
- Phase 6.9.7 Task 13：V3 R5 已失败封存，V4 R0--R5 已完成但尚无 Live quality authority；R6--R9 与产品验收没有开始。没有新的独立版本质量 authority 前，不得执行分支收尾、`--no-ff` 合并 main、main default-off 回放或远程推送。（不得开始）
...
V2 R7 与 V3 R5 均已失败封存；V3 一次性授权已经消费，不得重跑。V4 R0--R5 已完成，当前停在 R6 新的精确一次性 V4 Live 授权门前；产品验收、Task 13/main 合并和 Phase 6.10 均不得提前开始。任何 V4 网络运行都必须使用独立 identity/marker/journal/evidence，并重新取得新的精确一次性授权。
