...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, Memory, Orchestrator, Agent Eval, Layered Memory                                                 | Phase 6.9.3.2 已完成，继续补强 |
...
- Phase 6.9.3.2 已完成 state/prepare/cache：鉴权 prepare API 先校验 conversation ownership，再读写 PostgreSQL 状态；Redis 只缓存 tenant-hashed、strict sanitized public state并在任何故障时回源，Chat history 可恢复未过期状态，清会话会级联删除 PG state 并 best-effort 清缓存。本 slice 只复用已有 summary，不生成摘要、不调用模型、不接入 Chat prompt。
...
- Phase 6.9.3.3：滚动摘要、ModelAgentRuntime、source hash 与并发 CAS。（下一任务）
