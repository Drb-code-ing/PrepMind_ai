...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition 与 V10 最小语义修复。本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限；模型只返回产品实际合并的 `focusIndexes` / `blockOrder`。V1--V9 都是不可重跑的历史，V9 唯一 Live 为 `quality_gate_failed`。V10 唯一 controlled-Live 已通过 `23/22`、`48/48` strict/quality、critical `0`、P95 `1465ms`、usage `5764/232` 和 CNY `0.018684/1.00`，并是唯一语义质量 authority；V10 product terminal 保持 recovery-only。V11 execution bridge 已完成独立 success ledger、private manifest、runner adapter、default composition、CLI 与 exact recovery selector 的离线检查。两个业务 gate 继续为 `false`，尚未执行 V11 Docker/browser/provider；独立复审通过后只允许一次 branch command，post-manifest 失败自动 recovery 完成时输出 `operation_failed_recovered` 并停止，只有 `recovery_required` 或 crash 后 preflight 授权时才手工 recovery 一次，成功才进入 main replay、合并和推送。完整离线记录见 `docs/acceptance/phase-6-9-5-review-planner-v11-offline-checkpoint.md`。
...
下一会话可以复制：“请复审 Phase 6.9.5 V11 execution bridge；保持产品 gate=false，仅在通过离线 gates 后执行一次 V11 branch product command。`operation_failed_recovered` 后停止；只有 `recovery_required` 或 crash 后 preflight 授权时才手工 recovery 一次。”
