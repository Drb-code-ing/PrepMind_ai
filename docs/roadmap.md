...
- Phase 6.9.7 Small-sample G1：实现 manifest/baseline/report/scorer/gate 与 oracle/candidate/Mock 单向隔离，正式复现并冻结 subset baseline report SHA；不得读取 credential 或执行 Live。（下一原子任务，zero-provider）
- Phase 6.9.7 Small-sample G2/S2/L2/P2：依次完成 one-shot durability、reviewed Mock/static、未来 exact-authorized 唯一 L2 和 sealed 终态后的路线决策；G1 前均被阻断。L2 即使通过也只解锁 P2 zero-provider full-gate 设计，不直接授权产品/main。（规划中/被阻断）
