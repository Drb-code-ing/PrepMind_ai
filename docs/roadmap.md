...
- 当前实现事实：Router/Verifier 已完成混合模型生产接入且默认 gate 已恢复关闭；Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup 与 KnowledgeOrganizer 仍是 deterministic policy。Review/Planner 已有受限只读 candidate，但两个业务 gate 默认关闭；v1--v5 都是关闭的不可重跑 controlled-Live 证据，V6 只有 typed non-thinking zero-network transport，故当前项目仍返回 deterministic 建议，不能称为模型可用。FinalResponse 由既有 `/api/chat` mock/live 模型链路承担；Retriever 由 Qwen embedding + pgvector/关键词混合检索承担。后续必须先完成 V6 factory/evidence/CLI/Mock/复审，之后再申请一次独立 Live 授权。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1--v5 controlled-Live 均已独立关闭，V6 已完成 non-thinking transport 的零网络验证，当前项目保持 deterministic、两个业务 gate 默认 `false`；后续为 V6 resolver/factory/evidence/CLI/Mock/复审，之后才可申请一次独立 Live。（验收未完成）
...
下一会话可以复制：“请继续 Phase 6.9.5 V6 Task 2：仅让精确 DeepSeek V4 Pro `/v1` Review/Planner resolver 选择 typed non-thinking mode，保持两个业务 gate 为 `false`；不要创建 V6 marker、调用 provider、运行 48-case、Docker 或浏览器，也不要提前进入分层记忆系统。”
