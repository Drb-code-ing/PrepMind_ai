...
> 当前状态：项目按 Phase 0 ~ Phase 10 顺序推进，当前 Phase 7.7 已完成，后续继续 Phase 7 durable outbox、metrics 和更多后台任务生产化增强。
...
| Phase 7 | 工程化增强 | BullMQ, BackgroundJob, RAG SafetyGuard, EventBus, Swagger, Docker, Worker Observability | 已启动，Phase 7.7 已完成 |
...
- EventBus 已完成第一版 in-process 事件地基：发布时隔离 handler 失败并记录脱敏 warning；当前仍不是 durable outbox。
- Swagger / OpenAPI 已完成 debug docs 和首批 request body 示例：`/api-docs` 和 `/api-docs-json` 默认在非 production 开启，production 默认关闭；`SWAGGER_ENABLED=true` 只适合受控环境、内网或临时诊断，不放宽 `JwtAuthGuard`。
- Worker Observability 已完成第一版：worker-only 不监听 HTTP，worker / both 角色写 Redis heartbeat，`/worker-observability/summary` 聚合 BullMQ queue counts、worker heartbeat 和 BackgroundJob summary；production 默认关闭该接口，后续按部署形态补 CLI health check、BullMQ metrics 或容器 readiness。
- Durable outbox / metrics：当事件需要跨进程可靠投递时，再把 in-process EventBus 升级为持久化 outbox 或指标系统接入。
