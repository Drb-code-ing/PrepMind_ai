...
- Phase 6.9.7 Small-sample G2：实现 one-shot runner、durability、journal/marker/artifact/validator/seal；仍不得读取 credential 或调用 Provider。（下一原子任务，zero-provider）
- Phase 6.9.7 Small-sample S2/L2/P2：依次完成 reviewed Mock/static、未来 exact-authorized 唯一 L2 和 sealed 终态后的路线决策；当前均被 G2 阻断。L2 即使通过也只解锁 P2 zero-provider full-gate 设计，不直接授权产品/main。（规划中/被阻断）
