...
下一会话可以复制：“请继续 Phase 6.9.7 Task 5：把 Tutor candidate 接入 Web server-only default-off runtime、Chat 编排与安全 Trace；保持独立 credential/预算和既有 streaming/RAG/abort 语义，不调用真实 provider。”
