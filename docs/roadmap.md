...
下一会话可以复制：“请继续 Phase 6.9.7 Task 6：建立 WrongQuestionOrganizer owner-scoped 不可变快照、双 stale fence 与 model-free 授权写 command；provider 不得位于事务内，不读取或调用真实 provider。”
