...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition、受控诊断和前端降级状态的工程准备；本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v6 都是独立、不可重跑的终态，V6 为 `finalized / invalid_attempted / closed / 1 / false / usage_unverifiable`。V7 Task 1--7 已完成 preview/actual parity、无数值 raw usage audit、immutable evidence、one-shot CLI、composition parity、success seal、全量离线 gate 与两轮独立复审。成功必须由 `success_candidate` 与 exclusive、hash/history-bound seal 共同构成，standalone/mismatched candidate 一律回退为 `evidence_io`。V1--V6 snapshot 为 `9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba` / 18 entries。focused V7 为 AI 190、Server 86、native 15/130 assertions；全量为 AI 190、Agent 406、Server 980 passed/30 skipped、Web 409；两轮复审均 PASS 且无 Critical/Important/Minor。真实 V7 marker/evidence 尚不存在，controlled-Live 未运行且未授权，两个业务 gate 默认保持 `false`。下一步只能在新的单独授权后执行唯一 V7 Live。设计与计划见 `docs/superpowers/specs/2026-07-17-phase-6-9-5-deepseek-v4-pro-usage-recovery-v7-design.md`、`docs/superpowers/plans/2026-07-17-phase-6-9-5-deepseek-v4-pro-usage-recovery-v7.md`。
...
- 当前实现事实：Router/Verifier 已完成混合模型生产接入且默认 gate 已恢复关闭；Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup 与 KnowledgeOrganizer 仍是 deterministic policy。Review/Planner 已有受限只读 candidate，但两个业务 gate 默认关闭；v1--v6 都是关闭且不可重跑的 controlled-Live evidence。V7 Task 1--7 只证明离线工程 ready，新的 controlled-Live 授权仍未发生，因此当前项目仍返回 deterministic 建议，不能称为 Review/Planner 真实模型可用。FinalResponse 由既有 `/api/chat` mock/live 模型链路承担；Retriever 由 Qwen embedding + pgvector/关键词混合检索承担。
...
- ReviewAgent / PlannerAgent 当前边界：只读建议，不创建 `ReviewTask(source=PLANNER)`，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck，不进入 Dexie `mutationQueue`。Phase 6.9.5 已完成受限模型 candidate 与 V7 离线诊断工程，但 controlled-Live 和产品启用未完成；FSRS、容量事实、写操作和用户确认始终由后端控制。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1--v6 controlled-Live 均已独立关闭；V7 Task 1--7 离线工程、success seal、全量门禁与两轮复审已完成，controlled-Live 与产品验收尚未执行。当前项目保持 deterministic、两个业务 gate 默认 `false`。（验收未完成）
...
下一会话可以复制：“我明确授权执行唯一一次 Phase 6.9.5 V7 controlled-Live；不启用产品 gate，严格按 success seal、费用与历史不可变边界收口，也不要提前进入分层记忆系统。”
