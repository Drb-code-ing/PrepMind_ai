...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition 与 V10 最小语义修复。本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限；模型只返回产品实际合并的 `focusIndexes` / `blockOrder`。V1--V9 都是不可重跑的历史，V9 唯一 Live 为 `quality_gate_failed`。V10 唯一 Live 已通过 `23/22`、`48/48` strict/quality、critical `0`、P95 `1465ms`、usage `5764/232` 和 CNY `0.018684/1.00`；V1--V9 manifest 未变，V10 evidence/success seal immutable。旧 V8 branch 产品验收尝试已 recovery-only 归档：preflight 参数遗漏为 `0-call`，首次实际尝试的 runner parse bug 由零新 provider 调用的恢复收口；它不是 V10 Live failure，不可 reset 或重用。两个业务 gate 保持 `false`，下一步是新的隔离 V10 product-acceptance lineage；main 合并/replay 与推送尚未执行。完整结果见 `docs/acceptance/phase-6-9-5-review-planner-v10-offline-checkpoint.md`。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V9 唯一 Live 已 `quality_gate_failed` 封存。V10 唯一 controlled-Live 已通过；旧 V8 branch 产品尝试 recovery-only 关闭，需建立新的隔离 V10 product-acceptance lineage，产品 gate 默认关闭。（验收未完成）
...
下一会话可以复制：“请为 Phase 6.9.5 建立新的隔离 V10 product-acceptance lineage；旧 V8 branch evidence 只读且不得重用，保持产品 gate=false，不提前合并 main。”
