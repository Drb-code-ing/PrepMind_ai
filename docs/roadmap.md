...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition 与 V8 离线工程。本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v8 都是不可重跑的关闭终态；V8 唯一 Live 执行 23 次 provider attempt 后停在 `.stage-080-paired-returned / invalid_response`，没有 `.stage-090` 或 success seal。产品验收、main 合并/replay 与推送均未执行；两个业务 gate 保持 `false`。详细证据见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`。
- 截至 `683a209`，V9 Task 1--5 已离线实现 paired aggregate、durable evidence、一次性 controlled-Live CLI 和 product authority。V9 Live 尚未运行，V9 evidence directory、once marker 与 success seal 不存在；V1--V8 继续只读。独立 V9 eval gate 与 Review/Planner 产品 gate 当前均未设置、缺省关闭，产品仍 deterministic。Product authority 只接受 `finalized / complete / closed / passed + 23 provider / 22 paired admission + lowercase SHA-256`，并要求完整 leaf 全为 Git ordinary `H` 且前后 snapshot 稳定；失败在 ledger、Prisma、Docker、浏览器前阻断，不回退 V8 或 `git show`。离线证据与非结论见 `docs/acceptance/phase-6-9-5-review-planner-v9-offline-checkpoint.md`。
...
- ReviewAgent / PlannerAgent 当前边界：只读建议，不创建 `ReviewTask(source=PLANNER)`，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck，不进入 Dexie `mutationQueue`。V1--V8 是只读历史，V9 Live 尚未运行且产品启用未完成；FSRS、容量事实、写操作和用户确认始终由后端控制。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V9 Task 1--5 已离线实现，但 Live 尚未运行、无 V9 evidence/once/seal，产品 gate 默认关闭且产品验收被阻断。（验收未完成）
...
下一会话可以复制：“请复核 Phase 6.9.5 V9 offline checkpoint 与单独 Live 授权前提；保持产品 gate 关闭，不改写 V1--V8，不提前进入产品验收或分层记忆系统。”
