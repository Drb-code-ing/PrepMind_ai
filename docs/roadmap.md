...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition、受控诊断和前端降级状态的工程准备；本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v4 分别以 `invalid_attempted / structured_output` 关闭，v3/v4 为私有 `structuredOutputStage=provider_json_parse`；v5 使用与生产候选对齐的 `deepseek-v4-pro` JSON-object executor，但唯一 canary 仍关闭为 `invalid_attempted / closed / providerAttemptCount=1 / usageKnown=false / structured_output`。V6 唯一 canary 已封存为 `finalized / invalid_attempted / closed / providerAttemptCount=1 / usageKnown=false / usage_unverifiable`，不可重跑。V7 零网络复现确认 V6 把 96-token preview 错当 provider actual input usage 上限；合法 `97/4` fixture 被误关，但该复现不能改写历史 provider 事实。V7 将保留生产 OpenAI-compatible non-thinking executor，修复 preview/actual parity，并加入无数值 `missing/invalid/positive` usage-shape audit。七任务 TDD 实施计划已经固定 transport、factory、evidence、CLI、48-case parity、离线门禁和两轮复审顺序；当前尚未实施、未创建 V7 evidence/marker、未授权 Live，两个业务 gate 默认保持 `false`。设计与计划见 `docs/superpowers/specs/2026-07-17-phase-6-9-5-deepseek-v4-pro-usage-recovery-v7-design.md`、`docs/superpowers/plans/2026-07-17-phase-6-9-5-deepseek-v4-pro-usage-recovery-v7.md`。
...
- 当前实现事实：Router/Verifier 已完成混合模型生产接入且默认 gate 已恢复关闭；Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup 与 KnowledgeOrganizer 仍是 deterministic policy。Review/Planner 已有受限只读 candidate，但两个业务 gate 默认关闭；v1--v6 都是关闭且不可重跑的 controlled-Live evidence，最新 V6 为 `usage_unverifiable`，因此当前项目仍返回 deterministic 建议，不能称为模型可用。V7 usage parity 已完成缺陷复现、设计和 TDD 实施计划，下一步从离线 Task 1 开始；不能从本地 fixture 或计划推断下一次 Live 授权。FinalResponse 由既有 `/api/chat` mock/live 模型链路承担；Retriever 由 Qwen embedding + pgvector/关键词混合检索承担。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；v1--v6 controlled-Live 均已独立关闭；V7 已复现 preview/actual usage validator 缺陷并完成 parity + safe usage-shape 设计与七任务 TDD 计划，尚未实现、Mock 或授权 Live。当前项目保持 deterministic、两个业务 gate 默认 `false`。（验收未完成）
...
下一会话可以复制：“请按 Phase 6.9.5 V7 usage recovery 实施计划从 Task 1 开始离线 TDD；一步一提交，不运行 Live、Docker 或浏览器，也不要提前进入分层记忆系统。”
