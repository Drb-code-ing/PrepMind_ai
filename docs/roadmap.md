...
| Phase 6   | 多 Agent 系统     | LangGraph, Router, Retriever, Tutor, Verifier, Planner, MemoryAgent, Orchestrator, Agent Eval                                                            | Phase 6.9.6 已完成；Phase 6.9.7 V1 失败封存，V2 R0--R3 已完成，下一步 R4 |
...
- Phase 6.4 已完成 WrongQuestionOrganizerAgent 的确定性产品能力：`@repo/agent/wrong-question-organizer` 根据错题结构化字段和已有 deck 摘要推荐学科组与专题 deck；Phase 6.9.7 Task 4 又完成 package 级受治理 candidate/merger，Task 6/7 已接入 NestJS owner/write fencing 与 server-only default-off runtime/Trace/HTTP abort。Task 12 V1 已调用真实 provider，但语义质量门失败，尚未进入产品 Docker/API/浏览器验收；V2 R1 只增加离线 bounded diagnostics 与 V1 report compatibility，不改变该产品可用性结论。
...
下一会话可以复制：“请按 Phase 6.9.7 V2 remediation 计划执行 R4 held-out/metamorphic anti-overfit，并保持 dataset、质量门与 V1 evidence 不变。”V1 不得重跑；R4--R6 的 identity、测试与 checkpoint 未完成前，不读取 credential、不调用 provider 或启动产品 Docker/API/浏览器。
