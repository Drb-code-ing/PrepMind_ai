...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition 与 V10 最小语义修复。本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限；模型只返回产品实际合并的 `focusIndexes` / `blockOrder`。V1--V9 都是不可重跑的历史，V9 唯一 Live 为 `quality_gate_failed`。V10 离线门已通过但 evidence directory、once marker 与 success seal 尚不存在；两个业务 gate 保持 `false`，产品验收、main 合并/replay 与推送尚未执行。详细预检见 `docs/acceptance/phase-6-9-5-review-planner-v10-offline-checkpoint.md`。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V9 唯一 Live 已 `quality_gate_failed` 封存。V10 最小质量修复和离线门已完成，唯一 controlled-Live、产品验收、main replay 与 push 尚未执行，产品 gate 默认关闭。（验收未完成）
...
下一会话可以复制：“请从 Phase 6.9.5 V10 offline checkpoint 开始独立复审；保持产品 gate 关闭，不改写 V1--V9、`.env`，不提前进入产品验收或分层记忆系统。”
