...
- Phase 6.9.7 Small-sample L2/P2：未来 exact-authorized 唯一 L2 与 sealed 终态后的路线决策；S2 已完成，但 L2 仍缺运行当时的数据边界接受、exact authorization 与独立 source/tag admission。L2 即使通过也只解锁 P2 zero-provider full-gate 设计，不直接授权产品/main。（规划中/未授权）
