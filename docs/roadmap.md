...
- Phase 6.9.5 的 ReviewAgent / PlannerAgent 已完成受限只读 candidate、固定 Mock、server-only composition 与受控诊断工程。本地 merger 继续掌握 owner-scoped facts、FSRS、分钟数、链接、任务与全部写权限。v1--v7 都是不可重跑的关闭终态；V7 为 `finalized / invalid_attempted / closed / 23 / false / evidence_io`，无 success seal、token/cost 或质量 counters。2026-07-18 已冻结完全隔离的 V8 completion 设计：零字节 append-only stage markers、V1--V7 snapshot、原有 48/26/22 质量门、Review-only/Planner-only 产品验收、main 不重跑 paired lineage 的复验语义和保留 Docker 资源的关机协议。当前两个业务 gate 保持 `false`，V8 尚未实现或运行。详细设计见 `docs/superpowers/specs/phase-6-9-5-v8-stage-diagnostics-completion-design.md`。
...
- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径与只读权限边界；V7 已关闭且不可重跑，V8 stage-diagnostics completion 设计已冻结，待 TDD、Live、产品验收、main 复验与推送。（验收未完成）
...
下一会话可以复制：“请按 `phase-6-9-5-v8-stage-diagnostics-completion-design.md` 与实施计划继续 Phase 6.9.5；严格 TDD，不得重跑或改写 V7，也不要提前进入分层记忆系统。”
