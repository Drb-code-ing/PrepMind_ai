...
- 暴露 `knowledge-document-processing` BullMQ 队列的安全计数摘要，例如 waiting / active / delayed / completed / failed。
...
- 不做公开、匿名或默认生产开放的全局监控接口；本阶段 API 仍经过 `JwtAuthGuard`，并受 `WORKER_OBSERVABILITY_ENABLED` 控制。该开关默认非 production 开启、production 关闭，避免普通生产登录用户看到系统级队列和 worker 拓扑信号。
...
第一版只观测 `knowledge-document-processing` 队列，因为当前 BullMQ 只服务知识库文档处理。后续新增 OCR / reminder / batch embedding 队列时，再扩展为多队列数组。
...
- 经过 `JwtAuthGuard`。
- 受 `WORKER_OBSERVABILITY_ENABLED` 控制；production 默认关闭，关闭时返回 404，避免暴露运维接口存在性和系统级负载信号。
- 不返回全局用户数据。
...
- Controller 测试：
  - 接口经过 `JwtAuthGuard`。
  - production 默认关闭 worker observability；显式开启时才返回 summary。
  - 关闭时不调用 summary service，返回 404。
  - 返回 schema 结构正确。
...
- 因为 Queue counts 和 heartbeat 是系统级运维信号，production 默认不暴露给普通登录用户；如需生产诊断，必须通过 `WORKER_OBSERVABILITY_ENABLED=true` 在受控环境临时开启。
