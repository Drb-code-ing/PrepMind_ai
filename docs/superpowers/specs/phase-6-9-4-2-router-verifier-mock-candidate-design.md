...
> **文档定位：历史设计依据 / decision record。** 本文件记录 Phase 6.9.4.2 的设计决策，不是持续行为 contract 的 canonical source，也不表示设计已废弃或错误。当前持续行为 contract 以 `docs/ai-behavior-acceptance.md` 的 Phase 6.9.4.2 段为准，当前实现以 code 为准，fresh 运行证据见 `docs/acceptance/phase-6-9-4-2-router-verifier-mock-candidate.md`。未来行为变更应优先更新 canonical contract，并同步必要的历史说明。
...
不采用通用 Candidate Orchestrator，因为 Router 的“保留已验证 deterministic 语义”和 Verifier 的“保守收紧”语义不同，
...
packages/agent/src/model-candidates/model-candidate-runtime-result.ts
  -> Router / Verifier 共用的 strict runtime result/usage/Trace sanitizer
  -> 校验预算演进、provider input usage、output cap 与 hostile telemetry，不传播 raw runtime object
...
packages/agent/src/model-candidates/router-model-candidate.ts
  ...
  -> final review 已迁移到共享 strict runtime sanitizer
...
`result` 始终存在，observation 是三态 contract：

- runtime 未调用：`attempted=false`、usage 为 0、budget 为传入快照且没有 trace；
- runtime 已调用且 telemetry 通过共享 strict sanitizer：`attempted=true`，只向上传播 sanitizer 重建后的 budget/usage/trace；
- runtime 已调用但 throw、malformed envelope、hostile accessor 或 telemetry 不可验证：`attempted=true`、
  `traceUnavailable=true`、`usageUnavailable=true`，不伪造 trace，usage 数值保持 0，并按 preview budget 记账防止无证据重试超卖。

envelope 不得附加 prompt、模型 object、stack、raw error 或未经 sanitizer 验证的 runtime 字段。
...
3. 如未命中安全信号且 `candidateEligible=false`，返回 `not_eligible`：保留 strict schema 已验证并重建的 deterministic Router 结果，剥离原始对象额外字段，不产生 candidate provenance。
...
runtime 失败都保留 strict schema 已验证并重建的 deterministic Router 结果，剥离原始对象额外字段。
不重试，不降低权限位，也不将普通 candidate 失败
强制改为 `chat` 来隐藏基线结果。
...
3. `candidateEligible=false`：`not_eligible`；保留已验证 deterministic status（包括 `trusted`）与限制性语义，
   但使用本地 deterministic 固定模板重建 reason/notice/debug/promptAddition，不传播 raw 正文或 candidate provenance。
...
- `not_eligible`：保留已验证 deterministic status，因此可以保留 `trusted`；reason、notice、debug、promptAddition
  由本地 deterministic policy 固定模板安全重建，不传播 raw deterministic 正文，也不声称 candidate 曾运行；
...
输入
  ...
  -> strict schema parse + immutable budget + safe Trace
     ...
     -> failure: Router 保留已验证 deterministic 语义 / Verifier 保守收紧
...
| --- | ---: | --- | --- | ---: |
| not eligible | 0 | 保留 strict schema 重建后的 deterministic route/confidence/reason/权限位 | 保留已验证 status/限制性语义，本地固定模板重建 reason/notice/debug/promptAddition | no |
...
| runtime throw/malformed/hostile telemetry | 1 | deterministic + `traceUnavailable/usageUnavailable`，preview budget | 保守 fallback + `traceUnavailable/usageUnavailable`，preview budget | no |

“调用数”指 adapter 对 `invokeStructured()` 的尝试次数。有效 runtime telemetry 可以提供安全 Trace；已经尝试但
telemetry 不可验证时不得伪造 Trace，而必须以 `traceUnavailable/usageUnavailable` 明示并推进 preview budget。
...
2. Router ineligible 为 0 次 invoke并保留 strict schema 重建后的 deterministic 结果、剥离额外字段；8 个 safety boundary case 为 0 次 invoke且返回本地 safe `chat`。
...
6. Router 所有 runtime 失败保留 strict schema 重建后的 deterministic 结果，不传播原始对象额外字段。
...
12. envelope 必须断言三态：`attempted=false -> trace 不存在`；`attempted=true` 且 runtime telemetry valid
    `->` sanitizer 重建的 trace 存在；`attempted=true` 但 throw/malformed/hostile telemetry `->`
    `traceUnavailable/usageUnavailable=true`、trace 不存在、usage 数值为 0、budget 为 preview。`reasonCodes`
    首项为 disposition、无重复且按 canonical 顺序。
...
- observation 三态、reasonCodes、evidence 去重排序、chunkId/score 校验与 Unicode/40-code-point gate 边界均有回归测试；
