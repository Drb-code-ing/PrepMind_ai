...
> **2026-07-15 路线修订：** 本文保留 Phase 6.9.1～6.9.4 与已完成短期记忆工程的历史设计。后续执行顺序、12 个 Agent/Orchestrator 的模型路径、通信、权限和博客拆分，以 `docs/superpowers/specs/2026-07-15-phase-6-9-agent-architecture-completion-design.md` 为权威来源：先完成全部 Agent 架构，再进入 Phase 6.10 分层记忆；多 Agent 与记忆系统分别成文。
...
- 用固定评测集决定各 Agent 的模型调用范围、zero-call 范围、预算、降级和生产 gate。
- Agent 与记忆两个阶段分别交付详细面试学习博客，最终题目和结构由用户另行确认。
...
| TutorAgent | 明确指令 deterministic + 复杂教学意图 LLM | 模型负责隐含教学策略，FinalResponse 负责最终表达 |
| RetrieverAgent | Qwen embedding + 混合检索 + 可选 LLM query rewrite | 检索事实与权限保持本地权威，模型只处理复杂多轮查询表达 |
| FinalResponseAgent | 真实模型必需 | 汇总受控上下文、Agent 决策和工具结果生成最终回答 |
| ReviewAgent / PlannerAgent | 真实模型必须参与 + deterministic facts/constraints | FSRS 与容量由程序计算，模型负责诊断、权衡与个性化计划 |
| WrongQuestionOrganizerAgent | 高置信 deterministic + 低置信 LLM 分类 | 模型处理语义归类，用户锁定名称和写库权限保持本地权威 |
| KnowledgeDedup / Organizer | hash/权限 deterministic + embedding/LLM semantic judgment | 模型判断语义重复、版本、互补、标签和集合，但不能自动删除、替换或归类 |

这张表是 2026-07-15 确认的目标路径。对“真实模型必须参与”的 Agent，评测决定调用条件、预算、降级和生产 gate；不达标时保持 gate 关闭并继续优化，不能把产品目标静默改回纯 deterministic。每项最终状态必须写入评测报告。
...
不达标时保持模型生产 gate 关闭、使用安全 fallback 并记录原因，不为了“多 Agent 都用了真实模型”降低门槛，也不把必须模型化的 Agent 永久退回纯 deterministic。
...
### Phase 6.9.5～6.9.10：完成全部 Agent 架构

- Phase 6.9.5：ReviewAgent / PlannerAgent 真实模型路径。
- Phase 6.9.6：KnowledgeDedupAgent / KnowledgeOrganizerAgent 的 embedding + 真实模型语义路径。
- Phase 6.9.7：TutorAgent / WrongQuestionOrganizerAgent 混合模型路径。
- Phase 6.9.8：RetrieverAgent / FinalResponseAgent 职责正式化与通信 contract。
- Phase 6.9.9：MemoryAgent 敏感凭据缺陷修复、40-case paired eval 和真实模型候选提取；不在本阶段注入 Chat。
- Phase 6.9.10：MCP-ready Orchestrator、工具权限、可执行 LangGraph、Mock/Live/Docker/可见浏览器全 Agent 验收。

### Phase 6.10：分层记忆系统

- ACTIVE 结构化长期记忆混合检索、独立预算和 Chat 注入。
- `UserMemoryEpisode`、pgvector、自动低敏摘要、30 天 TTL、查看、删除、过期与遗忘闭环。
- embedding 降级、冲突、替代、多用户隔离和全链路验收。
...
博客拆为两个独立交付物：Phase 6.9 全部 Agent 与 Orchestrator 验收后编写《多 Agent 架构》；Phase 6.10 完成后编写《记忆系统》。两篇博客都不得只罗列代码，必须能回答设计取舍、Agent 通信与权限、失败方案、测试证据和面试追问；最终题目与结构由用户届时确认。

## 13. 验收标准

Phase 6.9 Agent 架构完成时必须满足：

- 共享模型运行时能在 Mock 和受控 Live 模式运行，并有统一预算、超时、降级和脱敏 Trace。
- 12 个 Agent/Orchestrator 的职责、通信、权限和触发边界有代码与文档依据。
- 必须模型化的 Agent 存在真实模型路径；混合 Agent 的 zero-call 与 model-call 范围有固定评测。
- Retriever、FinalResponse 与 Orchestrator 不再只是逻辑名称，LangGraph 不再只是 descriptor。
- Router、Verifier、Review、Planner、Tutor、WrongQuestionOrganizer、Memory、KnowledgeDedup/Organizer 与 Orchestrator 都有适配其职责的 deterministic/Mock/Live 证据。
- 未知工具、越权工具和未确认写操作执行数为 0。
- 通过 types、ai、agent、database、server、web、e2e、Docker 和可见浏览器验收。
- 临时账号与验收数据已清理，配置恢复 Mock，代码合并 `main` 并推送远程。
- 项目核心文档已经同步；多 Agent 博客按用户确认单独编写。

Phase 6.10 另行验收滚动摘要延续性、结构化长期记忆注入、Episodic Memory、30 天过期、查看、删除、遗忘和独立《记忆系统》博客，不再混入 Phase 6.9 完成条件。
