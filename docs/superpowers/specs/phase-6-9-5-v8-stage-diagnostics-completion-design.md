...
> 状态：V8 离线实现已完成，唯一 controlled-Live 已执行并在 `.stage-080-paired-returned / invalid_response` 终态关闭；无 `.stage-090` 或 success seal，因此产品验收与 main 路径被阻断。V1--V8 的 marker、evidence、confirmation 和计数均只读且不可重跑、删除、重建或改写。
