...
上述独立 controlled-Live 已作为 Attempt E 从 `main@5d964c51a948d4603a1fcff5c52dba66b0581725` 执行。新 runner v2 / `deepseek_strict_tool_v1` 在首个 eligible case 得到 `PROVIDER_ERROR / http_client`，`providerAttempts/strictSuccesses=1/0` 后严格停止；evidence validator exit 0 只证明该 incomplete 报告合法。Chat Completion 文档列出该模型，Tool Calls 指南另行描述通用 strict Beta contract，本地 fake-fetch wire 符合公开基础约束；模型级 feature/provider compatibility 仍未证实，现有 `http_client` 也无法区分 400、402、422 等 4xx。所以下一独立任务改为零网络 Provider compatibility diagnostics，不再直接重跑 Live；完整证据见阶段 acceptance。
