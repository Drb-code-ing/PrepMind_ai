...
pricing/cap 只从 flags 读取，env 没有别名。`effectiveMaxCostUsd=Math.min(cliMaxCostUsd,0.10)`；当前持续合同的 preflight 用 96,000 input / 11,200 output 与 non-cache input/output price 计算 worst case，`<=` 才允许。所有 config failure 生成 strict `invalid_run/live_config_invalid` 与两个 disabled/invalid_report decision；4,080 仅保留在旧代码附录与 Attempt A/B/C 历史说明中。
...
```powershell
bun run --cwd packages/agent eval:phase-6-9-4-3:accept-mock
if ($LASTEXITCODE -ne 1) { throw "MOCK_EXIT_CODE:$LASTEXITCODE" }
...
bun run --cwd packages/agent eval:phase-6-9-4-3:live -- --live `
...
不要回显、读取文件中的 key或把价格猜成默认值。后续 controlled-Live 的持续合同已经更新，preflight 必须证明 96,000 input / 11,200 output 的 worst-case cost `<= effectiveMaxCostUsd`；4,080 只属于 Attempt A/B/C 运行时的历史 output cap，不得用于新 run。
...
> 后续状态（2026-07-14）：上述 headroom 已在独立设计与实施计划中完成；Router/Verifier 单次 output 为 400，global output cap 为 11,200。详见 `docs/superpowers/specs/2026-07-14-phase-6-9-4-3-structured-output-headroom-design.md` 与对应 plan。本文保留 120/180/4,080 代码片段作为原实施历史，不作为后续 Live 的持续预算合同。

> Attempt D 检查点（2026-07-14）：从已推送 `main@c72a9f03a7709fce1aa1e0210a378f3c380b79fb` 创建独立分支；零调用 preflight 为 exit 3 且 evidence 3 → 3，diagnostics/paired 精确测试 250/250 通过。唯一 controlled-Live 为 exit 2，`observed/notRun=52/48`、`providerAttempts/strictSuccesses=16/15`，固定失败 case `router_ambiguous_mixed_chat_16` 为 `PROVIDER_ERROR / structured_output`；strict validator exit 0。成功 output 为 59~341，因此下一任务先做零网络 prompt/schema/provider compatibility 韧性设计，不直接重跑 Live或继续盲目提高 cap。
...
$liveJson=(& bun run --cwd packages/agent eval:phase-6-9-4-3:live -- --live `
...
bun run --cwd packages/agent eval:phase-6-9-4-3:validate -- --profile live --file $relative
...
| Budget | 32,000 local input、11,200 local/provider output、96,000 provider input、effective cost ≤USD0.10、等号允许 |
