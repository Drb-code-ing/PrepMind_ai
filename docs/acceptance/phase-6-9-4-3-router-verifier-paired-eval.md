...
Phase 6.9.4.3 的评测工程、Mock 验收与历史五次受控 Live 尝试已经形成可复核证据。2026-07-14 又完成了 JSON-mode resolution 的零网络实施 checkpoint，并从新 main 独立执行了唯一一次完整 JSON-mode controlled-Live。该 run 的 28 个 eligible case 全部 strict success、72 个 zero-call case 保持零调用，Verifier 通过质量/延迟门槛；Router 的 additional P95 为 4264ms，超过固定延迟门槛，因此 Router 仍 `enabled=false`，阶段验收仍未完成，生产 Chat 继续使用 deterministic policy。
...
- 历史 controlled-Live 使用 `deepseek-v4-flash`、单 case 10 秒、串行执行、无自动重试；Attempt A~D 使用 JSON structured output，Attempt E 使用 `deepseek_strict_tool_v1`。本次唯一新 run 固定使用标准 `https://api.deepseek.com`、`response_format: { type: 'json_object' }`，不带 tools/tool_choice/json_schema。
- 五次已越过 provider boundary 的运行均原样保留。Attempt A 在第 3 次 provider attempt 停止；Attempt B/C 都在第 1 次停止；400-token headroom 后的 Attempt D 在第 16 次停止，之前已有 15 次 strict success；strict-tool Attempt E 在第 1 次 provider attempt 收到非鉴权、非限流的 4xx 后停止。
- Attempt E 是当前 canonical Live evidence：identity/schema/privacy validator 全部通过，首个 eligible case `router_ambiguous_notes_tutor_01` 由共享 diagnostics 分类为 `PROVIDER_ERROR / http_client`；它仍为 `incomplete`，Router / Verifier 都是 `enabled=false / usage_unverifiable`。`http_client` 只证明状态属于 4xx 且不是 401/403/429，不能区分 400、402、422 或其他 4xx。
...
- strict-tool checkpoint 作为历史实验保留，不再作为 transport。新的 JSON-mode checkpoint 完成标准 URL、共享 executor wire、runner-v3 identity、prompt/evidence 版本绑定和零网络门禁；真实 run 证明 Provider/结构化输出可用，但 Router 延迟门槛未通过，不能启用 Router candidate。
...
| runner / prompt | 新 Live：`phase-6.9.4.3-runner-v3` / `phase-6.9.4.3-json-mode-v1`；历史 v1/v2 只读兼容 |
| structured transport | 新 Live：`deepseek_json_object_v1`；历史 `deepseek_strict_tool_v1` 只读保留；Mock 禁止携带 transport 字段 |
...
### 4.6 Attempt E：strict-tool controlled-Live incomplete evidence

2026-07-14 从已推送 `main@5d964c51a948d4603a1fcff5c52dba66b0581725` 创建独立分支。paired 精确测试为 96/96、845 assertions，Agent typecheck/lint exit 0；负向 zero-call preflight 为 exit 3，Live evidence 保持 4 份。随后使用根 `.env` 中的 key 只在当前 PowerShell 进程内注入，执行唯一一次完整 controlled-Live 命令。运行从 100 条 case 开头串行开始，在首个 eligible case 失败后严格停止，没有重试、补跑或拼接历史 evidence；结束后恢复 Mock 并移除进程 key。

第一次命令曾错误附加 `bun --env-file=.env`，根 `.env` 的 Mock 配置覆盖父进程 Live 配置，因此以 exit 3 / `live_config_invalid` 在 UUID、evidence 和 Provider boundary 前结束。该次为零调用配置失败，不是 Live attempt，不产生费用或 evidence。

| 字段 | 结果 |
| --- | ---: |
| evidence | `live-20260714T071444506Z-65042475cbaf.json` |
| Git blob hash | `368c91f817ad76272a495f77ff1d4d6f90695429` |
| runner / transport | `phase-6.9.4.3-runner-v2` / `deepseek_strict_tool_v1` |
| run status / qualityEvidence | `incomplete` / `true` |
| started / duration | 2026-07-14T07:14:44.506Z / 204ms |
| observed / notRun | 37 / 63 |
| case entries / adapter executions | 100 / 37 |
| runtime invocations / provider attempts | 1 / 1 |
| strict successes / runtime failures | 0 / 1 |
| observed zero-call cases | 36 |
| failing case | `router_ambiguous_notes_tutor_01` |
| fixed failure | `PROVIDER_ERROR / http_client` |
| aggregate usage / estimated cost | 0 / 0，`providerReported=false` / USD 0 |
| Router / Verifier decision | `enabled=false / usage_unverifiable` |
| strict validator | exit 0，`ok=true / live / incomplete` |

官方 DeepSeek 文档与本地 fake-fetch wire 对照得到以下有界事实：

- 官方 Chat Completion 文档列出 `deepseek-v4-flash`；独立的 Tool Calls 指南描述通用 strict Beta contract：使用 `https://api.deepseek.com/beta`、函数内 `strict:true`，并要求每个 object 的全部 properties 都进入 `required`、`additionalProperties:false`。该指南没有明确声明 `deepseek-v4-flash` 的模型级 strict-tool compatibility；
- 本地实际 SDK wire 为 `POST https://api.deepseek.com/beta/chat/completions`，body 只有 `model/messages/max_tokens/temperature/tool_choice/tools`；唯一 forced `model_agent_result` 的 Router schema 满足上述 object 约束，且不携带 `response_format`；
- 因此可以排除客户端误用了未公开的模型标识、非 Beta endpoint、缺失 forced tool 或基础 object schema 字段；但不能排除 `deepseek-v4-flash` 在当前端点、账号或 Provider 版本下的模型级 strict-tool compatibility 问题，也不能证明 Provider 已接受该次请求；
- 当前安全诊断把 401/403 映射为 `http_auth`、429 映射为 `http_rate_limit`，其余 4xx 都映射为 `http_client`。所以 Attempt E 可排除鉴权与限流，但不能区分 400 Invalid Format、402 Insufficient Balance、422 Invalid Parameters 或其他 4xx。0/0 usage 与 USD 0 只表示没有 provider-reported usage，不能冒充供应商账单或余额证明。

因此 Attempt E 把故障边界从“structured output 生成/解析”前移到了“Provider 4xx 接收/参数边界”，但尚未得到可验证根因。下一任务必须先做零网络的安全诊断细化与 Provider compatibility 设计，例如把 402 与参数类 4xx 以固定白名单分类区分，并对 SDK wire 做版本化 contract；在该设计实施、Mock 验收、合并 main 前，不再调用真实模型。
...
修复前后 worst-case admission 都明显低于 USD 0.10。Attempt B/C/E 都因首个 provider failure 没有可验证 usage，所以各自 evidence 的估算成本为 0；Attempt D 对 15 条 strict success 的部分可审计估算成本为 USD 0.001297004654663。上述数值都不冒充供应商最终账单、余额或未计费证明；历史 evidence 保留运行时合同，不按新 11,200 cap 回算或改写。
...
Attempt E 已使用新的 strict-tool transport，但首个 eligible case 即得到 `http_client`。该类别排除了 401/403/429，却仍混合 400/402/422 等互不相同的处理方向；此时重跑不能增加根因信息，只会重复消耗外部配额。官方文档与本地 wire 只证明客户端使用了公开模型标识、Beta endpoint、forced tool 和基础 object schema 形状，不能证明模型级 feature/provider compatibility。下一步应先提高**固定码诊断分辨率**，而不是读取 raw provider body、绕过 runner 或再次调用模型。
...
| Attempt E zero-call preflight | exit 3，`live_config_invalid`，evidence 4 → 4 |
| Attempt E paired 精确测试 | 96 passed / 0 failed / 845 assertions |
| Attempt E controlled-Live | exit 2，1 attempt / 0 strict success / `http_client` |
| Attempt E strict validator | exit 0，`ok=true / live / incomplete` |

现有 `mock.json` 在新 strict contract 下继续合法，不需要为了改变不可见的 request reservation 而重写；历史 Attempt A/B/C/D blob 保持不变。表格中 candidate RED/GREEN、paired RED/GREEN、全量测试与 headroom contract 验收只使用 Mock/fake/历史只读 evidence，没有设置 Live 双开关或发起 Provider 请求；其后的 Attempt D/E 行是独立 controlled-Live 任务的真实运行结果，均在单次子进程内显式开启双开关并调用 Provider，结束后已恢复 Mock、清除进程 key，且没有重试。
...
实施提交为 `303b88a`（schema compiler）、`bdb7cb5`（strict-tool Provider transport）、`2100e10`（paired CLI / evidence）。Attempt E 是其后的独立 controlled-Live 结果，不把失败重新解释为零网络通过。以上零网络门禁未读取真实 key；Attempt E 的 key 只存在于单次进程内，结束后已清除。
...
- [Attempt E：strict-tool 4xx 后 incomplete Live evidence](./evidence/phase-6-9-4-3/live-20260714T071444506Z-65042475cbaf.json)
...
canonical 选择理由：Attempt E 是 strict-tool resilience checkpoint 合并并在新 main 上启动后的最新独立 run，正文身份、文件名、strict schema、隐私、usage/cost 与 decision 校验全部通过；它记录了新 transport 首次真实越过 Provider boundary 后的 `http_client` 结果。Attempt A~D 继续作为不可覆盖的历史事实保留。Attempt E 仍是 incomplete，因此“canonical”不等于“质量通过”；它也不能凭 `http_client` 区分 400/402/422 等具体 Provider 原因。

Attempt A / B 的 Git blob hash 分别保持 `330a5cfcfda64a4c90b60e0e711ee6f2ce69b6c6` 与 `dd6cb8f2e543c4b89c009d9198b3d89f344ce594`，证明历史文件未被覆盖改写；Attempt C / D 的初始 Git blob hash 分别为 `ede0a9f5576996a2bad7a9dfb60cd135047d4edf` 与 `bc9f4e2efc70d26723d56418bebf327e1e75383e`；Attempt E blob hash 为 `368c91f817ad76272a495f77ff1d4d6f90695429`。

## 9.1 2026-07-14 JSON-mode resolution 零网络 checkpoint

本轮按已批准方案停止继续增加 transport：controlled-Live 回到已在 Phase 6.9.3.5 真实验证过的 DeepSeek JSON Output。实现只改变评测 composition 与 evidence identity，不接入生产 Chat，也不启用 Router / Verifier candidate。

- Provider wire 固定为标准 `https://api.deepseek.com` 与 `response_format: { type: 'json_object' }`；没有 `tools`、`tool_choice` 或 `json_schema`。共享 `@repo/ai` executor 仍负责超时、`maxRetries=0`、usage provenance、canonical Zod 解析和安全错误映射。
- 新报告身份为 `phase-6.9.4.3-runner-v3` / `deepseek_json_object_v1` / `phase-6.9.4.3-json-mode-v1`。v1/v2 历史 evidence 只读兼容；strict-tool 仅作为历史实验保留，不与新 run 拼接。
- 新增 fail-closed 关系校验：runner 版本决定允许的顶层 promptVersion，Mock/Live candidate entries 的 promptVersion 必须与顶层一致。这样不能把旧 prompt 或其他 transport 的 entry 伪装进新报告。
- 零网络门禁：Agent `345 pass / 0 fail / 3242 assertions`；AI `151 pass / 0 fail / 817 assertions`；Agent/AI typecheck 与 lint 均 exit 0；deterministic baseline 仍为 `74/100`、critical `2`。
- 新鲜 Mock（不持久化）为 `complete`，`100 cases / 28 runtime invocations / 0 provider attempts / 28 strict successes / 72 zero-call`；CLI exit `1` 是固定的“候选仍关闭”结果，不是运行失败。现有 tracked `mock.json` 保持历史 v1 evidence，validator exit `0`。
- 负 Live preflight 在未提供完整受控配置时 exit `3`、`live_config_invalid`，没有 UUID、provider call 或新 evidence。持久化 Mock 命令因 tracked `mock.json` 已存在而安全拒绝覆盖，返回 `unexpected_runner_error`；这不是模型调用或 JSON-mode 失败，现有 evidence 未被修改。

本 checkpoint 仍不是 Phase 6.9.4.3 完成证明，原因从“尚未执行真实模型”收敛为“Router 延迟门槛失败”。不再重跑、不再提高 token cap、不再引入第三种 transport；Router 采用 deterministic terminal fallback，Verifier 的 paired decision 仅作为评测证据，不自动改写生产 Chat。

## 9.2 2026-07-14 JSON-mode controlled-Live canonical evidence

新 main 复验并推送后，从 `codex/phase-6-9-4-3-controlled-live-json-mode` 执行一次完整 100-case run，证据文件为 [`live-20260714T084632914Z-4145ce0ffea0.json`](./evidence/phase-6-9-4-3/live-20260714T084632914Z-4145ce0ffea0.json)。API key 只存在于该次 PowerShell 子进程，结束后已恢复 Mock 并清除进程变量；没有启动或清理 Docker。

| 项目 | 结果 |
| --- | ---: |
| runner / transport / prompt | `runner-v3` / `deepseek_json_object_v1` / `phase-6.9.4.3-json-mode-v1` |
| run status | `complete` |
| cases / eligible / zero-call | `100 / 28 / 72` |
| provider attempts / strict successes | `28 / 28` |
| provider usage | `input 10,677 / output 4,323` |
| estimated cost | `$0.002842788219846` |
| Router decision | `enabled=false / latency_budget_exceeded` |
| Router additional P95 | `4264ms`（超过固定门槛） |
| Verifier decision | `enabled=true / quality_gate_passed` |
| Verifier additional P95 | `2872ms` |
| evidence validator | exit `0`, `live / complete` |

该结果证明 DeepSeek JSON mode、canonical Zod、usage provenance、zero-call safety 与完整 evidence 链路均可工作；它没有证明 Router 达到产品延迟门槛。由于阶段完成条件要求 Router / Verifier 的质量、安全、权限、延迟、token、usage provenance 与成本门槛同时通过，Phase 6.9.4.3 仍标记为“验收未完成”。Verifier 的 `enabled=true` 是 paired eval decision，不等于生产路径已经接入；当前生产 Chat 与 Router 继续 deterministic。

## 10. 下一任务与回顾问题

下一步先完成本 controlled-Live 分支的 evidence、终局结论与文档提交，经独立审查后 `--no-ff` 合并 main，在 main 复验并推送远程。Phase 6.9.4.3 随后以“真实 JSON-mode 完整运行成功，但 Router 延迟门槛失败，terminal deterministic fallback”冻结，不再发起新的 Router/Verifier Live；下一开发任务从新 main 进入 Phase 6.9.5。

本次真实 Live 已从第 1 条完整执行 100 cases，没有补跑或与 A/B/C/D/E 拼接；28/28 strict success 与 72/72 zero-call 均通过，但 Router 延迟门槛失败。因此 Phase 6.9.4.3 不标记完成，Router 继续 `enabled=false`，生产继续 deterministic；Verifier 的通过结论保留为后续集成依据。

下一会话可以直接问：`请收尾 Phase 6.9.4.3 JSON-mode controlled-Live evidence 与文档，审查后合并 main、复验并推送；然后从新 main 开 Phase 6.9.5，Router 保持 deterministic terminal fallback。`
...
- 零网络验收已经 151/345 passed，为什么 Router / Verifier 仍必须 `enabled=false`？
- Attempt E 的 `http_client` 为什么能排除 401/403/429，却不能区分 400/402/422？
- 本地 wire 符合官方公开基础约束，为什么仍不能宣布模型级 Provider strict-tool 兼容性已经通过？
- 为什么 Attempt E 的 0/0 usage 与 USD 0 不能证明账户余额充足或供应商没有计费？
