...
Phase 6.9.4.3 的评测工程、Mock 验收与历史五次受控 Live 尝试已经形成可复核证据。2026-07-14 又完成了 JSON-mode resolution 的零网络实施 checkpoint：Router / Verifier controlled-Live 已收敛回已验证的 DeepSeek `json_object` transport，新增 runner-v3 / `deepseek_json_object_v1` identity，且候选仍为 `enabled=false`。下一步才是从合并后的新 `main` 独立执行一次完整 JSON-mode controlled-Live；在该 run 完成前，阶段验收仍未完成，生产 Chat 继续使用 deterministic policy。
...
- 历史 controlled-Live 使用 `deepseek-v4-flash`、单 case 10 秒、串行执行、无自动重试；Attempt A~D 使用 JSON structured output，Attempt E 使用 `deepseek_strict_tool_v1`。新的待执行 run 固定使用标准 `https://api.deepseek.com`、`response_format: { type: 'json_object' }`，不带 tools/tool_choice/json_schema。
...
- strict-tool checkpoint 作为历史实验保留，不再作为下一轮 transport。新的 JSON-mode checkpoint 完成标准 URL、共享 executor wire、runner-v3 identity、prompt/evidence 版本绑定和零网络门禁；它仍不证明真实模型语义质量通过。
...
| runner / prompt | 待执行新 Live：`phase-6.9.4.3-runner-v3` / `phase-6.9.4.3-json-mode-v1`；历史 v1/v2 只读兼容 |
| structured transport | 待执行新 Live：`deepseek_json_object_v1`；历史 `deepseek_strict_tool_v1` 只读保留；Mock 禁止携带 transport 字段 |
...
本 checkpoint 仍不是 Phase 6.9.4.3 完成证明。合并 main 并在 main 复验、推送远程后，才允许从新 main 创建独立分支，使用真实 key 串行完整跑一次 100 cases；若 JSON mode 仍未同时满足 28/28 strict success、72/72 zero-call 以及质量/安全/延迟/token/usage/cost 全部门槛，则记录终局 fallback，保持 `enabled=false`，不再引入第三种 transport。

## 10. 下一任务与回顾问题

下一步仍属于 Phase 6.9.4.3，但必须遵守分支边界：先将本分支实现与 JSON-mode checkpoint 文档以 `--no-ff` 合并 main，在 main 重跑验收并推送远程；然后从该新 main 创建独立的 controlled-Live 分支。真实 key 只允许用于一次完整 JSON-mode run，不重跑历史 strict-tool，也不做单 case 补跑或 evidence 拼接。

下一次真实 Live 必须从第 1 条开始完整重跑 100 cases，不能只补失败 case，也不能与 A/B/C/D/E 拼接。只有同时得到 28/28 strict success、72/72 design-time zero-call，并通过 Router/Verifier 全部质量、安全、权限、延迟、token、usage provenance 与成本门槛，Phase 6.9.4.3 才能标记完成。否则继续 `enabled=false`，生产继续 deterministic。

下一会话可以直接问：`请先将 Phase 6.9.4.3 JSON-mode resolution 分支 --no-ff 合并 main，在 main 复验并推送；然后从新 main 开独立 controlled-Live 任务，完整跑 100 cases，失败就记录终局 fallback，不要拼接历史 evidence。`
