...
P1 L2 controlled-Live 的失败证据、根 artifact 与相关开发文档已经从受控分支提交并合并到 `main`，合并后的
`main` 已推送到远程并完成一次只读、零 Provider 回归。该验收确认的是源码、文档和封存证据的分支一致性与工程回归，
不是把失败的 Live 提升为语义或产品通过。

固定结论：

- main merge commit：`f4fac048919461c26957c1aed11488fda7e5dbee`；
- `HEAD == main == origin/main`：`f4fac048919461c26957c1aed11488fda7e5dbee`；
...
3. 从干净的 `main` 使用 `--no-ff` 合并，生成 `f4fac048`；
4. 推送 `origin/main`，随后在 `main` 执行本文件第 3 节的零 Provider 验收。
...
git rev-parse HEAD main origin/main  ## 三者均为 f4fac048...
