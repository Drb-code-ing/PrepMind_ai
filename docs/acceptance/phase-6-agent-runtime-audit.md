...
- ChatTurn 第一原子实现与第二步可靠入队已完成：`ChatTurn + BackgroundJob + chat.response.requested OutboxEvent` 在同一 Serializable 事务中创建，并通过回滚、幂等、跨 owner 与 serialization retry 测试；这仍不等于 Worker、Replay 或 `/api/chat` durability 已完成。证据见 `docs/acceptance/phase-6-chat-enqueue-outbox.md`。

## 2. Agent 总矩阵

Chat 入队的当前实现状态：可靠入队已由 `ChatTurnEnqueueService` 完成（同事务 Turn/Job/Outbox）；矩阵中各 Agent 的执行、Worker claim、
终态提交与产品切换仍按下表缺口推进，不因入队完成而自动升级为真实模型或生产使用证据。
...
| ReviewAgent | `GET /review-agent/suggestions`，生成复习分析/建议 | 确定性 tasks/preferences/cards/logs 为权威；可选 Review candidate | Review/Planner 共享 `2 / 1950 / 440`；默认 4.5s | JWT；只读业务快照；candidate 不得写业务 | AgentTrace best-effort；无本轮产品 live endpoint smoke | Controller 未向 service 传递断连 AbortSignal；candidate 外层异常 fallback 需补强并测试 |
| PlannerAgent | 同一 suggestions endpoint 的学习计划生成 | 确定性 `planStudy` 为权威；可选 Planner candidate | 与 Review 共享 `2 / 1950 / 440`；默认 4.5s | JWT；只读快照；输出计划候选，不得改任务数据 | AgentTrace best-effort；无本轮产品 live endpoint smoke | 同 ReviewAgent；需要验证并发预算与请求取消边界 |
...
5. Review/Planner controller 当前没有把客户端断连信号传入 service；服务端可能在客户端已断开后继续消耗模型预算。
...
3. ~~为 Chat 增加全链路预算/断连 durability 的明确合同和测试。~~ 设计 checkpoint 与可靠入队已完成：明确 ChatTurn、BackgroundJob+Outbox 同事务、worker/replay、owner/幂等和 run-level budget；Worker、Replay 和产品切换仍未完成，详见 `docs/acceptance/phase-6-chat-durability-budget-design.md` 与 `docs/acceptance/phase-6-chat-enqueue-outbox.md`。
   ChatTurn schema/migration 与 owner-scoped repository focused `10/10`、schema `9/9`，可靠入队 focused `8/8`；Server build 通过。BackgroundJob+Outbox 同事务已落地，但 Worker、Replay 和产品切换仍未完成。
