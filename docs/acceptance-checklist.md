...
若要继续，必须先完成新的零网络根因设计与独立复审；只有用户之后明确批准一个新的、隔离 profile，才可考虑一次新的受控诊断。它不得被命名为或实现为 V6 retry，必须有自己的 evidence、once marker、计数与预算。两个业务 gate 始终为 `false`，直至未来 profile 完成全部独立质量与产品验收。本阶段的终态记录见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`；该记录不包含真实模型通过结论。回顾时可以问：“为什么 `usage_unverifiable` 不能推导实际费用？”“为什么新的 profile 不能复用 V6 marker 或计数？”
