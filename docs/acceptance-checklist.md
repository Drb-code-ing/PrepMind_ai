...
3. 复核 `main...HEAD`、server/web/worker 的环境 allowlist、owner isolation、zero-call safety、默认 `false` gate、一次 attempt/零 retry 和 evidence 脱敏。不得把 prompt、用户 facts、provider 原文、base URL、key、header、cookie 或 stack 写进报告。
4. 只有前三步全部通过，才可在独立进程中用精确确认参数执行一次 server-only controlled diagnostic。诊断失败时保存固定类别、保持 gate 关闭并停止；禁止用历史 run、重试或 Docker 成功替代诊断结论。
...
本阶段的最新无凭据/Mock 记录见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`。该记录不包含真实模型通过结论；main 合并、main 复验与远程推送仍属于后续收尾任务。
