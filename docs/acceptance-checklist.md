...
当前 v1/v2 profile 都以 `invalid_attempted / structured_output`、`gate=closed`、`providerAttemptCount=1`、`usageKnown=false` 关闭；两条 once marker 已消耗，计数不得拼接。48-case、Docker 和浏览器验收均未执行且不得借任一 profile 继续。本阶段的最新无凭据/Mock/关闭记录见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`；该记录不包含真实模型通过结论，main 合并、main 复验与远程推送仍属于后续收尾任务。
