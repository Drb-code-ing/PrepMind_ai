...
RAG Docker 验收前在根 `.env` 或宿主环境明确配置 `RAG_EMBEDDING_PROVIDER=qwen`、`RAG_EMBEDDING_MODEL=text-embedding-v4`、`RAG_EMBEDDING_DIMENSIONS=1536`、无凭据 HTTPS `RAG_EMBEDDING_BASE_URL` 和 `QWEN_API_KEY`。server/worker 共用同一 RAG runtime allowlist；宿主别名 `Qwen_API_KEY` / `DASHSCOPE_API_KEY` 仅用于兼容输入，容器内规范化为 `QWEN_API_KEY`。不允许 provider fallback，不允许 production fake。
...
- fake embedding 只能证明上传、解析、分块、入库和检索 API 可用；当前真实语义质量标准验收使用 Qwen `text-embedding-v4` / 1536。Phase 7.8.5 实施完成不等于真实 Docker 验收已完成。
...
- 真实 RAG 验收要明确记录 Qwen `text-embedding-v4` / 1536、queue `BackgroundJob=SUCCEEDED`、hybrid `keywordScore` / `vectorScore` 证据和无重复 `chunkId`；不得以 provider fallback 或 reranker 解释结果。
