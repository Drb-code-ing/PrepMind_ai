...
Task 0--7 已完成：72-case baseline、strict contract/projection、Tutor/Organizer package candidate/merger、Tutor Web server-only default-off composition，以及 Organizer owner snapshot/三阶段 stale fence/model-free command、server-only default-off runtime、single/batch 单次 dispatch、两阶段 Trace 与 HTTP abort 已落地。当前仍未读取根 `.env`/credential、调用 provider 或进行 controlled-Live/Docker/浏览器产品验收。执行时逐项确认：
