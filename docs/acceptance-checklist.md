...
DeepSeek V4 Pro v5 的离线 CLI 已就绪，但尚未执行：只能通过 `bun --filter @repo/server eval:review-planner:live:v5:deepseek -- --confirm-controlled-live-v5-deepseek-v4-pro` 进入。它在任何 evaluator/provider 创建前先完成 preflight、v1--v4 历史 snapshot、v5 专属 reserve 和历史复核；随后写入一次 attempt marker、运行单个 canary，并只在 `complete + positive verified usage + providerAttemptCount=1` 时运行 22 个 eligible runtime case。最终只有 48 entries、26 个 `zeroCallVerified`、22 runtime、23 total attempts、`quality_gate_passed`、P95 `<=4500ms`、受控 price profile `deepseek-v4-pro-non-cached-cny-v1` 与 CNY aggregate `<=1` 才可输出 `complete/open`。成功或失败 stdout 均为严格白名单 JSON；若任一 preflight、历史完整性、usage、质量、费用或 evidence I/O 检查失败则关闭，绝不 retry。当前只完成离线 Jest 和不触网 Mock（48/26/22/48 strict/0 critical/`mock_quality_not_evidence`）；未运行 v5 Live、Docker、浏览器、main 合并或远程推送，且两个业务 gate 保持默认 `false`。
