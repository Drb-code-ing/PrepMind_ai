...
## 0M. Phase 6 Agent 运行时总审计（2026-09-05，进行中）
...
- [x] production launcher 以受控子进程统一 preflight 与 Provider transport 的 direct-host 环境
- [x] 大小写不敏感移除 `HTTP(S)_PROXY/ALL_PROXY/NO_PROXY`，保留系统与 V12 授权变量
...
- [x] 功能提交 `243a4b97` 推送，`main --no-ff` merge/push 为 `dde5c24a`，功能验收时 main/origin parity
...
- [x] 确认旧 outer `response=0` 只表示 typed call 未返回，不证明 HTTP response 未到达
- [x] bounded `adapterFailureCategory/structuredOutputStage` 投影，不保留 raw content/field/value
- [x] Task9/SR5 failure terminal 前补写 durability `response_received`
- [x] 历史 call entry 无新增字段仍兼容 strict validator
...
验收：`docs/acceptance/phase-6-9-8-retriever-final-response-schema-recovery-sr5-v10-schema-adapter-postmortem-zero-provider.md`。
...
- [x] focused `25/25`、SR5 六文件 `50/50`（162 assertions）、typecheck/lint/diff check
...
- [x] L2 admission strict source/tag/anchor/data-boundary/authorization/budget parser 与 single-use capability（focused `4/4`、Provider/credential/formal evidence `0`）
...
BackgroundJob/Outbox、业务写入或 `main` authority。S1 已由唯一 V2 L1 sealed run 收口；当时的下一步是 L1，当前
路线已转为 P1/G1，未来
L2 仍必须重新接受 DeepSeek/Qwen 数据边界并提供 exact authorization，普通“继续/好的”不替代授权。详见
...
T1 已把 strict no-raw diagnostic parser、五阶段/boundary/wire 校验和三个 family 私有 single-consume capability
落地；T2 又完成 `30/30` matrix、`15/15` classifier、strict journal state machine、partial/terminal prefix recovery、
幂等 publication recovery、multiple-marker rejection、hard-link artifact 与 Windows/Bun fsync compatibility。最终
...
### 历史封存：Transport Re-entry V2 L1（2026-08-08）
