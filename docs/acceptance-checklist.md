...
V7 不是 V6 retry。Task 1--6 已完成离线 usage-parity recovery：合法 actual input `97` 不再被 preview `96` 拒绝；raw audit 只保留冻结的 `missing/invalid/positive` shape；预算固定为 1 canary + 22 paired runtime，即最多 23 attempts，最坏 reservation CNY `0.18726 < 1.00`。V7 使用独立 evidence/once marker/CLI，当前 V1--V6 immutable snapshot 为 integrity-v3、18 entries、tree hash `9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba`；真实 V7 marker/evidence 尚不存在。direct Mock report 为 `mock_quality_not_evidence`，strict-fake evaluator 回归另标 `mock_quality_not_live_evidence`，两者均为 `48/26/22/48/48/0` 的离线证据。focused gate 为 AI 190、Server 86、native 9/40 assertions；全量 gate 为 AI 190、Agent 406、Server 980 passed/30 skipped、Web 409，相关 lint/typecheck/build、Compose 静态 `config --quiet` 与 diff check 均通过。V7 offline engineering ready；controlled-Live 未运行且未授权，两个产品 gate 继续为 `false`。
...
若要继续，先完成 V7 Task 7 两轮最终离线复审并停止。只有用户之后重新明确授权精确 V7 confirmation，才可执行唯一一次 controlled-Live；不得用当前离线证据、V6、Docker 或历史 run 替代。即使 V7 Live 成功，也仍需 Docker/API/可见浏览器/Trace 产品验收、合并 main 后复验与远程推送。两个业务 gate 始终为 `false`，直至独立质量与产品验收完成。本阶段记录见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`；当前不包含 V7 provider 质量、成本、延迟或产品启用结论。

Task 7 第一轮复审 contract/security（preview/actual、usage mapping、预算/output/reservation/cost、one-fetch/no-retry、脱敏、V1--V6 bytes、marker race、只读权限、default-off gate），第二轮复审 exact confirmation 到 terminal seal 的唯一执行路径及 Mock/failed-evidence 语义；未关闭 Critical/Important 为 0 才能通过。未来 V7 Live complete 门固定为 `48 cases / 26 verified zero-call / 22 runtime / 48 strict / 48 quality / 0 critical`、semantic quality `>=90%`、P95 `<=4500ms`、attempts `=23`、usage 在 `42_996/9_712` reservation 内且 observed cost `<= CNY 1.00`。产品验收必须分别覆盖 Review 与 Planner，两个 gate 独立且一次只临时开启一个；每个组件验完立即恢复 `false`，全部通过后才进入 main 复验。
