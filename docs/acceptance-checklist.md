...
**2026-07-20 host-wiring correction.** The old references below to an
injectable fake default host are superseded. The V12 default product and
recovery compositions now use a real, default-off host lifecycle: read-only
preflight, durable reservation and non-secret resource selectors precede any
synthetic account, Docker, API, browser or provider operation. This is still
offline code evidence only. The refreshed independent contract and operations
reviews have no unresolved P0/P1; no V12 product command may run until the
user gives fresh standalone authorization.

V12 also revalidates repository/evidence/default-off/database identity after
the product owner lock and before reservation. Its execution manifest retains
only a `DATABASE_URL` SHA-256 fingerprint, never the URL. Recovery obtains its
own owner lock, rereads the attempt, rejects fingerprint drift before writes,
and writes a one-time `recovered` terminal only after restore plus exact
cleanup. The headed V12 browser remains visible for 30 seconds before its
required exact cleanup; this does not constitute runtime evidence yet.
...
V12 product 报告 `operation_failed` 时必须停止，不能重试 product。只有 failure state 被 V12 recovery preflight 明确接纳，并取得另一次单独的用户授权时，才运行一次 standalone recovery；成功的 recovery CLI 终态为 `recovered`：
...
只有已包含 default-off restoration 与精确 cleanup receipts 的 branch `passed` 结果，才允许合并 main、执行 main replay、更新证据并推送；任何 standalone recovery 后均停止。V12 runtime 前后禁止 `down -v`、prune、volume 清理或 Docker 全量清空。完整离线记录见 `docs/acceptance/phase-6-9-5-review-planner-v12-offline-checkpoint.md`。
