...
V7 不是 V6 retry。Task 1--7 已完成离线 usage-parity recovery：合法 actual input `97` 不再被 preview `96` 拒绝；raw audit 只保留 `missing/invalid/positive`；预算固定为 1 canary + 22 paired runtime，最多 23 attempts，最坏 reservation CNY `0.18726 < 1.00`。V7 使用独立 evidence/once marker/CLI，V1--V6 snapshot 为 integrity-v3、18 entries、tree hash `9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba`；真实 V7 marker/evidence 尚不存在。成功必须由私有 `success_candidate` 与 exclusive、hash/history-bound success seal 共同构成；任一 unsealed/mismatched candidate 只能读为不含 token/cost 的 `evidence_io`。direct Mock 与 strict-fake 的 `48/26/22/48/48/0` 均为离线证据。focused gate 为 AI 190、Server 86、native 15/130 assertions；全量 gate 为 AI 190、Agent 406、Server 980 passed/30 skipped、Web 409。两轮复审均 PASS 且无 Critical/Important/Minor。V7 offline engineering ready；controlled-Live 未运行且未授权，两个产品 gate 继续为 `false`。
...
若要继续，必须由用户重新明确授权精确 V7 confirmation，才可执行唯一一次 controlled-Live；不得用当前离线证据、V6、Docker 或历史 run 替代。即使 V7 Live 成功，也仍需 Docker/API/可见浏览器/Trace 产品验收、合并 main 后复验与远程推送。两个业务 gate 始终为 `false`，直至独立质量与产品验收完成。本阶段记录见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`；当前不包含 V7 provider 质量、成本、延迟或产品启用结论。

Task 7 contract/security 与 acceptance/operations 两轮复审已通过，Critical/Important/Minor 均为 0；success seal、failed-evidence、唯一 CLI、只读权限和 default-off gate 均已复核。未来 V7 Live complete 门固定为 `48 cases / 26 verified zero-call / 22 runtime / 48 strict / 48 quality / 0 critical`、semantic quality `>=90%`、P95 `<=4500ms`、attempts `=23`、usage 在 `42_996/9_712` reservation 内且 observed cost `<= CNY 1.00`。产品验收必须分别覆盖 Review 与 Planner，两个 gate 独立且一次只临时开启一个；每个组件验完立即恢复 `false`，全部通过后才进入 main 复验。
