...
> Phase 6.9.8 Architecture Recovery R0--R3 后续状态：R0 已冻结三类调用阶段机、`providerWire/runnerWire` 双层观察、
...
> journal、hard-link artifact、strict validator 与 crash-only recovery。R0--R3 未读取 credential、调用 Provider 或创建
> 正式 Recovery evidence；当前只解锁 R4 zero-provider reviewed Mock/static。
...
- Phase 6.9.8 Architecture Recovery R0--R3 职责：新 lineage 只定义独立评测 diagnostic/durability 信任边界，不进入当前产品数据流。R0
  ...
  answer、unknown key、credential、URL 或 raw error。R0--R3 external `providerCalls=0`，只形成 runner/durability/
  admission authority，不形成 reviewed Mock、Live、产品或质量 authority。
...
Architecture Recovery Provider Canary V2（D0/C1/C2/S1/L1 complete）
  ...
  -> Architecture Recovery R0--R3 complete / providerCalls=0 / qualityAuthority=none
       ...
       -> formal tag/marker/journal/artifact/claim = 0；next only R4 reviewed Mock/static
