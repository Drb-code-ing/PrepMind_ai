...
- Operator Audit：operator/admin 诊断写操作审计是在线运维留痕能力，不进入 Dexie `mutationQueue`；审计写入失败只记录脱敏 warning，不影响主操作。
