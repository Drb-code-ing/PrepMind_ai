...
> 当前版本：2026-07-17。Phase 7 核心工程化与 Phase 7.8.5 RAG runtime parity 已完成真实 Docker 验收。Router/Verifier 已完成混合模型生产验收并恢复默认关闭；Review/Planner 的受限只读候选路径仍在 Phase 6.9.5 验收未完成状态。v1--v6 controlled-Live profile 都是不可重跑的独立终态；V7 Task 1--6 已完成离线工程与全量门禁，但 controlled-Live 未运行且未授权。两条业务 gate 默认 `false`，当前产品路径仍 deterministic；下一步是 V7 Task 7 两轮最终离线复审并停在新的授权边界。全部 Agent 架构完成前不进入 Phase 6.10 分层记忆；权威路线见 `docs/superpowers/specs/2026-07-15-phase-6-9-agent-architecture-completion-design.md`。
...
- `@repo/ai` 的 `ModelAgentRuntime` 不替换最终流式 provider；Router/Verifier 已完成结构化候选的生产验收且组件 gate 默认关闭。Review/Planner 已接入受限只读 candidate，但 v1--v6 controlled-Live 均为关闭终态，V7 仅完成离线 Task 1--6，当前业务 gate 默认 `false`。Memory 与其他业务 Agent 尚未接入该 runtime。
...
- V7 Task 1--6 只增加独立诊断工程，不改变上述产品数据流。它修复 preview `96` 与 actual input `97` 的 usage parity，raw audit 只留下 `missing/invalid/positive`，sanitized composition identity 只暴露 provider/model/base identity/structured mode/timeout/schemaId；真实 URL、credential、pricing、executor、prompt 与 owner facts 不进入公开 config、Trace 或 DTO。V7 eval gate 不在 Docker、Web、worker 或 server config allowlist，也不能替代两个产品 gate。直接 Mock 与 strict-fake evaluator 的 `48/26/22/48/48/0` 都是离线 evidence；controlled-Live 未运行且未授权，真实 V7 marker/evidence 不存在，因此当前 `/review-agent/suggestions` 仍走 deterministic read-only path。
