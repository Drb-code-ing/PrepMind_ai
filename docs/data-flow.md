...
>
> 用户随后决定停止整套 Vn 重试并进入独立 Architecture Recovery。R1 新增 transport diagnostic wrapper：sealed V1 direct adapter 与公共 `transport` category 不变，future delegate throw 只在新 adapter 内存中被安全映射为九个固定 subtype。R2 已完成 zero-network Provider health canary contract/runner：closed scenario enum 只进入模块内 synthetic responder，外部不能注入 fetch/transport/credential；Mock 与 `21/21` fault matrix authority 固定为 `synthetic_test`。R3 已完成独立 controlled-Live composition、专用授权/credential、一次性 marker、wire journal、crash-only seal、exclusive evidence 与 validator。首次授权 CLI 只完成 source/credential preflight，随后因 Windows 默认 evidence root 尾分隔符的旧字符串围栏误判在 reservation 前 zero-call 失败；根因已修复，正式 artifact 仍为 0。旧 exact confirmation 不复用，下一步真实 canary 仍需用户新的精确授权。
...
Architecture Recovery R3（zero-provider controlled-Live boundary）
  ...
  -> 当前正式 marker/journal/claim/artifact=0；已重新接受数据边界，等待新的精确 Live 授权
```
