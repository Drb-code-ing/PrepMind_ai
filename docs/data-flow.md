...
> 当前 P1（2026-08-08）：L1 transport success 不进入产品语义流；下一条 zero-provider 语义流固定为
> `8 guards -> 6 rewrite baseline/candidate pairs -> 6 FinalResponse projector/candidate lanes -> local ledger/scorer`。
> manifest/policy/baseline anchor SHA 为 `e7216d07...4fab2`、`ab6a453a...d146`、`63748b92...f503b`；最大并发 `1`，
...
> BackgroundJob、Outbox 或业务数据；S2 gate 固定 `p1_mock_quality_not_evidence / qualityAuthority=none`。P1 设计、计划与验收见
