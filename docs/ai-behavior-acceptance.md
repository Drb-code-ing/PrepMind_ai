...
重跑。其解锁的 P1 zero-provider 小样本 semantic gate 设计与 G1 contract/baseline 已完成；当前只允许 G2
zero-provider runner/durability。
...
backfill。P1/G1 未读取 credential 或调用 Provider；G2/S2 完成并推送前，L2 精确授权门不会开放。
...
`executorProvenance=deepseek_network` 的 schema pass 也不构成真实 provenance。G2 必须由不可注入 one-shot
runner、durable journal/marker/artifact 与重算 validator 建立来源，S2 reviewed Mock 之后才可能请求 L2。

G1 全程没有改 prompt、candidate 或最终 Chat 输出，也未运行 Mock/Live，因此本阶段不需要、也不允许用一般
live smoke 规则追加 Provider 调用。当前 48-case、产品 Docker/API/browser 和 main 仍被阻断。
