...
V10 是独立的最小修复 lineage，不重跑或改写 V1--V9：生产有效模型输出收窄为 Review `focusIndexes` 与 Planner `blockOrder`，安全扫描仍覆盖完整 snapshot，本地继续重建 facts、策略、FSRS、分钟数、链接和写权限。当前仅完成离线门：V1--V9 fresh manifest 为 `36` entries / `61a6e4a956784a59a8b8639d4c94d6fd870bce5dd8549a026abf02a0e7cb769d`，V10 evidence/once/success seal 不存在；Jest `266/266`、Agent `409/409` 与 typecheck、server lint/build、V10 native `3/3`、diff check 均通过。唯一 Live 必须用根目录 `bun --env-file=.env` 注入凭据，绝不改写 `.env`；同一独立进程只设 V10 eval gate=true，并显式设 V8/V9 eval gates 和 Review/Planner 产品 gates=false。配置固定 `deepseek-v4-pro`、JSON-object non-thinking、`4500ms`、最多 `23` attempts/`22` paired admissions、CNY `1.00` cap。V10 safe writer/reader 只接受严格 lane aggregate，拒绝 prompt、snapshot、model output、raw error、URL、credential、cookie、stack 与 per-case timing/usage；任何非成功终态都禁止 Docker、浏览器、main 或 push。完整 preflight 与精确命令见 `docs/acceptance/phase-6-9-5-review-planner-v10-offline-checkpoint.md`。
