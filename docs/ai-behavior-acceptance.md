...
每个获批 controlled-Live profile 必须是 server-only、单诊断/单 run、零 retry、原子脱敏 evidence。任一 `diagnostic_blocked`、`invalid_attempted`、质量/安全/权限/延迟/usage/cost 门失败都会保持两个 gate 关闭；不得用 Docker HTTP 成功、浏览器文案或历史证据替代本次语义评测。新 profile 只能在新的零网络根因设计与复审后创建，且不得覆盖、复用或拼接既有 evidence、once marker 或计数。当前 v1--v4 皆为 `invalid_attempted / structured_output`（v3/v4 私有 evidence 记录 `structuredOutputStage=provider_json_parse`），v5 则为 `invalid_attempted / closed / providerAttemptCount=1 / usageKnown=false / structured_output`；五个 profile 都不可重跑。V6 Task 1--6 已完成 default-off 的离线 transport/resolver/factory/evidence/CLI/Mock/复审：只允许精确 DeepSeek V4 Pro `/v1` JSON request 写入 `thinking:{type:'disabled'}`，并在本地拒绝 tool/schema drift 与任何暴露 reasoning 的 response。complete evidence 的 reasoning audit 只允许 `not_reported` 或 `reported_zero`，但 CNY 仍按完整 provider completion aggregate 记账；immutable no-reparse V1--V5 snapshot 与 V6 private provisional/seal capability 防止历史重解析或伪造终态。hardening 后 focused V6 suite 为 61/61、native evidence 为 15/15；一次 Mock proof 为 48 cases / 26 zero-call / 22 runtime / 48 strict / 0 critical / `mock_quality_not_evidence`，临时 `.tmp` 已删除；fake CLI 的 31/31 是 hardening 前离线回归。V6 CLI 只接受精确确认参数但本任务未执行，V6 evidence/marker/provider attempt 仍不存在。最多 1 canary + 22 paired case（23 次），worst-case reservation CNY `0.18726`、hard cap CNY `1.00` 不代表实际费用或真实模型通过。两条业务 gate 继续默认 `false`；必须由用户另行明确授权一次后才可执行 V6 controlled-Live，且其结果也不自动开启产品 gate。证据见 `docs/acceptance/phase-6-9-5-review-planner-live-diagnostic.md`，不声明真实模型通过。
