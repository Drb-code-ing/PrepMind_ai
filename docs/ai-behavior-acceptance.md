...
typecheck/lint 通过；未读取 credential、调用 Provider、创建 V4 Live artifact 或启动产品验收。下一步
仅 R2 Tutor policy，完整证据见
