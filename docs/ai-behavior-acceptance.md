...
- structured-output resilience 零网络 checkpoint 已完成，但不改变上述 Live 结论。默认 `json_object` 只保证合法 JSON；显式 `deepseek_strict_tool` 只允许 `deepseek-v4-flash` 与精确 `https://api.deepseek.com/beta`，使用唯一 forced synthetic function `model_agent_result`、`strict: true`，不发送 `response_format/json_schema`。该 function 不得有 handler、业务执行、副作用或 MCP 调用；
...
- Live 受控 preflight 必须先完成 schema 编译/校验、安全 start timestamp 和 dependencies/strict executor 本地初始化，再允许 UUID、evidence reservation/fs、runner 或 Provider attempt。schema 只有明确 `true` 才继续，本地初始化抛错也必须以 `live_config_invalid` 零副作用结束。新 Live 报告必须使用 `phase-6.9.4.3-runner-v2` + `deepseek_strict_tool_v1`；历史 runner v1 Live 只读兼容，Mock v1/v2 均禁止携带 Live transport 字段；
...
- fresh 零网络 gates 为 AI 151 passed、Agent 344 passed、typecheck/lint exit 0，deterministic 74/100 critical=2；Mock complete 的 `caseEntries/runtimeInvocations/providerAttempts/strictSuccesses/zeroCallCases = 100/28/0/28/72`；zero-call Live config exit 3 且 evidence `4 -> 4`。这些只证明工程 contract；Router / Verifier 仍 `enabled=false`，production 继续 deterministic；
- 下一任务只能在当前分支合并 main、main 复验并推送后，从新 main 创建零网络 Provider compatibility diagnostics；诊断固定码与 Mock 门禁通过后，新的 controlled-Live 才能完整重跑 100 cases，只有 28/28 strict success、72/72 zero-call 且所有质量、安全、权限、延迟、token、usage provenance 和成本门槛通过才能完成；
- 完整证据与 pricing/cost/decision 解释见 `docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md`。

回顾时可以问：“为什么 `deepseek_strict_tool` 仍不能取代 canonical Zod？”“为什么 zero-call preflight 要早于 evidence reservation？”“什么证据才足以把 Router / Verifier 从 `enabled=false` 改为可启用？”

下一会话可以复制：“请先合并并在 main 复验 Attempt E evidence，推送后从新 main 开零网络 Provider compatibility diagnostics，不要重跑 Live。”
