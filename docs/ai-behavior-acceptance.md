...
- controlled-Live 必须显式双开关、固定 provider/model、串行、单 case 10 秒、无自动重试，并在运行前用 pricing snapshot 验证 96,000 input + 4,080 output 的 worst-case 成本不超过 USD 0.10；
...
- 2026-07-14 diagnostics 后的 canonical Attempt C 在首个 eligible case 得到 `PROVIDER_ERROR / structured_output` 后停止：`observed/notRun=37/63`、`providerAttempts/strictSuccesses=1/0`、Router/Verifier 均为 `enabled=false / usage_unverifiable`。此前 Attempt A/B 原样保留，A 的 18ms evidence identity 缺陷已修复但 artifact 仍非 canonical；
...
- 本次仍没有 complete Live 质量证据，Phase 6.9.4.3 验收未完成并禁止进入 enablement。下一任务先按 TDD 把 Router/Verifier 单次 structured-output 上限统一提高到 400，并同步 per-agent/global budget、provider ceiling、pricing 与 strict evidence contract；修复合并后才允许新 controlled-Live。只有新 run 达到 28 次 strict success 并通过全部门槛后才可标记阶段完成；
