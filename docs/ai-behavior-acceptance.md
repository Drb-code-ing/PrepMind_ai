...
以上是 Phase 6.9.4.3 的历史评测与当时生产结论，必须原样保留，但不得解释为永久禁止 Router 模型。2026-07-15 的当前产品决策是 Phase 6.9.4.4 使用高置信/安全 zero-call、歧义 Router 真实模型、semantic-needed Verifier 与 deterministic fallback 的混合生产路径；该路径在完成 controlled-Live、Docker、可见浏览器和 main 复验前保持默认关闭。
...
V7 usage-parity profile 已完成 Task 1--7 离线工程：provider actual input `97` 可高于工程 preview `96`，同时 output、aggregate reservation、23 attempts 与 CNY hard cap 继续受控；cloned raw audit 只保存冻结的 `missing/invalid/positive` shape，不保存 token 数值。V7 evidence/once marker/CLI 与 v1--v6 完全隔离，historical integrity-v3 snapshot 为 18 entries / tree hash `9f8cc9a7d5ba83d630fa5806f19aaa74066352de92bb04631813c17feaa230ba`，真实 V7 marker/evidence 尚未创建。成功证据必须由私有 `success_candidate` 与 exclusive、hash/history-bound success seal 共同构成；standalone、unsealed、reparse 或 hash/tree/commitment 不匹配一律由唯一公开 reader 投影为不含 token/cost 的 `evidence_io`。production parity 只公开六字段 sanitized identity，V7 eval gate 不进入产品 allowlist，也不能替代两个产品 gate。direct Mock 与 strict-fake 的 `48/26/22/48/48/0` 都只是离线证据。focused V7 gate 为 AI 190、Server 86、native 15/130 assertions；全量离线 gate 为 AI 190、Agent 406、Server 980 passed/30 skipped、Web 409。contract/security 与 acceptance/operations 两轮复审均 PASS 且无 Critical/Important/Minor。当前结论只能是 `V7 offline engineering ready; controlled-Live not run and not authorized`；产品路径仍 deterministic。下一步只能在新的单独授权后执行唯一 V7 Live。
