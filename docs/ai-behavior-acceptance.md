...
V8 stage-diagnostics completion contract 与离线实现已完成，但唯一 controlled-Live 尚未运行。它使用全新 profile/eval gate/confirmation/evidence/success seal，V1--V7 immutable snapshot 与 15 个零字节、固定文件名、append-only stage markers；marker 缺口、乱序、重复、未知项、reparse 或 manifest mismatch 均 fail-closed。V8 保留 DeepSeek V4 Pro non-thinking、48/26/22、23 attempts、P95 4500ms、`42_996 / 9_712` reservation 和 CNY 1.00 hard cap，paired Live 时两个产品 gate 必须为 `false`。branch/main 产品 runner 还以 durable slot/usage ledger、双层原子 admission、精确 container/profile identity、Trace/facts/owner isolation 与 default-off restore 限制每环境四次产品请求。最终离线门与两轮独立复审已通过；真实 V8 evidence/once marker、provider 调用和产品 evidence 均不存在。只有 committed success 才能进入 Review-only 与 Planner-only 产品验收，且每个组件验完必须恢复 gate 并重建 Nest `server`；V8 paired lineage 在 main 不重跑。
