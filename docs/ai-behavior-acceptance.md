...
V8 stage-diagnostics completion contract 已冻结但尚未实现或运行。它使用全新 profile/eval gate/confirmation/evidence/success seal，V1--V7 immutable snapshot 与 15 个零字节、固定文件名、append-only stage markers；marker 缺口、乱序、重复、未知项、reparse 或 manifest mismatch 均 fail-closed。V8 保留 DeepSeek V4 Pro non-thinking、48/26/22、23 attempts、P95 4500ms、`42_996 / 9_712` reservation 和 CNY 1.00 hard cap，paired Live 时两个产品 gate 必须为 `false`。只有 committed success 才能进入 Review-only 与 Planner-only 产品验收，且每个组件验完必须恢复 gate 并重建 Nest `server`；V8 paired lineage 在 main 不重跑。
