...
V9 是在不改写 V1--V8 的前提下建立的独立 gate-diagnostics lineage。截至 `683a209`，paired aggregate、durable evidence、一次性 CLI 与只接受 V9 committed success 的 product authority 已完成离线实现；V9 Live 尚未运行，evidence directory、once marker 与 success seal 均不存在。独立 V9 eval gate 与 `REVIEW_AGENT_MODEL_ENABLED` / `PLANNER_AGENT_MODEL_ENABLED` 当前未设置，缺省关闭，产品仍 deterministic。V9 product authority 只接受 `finalized / complete / closed / passed`、23 provider attempts、22 paired admissions 和合法 evidence SHA，并要求完整 V9 leaf 全部以 ordinary `H` 被 Git 精确跟踪且读取前后 repository snapshot 稳定；任一失败都在 ledger、Prisma、Docker、浏览器前阻断，不回退 V8 reader 或 `git show`。离线测试、Mock、build 和 Compose 不得解释为 V9 Live、provider quality 或 Phase 6.9.5 完成；完整证据见 `docs/acceptance/phase-6-9-5-review-planner-v9-offline-checkpoint.md`。
