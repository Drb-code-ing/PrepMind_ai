  ...
  return (
    <main className="mx-auto flex h-full w-full max-w-[27rem] flex-col justify-center">
      ...
      <section className="pm-auth-panel pm-enter rounded-[1.35rem] p-[clamp(0.75rem,1.8dvh,1rem)]">
        ...
        <div className="mt-2.5 flex items-center justify-center text-sm">
...
function ErrorBanner({ message }: { message: string | null }) {
  ...
  return (
    <div className="mb-2.5 rounded-[1rem] border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold leading-5 text-red-600">
      {message}
    </div>
  );
}
...
function FieldHint({ id, error }: { id: string; error: FieldError | undefined }) {
  return (
    <p id={id} className="min-h-[0.8rem] pt-0.5 text-[11px] font-semibold leading-none text-red-500">
      {error ?? ''}
    </p>
  );
}
