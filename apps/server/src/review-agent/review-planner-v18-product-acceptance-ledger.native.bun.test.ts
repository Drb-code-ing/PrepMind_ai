...
import {
  finalizeReviewPlannerV18ProductAcceptanceRecovery,
  readReviewPlannerV18ProductAcceptanceLedger,
...
import {
  ...
  openReviewPlannerV18ProductAcceptanceRecoveryJournal,
...
describeWindows('Review/Planner V18 durable product acceptance ledger', () => {
  let root = '';

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'prepmind-v18-ledger-'));
  ...
  it('creates one V18-only reservation whose public, binding, and private execution hashes agree', async () => {
    const state = await prepareEarliestV18State(root);
  ...
  it('permits only one V18 reservation even after the first owner releases its private lock', async () => {
    const state = await prepareEarliestV18State(root);
    const next = await acquireReviewPlannerV18ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V18_PRODUCT_ACCEPTANCE_ALREADY_RESERVED');
    } finally {
      next.owner.close();
    }
  ...
  it('rolls back a verified unstarted reservation so a fresh V18 reservation can be created', async () => {
    const acquired = await acquireReviewPlannerV18ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    const ledger = await reserveReviewPlannerV18ProductAcceptanceLedger({
      repoRoot: root,
      environment: 'branch',
    ...
    try {
      ...
      ledger.writeManifest({
        schemaVersion: 'phase-6.9.5-v18-product-acceptance-manifest-v1',
        environment: 'branch',
        attemptSha256,
      });
    ...
    const fresh = await acquireReviewPlannerV18ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      const next = await reserveReviewPlannerV18ProductAcceptanceLedger({
        repoRoot: root,
        environment: 'branch',
        owner: fresh.owner,
      });
  ...
  it('admits the strict earliest V18 state with no checkpoint for recovery', async () => {
    const state = await prepareEarliestV18State(root);
    const owner = await acquireReviewPlannerV18ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      const journal =
        await openReviewPlannerV18ProductAcceptanceRecoveryJournal({
          repoRoot: root,
          environment: 'branch',
          owner: owner.owner,
        });
  ...
  it('fails closed for a bare reservation and a malformed checkpoint', async () => {
    const publicRoot = join(root, ...v18PublicSegments('branch'));
  ...
  it('fails closed for a V11 manifest injected into a V18 reservation', async () => {
    const state = await prepareEarliestV18State(root);
    const publicRoot = join(root, ...v18PublicSegments('branch'));
    await writeFile(
  ...
  it('seals exactly four V18 slots and leaves V11/V12/V13/V14/V15 root bytes untouched', async () => {
    ...
    try {
      ...
      for (const [slot, traceSha256] of [
        ...
        ledger.recordSlotResult({
          schemaVersion: 'phase-6.9.5-v18-product-acceptance-slot-result-v1',
          slot,
          traceSha256,
        });
      }
      ...
      ledger.recordOwnerIsolation({
        schemaVersion: 'phase-6.9.5-v18-product-acceptance-owner-isolation-v1',
      ...
      ledger.recordCleanup({
        schemaVersion: 'phase-6.9.5-v18-product-acceptance-cleanup-v1',
        syntheticAccounts: 0,
        fixtures: 0,
        traces: 0,
        browserProfiles: 0,
      ...
      await ledger.finalizeSuccess({
        schemaVersion: 'phase-6.9.5-v18-product-acceptance-aggregate-v1',
        environment: 'branch',
        attemptSha256,
        requests: 4,
        durationMs: 4_000,
  ...
  it('rejects a success terminal after a failure terminal has been written', async () => {
    ...
    try {
      ...
      ledger.recordFailure({
        schemaVersion: 'phase-6.9.5-v18-product-acceptance-failure-v1',
        environment: 'branch',
        attemptSha256,
        component: 'review',
        slot: 'api',
      ...
      ).toThrow('V18_PRODUCT_ACCEPTANCE_LEDGER_RECORD_INVALID');
    } finally {
      ledger.close();
      acquired.owner.close();
    }
  ...
  it('seals recovery exactly once and rejects a second recovery owner', async () => {
    const state = await prepareFailedV18State(root);
    const recovery = await acquireReviewPlannerV18ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      await finalizeReviewPlannerV18ProductAcceptanceRecovery({
        repoRoot: root,
        environment: 'branch',
    ...
    const second = await acquireReviewPlannerV18ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V18_PRODUCT_ACCEPTANCE_RECOVERY_NOT_AUTHORIZED');
    } finally {
      second.owner.close();
    }
    expect(state.attemptSha256).toHaveLength(64);
  ...
  it('fails closed when a failure terminal disagrees with its attempt-bound latest checkpoint', async () => {
    await prepareFailedV18State(root);
    await writeFile(
      join(root, ...v18PublicSegments('branch'), 'failure.json'),
      JSON.stringify({
        schemaVersion: 'phase-6.9.5-v18-product-acceptance-failure-v1',
        environment: 'branch',
...
async function prepareEarliestV18State(root: string) {
  const acquired = await acquireReviewPlannerV18ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
...
async function prepareFailedV18State(root: string) {
  const acquired = await acquireReviewPlannerV18ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
  ...
  let journal:
    | Awaited<
        ReturnType<
          typeof prepareReviewPlannerV18ProductAcceptanceRecoveryJournal
        >
      >
    | undefined;
  try {
    ...
    journal = await prepareReviewPlannerV18ProductAcceptanceRecoveryJournal({
      repoRoot: root,
      environment: 'branch',
      owner: acquired.owner,
    });
    journal.appendCheckpoint({
      schemaVersion: 'phase-6.9.5-v18-product-acceptance-checkpoint-v1',
      component: 'review',
      slot: 'api',
      checkpoint: 'review_api_setup',
      providerCallState: 'not_started',
...
function v18PublicSegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_PROFILE.publicLedgerSegments(
    environment,
  );
}
...
function v18RecoverySegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_PROFILE.recoverySegments(
    environment,
  );
}
...
function executionManifest(
  ...
  return {
    schemaVersion: 'phase-6.9.5-v18-product-acceptance-execution-manifest-v1',
    environment,
    attemptSha256,
    databaseUrlSha256: 'e'.repeat(64),
    resources: {
      accountId: {
        review: `v18-synthetic-account-review-${'a'.repeat(32)}`,
        planner: `v18-synthetic-account-planner-${'b'.repeat(32)}`,
      },
      fixtureId: {
        review: `v18-synthetic-fixture-review-${'c'.repeat(32)}`,
        planner: `v18-synthetic-fixture-planner-${'d'.repeat(32)}`,
      },
      browser: {
        executablePath:
...
function defaultOffReceipt(component: 'review' | 'planner') {
  ...
  return {
    schemaVersion: 'phase-6.9.5-v18-product-acceptance-default-off-v1',
