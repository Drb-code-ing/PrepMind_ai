...
import {
  assertReviewPlannerV21DefaultOffEnvironment,
  finalizeReviewPlannerV21ProductAcceptanceRecovery,
  readReviewPlannerV21ProductAcceptanceLedger,
  readReviewPlannerV21ProductAcceptanceExecutionManifest,
  type ReviewPlannerV21ProductAcceptanceExecutionManifest,
  type ReviewPlannerV21ProductAcceptanceLedger,
} from './review-planner-v21-product-acceptance-ledger';
import type { ReviewPlannerV21ReadyPreflight } from './review-planner-v21-product-acceptance-composition';
import {
  acquireReviewPlannerV21ProductAcceptanceOwner,
  type ReviewPlannerV21ProductAcceptanceRecoveryJournal,
...
const V21_BUN_AUTHORITY_TIMEOUT_MS = 5_000;
const V21_BUN_AUTHORITY_MAX_BUFFER = 8 * 1024;
const V21_BUN_AUTHORITY_HELPER_RELATIVE_PATH =
  'apps/server/scripts/review-planner-v21-v10-paired-evidence-authority.ts';

type ReviewPlannerV21BunAuthorityHelper = (
  input: Readonly<{
    file: 'bun';
...
type ReviewPlannerV21BunPairedEvidenceAuthorityOptions = Readonly<{
  executeBunHelper?: ReviewPlannerV21BunAuthorityHelper;
}>;

export function createReviewPlannerV21BunPairedEvidenceAuthority(
  repoRoot: string,
  options: ReviewPlannerV21BunPairedEvidenceAuthorityOptions = {},
): PairedEvidenceAuthority {
  const root = resolve(repoRoot);
  const helperPath = resolve(root, V21_BUN_AUTHORITY_HELPER_RELATIVE_PATH);
  ...
  return createReviewPlannerV10PairedEvidenceAuthority({
    readEvidence: async (requestedRoot) => {
      if (resolve(requestedRoot) !== root) {
        return v21BunAuthorityFallback();
      }
      try {
        const result = await executeBunHelper({
          ...
          cwd: root,
          timeoutMs: V21_BUN_AUTHORITY_TIMEOUT_MS,
          maxBuffer: V21_BUN_AUTHORITY_MAX_BUFFER,
        });
        return parseReviewPlannerV21BunAuthorityRecord(result.stdout);
      } catch {
        return v21BunAuthorityFallback();
      }
    },
...
function parseReviewPlannerV21BunAuthorityRecord(stdout: string) {
  if (
    typeof stdout !== 'string' ||
    stdout.length > V21_BUN_AUTHORITY_MAX_BUFFER
  ) {
    return v21BunAuthorityFallback();
  }
  try {
...
function v21BunAuthorityFallback() {
  return Object.freeze({
...
export type ReviewPlannerV21ProductAcceptanceHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
  }): Promise<Readonly<{ status: 'blocked' }> | ReviewPlannerV21ReadyPreflight>;
  revalidate(
    preflight: ReviewPlannerV21ReadyPreflight,
  ): Promise<Readonly<{ status: 'blocked' }> | ReviewPlannerV21ReadyPreflight>;
  prepareExecution(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
    preflight: ReviewPlannerV21ReadyPreflight;
    ledger: ReviewPlannerV21ProductAcceptanceLedger;
    journal: ReviewPlannerV21ProductAcceptanceRecoveryJournal;
    executionManifest: ReviewPlannerV21ProductAcceptanceExecutionManifest;
  }): Promise<RuntimeResources>;
  dispose(): Promise<void>;
}>;

export type ReviewPlannerV21ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V21-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V21 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV21ProductAcceptanceHost(
  repoRoot: string,
): ReviewPlannerV21ProductAcceptanceHost {
  const root = resolve(repoRoot);
  const hostDatabaseUrlSha256 = sha256(readRootDatabaseUrl(root));
  const pairedEvidenceAuthority =
    createReviewPlannerV21BunPairedEvidenceAuthority(root);
  let latest:
    | Awaited<
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV21ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V21_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
  ...
  const getReadyPreflight = async (input: {
    ...
    Readonly<{ status: 'blocked' }> | ReviewPlannerV21ReadyPreflight
  > => {
    if (
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV21ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v21-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV21RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV21ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV21ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV21ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV21ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV21ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV21RecoveryHostOptions = {},
): ReviewPlannerV21ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
    options.readLedger ?? readReviewPlannerV21ProductAcceptanceLedger;
  const readExecutionManifest =
    options.readExecutionManifest ??
    readReviewPlannerV21ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV21ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV21ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV21ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV21ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV21ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV21ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV21ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V21_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV21DatabaseFingerprintStable(
