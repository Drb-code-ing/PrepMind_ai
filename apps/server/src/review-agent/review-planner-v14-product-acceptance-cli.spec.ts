...
import {
  parseReviewPlannerV14ProductAcceptanceArguments,
  runReviewPlannerV14ProductAcceptanceProductCli,
  serializeReviewPlannerV14ProductAcceptanceCliFailure,
} from './review-planner-v14-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V14 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V14 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V14_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V14 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V14 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V14 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV14ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V14_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV14ProductAcceptanceCliFailure(
      'product',
