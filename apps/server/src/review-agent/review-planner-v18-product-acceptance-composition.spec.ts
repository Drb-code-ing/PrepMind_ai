...
describe('Review Planner V18 default product composition', () => {
  it('keeps the unconfigured host boundary default-off before any runtime capability', async () => {
    ...
    await expect(
      composition.ports.preflight({
        environment: 'branch',
        repoRoot: 'E:\\v18-default-off',
      }),
    ).resolves.toEqual({ status: 'blocked' });
  });

  it('accepts an injected V18-only ready preflight without touching a V11 boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
        repoRoot: 'E:\\v18-ready',
    ...
    const create =
      createDefaultReviewPlannerV18ProductAcceptanceComposition as unknown as (
        root: string,
        options: { boundary: { preflight: typeof preflight } },
      ) => ReturnType<
        typeof createDefaultReviewPlannerV18ProductAcceptanceComposition
      >;
    const composition = create('E:\\v18-ready', {
      boundary: { preflight },
    });
  ...
  it('obtains the default preflight from a V18 host factory instead of a test-only boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
    ...
    const create =
      createDefaultReviewPlannerV18ProductAcceptanceComposition as unknown as (
        ...
        typeof createDefaultReviewPlannerV18ProductAcceptanceComposition
      >;
    const composition = create('E:\\v18-host', { hostFactory });
    ...
    expect(hostFactory).toHaveBeenCalledWith('E:\\v18-host');
    expect(preflight).toHaveBeenCalledTimes(1);
  });

  it('prepares the runtime only after the V18 reservation and journal exist', async () => {
    const order: string[] = [];
    const owner = {
      assertHeld: jest.fn(),
      close: jest.fn(),
  ...
  it('revalidates the ready identity after owner acquisition and before reserving V18', async () => {
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v18-revalidate',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('projects a reservation failure as default-off rather than an operation failure', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v18-reserve-failure',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('rolls back the verified unstarted reservation when journal setup fails after reservation', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v18-journal-failure',
      commitSha: 'b'.repeat(40),
      pairedEvidenceSha256: 'c'.repeat(64),
      databaseUrlSha256: 'f'.repeat(64),
    };
  ...
  it('fails closed when the unstarted reservation cannot be rolled back', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v18-rollback-failure',
      commitSha: 'b'.repeat(40),
      pairedEvidenceSha256: 'c'.repeat(64),
      databaseUrlSha256: 'f'.repeat(64),
    };
