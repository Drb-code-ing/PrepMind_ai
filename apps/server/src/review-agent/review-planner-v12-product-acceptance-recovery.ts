...
export type ReviewPlannerV12ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV12ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v12-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    ...
    checkpoint: z.enum(REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV12ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV12ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV12ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV12ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV12ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV12ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV12ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
export async function acquireReviewPlannerV12ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV12ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V12_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    const owner: ReviewPlannerV12ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V12_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V12_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV12ProductAcceptanceOwner(
  owner: ReviewPlannerV12ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV12ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V12_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV12ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV12ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
...
export async function bindReviewPlannerV12ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV12ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v12-product-acceptance-attempt-v1',
    ...
    throw new Error('V12_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV12ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV12ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV12ProductAcceptanceOwner;
...
export async function openReviewPlannerV12ProductAcceptanceRecoveryJournal(input: {
  ...
  const binding =
    await readReviewPlannerV12ProductAcceptanceAttemptBinding(input);
  return openV12Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV12ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV12ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV12ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV12ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV12ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V12_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV12Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V12_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV12ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V12_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV12ProductAcceptanceCheckpoint(value);
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV12ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV12ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV12ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV12ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V12_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function expectedProviderCallState(
  ...
  return record.checkpoint.endsWith('_dispatch') ||
    REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
      record.checkpoint,
    ) >
      REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV12ProductAcceptanceOwnerRole {
