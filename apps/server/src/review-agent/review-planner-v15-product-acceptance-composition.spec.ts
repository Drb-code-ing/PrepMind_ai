...
describe('Review Planner V15 default product composition', () => {
  it('keeps the unconfigured host boundary default-off before any runtime capability', async () => {
    ...
    await expect(
      composition.ports.preflight({
        environment: 'branch',
        repoRoot: 'E:\\v15-default-off',
      }),
    ).resolves.toEqual({ status: 'blocked' });
  });

  it('accepts an injected V15-only ready preflight without touching a V11 boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
        repoRoot: 'E:\\v15-ready',
    ...
    const create =
      createDefaultReviewPlannerV15ProductAcceptanceComposition as unknown as (
        root: string,
        options: { boundary: { preflight: typeof preflight } },
      ) => ReturnType<
        typeof createDefaultReviewPlannerV15ProductAcceptanceComposition
      >;
    const composition = create('E:\\v15-ready', {
      boundary: { preflight },
    });
  ...
  it('obtains the default preflight from a V15 host factory instead of a test-only boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
    ...
    const create =
      createDefaultReviewPlannerV15ProductAcceptanceComposition as unknown as (
        ...
        typeof createDefaultReviewPlannerV15ProductAcceptanceComposition
      >;
    const composition = create('E:\\v15-host', { hostFactory });
    ...
    expect(hostFactory).toHaveBeenCalledWith('E:\\v15-host');
    expect(preflight).toHaveBeenCalledTimes(1);
  });

  it('prepares the runtime only after the V15 reservation and journal exist', async () => {
    const order: string[] = [];
    const owner = {
      assertHeld: jest.fn(),
      close: jest.fn(),
  ...
  it('revalidates the ready identity after owner acquisition and before reserving V15', async () => {
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v15-revalidate',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('projects a reservation failure as default-off rather than an operation failure', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v15-reserve-failure',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('rolls back the verified unstarted reservation when journal setup fails after reservation', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v15-journal-failure',
      commitSha: 'b'.repeat(40),
      pairedEvidenceSha256: 'c'.repeat(64),
      databaseUrlSha256: 'f'.repeat(64),
    };
  ...
  it('fails closed when the unstarted reservation cannot be rolled back', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v15-rollback-failure',
      commitSha: 'b'.repeat(40),
      pairedEvidenceSha256: 'c'.repeat(64),
      databaseUrlSha256: 'f'.repeat(64),
    };
