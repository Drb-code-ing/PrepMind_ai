...
describe('Review Planner V21 default product composition', () => {
  it('keeps the unconfigured host boundary default-off before any runtime capability', async () => {
    ...
    await expect(
      composition.ports.preflight({
        environment: 'branch',
        repoRoot: 'E:\\v21-default-off',
      }),
    ).resolves.toEqual({ status: 'blocked' });
  });

  it('accepts an injected V21-only ready preflight without touching a V11 boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
        repoRoot: 'E:\\v21-ready',
    ...
    const create =
      createDefaultReviewPlannerV21ProductAcceptanceComposition as unknown as (
        root: string,
        options: { boundary: { preflight: typeof preflight } },
      ) => ReturnType<
        typeof createDefaultReviewPlannerV21ProductAcceptanceComposition
      >;
    const composition = create('E:\\v21-ready', {
      boundary: { preflight },
    });
  ...
  it('obtains the default preflight from a V21 host factory instead of a test-only boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
    ...
    const create =
      createDefaultReviewPlannerV21ProductAcceptanceComposition as unknown as (
        ...
        typeof createDefaultReviewPlannerV21ProductAcceptanceComposition
      >;
    const composition = create('E:\\v21-host', { hostFactory });
    ...
    expect(hostFactory).toHaveBeenCalledWith('E:\\v21-host');
    expect(preflight).toHaveBeenCalledTimes(1);
  });

  it('prepares the runtime only after the V21 reservation and journal exist', async () => {
    const order: string[] = [];
    const owner = {
      assertHeld: jest.fn(),
      close: jest.fn(),
  ...
  it('revalidates the ready identity after owner acquisition and before reserving V21', async () => {
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v21-revalidate',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('projects a reservation failure as default-off rather than an operation failure', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v21-reserve-failure',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('rolls back the verified unstarted reservation when journal setup fails after reservation', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v21-journal-failure',
      commitSha: 'b'.repeat(40),
      pairedEvidenceSha256: 'c'.repeat(64),
      databaseUrlSha256: 'f'.repeat(64),
    };
  ...
  it('fails closed when the unstarted reservation cannot be rolled back', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v21-rollback-failure',
      commitSha: 'b'.repeat(40),
      pairedEvidenceSha256: 'c'.repeat(64),
      databaseUrlSha256: 'f'.repeat(64),
    };
