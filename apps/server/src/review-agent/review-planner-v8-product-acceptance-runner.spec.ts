...
import { REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS } from './review-planner-v20-product-acceptance-recovery';
import { REVIEW_PLANNER_V21_PRODUCT_ACCEPTANCE_CHECKPOINTS } from './review-planner-v21-product-acceptance-recovery';
import { REVIEW_PLANNER_V22_PRODUCT_ACCEPTANCE_CHECKPOINTS } from './review-planner-v22-product-acceptance-recovery';
import {
...
describe('Review Planner V8 product acceptance runner', () => {
  ...
  it('admits the isolated V18 profile and records its complete checkpoint sequence', async () => {
    const fixture = createFixture();
    const diagnostics = {
      checkpoint: jest.fn(),
      publishFailure: jest.fn(),
  ...
  it('admits the isolated V20 profile and records its complete checkpoint sequence', async () => {
    const fixture = createFixture();
    const diagnostics = {
      checkpoint: jest.fn(),
      publishFailure: jest.fn(),
    ...
    expect(diagnostics.checkpoint.mock.calls.map(([value]) => value)).toEqual(
      REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS.slice(1),
    );
    expect(diagnostics.publishFailure).not.toHaveBeenCalled();
  });

  it('admits the isolated V21 profile and records its complete checkpoint sequence', async () => {
    const fixture = createFixture();
    const diagnostics = {
      checkpoint: jest.fn(),
      publishFailure: jest.fn(),
    ...
    expect(diagnostics.checkpoint.mock.calls.map(([value]) => value)).toEqual(
      REVIEW_PLANNER_V21_PRODUCT_ACCEPTANCE_CHECKPOINTS.slice(1),
    );
    expect(diagnostics.publishFailure).not.toHaveBeenCalled();
  });

  it('admits the isolated V22 profile and records its complete checkpoint sequence', async () => {
    const fixture = createFixture();
    const diagnostics = {
      checkpoint: jest.fn(),
      publishFailure: jest.fn(),
    ...
    expect(diagnostics.checkpoint.mock.calls.map(([value]) => value)).toEqual(
      REVIEW_PLANNER_V22_PRODUCT_ACCEPTANCE_CHECKPOINTS.slice(1),
    );
    expect(diagnostics.publishFailure).not.toHaveBeenCalled();
  });
