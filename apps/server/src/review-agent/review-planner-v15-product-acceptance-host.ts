...
import {
  ...
  readReviewPlannerV15ProductAcceptanceExecutionManifest,
...
import type { ReviewPlannerV15ReadyPreflight } from './review-planner-v15-product-acceptance-composition';
import {
...
export type ReviewPlannerV15ProductAcceptanceHost = Readonly<{
  preflight(input: {
...
export type ReviewPlannerV15ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V15-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V15 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV15ProductAcceptanceHost(
  repoRoot: string,
): ReviewPlannerV15ProductAcceptanceHost {
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV15ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V15_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
  ...
  > => {
    if (
      ...
      !isReviewPlannerV15DatabaseFingerprintStable(
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV15ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v15-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV15RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV15ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV15ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV15ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV15ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV15ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV15RecoveryHostOptions = {},
): ReviewPlannerV15ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
    options.readLedger ?? readReviewPlannerV15ProductAcceptanceLedger;
  const readExecutionManifest =
    options.readExecutionManifest ??
    readReviewPlannerV15ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV15ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV15ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV15ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV15ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV15ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV15ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV15ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V15_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV15DatabaseFingerprintStable(
