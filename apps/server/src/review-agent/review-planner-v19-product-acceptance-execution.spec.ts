import { createReviewPlannerV19ProductAcceptanceDiagnosticsPort } from './review-planner-v19-product-acceptance-diagnostics';
import { createReviewPlannerV19ProductAcceptanceRunnerLedgerAdapter } from './review-planner-v19-product-acceptance-execution';
import { runReviewPlannerV19ProductAcceptanceComposition } from './review-planner-v19-product-acceptance-composition';
import { REVIEW_PLANNER_V19_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V19 execution bridge', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('emits only fixed V19 checkpoints and a nonsecret failure projection', () => {
    ...
    const diagnostics = createReviewPlannerV19ProductAcceptanceDiagnosticsPort({
      environment: 'branch',
  ...
  it('durably commits exactly four V19 runner slots without V11 records', async () => {
    const ledger = {
      attemptSha256: () => attemptSha256,
      claimSlot: jest.fn(),
    ...
    const adapter = createReviewPlannerV19ProductAcceptanceRunnerLedgerAdapter({
      environment: 'branch',
      ledger,
      manifest: {
        schemaVersion:
          'phase-6.9.5-v19-product-acceptance-manifest-v1' as const,
  ...
  it('rejects a component-only default-off record instead of synthesizing evidence', () => {
    ...
    expect(() => adapter.recordDefaultOff({ component: 'review' })).toThrow(
      'V19_PRODUCT_ACCEPTANCE_EXECUTION_INVALID',
    );
    expect(ledger.recordDefaultOff).not.toHaveBeenCalled();
  });

  it('projects a strict V8 runner receipt into the V19 durable default-off record', () => {
    const ledger = {
      attemptSha256: () => attemptSha256,
      claimSlot: jest.fn(),
      recordSlotResult: jest.fn(),
  ...
  it('rejects an extra raw model instead of allowing it to influence the V8 projection', () => {
    ...
    ).toThrow('V19_PRODUCT_ACCEPTANCE_EXECUTION_INVALID');
    expect(ledger.recordDefaultOff).not.toHaveBeenCalled();
  });

  it('stops at V19 preflight before owner, ledger, runner, or external work', async () => {
    const ports = {
