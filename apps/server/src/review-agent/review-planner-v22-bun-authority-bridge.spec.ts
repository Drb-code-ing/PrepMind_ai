...
import * as subject from './review-planner-v22-product-acceptance-host';
...
function bridgeFactory(): BridgeFactory {
  const candidate = (
    subject as typeof subject & {
      createReviewPlannerV22BunPairedEvidenceAuthority?: unknown;
    }
  ).createReviewPlannerV22BunPairedEvidenceAuthority;
  if (typeof candidate !== 'function') {
    throw new Error('V22_BUN_AUTHORITY_BRIDGE_MISSING');
  }
  return candidate;
}

describe('Review Planner V22 Bun paired-evidence authority bridge', () => {
  beforeEach(() => {
    ...
    mockRunDefaultHostPreflight.mockImplementation(
      async (
        ...
        return Object.freeze({
          ...
          branchName: 'codex/v22-bun-authority-bridge',
  ...
  it('returns ready from the Node V22 host preflight when started in apps/server', async () => {
    ...
    try {
      ...
      if (preflight.status !== 'ready') {
        throw new Error('V22_BUN_AUTHORITY_PRELIGHT_BLOCKED');
      }
