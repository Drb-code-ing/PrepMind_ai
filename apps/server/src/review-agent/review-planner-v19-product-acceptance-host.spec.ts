...
import {
  createDefaultReviewPlannerV19ProductAcceptanceRecoveryHost,
  isReviewPlannerV19DatabaseFingerprintStable,
} from './review-planner-v19-product-acceptance-host';

describe('Review Planner V19 recovery host', () => {
  const databaseUrl = 'postgresql://v19-recovery.example.invalid/prepmind';
  const databaseUrlSha256 = createHash('sha256')
    .update(databaseUrl)
    .digest('hex');
  const executionManifest = {
    schemaVersion:
      'phase-6.9.5-v19-product-acceptance-execution-manifest-v1' as const,
    environment: 'branch' as const,
    attemptSha256: 'a'.repeat(64),
    databaseUrlSha256,
    resources: {
      accountId: {
        review: `v19-synthetic-account-review-${'b'.repeat(32)}`,
        planner: `v19-synthetic-account-planner-${'c'.repeat(32)}`,
      },
      fixtureId: {
        review: `v19-synthetic-fixture-review-${'d'.repeat(32)}`,
        planner: `v19-synthetic-fixture-planner-${'e'.repeat(32)}`,
      },
      browser: {
        executablePath:
  ...
  it('takes the recovery owner, revalidates its attempt, and seals only after restore plus exact cleanup', async () => {
    ...
    const host = createDefaultReviewPlannerV19ProductAcceptanceRecoveryHost(
      'E:\\v19-recovery-host',
      {
        platform: 'win32',
        readLedger,
  ...
  it('injects the V19 default-off validator into recovery restore without relaxing unsafe runtime values', async () => {
    mockRestoreDefaultReviewPlannerProductAcceptanceServer.mockClear();
    injectedDefaultOffEnvironmentValidator = undefined;
    const root = 'E:\\v19-default-validator';
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const host = createDefaultReviewPlannerV19ProductAcceptanceRecoveryHost(
      root,
      {
    ...
    if (typeof validator !== 'function') {
      throw new Error('V19 default-off validator was not injected');
    }
