...
import * as subject from './review-planner-v17-product-acceptance-host';
...
function bridgeFactory(): BridgeFactory {
  const candidate = (
    subject as typeof subject & {
      createReviewPlannerV17BunPairedEvidenceAuthority?: unknown;
    }
  ).createReviewPlannerV17BunPairedEvidenceAuthority;
  if (typeof candidate !== 'function') {
    throw new Error('V17_BUN_AUTHORITY_BRIDGE_MISSING');
  }
  return candidate;
}

describe('Review Planner V17 Bun paired-evidence authority bridge', () => {
  beforeEach(() => {
    ...
    mockRunDefaultHostPreflight.mockImplementation(
      async (
        ...
        return Object.freeze({
          ...
          branchName: 'codex/v17-bun-authority-bridge',
  ...
  it('returns ready from the Node V17 host preflight when started in apps/server', async () => {
    const previousCwd = process.cwd();
    process.chdir(appsServerRoot);
    try {
      const host = subject.createDefaultReviewPlannerV17ProductAcceptanceHost(
        root,
        {
      ...
      if (preflight.status !== 'ready') {
        throw new Error('V17_BUN_AUTHORITY_PRELIGHT_BLOCKED');
      }
