...
const V14_PUBLIC_SLOT_LEAF = /^slot-(review|planner)-(api|browser)\.json$/;
const V14_PUBLIC_DEFAULT_OFF_LEAF = /^default-off-(review|planner)\.json$/;
const V14_PUBLIC_RECEIPT_LEAVES = new Set([
...
export const REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_CHECKPOINTS = [
  'review_api_setup',
  'review_api_activate',
...
export type ReviewPlannerV14ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV14ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v14-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    ...
    checkpoint: z.enum(REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV14ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV14ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV14ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV14ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV14ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV14ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV14ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
export async function acquireReviewPlannerV14ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV14ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V14_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    directory = await openWindowsNoReparseFrozenDirectory(repoRoot, [
      ...REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_PROFILE.recoverySegments(
        input.environment,
      ),
    ]);
    ...
    const owner: ReviewPlannerV14ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V14_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V14_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV14ProductAcceptanceOwner(
  owner: ReviewPlannerV14ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV14ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V14_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV14ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV14ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
  assertReviewPlannerV14ProductAcceptanceOwner(owner, environment, ['product']);
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V14_PRODUCT_ACCEPTANCE_ATTEMPT_INVALID');
  }
  state.attemptSha256 = attemptSha256;
}

export async function bindReviewPlannerV14ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV14ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v14-product-acceptance-attempt-v1',
    ...
    throw new Error('V14_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV14ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV14ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV14ProductAcceptanceOwner;
}): Promise<ReviewPlannerV14ProductAcceptanceRecoveryJournal> {
  assertReviewPlannerV14ProductAcceptanceOwner(input.owner, input.environment, [
    'product',
  ]);
  ...
  return openV14Journal(input, state.attemptSha256, true);
}

export async function openReviewPlannerV14ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV14ProductAcceptanceOwner;
}): Promise<ReviewPlannerV14ProductAcceptanceRecoveryJournal> {
  ...
  return openV14Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV14ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV14ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV14ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV14ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV14ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V14_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV14Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V14_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV14ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V14_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV14ProductAcceptanceCheckpoint(value);
    ...
    if (
      ...
      leaves.some(
        (leaf) =>
          ...
          !V14_PUBLIC_SLOT_LEAF.test(leaf) &&
          !V14_PUBLIC_DEFAULT_OFF_LEAF.test(leaf) &&
          !V14_PUBLIC_RECEIPT_LEAVES.has(leaf),
      )
    ) {
      throw new Error();
    }
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV14ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV14ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV14ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV14ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V14_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function checkpointMatchesBoundary(
  value: ReviewPlannerV14ProductAcceptanceCheckpointRecord,
) {
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV14ProductAcceptanceOwnerRole {
