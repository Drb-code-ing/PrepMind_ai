...
const V20_PUBLIC_SLOT_LEAF = /^slot-(review|planner)-(api|browser)\.json$/;
const V20_PUBLIC_DEFAULT_OFF_LEAF = /^default-off-(review|planner)\.json$/;
const V20_PUBLIC_RECEIPT_LEAVES = new Set([
...
export const REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS = [
  'review_api_setup',
  'review_api_activate',
...
export type ReviewPlannerV20ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV20ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v20-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    schemaVersion: z.literal(
      REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_PROFILE.schemas.checkpoint,
    ),
    component: z.enum(['review', 'planner']),
    slot: z.enum(['api', 'browser']),
    checkpoint: z.enum(REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV20ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV20ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV20ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV20ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV20ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV20ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV20ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
const journalState = new WeakMap<
  ReviewPlannerV20ProductAcceptanceRecoveryJournal,
  JournalState
>();

export async function acquireReviewPlannerV20ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV20ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V20_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    const owner: ReviewPlannerV20ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V20_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V20_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV20ProductAcceptanceOwner(
  owner: ReviewPlannerV20ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV20ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V20_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV20ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV20ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
  assertReviewPlannerV20ProductAcceptanceOwner(owner, environment, ['product']);
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V20_PRODUCT_ACCEPTANCE_ATTEMPT_INVALID');
  }
  state.attemptSha256 = attemptSha256;
}

export async function bindReviewPlannerV20ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV20ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v20-product-acceptance-attempt-v1',
    ...
    throw new Error('V20_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV20ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV20ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV20ProductAcceptanceOwner;
  ...
  return openV20Journal(input, state.attemptSha256, true);
}

export async function openReviewPlannerV20ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV20ProductAcceptanceOwner;
}): Promise<ReviewPlannerV20ProductAcceptanceRecoveryJournal> {
  ...
  return openV20Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV20ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV20ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV20ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV20ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV20ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V20_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV20Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V20_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV20ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V20_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV20ProductAcceptanceCheckpoint(value);
    ...
    if (
      ...
      leaves.some(
        (leaf) =>
          ...
          !V20_PUBLIC_SLOT_LEAF.test(leaf) &&
          !V20_PUBLIC_DEFAULT_OFF_LEAF.test(leaf) &&
          !V20_PUBLIC_RECEIPT_LEAVES.has(leaf),
      )
    ) {
      throw new Error();
    }
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV20ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV20ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV20ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV20ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V20_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function expectedProviderCallState(
  ...
  return record.checkpoint.endsWith('_dispatch') ||
    REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
      record.checkpoint,
    ) >
      REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
...
function checkpointMatchesBoundary(
  value: ReviewPlannerV20ProductAcceptanceCheckpointRecord,
) {
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV20ProductAcceptanceOwnerRole {
