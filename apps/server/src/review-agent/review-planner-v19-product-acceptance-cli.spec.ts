...
import {
  parseReviewPlannerV19ProductAcceptanceArguments,
  runReviewPlannerV19ProductAcceptanceProductCli,
  serializeReviewPlannerV19ProductAcceptanceCliFailure,
} from './review-planner-v19-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V19_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V19 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V19_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V19 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V19_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V19 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V19 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V19 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV19ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V19_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV19ProductAcceptanceCliFailure(
      'product',
