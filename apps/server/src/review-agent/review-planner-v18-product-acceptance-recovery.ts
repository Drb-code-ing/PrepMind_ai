...
const V18_PUBLIC_SLOT_LEAF = /^slot-(review|planner)-(api|browser)\.json$/;
const V18_PUBLIC_DEFAULT_OFF_LEAF = /^default-off-(review|planner)\.json$/;
const V18_PUBLIC_RECEIPT_LEAVES = new Set([
...
export const REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_CHECKPOINTS = [
  'review_api_setup',
  'review_api_activate',
...
export type ReviewPlannerV18ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV18ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v18-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    schemaVersion: z.literal(
      REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_PROFILE.schemas.checkpoint,
    ),
    component: z.enum(['review', 'planner']),
    slot: z.enum(['api', 'browser']),
    checkpoint: z.enum(REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV18ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV18ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV18ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV18ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV18ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV18ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV18ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
const journalState = new WeakMap<
  ReviewPlannerV18ProductAcceptanceRecoveryJournal,
  JournalState
>();

export async function acquireReviewPlannerV18ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV18ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V18_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    const owner: ReviewPlannerV18ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V18_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V18_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV18ProductAcceptanceOwner(
  owner: ReviewPlannerV18ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV18ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V18_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV18ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV18ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
  assertReviewPlannerV18ProductAcceptanceOwner(owner, environment, ['product']);
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V18_PRODUCT_ACCEPTANCE_ATTEMPT_INVALID');
  }
  state.attemptSha256 = attemptSha256;
}

export async function bindReviewPlannerV18ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV18ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v18-product-acceptance-attempt-v1',
    ...
    throw new Error('V18_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV18ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV18ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV18ProductAcceptanceOwner;
  ...
  return openV18Journal(input, state.attemptSha256, true);
}

export async function openReviewPlannerV18ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV18ProductAcceptanceOwner;
}): Promise<ReviewPlannerV18ProductAcceptanceRecoveryJournal> {
  ...
  return openV18Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV18ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV18ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV18ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV18ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV18ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V18_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV18Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V18_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV18ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V18_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV18ProductAcceptanceCheckpoint(value);
    ...
    if (
      ...
      leaves.some(
        (leaf) =>
          ...
          !V18_PUBLIC_SLOT_LEAF.test(leaf) &&
          !V18_PUBLIC_DEFAULT_OFF_LEAF.test(leaf) &&
          !V18_PUBLIC_RECEIPT_LEAVES.has(leaf),
      )
    ) {
      throw new Error();
    }
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV18ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV18ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV18ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV18ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V18_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function expectedProviderCallState(
  ...
  return record.checkpoint.endsWith('_dispatch') ||
    REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
      record.checkpoint,
    ) >
      REVIEW_PLANNER_V18_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
...
function checkpointMatchesBoundary(
  value: ReviewPlannerV18ProductAcceptanceCheckpointRecord,
) {
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV18ProductAcceptanceOwnerRole {
