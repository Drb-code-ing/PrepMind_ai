...
describe('Review/Planner V20 Node runner', () => {
  ...
  it('fails closed for an ordinary unknown entry before loading TypeScript', () => {
    const result = spawnSync(
      process.execPath,
      [
        resolve(__dirname, '../../scripts/review-planner-v20-node-runner.cjs'),
        'not-an-entry.ts',
      ],
      {
        encoding: 'utf8',
  ...
  it('removes one Bun separator before forwarding confirmed arguments to the product entry', () => {
    ...
    const entry = JSON.stringify(
      resolve(
        __dirname,
        '../../scripts/review-planner-v20-product-acceptance.ts',
      ),
    );
    const expectedArguments = JSON.stringify([
      '--confirm-v20-review-planner-product-acceptance',
      '--environment=branch',
    ]);
    const result = spawnSync(
      process.execPath,
      [
        ...
        [
          ...
          `const entry = ${entry};`,
          `const expectedArguments = ${expectedArguments};`,
          'const load = Module._load;',
          'Module._load = function(request, parent, isMain) {',
          "  if (request === entry || request.endsWith('review-planner-v20-preflight.ts')) {",
          '    const actualArguments = process.argv.slice(2);',
          '    if (JSON.stringify(actualArguments) !== JSON.stringify(expectedArguments)) {',
          '      process.stderr.write(`unexpected arguments: ${JSON.stringify(actualArguments)}\\n`);',
          '      process.exitCode = 2;',
          '    }',
          '    return {};',
          '  }',
          ...
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v20-product-acceptance.ts', '--', '--confirm-v20-review-planner-product-acceptance', '--environment=branch'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
  ...
  it('keeps a forwarded unexpected argument fail-closed after removing the Bun separator', () => {
    const result = spawnSync(
      process.execPath,
      [
        resolve(__dirname, '../../scripts/review-planner-v20-node-runner.cjs'),
        'review-planner-v20-product-acceptance.ts',
        '--',
        '--confirm-v20-review-planner-product-acceptance',
        '--environment=branch',
        '--unexpected=blocked',
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v20-product-acceptance.ts'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          'const resolveFilename = Module._resolveFilename;',
          'Module._resolveFilename = function(request, parent, isMain, options) {',
          "  if (request === '../src/review-agent/review-planner-v20-product-acceptance-cli') return " +
  ...
  it('blocks a resolved outside JavaScript dependency before it executes', () => {
    const fixtureDirectory = mkdtempSync(
      join(tmpdir(), 'review-planner-v20-node-runner-'),
    );
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            'const resolveFilename = Module._resolveFilename;',
            'Module._resolveFilename = function(request, parent, isMain, options) {',
            "  if (request === '../src/review-agent/review-planner-v20-product-acceptance-cli') return unapprovedJavaScript;",
            '  return resolveFilename.call(this, request, parent, isMain, options);',
            '};',
            "process.argv = ['node', 'runner', 'review-planner-v20-product-acceptance.ts'];",
            `require(${runner});`,
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '  return value;',
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v20-product-acceptance.ts', '--invalid', '--environment=branch'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
