import { createReviewPlannerV13ProductAcceptanceDiagnosticsPort } from './review-planner-v13-product-acceptance-diagnostics';
import { createReviewPlannerV13ProductAcceptanceRunnerLedgerAdapter } from './review-planner-v13-product-acceptance-execution';
import { runReviewPlannerV13ProductAcceptanceComposition } from './review-planner-v13-product-acceptance-composition';
import { REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V13 execution bridge', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('emits only fixed V13 checkpoints and a nonsecret failure projection', () => {
    ...
    const diagnostics = createReviewPlannerV13ProductAcceptanceDiagnosticsPort({
      environment: 'branch',
  ...
  it('durably commits exactly four V13 runner slots without V11 records', async () => {
    const ledger = {
      attemptSha256: () => attemptSha256,
      claimSlot: jest.fn(),
    ...
    const adapter = createReviewPlannerV13ProductAcceptanceRunnerLedgerAdapter({
      environment: 'branch',
      ledger,
      manifest: {
        schemaVersion:
          'phase-6.9.5-v13-product-acceptance-manifest-v1' as const,
  ...
  it('stops at V13 preflight before owner, ledger, runner, or external work', async () => {
    const ports = {
