...
import {
  parseReviewPlannerV17ProductAcceptanceArguments,
  runReviewPlannerV17ProductAcceptanceProductCli,
  serializeReviewPlannerV17ProductAcceptanceCliFailure,
} from './review-planner-v17-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V17_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V17 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V17_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V17 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V17_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V17 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V17 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V17 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV17ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V17_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV17ProductAcceptanceCliFailure(
      'product',
