import {
  createReviewPlannerV19ProductAcceptanceDiagnosticsPort,
  type ReviewPlannerV19ProductAcceptanceFailure,
} from './review-planner-v19-product-acceptance-diagnostics';
import {
  createReviewPlannerV19ProductAcceptanceRunnerLedgerAdapter,
  type ReviewPlannerV19ProductAcceptanceSafeEvent,
} from './review-planner-v19-product-acceptance-execution';
import {
  createReviewPlannerV19ProductAcceptanceExecutionManifest,
...
import {
  createDefaultReviewPlannerV19ProductAcceptanceHost,
  type ReviewPlannerV19ProductAcceptanceHost,
} from './review-planner-v19-product-acceptance-host';
import { REVIEW_PLANNER_V19_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

export type ReviewPlannerV19ReadyPreflight = Readonly<{
  status: 'ready';
...
export type ReviewPlannerV19ProductAcceptanceCompositionPorts = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
  ...
  acquireOwner(input: {
    ...
    repoRoot: string;
    role: 'product';
  }): ReturnType<typeof acquireReviewPlannerV19ProductAcceptanceOwner>;
  reserveLedger(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
  ...
  record(event: ReviewPlannerV19ProductAcceptanceSafeEvent): void;
  runRunner(input: unknown): Promise<unknown>;
  dispose?(): Promise<void>;
}>;

export async function runReviewPlannerV19ProductAcceptanceComposition(input: {
  environment: 'branch' | 'main';
  repoRoot: string;
  ports: ReviewPlannerV19ProductAcceptanceCompositionPorts;
}): Promise<
...
> {
  let owner: ReviewPlannerV19ProductAcceptanceOwner | undefined;
  let ledger: ReviewPlannerV19ProductAcceptanceLedger | undefined;
  let journal: ReviewPlannerV19ProductAcceptanceRecoveryJournal | undefined;
  let diagnostics:
    | ReturnType<typeof createReviewPlannerV19ProductAcceptanceDiagnosticsPort>
    | undefined;
  try {
    ...
    if (!isSameV19ReadyPreflight(preflight, revalidated)) {
      return Object.freeze({
        status: 'blocked' as const,
        stage: 'owner' as const,
      });
    ...
    const manifest = {
      schemaVersion: 'phase-6.9.5-v19-product-acceptance-manifest-v1' as const,
      environment: input.environment,
      attemptSha256,
    };
    ...
    diagnostics = createReviewPlannerV19ProductAcceptanceDiagnosticsPort({
      environment: input.environment,
      journal,
...
export type ReviewPlannerV19DefaultCompositionOptions = Readonly<{
  hostFactory?: (repoRoot: string) => ReviewPlannerV19ProductAcceptanceHost;
  boundary?: Readonly<{
    preflight(input: {
      environment: 'branch' | 'main';
      repoRoot: string;
...
export function createDefaultReviewPlannerV19ProductAcceptanceComposition(
  repoRoot = '',
  options: ReviewPlannerV19DefaultCompositionOptions = {},
): Readonly<{
  ports: ReviewPlannerV19ProductAcceptanceCompositionPorts;
}> {
  const host =
    ...
    (!options.boundary && repoRoot
      ? createDefaultReviewPlannerV19ProductAcceptanceHost(repoRoot)
      : undefined);
  return Object.freeze({
    ports: Object.freeze({
      preflight: (input: {
      ...
      revalidate: (preflight: ReviewPlannerV19ReadyPreflight) =>
        ...
        Promise.resolve(Object.freeze({ status: 'blocked' as const })),
      acquireOwner: acquireReviewPlannerV19ProductAcceptanceOwner,
      reserveLedger: reserveReviewPlannerV19ProductAcceptanceLedger,
      prepareJournal: prepareReviewPlannerV19ProductAcceptanceRecoveryJournal,
      prepareExecution: (
        input: Parameters<
          ReviewPlannerV19ProductAcceptanceCompositionPorts['prepareExecution']
        >[0],
      ) => {
        ...
        if (!accountIdSha256 || !capabilities || !dependencies) {
          return Promise.reject(
            new Error('V19_PRODUCT_ACCEPTANCE_RUNTIME_UNAVAILABLE'),
          );
        }
...
function isSameV19ReadyPreflight(
  expected: ReviewPlannerV19ReadyPreflight,
  value: Readonly<{ status: 'blocked' }> | ReviewPlannerV19ReadyPreflight,
): value is ReviewPlannerV19ReadyPreflight {
  return (
    value.status === 'ready' &&
