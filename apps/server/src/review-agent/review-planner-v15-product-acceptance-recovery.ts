...
const V15_PUBLIC_SLOT_LEAF = /^slot-(review|planner)-(api|browser)\.json$/;
const V15_PUBLIC_DEFAULT_OFF_LEAF = /^default-off-(review|planner)\.json$/;
const V15_PUBLIC_RECEIPT_LEAVES = new Set([
...
export const REVIEW_PLANNER_V15_PRODUCT_ACCEPTANCE_CHECKPOINTS = [
  'review_api_setup',
  'review_api_activate',
...
export type ReviewPlannerV15ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V15_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV15ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v15-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    ...
    checkpoint: z.enum(REVIEW_PLANNER_V15_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV15ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV15ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV15ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV15ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV15ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV15ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV15ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
export async function acquireReviewPlannerV15ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV15ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V15_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    const owner: ReviewPlannerV15ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V15_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V15_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV15ProductAcceptanceOwner(
  owner: ReviewPlannerV15ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV15ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V15_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV15ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV15ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
  assertReviewPlannerV15ProductAcceptanceOwner(owner, environment, ['product']);
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V15_PRODUCT_ACCEPTANCE_ATTEMPT_INVALID');
  }
  state.attemptSha256 = attemptSha256;
}

export async function bindReviewPlannerV15ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV15ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v15-product-acceptance-attempt-v1',
    ...
    throw new Error('V15_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV15ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV15ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV15ProductAcceptanceOwner;
}): Promise<ReviewPlannerV15ProductAcceptanceRecoveryJournal> {
  assertReviewPlannerV15ProductAcceptanceOwner(input.owner, input.environment, [
    'product',
  ]);
  ...
  return openV15Journal(input, state.attemptSha256, true);
}

export async function openReviewPlannerV15ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV15ProductAcceptanceOwner;
}): Promise<ReviewPlannerV15ProductAcceptanceRecoveryJournal> {
  ...
  return openV15Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV15ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV15ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV15ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV15ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV15ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V15_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV15Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V15_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV15ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V15_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV15ProductAcceptanceCheckpoint(value);
    ...
    if (
      ...
      leaves.some(
        (leaf) =>
          ...
          !V15_PUBLIC_SLOT_LEAF.test(leaf) &&
          !V15_PUBLIC_DEFAULT_OFF_LEAF.test(leaf) &&
          !V15_PUBLIC_RECEIPT_LEAVES.has(leaf),
      )
    ) {
      throw new Error();
    }
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV15ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV15ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV15ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV15ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V15_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V15_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function checkpointMatchesBoundary(
  value: ReviewPlannerV15ProductAcceptanceCheckpointRecord,
) {
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV15ProductAcceptanceOwnerRole {
