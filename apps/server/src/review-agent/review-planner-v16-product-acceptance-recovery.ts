...
const V16_PUBLIC_SLOT_LEAF = /^slot-(review|planner)-(api|browser)\.json$/;
const V16_PUBLIC_DEFAULT_OFF_LEAF = /^default-off-(review|planner)\.json$/;
const V16_PUBLIC_RECEIPT_LEAVES = new Set([
...
export const REVIEW_PLANNER_V16_PRODUCT_ACCEPTANCE_CHECKPOINTS = [
  'review_api_setup',
  'review_api_activate',
...
export type ReviewPlannerV16ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V16_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV16ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v16-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    ...
    checkpoint: z.enum(REVIEW_PLANNER_V16_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV16ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV16ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV16ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV16ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV16ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV16ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV16ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
export async function acquireReviewPlannerV16ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV16ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V16_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    const owner: ReviewPlannerV16ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V16_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V16_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV16ProductAcceptanceOwner(
  owner: ReviewPlannerV16ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV16ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V16_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV16ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV16ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
  assertReviewPlannerV16ProductAcceptanceOwner(owner, environment, ['product']);
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V16_PRODUCT_ACCEPTANCE_ATTEMPT_INVALID');
  }
  state.attemptSha256 = attemptSha256;
}

export async function bindReviewPlannerV16ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV16ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v16-product-acceptance-attempt-v1',
    ...
    throw new Error('V16_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV16ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV16ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV16ProductAcceptanceOwner;
  ...
  return openV16Journal(input, state.attemptSha256, true);
}

export async function openReviewPlannerV16ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV16ProductAcceptanceOwner;
}): Promise<ReviewPlannerV16ProductAcceptanceRecoveryJournal> {
  ...
  return openV16Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV16ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV16ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV16ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV16ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV16ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V16_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV16Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V16_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV16ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V16_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV16ProductAcceptanceCheckpoint(value);
    ...
    if (
      ...
      leaves.some(
        (leaf) =>
          ...
          !V16_PUBLIC_SLOT_LEAF.test(leaf) &&
          !V16_PUBLIC_DEFAULT_OFF_LEAF.test(leaf) &&
          !V16_PUBLIC_RECEIPT_LEAVES.has(leaf),
      )
    ) {
      throw new Error();
    }
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV16ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV16ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV16ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV16ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V16_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V16_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function checkpointMatchesBoundary(
  value: ReviewPlannerV16ProductAcceptanceCheckpointRecord,
) {
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV16ProductAcceptanceOwnerRole {
