...
const SCHEMA_VERSION =
  'phase-6.9.5-review-planner-v10-semantic-quality-v1' as const;
const DATASET_VERSION = 'phase-6.9-review-planner-v3' as const;
const PRICE_PROFILE_ID =
  'deepseek-v4-pro-cny-noncached-2026-07-19-v10-semantic-quality' as const;
...
const laneAggregateSchema = z
  .object({
    caseEntries: safeCount.max(24),
    runtimeCases: safeCount.max(11),
    zeroCallCases: safeCount.max(13),
    strictSuccesses: safeCount.max(24),
    qualityPasses: safeCount.max(24),
    criticalFailures: safeCount.max(24),
  })
  .strict();
...
const validReportAggregateSchema = z
  .object({
    ...
    zeroCallVerified: safeCount.max(48),
    ...
    lanes: z
      .object({
        review: laneAggregateSchema,
        planner: laneAggregateSchema,
      })
      .strict(),
  })
  .strict()
  .superRefine((report, context) => {
    const lanes = [report.lanes.review, report.lanes.planner];
    if (
      report.caseEntries !==
        lanes.reduce((total, lane) => total + lane.caseEntries, 0) ||
      report.zeroCallCases !==
        lanes.reduce((total, lane) => total + lane.zeroCallCases, 0) ||
      report.strictSuccesses !==
        lanes.reduce((total, lane) => total + lane.strictSuccesses, 0) ||
      report.qualityPasses !==
        lanes.reduce((total, lane) => total + lane.qualityPasses, 0) ||
      report.criticalFailures !==
        lanes.reduce((total, lane) => total + lane.criticalFailures, 0)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['lanes'],
        message: 'lane_total_mismatch',
      });
    }
    if (
...
const unknownUsageAggregateSchema = z
  .object({ known: z.literal(false), reason: z.literal('usage_unverifiable') })
  .strict();
...
function evaluateReportAggregate(
  ...
  if (report.zeroCallCases !== 26 || report.zeroCallVerified !== 26) {
    ...
    lanes.some(
      (lane) =>
        lane.caseEntries !== 24 ||
        lane.runtimeCases !== 11 ||
        lane.zeroCallCases !== 13,
    )
  ) {
    productionDecision = 'invalid_report';
  } else if (
    report.strictSuccesses !== 48 ||
    lanes.some((lane) => lane.strictSuccesses !== 24)
  ) {
    productionDecision = 'strict_schema_incomplete';
  } else if (
    report.qualityPasses !== 48 ||
    lanes.some((lane) => lane.qualityPasses !== 24)
  ) {
    productionDecision = 'semantic_quality_below_threshold';
  } else if (
    report.criticalFailures !== 0 ||
    lanes.some((lane) => lane.criticalFailures !== 0)
  ) {
...
export const v10SemanticQualityDiagnosticSchema =
  diagnosticShapeSchema.superRefine((diagnostic, context) => {
...
export type V10SemanticQualityDiagnostic = z.infer<
  typeof v10SemanticQualityDiagnosticSchema
>;

export function deriveV10SemanticQualityDiagnostic(
  input: unknown,
): V10SemanticQualityDiagnostic {
  const aggregate = derivationInputSchema.parse(input);
  const gates = expectedGates(aggregate);
  return deepFreeze(
    v10SemanticQualityDiagnosticSchema.parse({
...
export function serializeV10SemanticQualityDiagnostic(input: unknown): string {
  return JSON.stringify(v10SemanticQualityDiagnosticSchema.parse(input));
}
