...
import {
  parseReviewPlannerV21ProductAcceptanceArguments,
  executeReviewPlannerV21ProductAcceptanceProductCli,
  runReviewPlannerV21ProductAcceptanceProductCli,
  serializeReviewPlannerV21ProductAcceptanceCliFailure,
} from './review-planner-v21-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V21_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V21 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V21_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V21 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V21_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V21 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V21 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V21 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV21ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V21_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV21ProductAcceptanceCliFailure(
      'product',
