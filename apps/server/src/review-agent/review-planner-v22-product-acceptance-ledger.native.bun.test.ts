...
import {
  ...
  openReviewPlannerV22ProductAcceptanceRecoveryJournal,
...
describeWindows('Review/Planner V22 durable product acceptance ledger', () => {
  let root = '';

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'prepmind-v22-ledger-'));
  ...
  it('creates one V22-only reservation whose public, binding, and private execution hashes agree', async () => {
    const state = await prepareEarliestV22State(root);
  ...
  it('permits only one V22 reservation even after the first owner releases its private lock', async () => {
    const state = await prepareEarliestV22State(root);
    const next = await acquireReviewPlannerV22ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V22_PRODUCT_ACCEPTANCE_ALREADY_RESERVED');
    } finally {
      next.owner.close();
    }
  ...
  it('rolls back a verified unstarted reservation so a fresh V22 reservation can be created', async () => {
    const acquired = await acquireReviewPlannerV22ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    const ledger = await reserveReviewPlannerV22ProductAcceptanceLedger({
      repoRoot: root,
      environment: 'branch',
    ...
    try {
      ...
      ledger.writeManifest({
        schemaVersion: 'phase-6.9.5-v22-product-acceptance-manifest-v1',
        environment: 'branch',
        attemptSha256,
      });
    ...
    const fresh = await acquireReviewPlannerV22ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      const next = await reserveReviewPlannerV22ProductAcceptanceLedger({
        repoRoot: root,
        environment: 'branch',
        owner: fresh.owner,
      });
  ...
  it('admits the strict earliest V22 state with no checkpoint for recovery', async () => {
    const state = await prepareEarliestV22State(root);
    const owner = await acquireReviewPlannerV22ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      const journal =
        await openReviewPlannerV22ProductAcceptanceRecoveryJournal({
          repoRoot: root,
          environment: 'branch',
          owner: owner.owner,
        });
  ...
  it('fails closed for a bare reservation and a malformed checkpoint', async () => {
    const publicRoot = join(root, ...v22PublicSegments('branch'));
  ...
  it('fails closed for a V11 manifest injected into a V22 reservation', async () => {
    const state = await prepareEarliestV22State(root);
    const publicRoot = join(root, ...v22PublicSegments('branch'));
    await writeFile(
  ...
  it('seals exactly four V22 slots and leaves V11/V12/V13/V14/V15 root bytes untouched', async () => {
  ...
  it('seals recovery exactly once and rejects a second recovery owner', async () => {
    const state = await prepareFailedV22State(root);
    const recovery = await acquireReviewPlannerV22ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      await finalizeReviewPlannerV22ProductAcceptanceRecovery({
        repoRoot: root,
        environment: 'branch',
    ...
    const second = await acquireReviewPlannerV22ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V22_PRODUCT_ACCEPTANCE_RECOVERY_NOT_AUTHORIZED');
    } finally {
      second.owner.close();
    }
    expect(state.attemptSha256).toHaveLength(64);
  ...
  it('fails closed when a failure terminal disagrees with its attempt-bound latest checkpoint', async () => {
    await prepareFailedV22State(root);
    await writeFile(
      join(root, ...v22PublicSegments('branch'), 'failure.json'),
      JSON.stringify({
        schemaVersion: 'phase-6.9.5-v22-product-acceptance-failure-v1',
        environment: 'branch',
...
async function prepareEarliestV22State(root: string) {
  const acquired = await acquireReviewPlannerV22ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
...
async function prepareFailedV22State(root: string) {
  const acquired = await acquireReviewPlannerV22ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
  ...
  let journal:
    | Awaited<
        ReturnType<
          typeof prepareReviewPlannerV22ProductAcceptanceRecoveryJournal
        >
      >
    | undefined;
  try {
    ...
    journal = await prepareReviewPlannerV22ProductAcceptanceRecoveryJournal({
      repoRoot: root,
      environment: 'branch',
      owner: acquired.owner,
    });
    journal.appendCheckpoint({
      schemaVersion: 'phase-6.9.5-v22-product-acceptance-checkpoint-v1',
      component: 'review',
      slot: 'api',
      checkpoint: 'review_api_setup',
      providerCallState: 'not_started',
...
function v22PublicSegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V22_PRODUCT_ACCEPTANCE_PROFILE.publicLedgerSegments(
    environment,
  );
}
...
function v22RecoverySegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V22_PRODUCT_ACCEPTANCE_PROFILE.recoverySegments(
    environment,
  );
}
