...
describe('Review Planner V12 default product composition', () => {
  it('keeps the unconfigured host boundary default-off before any runtime capability', async () => {
    const composition = createDefaultReviewPlannerV12ProductAcceptanceComposition();
  ...
  it('accepts an injected V12-only ready preflight without touching a V11 boundary', async () => {
    const preflight = jest.fn(async () => ({
      ...
      capabilities: { review: 'review-capability', planner: 'planner-capability' },
      dependencies: {},
    }));
    const create = createDefaultReviewPlannerV12ProductAcceptanceComposition as unknown as (
      root: string,
      options: { boundary: { preflight: typeof preflight } },
    ) => ReturnType<typeof createDefaultReviewPlannerV12ProductAcceptanceComposition>;
