...
import {
  ...
  openReviewPlannerV14ProductAcceptanceRecoveryJournal,
...
describeWindows('Review/Planner V14 durable product acceptance ledger', () => {
  let root = '';

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'prepmind-v14-ledger-'));
  ...
  it('creates one V14-only reservation whose public, binding, and private execution hashes agree', async () => {
    const state = await prepareEarliestV14State(root);
  ...
  it('permits only one V14 reservation even after the first owner releases its private lock', async () => {
    const state = await prepareEarliestV14State(root);
    const next = await acquireReviewPlannerV14ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V14_PRODUCT_ACCEPTANCE_ALREADY_RESERVED');
    } finally {
      next.owner.close();
    }
  ...
  it('admits the strict earliest V14 state with no checkpoint for recovery', async () => {
    const state = await prepareEarliestV14State(root);
    const owner = await acquireReviewPlannerV14ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      const journal =
        await openReviewPlannerV14ProductAcceptanceRecoveryJournal({
          repoRoot: root,
          environment: 'branch',
          owner: owner.owner,
        });
  ...
  it('fails closed for a bare reservation and a malformed checkpoint', async () => {
    const publicRoot = join(root, ...v14PublicSegments('branch'));
  ...
  it('fails closed for a V11 manifest injected into a V14 reservation', async () => {
    const state = await prepareEarliestV14State(root);
    const publicRoot = join(root, ...v14PublicSegments('branch'));
    await writeFile(
  ...
  it('seals exactly four V14 slots and leaves V11/V12/V13 root bytes untouched', async () => {
    ...
    const acquired = await acquireReviewPlannerV14ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    const ledger = await reserveReviewPlannerV14ProductAcceptanceLedger({
      repoRoot: root,
      environment: 'branch',
    ...
    try {
      ...
      ledger.writeManifest({
        schemaVersion: 'phase-6.9.5-v14-product-acceptance-manifest-v1',
        environment: 'branch',
        attemptSha256,
      });
  ...
  it('seals recovery exactly once and rejects a second recovery owner', async () => {
    const state = await prepareFailedV14State(root);
    const recovery = await acquireReviewPlannerV14ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      await finalizeReviewPlannerV14ProductAcceptanceRecovery({
        repoRoot: root,
        environment: 'branch',
    ...
    const second = await acquireReviewPlannerV14ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V14_PRODUCT_ACCEPTANCE_RECOVERY_NOT_AUTHORIZED');
    } finally {
      second.owner.close();
    }
    expect(state.attemptSha256).toHaveLength(64);
  ...
  it('fails closed when a failure terminal disagrees with its attempt-bound latest checkpoint', async () => {
    await prepareFailedV14State(root);
    await writeFile(
      join(root, ...v14PublicSegments('branch'), 'failure.json'),
      JSON.stringify({
...
async function prepareEarliestV14State(root: string) {
  const acquired = await acquireReviewPlannerV14ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
...
async function prepareFailedV14State(root: string) {
  const acquired = await acquireReviewPlannerV14ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
  ...
  let journal:
    | Awaited<
        ReturnType<
          typeof prepareReviewPlannerV14ProductAcceptanceRecoveryJournal
        >
      >
    | undefined;
  try {
    ...
    journal = await prepareReviewPlannerV14ProductAcceptanceRecoveryJournal({
      repoRoot: root,
      environment: 'branch',
      owner: acquired.owner,
    });
    journal.appendCheckpoint({
      schemaVersion: 'phase-6.9.5-v14-product-acceptance-checkpoint-v1',
      component: 'review',
      slot: 'api',
      checkpoint: 'review_api_setup',
      providerCallState: 'not_started',
