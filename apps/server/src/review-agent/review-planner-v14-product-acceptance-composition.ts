import {
  createReviewPlannerV14ProductAcceptanceDiagnosticsPort,
  type ReviewPlannerV14ProductAcceptanceFailure,
} from './review-planner-v14-product-acceptance-diagnostics';
import {
  createReviewPlannerV14ProductAcceptanceRunnerLedgerAdapter,
  type ReviewPlannerV14ProductAcceptanceSafeEvent,
} from './review-planner-v14-product-acceptance-execution';
import {
  createReviewPlannerV14ProductAcceptanceExecutionManifest,
  reserveReviewPlannerV14ProductAcceptanceLedger,
  type ReviewPlannerV14ProductAcceptanceLedger,
  type ReviewPlannerV14ProductAcceptanceExecutionManifest,
} from './review-planner-v14-product-acceptance-ledger';
import {
  acquireReviewPlannerV14ProductAcceptanceOwner,
  prepareReviewPlannerV14ProductAcceptanceRecoveryJournal,
...
import {
  createDefaultReviewPlannerV14ProductAcceptanceHost,
  type ReviewPlannerV14ProductAcceptanceHost,
...
export type ReviewPlannerV14ReadyPreflight = Readonly<{
  status: 'ready';
...
export type ReviewPlannerV14ProductAcceptanceCompositionPorts = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
  }): Promise<Readonly<{ status: 'blocked' }> | ReviewPlannerV14ReadyPreflight>;
  revalidate(
    preflight: ReviewPlannerV14ReadyPreflight,
  ): Promise<Readonly<{ status: 'blocked' }> | ReviewPlannerV14ReadyPreflight>;
  acquireOwner(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
    role: 'product';
  }): ReturnType<typeof acquireReviewPlannerV14ProductAcceptanceOwner>;
  reserveLedger(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
  ...
  prepareExecution(input: {
    ...
    repoRoot: string;
    preflight: ReviewPlannerV14ReadyPreflight;
    ledger: ReviewPlannerV14ProductAcceptanceLedger;
    journal: ReviewPlannerV14ProductAcceptanceRecoveryJournal;
    executionManifest: ReviewPlannerV14ProductAcceptanceExecutionManifest;
  }): Promise<
    Readonly<{
      accountIdSha256: Readonly<{ review: string; planner: string }>;
      capabilities: Readonly<{ review: string; planner: string }>;
  ...
  record(event: ReviewPlannerV14ProductAcceptanceSafeEvent): void;
  runRunner(input: unknown): Promise<unknown>;
  dispose?(): Promise<void>;
}>;

export async function runReviewPlannerV14ProductAcceptanceComposition(input: {
  environment: 'branch' | 'main';
  repoRoot: string;
  ports: ReviewPlannerV14ProductAcceptanceCompositionPorts;
}): Promise<
...
> {
  let owner: ReviewPlannerV14ProductAcceptanceOwner | undefined;
  let ledger: ReviewPlannerV14ProductAcceptanceLedger | undefined;
  let journal: ReviewPlannerV14ProductAcceptanceRecoveryJournal | undefined;
  let diagnostics:
    | ReturnType<typeof createReviewPlannerV14ProductAcceptanceDiagnosticsPort>
    | undefined;
  try {
    ...
    if (!isSameV14ReadyPreflight(preflight, revalidated)) {
      return Object.freeze({
        status: 'blocked' as const,
        stage: 'owner' as const,
      });
    ...
    const manifest = {
      schemaVersion: 'phase-6.9.5-v14-product-acceptance-manifest-v1' as const,
      environment: input.environment,
      attemptSha256,
    };
    ...
    diagnostics = createReviewPlannerV14ProductAcceptanceDiagnosticsPort({
      environment: input.environment,
      journal,
...
export type ReviewPlannerV14DefaultCompositionOptions = Readonly<{
  hostFactory?: (repoRoot: string) => ReviewPlannerV14ProductAcceptanceHost;
  boundary?: Readonly<{
    preflight(input: {
      environment: 'branch' | 'main';
      repoRoot: string;
    }): Promise<
      Readonly<{ status: 'blocked' }> | ReviewPlannerV14ReadyPreflight
    >;
  }>;
}>;

export function createDefaultReviewPlannerV14ProductAcceptanceComposition(
  repoRoot = '',
  options: ReviewPlannerV14DefaultCompositionOptions = {},
): Readonly<{
  ports: ReviewPlannerV14ProductAcceptanceCompositionPorts;
}> {
  const host =
    ...
    (!options.boundary && repoRoot
      ? createDefaultReviewPlannerV14ProductAcceptanceHost(repoRoot)
      : undefined);
  return Object.freeze({
    ports: Object.freeze({
      preflight: (input: {
      ...
      revalidate: (preflight: ReviewPlannerV14ReadyPreflight) =>
        ...
        Promise.resolve(Object.freeze({ status: 'blocked' as const })),
      acquireOwner: acquireReviewPlannerV14ProductAcceptanceOwner,
      reserveLedger: reserveReviewPlannerV14ProductAcceptanceLedger,
      prepareJournal: prepareReviewPlannerV14ProductAcceptanceRecoveryJournal,
      prepareExecution: (
        input: Parameters<
          ReviewPlannerV14ProductAcceptanceCompositionPorts['prepareExecution']
        >[0],
      ) => {
        ...
        if (!accountIdSha256 || !capabilities || !dependencies) {
          return Promise.reject(
            new Error('V14_PRODUCT_ACCEPTANCE_RUNTIME_UNAVAILABLE'),
          );
        }
...
function isSameV14ReadyPreflight(
  expected: ReviewPlannerV14ReadyPreflight,
  value: Readonly<{ status: 'blocked' }> | ReviewPlannerV14ReadyPreflight,
): value is ReviewPlannerV14ReadyPreflight {
  return (
    value.status === 'ready' &&
