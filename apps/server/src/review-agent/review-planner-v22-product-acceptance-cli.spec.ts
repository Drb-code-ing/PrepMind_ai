...
import {
  parseReviewPlannerV22ProductAcceptanceArguments,
  executeReviewPlannerV22ProductAcceptanceProductCli,
  runReviewPlannerV22ProductAcceptanceProductCli,
  serializeReviewPlannerV22ProductAcceptanceCliFailure,
} from './review-planner-v22-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V22_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V22 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V22_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V22 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V22_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V22 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V22 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V22 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV22ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V22_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV22ProductAcceptanceCliFailure(
      'product',
