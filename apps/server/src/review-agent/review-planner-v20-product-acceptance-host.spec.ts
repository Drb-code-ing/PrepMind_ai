...
import {
  createDefaultReviewPlannerV20ProductAcceptanceRecoveryHost,
  isReviewPlannerV20DatabaseFingerprintStable,
} from './review-planner-v20-product-acceptance-host';

describe('Review Planner V20 recovery host', () => {
  const databaseUrl = 'postgresql://v20-recovery.example.invalid/prepmind';
  const databaseUrlSha256 = createHash('sha256')
    .update(databaseUrl)
    .digest('hex');
  const executionManifest = {
    schemaVersion:
      'phase-6.9.5-v20-product-acceptance-execution-manifest-v1' as const,
    environment: 'branch' as const,
    attemptSha256: 'a'.repeat(64),
    databaseUrlSha256,
    resources: {
      accountId: {
        review: `v20-synthetic-account-review-${'b'.repeat(32)}`,
        planner: `v20-synthetic-account-planner-${'c'.repeat(32)}`,
      },
      fixtureId: {
        review: `v20-synthetic-fixture-review-${'d'.repeat(32)}`,
        planner: `v20-synthetic-fixture-planner-${'e'.repeat(32)}`,
      },
      browser: {
        executablePath:
  ...
  it('takes the recovery owner, revalidates its attempt, and seals only after restore plus exact cleanup', async () => {
    ...
    const host = createDefaultReviewPlannerV20ProductAcceptanceRecoveryHost(
      'E:\\v20-recovery-host',
      {
        platform: 'win32',
        readLedger,
  ...
  it('injects the V20 default-off validator into recovery restore without relaxing unsafe runtime values', async () => {
    mockRestoreDefaultReviewPlannerProductAcceptanceServer.mockClear();
    injectedDefaultOffEnvironmentValidator = undefined;
    const root = 'E:\\v20-default-validator';
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const host = createDefaultReviewPlannerV20ProductAcceptanceRecoveryHost(
      root,
      {
    ...
    if (typeof validator !== 'function') {
      throw new Error('V20 default-off validator was not injected');
    }
