...
import * as subject from './review-planner-v20-product-acceptance-host';
...
function bridgeFactory(): BridgeFactory {
  const candidate = (
    subject as typeof subject & {
      createReviewPlannerV20BunPairedEvidenceAuthority?: unknown;
    }
  ).createReviewPlannerV20BunPairedEvidenceAuthority;
  if (typeof candidate !== 'function') {
    throw new Error('V20_BUN_AUTHORITY_BRIDGE_MISSING');
  }
  return candidate;
}

describe('Review Planner V20 Bun paired-evidence authority bridge', () => {
  beforeEach(() => {
    ...
    mockRunDefaultHostPreflight.mockImplementation(
      async (
        ...
        return Object.freeze({
          ...
          branchName: 'codex/v20-bun-authority-bridge',
  ...
  it('returns ready from the Node V20 host preflight when started in apps/server', async () => {
    const previousCwd = process.cwd();
    process.chdir(appsServerRoot);
    try {
      const host = subject.createDefaultReviewPlannerV20ProductAcceptanceHost(
        root,
        {
      ...
      if (preflight.status !== 'ready') {
        throw new Error('V20_BUN_AUTHORITY_PRELIGHT_BLOCKED');
      }
