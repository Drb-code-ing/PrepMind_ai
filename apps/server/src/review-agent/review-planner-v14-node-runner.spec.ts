...
describe('Review/Planner V14 Node runner', () => {
  ...
  it('fails closed for an ordinary unknown entry before loading TypeScript', () => {
    const result = spawnSync(
      process.execPath,
      [
        resolve(__dirname, '../../scripts/review-planner-v14-node-runner.cjs'),
        'not-an-entry.ts',
      ],
      {
        encoding: 'utf8',
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v14-product-acceptance.ts'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          'const resolveFilename = Module._resolveFilename;',
          'Module._resolveFilename = function(request, parent, isMain, options) {',
          "  if (request === '../src/review-agent/review-planner-v14-product-acceptance-cli') return " +
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '  return value;',
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v14-product-acceptance.ts', '--invalid', '--environment=branch'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
