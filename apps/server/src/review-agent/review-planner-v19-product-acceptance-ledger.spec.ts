import {
  ...
  parseReviewPlannerV19ProductAcceptanceAggregate,
  parseReviewPlannerV19ProductAcceptanceCleanup,
  parseReviewPlannerV19ProductAcceptanceDefaultOff,
  parseReviewPlannerV19ProductAcceptanceExecutionManifest,
  parseReviewPlannerV19ProductAcceptanceFailure,
  parseReviewPlannerV19ProductAcceptanceManifest,
  parseReviewPlannerV19ProductAcceptanceOwnerIsolation,
  parseReviewPlannerV19ProductAcceptanceSlotResult,
  parseReviewPlannerV19ProductAcceptanceSuccess,
} from './review-planner-v19-product-acceptance-ledger';
import * as reviewPlannerV19Ledger from './review-planner-v19-product-acceptance-ledger';
import { parseReviewPlannerV19ProductAcceptanceCheckpoint } from './review-planner-v19-product-acceptance-recovery';
import { REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V19 product-acceptance ledger contracts', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('rejects V11 record identities from every V19 durable contract', () => {
    expect(() =>
      parseReviewPlannerV19ProductAcceptanceManifest({
        schemaVersion:
    ...
    expect(() =>
      parseReviewPlannerV19ProductAcceptanceCheckpoint({
    ...
    ).toThrow('V19_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  });

  it('accepts only the complete V19 four-slot durable success vocabulary', () => {
    const slot = parseReviewPlannerV19ProductAcceptanceSlotResult({
      schemaVersion: 'phase-6.9.5-v19-product-acceptance-slot-result-v1',
      slot: 'review-api',
      traceSha256: 'b'.repeat(64),
    });
    const defaultOff = parseReviewPlannerV19ProductAcceptanceDefaultOff({
    ...
    const isolation = parseReviewPlannerV19ProductAcceptanceOwnerIsolation({
    ...
    const cleanup = parseReviewPlannerV19ProductAcceptanceCleanup({
    ...
    const success = parseReviewPlannerV19ProductAcceptanceSuccess({
      schemaVersion: 'phase-6.9.5-v19-product-acceptance-success-v1',
      environment: 'branch',
      attemptSha256,
    });
  ...
  it.each(['https://api.deepseek.com', 'https://api.deepseek.com/v1'] as const)(
    'accepts the V19 durable default-off receipt with approved base URL %s',
    (baseUrl) => {
      const receipt = {
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-default-off-v1',
        model: 'deepseek-v4-pro',
      ...
      expect(
        parseReviewPlannerV19ProductAcceptanceDefaultOff(receipt),
  ...
  ])('rejects a %s V19 durable default-off receipt base URL', (_, baseUrl) => {
    const receipt = {
      schemaVersion: 'phase-6.9.5-v19-product-acceptance-default-off-v1',
      model: 'deepseek-v4-pro',
      component: 'review',
  ...
  it('allows only Flash or Pro in the V19 default-off environment without relaxing its controls', () => {
    const assertDefaultOff = (
      reviewPlannerV19Ledger as typeof reviewPlannerV19Ledger & {
  ...
  it.each(['https://api.deepseek.com', 'https://api.deepseek.com/v1'])(
    ...
    (baseUrl) => {
      ...
      if (!assertDefaultOff) throw new Error('V19 validator unavailable');
      const entries = [
        'AI_PROVIDER_MODE=mock',
        'AI_ENABLE_LIVE_CALLS=false',
  ...
  it.each([
    ...
    'rejects a %s model from a V19 durable default-off receipt',
    (_, model) => {
      const receipt = {
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-default-off-v1',
        baseUrl: 'https://api.deepseek.com',
  ...
  it('fails closed when a V19 durable record carries secret-like content', () => {
    expect(() =>
      parseReviewPlannerV19ProductAcceptanceSlotResult({
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-slot-result-v1',
        slot: 'review-api',
  ...
  it('requires an attempt-bound hash of the selected database URL without persisting the URL', () => {
    ...
    expect(() =>
      parseReviewPlannerV19ProductAcceptanceExecutionManifest(record),
    ).toThrow('V19_PRODUCT_ACCEPTANCE_LEDGER_RECORD_INVALID');
    expect(
      parseReviewPlannerV19ProductAcceptanceExecutionManifest({
        ...record,
  ...
  it('generates distinct V19-only selectors that bind cleanup to the reserved attempt', () => {
    const execution = createReviewPlannerV19ProductAcceptanceExecutionManifest({
      environment: 'branch',
      attemptSha256,
      databaseUrlSha256: 'f'.repeat(64),
    });
    ...
    expect(execution.resources.accountId.review).toMatch(
      /^v19-synthetic-account-review-[a-f0-9]{32}$/,
    );
  ...
  it('accepts only the fixed V19 failure terminal vocabulary', () => {
    expect(
      parseReviewPlannerV19ProductAcceptanceFailure({
