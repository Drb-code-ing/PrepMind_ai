...
describe('Review/Planner V19 Node runner', () => {
  ...
  it('removes one Bun separator before forwarding confirmed arguments to the product entry', () => {
    ...
    const entry = JSON.stringify(
      resolve(
        __dirname,
        '../../scripts/review-planner-v19-product-acceptance.ts',
      ),
    );
    const expectedArguments = JSON.stringify([
      '--confirm-v19-review-planner-product-acceptance',
      '--environment=branch',
    ]);
    const result = spawnSync(
      process.execPath,
      [
        ...
        [
          ...
          "  if (request === entry || request.endsWith('review-planner-v19-preflight.ts')) {",
  ...
  it('keeps a forwarded unexpected argument fail-closed after removing the Bun separator', () => {
    const result = spawnSync(
      ...
      [
        resolve(__dirname, '../../scripts/review-planner-v19-node-runner.cjs'),
        'review-planner-v19-product-acceptance.ts',
        '--',
        '--confirm-v19-review-planner-product-acceptance',
        '--environment=branch',
        '--unexpected=blocked',
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v19-product-acceptance.ts'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          'const resolveFilename = Module._resolveFilename;',
          'Module._resolveFilename = function(request, parent, isMain, options) {',
          "  if (request === '../src/review-agent/review-planner-v19-product-acceptance-cli') return " +
  ...
  it('blocks a resolved outside JavaScript dependency before it executes', () => {
    const fixtureDirectory = mkdtempSync(
      join(tmpdir(), 'review-planner-v19-node-runner-'),
    );
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            'const resolveFilename = Module._resolveFilename;',
            'Module._resolveFilename = function(request, parent, isMain, options) {',
            "  if (request === '../src/review-agent/review-planner-v19-product-acceptance-cli') return unapprovedJavaScript;",
            '  return resolveFilename.call(this, request, parent, isMain, options);',
            '};',
            "process.argv = ['node', 'runner', 'review-planner-v19-product-acceptance.ts'];",
            `require(${runner});`,
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '  return value;',
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v19-product-acceptance.ts', '--invalid', '--environment=branch'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
