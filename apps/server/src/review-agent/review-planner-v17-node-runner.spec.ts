...
describe('Review/Planner V17 Node runner', () => {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v17-product-acceptance.ts'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          'const resolveFilename = Module._resolveFilename;',
          'Module._resolveFilename = function(request, parent, isMain, options) {',
          "  if (request === '../src/review-agent/review-planner-v17-product-acceptance-cli') return " +
  ...
  it('blocks a resolved outside JavaScript dependency before it executes', () => {
    const fixtureDirectory = mkdtempSync(
      join(tmpdir(), 'review-planner-v17-node-runner-'),
    );
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            'const resolveFilename = Module._resolveFilename;',
            'Module._resolveFilename = function(request, parent, isMain, options) {',
            "  if (request === '../src/review-agent/review-planner-v17-product-acceptance-cli') return unapprovedJavaScript;",
            '  return resolveFilename.call(this, request, parent, isMain, options);',
            '};',
            "process.argv = ['node', 'runner', 'review-planner-v17-product-acceptance.ts'];",
            `require(${runner});`,
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          '  return value;',
          '};',
          "process.argv = ['node', 'runner', 'review-planner-v17-product-acceptance.ts', '--invalid', '--environment=branch'];",
          `require(${runner});`,
        ].join('\n'),
      ],
      {
