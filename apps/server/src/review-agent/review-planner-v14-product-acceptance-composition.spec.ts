...
describe('Review Planner V14 default product composition', () => {
  it('keeps the unconfigured host boundary default-off before any runtime capability', async () => {
    ...
    await expect(
      composition.ports.preflight({
        environment: 'branch',
        repoRoot: 'E:\\v14-default-off',
      }),
    ).resolves.toEqual({ status: 'blocked' });
  });

  it('accepts an injected V14-only ready preflight without touching a V11 boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
        repoRoot: 'E:\\v14-ready',
    ...
    const create =
      createDefaultReviewPlannerV14ProductAcceptanceComposition as unknown as (
        root: string,
        options: { boundary: { preflight: typeof preflight } },
      ) => ReturnType<
        typeof createDefaultReviewPlannerV14ProductAcceptanceComposition
      >;
    const composition = create('E:\\v14-ready', {
      boundary: { preflight },
    });
  ...
  it('obtains the default preflight from a V14 host factory instead of a test-only boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
    ...
    const create =
      createDefaultReviewPlannerV14ProductAcceptanceComposition as unknown as (
        ...
        typeof createDefaultReviewPlannerV14ProductAcceptanceComposition
      >;
    const composition = create('E:\\v14-host', { hostFactory });
    ...
    expect(hostFactory).toHaveBeenCalledWith('E:\\v14-host');
    expect(preflight).toHaveBeenCalledTimes(1);
  });

  it('prepares the runtime only after the V14 reservation and journal exist', async () => {
    const order: string[] = [];
    const owner = {
      assertHeld: jest.fn(),
      close: jest.fn(),
  ...
  it('revalidates the ready identity after owner acquisition and before reserving V14', async () => {
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v14-revalidate',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('projects a reservation failure as default-off rather than an operation failure', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v14-reserve-failure',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
