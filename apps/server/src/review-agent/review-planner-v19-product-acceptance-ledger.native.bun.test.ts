...
import {
  ...
  reserveReviewPlannerV19ProductAcceptanceLedger,
...
import {
  ...
  openReviewPlannerV19ProductAcceptanceRecoveryJournal,
  prepareReviewPlannerV19ProductAcceptanceRecoveryJournal,
...
describeWindows('Review/Planner V19 durable product acceptance ledger', () => {
  let root = '';

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'prepmind-v19-ledger-'));
  ...
  it('creates one V19-only reservation whose public, binding, and private execution hashes agree', async () => {
    const state = await prepareEarliestV19State(root);
  ...
  it('permits only one V19 reservation even after the first owner releases its private lock', async () => {
    const state = await prepareEarliestV19State(root);
    const next = await acquireReviewPlannerV19ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V19_PRODUCT_ACCEPTANCE_ALREADY_RESERVED');
    } finally {
      next.owner.close();
    }
  ...
  it('rolls back a verified unstarted reservation so a fresh V19 reservation can be created', async () => {
    const acquired = await acquireReviewPlannerV19ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    const ledger = await reserveReviewPlannerV19ProductAcceptanceLedger({
      repoRoot: root,
      environment: 'branch',
    ...
    try {
      ...
      ledger.writeManifest({
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-manifest-v1',
        environment: 'branch',
        attemptSha256,
      });
    ...
    const fresh = await acquireReviewPlannerV19ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      const next = await reserveReviewPlannerV19ProductAcceptanceLedger({
        repoRoot: root,
        environment: 'branch',
        owner: fresh.owner,
      });
  ...
  it('admits the strict earliest V19 state with no checkpoint for recovery', async () => {
    const state = await prepareEarliestV19State(root);
    const owner = await acquireReviewPlannerV19ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      const journal =
        await openReviewPlannerV19ProductAcceptanceRecoveryJournal({
          repoRoot: root,
          environment: 'branch',
          owner: owner.owner,
        });
  ...
  it('fails closed for a bare reservation and a malformed checkpoint', async () => {
    const publicRoot = join(root, ...v19PublicSegments('branch'));
  ...
  it('fails closed for a V11 manifest injected into a V19 reservation', async () => {
    const state = await prepareEarliestV19State(root);
    const publicRoot = join(root, ...v19PublicSegments('branch'));
    await writeFile(
  ...
  it('seals exactly four V19 slots and leaves V11/V12/V13/V14/V15 root bytes untouched', async () => {
    ...
    try {
      ...
      for (const [slot, traceSha256] of [
        ...
        ledger.recordSlotResult({
          schemaVersion: 'phase-6.9.5-v19-product-acceptance-slot-result-v1',
          slot,
          traceSha256,
        });
      }
      ...
      ledger.recordOwnerIsolation({
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-owner-isolation-v1',
      ...
      ledger.recordCleanup({
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-cleanup-v1',
        syntheticAccounts: 0,
        fixtures: 0,
        traces: 0,
        browserProfiles: 0,
      ...
      await ledger.finalizeSuccess({
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-aggregate-v1',
        environment: 'branch',
        attemptSha256,
        requests: 4,
        durationMs: 4_000,
  ...
  it('rejects a success terminal after a failure terminal has been written', async () => {
    ...
    try {
      ...
      ).toThrow('V19_PRODUCT_ACCEPTANCE_LEDGER_RECORD_INVALID');
    } finally {
      ledger.close();
      acquired.owner.close();
    }
  ...
  it('seals recovery exactly once and rejects a second recovery owner', async () => {
    const state = await prepareFailedV19State(root);
    const recovery = await acquireReviewPlannerV19ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      await finalizeReviewPlannerV19ProductAcceptanceRecovery({
        repoRoot: root,
        environment: 'branch',
    ...
    const second = await acquireReviewPlannerV19ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V19_PRODUCT_ACCEPTANCE_RECOVERY_NOT_AUTHORIZED');
    } finally {
      second.owner.close();
    }
    expect(state.attemptSha256).toHaveLength(64);
  ...
  it('fails closed when a failure terminal disagrees with its attempt-bound latest checkpoint', async () => {
    await prepareFailedV19State(root);
    await writeFile(
      join(root, ...v19PublicSegments('branch'), 'failure.json'),
      JSON.stringify({
        schemaVersion: 'phase-6.9.5-v19-product-acceptance-failure-v1',
        environment: 'branch',
...
async function prepareEarliestV19State(root: string) {
  const acquired = await acquireReviewPlannerV19ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
...
async function prepareFailedV19State(root: string) {
  const acquired = await acquireReviewPlannerV19ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
  ...
  let journal:
    | Awaited<
        ReturnType<
          typeof prepareReviewPlannerV19ProductAcceptanceRecoveryJournal
        >
      >
    | undefined;
  try {
    ...
    journal = await prepareReviewPlannerV19ProductAcceptanceRecoveryJournal({
      repoRoot: root,
      environment: 'branch',
      owner: acquired.owner,
    });
...
function v19PublicSegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V19_PRODUCT_ACCEPTANCE_PROFILE.publicLedgerSegments(
    environment,
  );
}
...
function v19RecoverySegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V19_PRODUCT_ACCEPTANCE_PROFILE.recoverySegments(
    environment,
  );
}
...
function executionManifest(
  ...
  return {
    schemaVersion: 'phase-6.9.5-v19-product-acceptance-execution-manifest-v1',
    environment,
    attemptSha256,
    databaseUrlSha256: 'e'.repeat(64),
    resources: {
      accountId: {
        review: `v19-synthetic-account-review-${'a'.repeat(32)}`,
        planner: `v19-synthetic-account-planner-${'b'.repeat(32)}`,
      },
      fixtureId: {
        review: `v19-synthetic-fixture-review-${'c'.repeat(32)}`,
        planner: `v19-synthetic-fixture-planner-${'d'.repeat(32)}`,
      },
      browser: {
        executablePath:
...
function defaultOffReceipt(component: 'review' | 'planner') {
  ...
  return {
    schemaVersion: 'phase-6.9.5-v19-product-acceptance-default-off-v1',
