...
import {
  parseReviewPlannerV15ProductAcceptanceArguments,
  runReviewPlannerV15ProductAcceptanceProductCli,
  serializeReviewPlannerV15ProductAcceptanceCliFailure,
} from './review-planner-v15-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V15_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V15 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V15_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V15 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V15_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V15 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V15 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V15 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV15ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V15_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV15ProductAcceptanceCliFailure(
      'product',
