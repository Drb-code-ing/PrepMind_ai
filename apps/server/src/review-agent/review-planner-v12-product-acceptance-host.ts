...
import {
  ...
  readReviewPlannerV12ProductAcceptanceExecutionManifest,
...
import type { ReviewPlannerV12ReadyPreflight } from './review-planner-v12-product-acceptance-composition';
import {
...
export type ReviewPlannerV12ProductAcceptanceHost = Readonly<{
  preflight(input: {
...
export type ReviewPlannerV12ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V12-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V12 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV12ProductAcceptanceHost(
  repoRoot: string,
): ReviewPlannerV12ProductAcceptanceHost {
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV12ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V12_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
  ...
  > => {
    if (
      ...
      !isReviewPlannerV12DatabaseFingerprintStable(
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV12ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v12-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV12RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV12ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV12ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV12ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV12ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV12ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV12RecoveryHostOptions = {},
): ReviewPlannerV12ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
  ...
  const readExecutionManifest =
    ...
    readReviewPlannerV12ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV12ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV12ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV12ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV12ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV12ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV12ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV12ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V12_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV12DatabaseFingerprintStable(
