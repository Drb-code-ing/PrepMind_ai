...
import {
  assertReviewPlannerV19DefaultOffEnvironment,
  finalizeReviewPlannerV19ProductAcceptanceRecovery,
  readReviewPlannerV19ProductAcceptanceLedger,
  readReviewPlannerV19ProductAcceptanceExecutionManifest,
  type ReviewPlannerV19ProductAcceptanceExecutionManifest,
  type ReviewPlannerV19ProductAcceptanceLedger,
} from './review-planner-v19-product-acceptance-ledger';
import type { ReviewPlannerV19ReadyPreflight } from './review-planner-v19-product-acceptance-composition';
import {
  acquireReviewPlannerV19ProductAcceptanceOwner,
...
const V19_BUN_AUTHORITY_TIMEOUT_MS = 5_000;
const V19_BUN_AUTHORITY_MAX_BUFFER = 8 * 1024;
const V19_BUN_AUTHORITY_HELPER_RELATIVE_PATH =
  'apps/server/scripts/review-planner-v19-v10-paired-evidence-authority.ts';

type ReviewPlannerV19BunAuthorityHelper = (
  input: Readonly<{
    file: 'bun';
...
type ReviewPlannerV19BunPairedEvidenceAuthorityOptions = Readonly<{
  executeBunHelper?: ReviewPlannerV19BunAuthorityHelper;
}>;

export function createReviewPlannerV19BunPairedEvidenceAuthority(
  repoRoot: string,
  options: ReviewPlannerV19BunPairedEvidenceAuthorityOptions = {},
): PairedEvidenceAuthority {
  const root = resolve(repoRoot);
  const helperPath = resolve(root, V19_BUN_AUTHORITY_HELPER_RELATIVE_PATH);
  ...
  return createReviewPlannerV10PairedEvidenceAuthority({
    readEvidence: async (requestedRoot) => {
      if (resolve(requestedRoot) !== root) {
        return v19BunAuthorityFallback();
      }
      try {
        const result = await executeBunHelper({
          ...
          cwd: root,
          timeoutMs: V19_BUN_AUTHORITY_TIMEOUT_MS,
          maxBuffer: V19_BUN_AUTHORITY_MAX_BUFFER,
        });
        return parseReviewPlannerV19BunAuthorityRecord(result.stdout);
      } catch {
        return v19BunAuthorityFallback();
      }
    },
...
function parseReviewPlannerV19BunAuthorityRecord(stdout: string) {
  if (
    typeof stdout !== 'string' ||
    stdout.length > V19_BUN_AUTHORITY_MAX_BUFFER
  ) {
    return v19BunAuthorityFallback();
  }
  try {
...
function v19BunAuthorityFallback() {
  return Object.freeze({
...
export type ReviewPlannerV19ProductAcceptanceHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
  }): Promise<Readonly<{ status: 'blocked' }> | ReviewPlannerV19ReadyPreflight>;
  revalidate(
    preflight: ReviewPlannerV19ReadyPreflight,
  ): Promise<Readonly<{ status: 'blocked' }> | ReviewPlannerV19ReadyPreflight>;
  prepareExecution(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
    preflight: ReviewPlannerV19ReadyPreflight;
    ledger: ReviewPlannerV19ProductAcceptanceLedger;
    journal: ReviewPlannerV19ProductAcceptanceRecoveryJournal;
    executionManifest: ReviewPlannerV19ProductAcceptanceExecutionManifest;
  }): Promise<RuntimeResources>;
  dispose(): Promise<void>;
}>;

export type ReviewPlannerV19ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V19-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V19 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV19ProductAcceptanceHost(
  repoRoot: string,
): ReviewPlannerV19ProductAcceptanceHost {
  const root = resolve(repoRoot);
  const hostDatabaseUrlSha256 = sha256(readRootDatabaseUrl(root));
  const pairedEvidenceAuthority =
    createReviewPlannerV19BunPairedEvidenceAuthority(root);
  let latest:
    | Awaited<
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV19ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V19_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
  ...
  const getReadyPreflight = async (input: {
    ...
    Readonly<{ status: 'blocked' }> | ReviewPlannerV19ReadyPreflight
  > => {
    if (
      resolve(input.repoRoot) !== root ||
      !isReviewPlannerV19DatabaseFingerprintStable(
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV19ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v19-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV19RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV19ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV19ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV19ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV19ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV19ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV19RecoveryHostOptions = {},
): ReviewPlannerV19ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
    options.readLedger ?? readReviewPlannerV19ProductAcceptanceLedger;
  const readExecutionManifest =
    options.readExecutionManifest ??
    readReviewPlannerV19ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV19ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV19ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV19ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV19ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV19ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV19ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV19ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V19_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV19DatabaseFingerprintStable(
