...
const V13_PUBLIC_SLOT_LEAF = /^slot-(review|planner)-(api|browser)\.json$/;
const V13_PUBLIC_DEFAULT_OFF_LEAF = /^default-off-(review|planner)\.json$/;
const V13_PUBLIC_RECEIPT_LEAVES = new Set([
...
export const REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_CHECKPOINTS = [
  'review_api_setup',
  'review_api_activate',
...
export type ReviewPlannerV13ProductAcceptanceCheckpoint =
  (typeof REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_CHECKPOINTS)[number];
export type ReviewPlannerV13ProductAcceptanceOwnerRole = 'product' | 'recovery';

const attemptBindingSchema = z
  .object({
    schemaVersion: z.literal('phase-6.9.5-v13-product-acceptance-attempt-v1'),
...
const checkpointSchema = z
  .object({
    schemaVersion: z.literal(
      REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_PROFILE.schemas.checkpoint,
    ),
    component: z.enum(['review', 'planner']),
    slot: z.enum(['api', 'browser']),
    checkpoint: z.enum(REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_CHECKPOINTS),
    providerCallState: z.enum(['not_started', 'indeterminate']),
  })
  .strict();

type AttemptBinding = z.infer<typeof attemptBindingSchema>;
export type ReviewPlannerV13ProductAcceptanceCheckpointRecord = z.infer<
  typeof checkpointSchema
>;

export type ReviewPlannerV13ProductAcceptanceOwner = Readonly<{
  assertHeld(): void;
  close(): void;
}>;
...
type OwnerState = {
  ...
  role: ReviewPlannerV13ProductAcceptanceOwnerRole;
...
const ownerState = new WeakMap<
  ReviewPlannerV13ProductAcceptanceOwner,
  OwnerState
>();

export type ReviewPlannerV13ProductAcceptanceRecoveryJournal = Readonly<{
  attemptSha256(): string;
  appendCheckpoint(
    value: unknown,
  ): ReviewPlannerV13ProductAcceptanceCheckpointRecord;
  latestCheckpoint(): ReviewPlannerV13ProductAcceptanceCheckpointRecord | null;
  close(): void;
}>;
...
const journalState = new WeakMap<
  ReviewPlannerV13ProductAcceptanceRecoveryJournal,
  JournalState
>();

export async function acquireReviewPlannerV13ProductAcceptanceOwner(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  role: ReviewPlannerV13ProductAcceptanceOwnerRole;
}): Promise<
...
> {
  if (!isEnvironment(input.environment) || !isOwnerRole(input.role)) {
    throw new Error('V13_PRODUCT_ACCEPTANCE_OWNER_INPUT_INVALID');
  }
  ...
  try {
    ...
    directory = await openWindowsNoReparseFrozenDirectory(repoRoot, [
      ...REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_PROFILE.recoverySegments(
        input.environment,
      ),
    ]);
    ...
    const owner: ReviewPlannerV13ProductAcceptanceOwner = Object.freeze({
      assertHeld() {
        const state = ownerState.get(owner);
        if (!state || state.closed) {
          throw new Error('V13_PRODUCT_ACCEPTANCE_OWNER_CLOSED');
        }
    ...
    throw new Error('V13_PRODUCT_ACCEPTANCE_OWNER_IO');
  }
}

export function assertReviewPlannerV13ProductAcceptanceOwner(
  owner: ReviewPlannerV13ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  allowedRoles: readonly ReviewPlannerV13ProductAcceptanceOwnerRole[],
): void {
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V13_PRODUCT_ACCEPTANCE_OWNER_INVALID');
  }
  owner.assertHeld();
}

export function registerReviewPlannerV13ProductAcceptanceOwnerAttempt(
  owner: ReviewPlannerV13ProductAcceptanceOwner,
  environment: ReviewPlannerProductAcceptanceEnvironment,
  attemptSha256: string,
): void {
  assertReviewPlannerV13ProductAcceptanceOwner(owner, environment, ['product']);
  const state = ownerState.get(owner);
  if (
    !state ||
    ...
    throw new Error('V13_PRODUCT_ACCEPTANCE_ATTEMPT_INVALID');
  }
  state.attemptSha256 = attemptSha256;
}

export async function bindReviewPlannerV13ProductAcceptanceAttempt(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV13ProductAcceptanceOwner;
  attemptId: string;
  ...
  try {
    ...
    const binding = parseAttemptBinding({
      schemaVersion: 'phase-6.9.5-v13-product-acceptance-attempt-v1',
    ...
    throw new Error('V13_PRODUCT_ACCEPTANCE_ATTEMPT_IO');
  } finally {
    directory.close();
  }
}

export async function readReviewPlannerV13ProductAcceptanceAttemptBinding(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<Readonly<AttemptBinding>> {
  try {
...
export async function prepareReviewPlannerV13ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV13ProductAcceptanceOwner;
  ...
  return openV13Journal(input, state.attemptSha256, true);
}

export async function openReviewPlannerV13ProductAcceptanceRecoveryJournal(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
  owner: ReviewPlannerV13ProductAcceptanceOwner;
}): Promise<ReviewPlannerV13ProductAcceptanceRecoveryJournal> {
  ...
  return openV13Journal(input, binding.attemptSha256, false);
}

export async function inspectReviewPlannerV13ProductAcceptanceRecoveryCheckpoint(input: {
  repoRoot: string;
  environment: ReviewPlannerProductAcceptanceEnvironment;
}): Promise<ReviewPlannerV13ProductAcceptanceCheckpointRecord | null> {
  await readReviewPlannerV13ProductAcceptanceAttemptBinding(input);
  const directory = await openWindowsNoReparseExistingFrozenDirectory(
    input.repoRoot,
    [
...
export function parseReviewPlannerV13ProductAcceptanceCheckpoint(
  value: unknown,
): ReviewPlannerV13ProductAcceptanceCheckpointRecord {
  const parsed = checkpointSchema.safeParse(value);
  if (!parsed.success || !checkpointMatchesBoundary(parsed.data)) {
    throw new Error('V13_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  }
...
async function openV13Journal(
  ...
  try {
    ...
    if (requireEmpty && history.length !== 0) {
      throw new Error('V13_PRODUCT_ACCEPTANCE_CHECKPOINT_EXISTS');
    }
    const journal: ReviewPlannerV13ProductAcceptanceRecoveryJournal =
      Object.freeze({
        attemptSha256: () => attemptSha256,
        appendCheckpoint(value) {
          const state = journalState.get(journal);
          if (!state || state.closed) {
            throw new Error('V13_PRODUCT_ACCEPTANCE_RECOVERY_CLOSED');
          }
          ...
          const record =
            parseReviewPlannerV13ProductAcceptanceCheckpoint(value);
    ...
    if (
      ...
      leaves.some(
        (leaf) =>
          ...
          !V13_PUBLIC_SLOT_LEAF.test(leaf) &&
          !V13_PUBLIC_DEFAULT_OFF_LEAF.test(leaf) &&
          !V13_PUBLIC_RECEIPT_LEAVES.has(leaf),
      )
    ) {
      throw new Error();
    }
...
function readCheckpointHistory(
  directory: WindowsNoReparseChildDirectory,
): readonly ReviewPlannerV13ProductAcceptanceCheckpointRecord[] {
  assertRecoveryLeaves(directory, true);
  try {
    ...
    const records = entries.map((entry) => {
      const record = parseReviewPlannerV13ProductAcceptanceCheckpoint(
...
function assertNextCheckpoint(
  history: readonly ReviewPlannerV13ProductAcceptanceCheckpointRecord[],
  record: ReviewPlannerV13ProductAcceptanceCheckpointRecord,
) {
  const boundary = `${record.component}_${record.slot}`;
  const expected = REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_CHECKPOINTS.filter(
  ...
  if (
    ...
    throw new Error('V13_PRODUCT_ACCEPTANCE_CHECKPOINT_ORDER_INVALID');
  }
}
...
function expectedProviderCallState(
  ...
  return record.checkpoint.endsWith('_dispatch') ||
    REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
      record.checkpoint,
    ) >
      REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
...
function checkpointMatchesBoundary(
  value: ReviewPlannerV13ProductAcceptanceCheckpointRecord,
) {
...
function isOwnerRole(
  value: unknown,
): value is ReviewPlannerV13ProductAcceptanceOwnerRole {
