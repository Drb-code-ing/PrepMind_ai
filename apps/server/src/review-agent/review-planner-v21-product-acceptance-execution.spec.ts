import { createReviewPlannerV21ProductAcceptanceDiagnosticsPort } from './review-planner-v21-product-acceptance-diagnostics';
import { createReviewPlannerV21ProductAcceptanceRunnerLedgerAdapter } from './review-planner-v21-product-acceptance-execution';
import { runReviewPlannerV21ProductAcceptanceComposition } from './review-planner-v21-product-acceptance-composition';
import { REVIEW_PLANNER_V21_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V21 execution bridge', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('emits only fixed V21 checkpoints and a nonsecret failure projection', () => {
    ...
    const diagnostics = createReviewPlannerV21ProductAcceptanceDiagnosticsPort({
      environment: 'branch',
  ...
  it('durably commits exactly four V21 runner slots without V11 records', async () => {
    const ledger = {
      attemptSha256: () => attemptSha256,
      claimSlot: jest.fn(),
    ...
    const adapter = createReviewPlannerV21ProductAcceptanceRunnerLedgerAdapter({
      environment: 'branch',
      ledger,
      manifest: {
        schemaVersion:
          'phase-6.9.5-v21-product-acceptance-manifest-v1' as const,
  ...
  it('rejects a component-only default-off record instead of synthesizing evidence', () => {
    ...
    expect(() => adapter.recordDefaultOff({ component: 'review' })).toThrow(
      'V21_PRODUCT_ACCEPTANCE_EXECUTION_INVALID',
    );
    expect(ledger.recordDefaultOff).not.toHaveBeenCalled();
  });

  it('projects a strict V8 runner receipt into the V21 durable default-off record', () => {
    const ledger = {
      attemptSha256: () => attemptSha256,
      claimSlot: jest.fn(),
      recordSlotResult: jest.fn(),
  ...
  it('rejects an extra raw model instead of allowing it to influence the V8 projection', () => {
    ...
    ).toThrow('V21_PRODUCT_ACCEPTANCE_EXECUTION_INVALID');
    expect(ledger.recordDefaultOff).not.toHaveBeenCalled();
  });

  it('stops at V21 preflight before owner, ledger, runner, or external work', async () => {
    const ports = {
