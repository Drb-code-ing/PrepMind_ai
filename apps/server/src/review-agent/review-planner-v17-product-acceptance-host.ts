...
import {
  assertReviewPlannerV17DefaultOffEnvironment,
  finalizeReviewPlannerV17ProductAcceptanceRecovery,
  readReviewPlannerV17ProductAcceptanceLedger,
  readReviewPlannerV17ProductAcceptanceExecutionManifest,
...
import type { ReviewPlannerV17ReadyPreflight } from './review-planner-v17-product-acceptance-composition';
import {
...
const V17_BUN_AUTHORITY_TIMEOUT_MS = 5_000;
const V17_BUN_AUTHORITY_MAX_BUFFER = 8 * 1024;
const V17_BUN_AUTHORITY_HELPER_RELATIVE_PATH =
  'apps/server/scripts/review-planner-v17-v10-paired-evidence-authority.ts';

type ReviewPlannerV17BunAuthorityHelper = (
  input: Readonly<{
    file: 'bun';
...
type ReviewPlannerV17BunPairedEvidenceAuthorityOptions = Readonly<{
  executeBunHelper?: ReviewPlannerV17BunAuthorityHelper;
}>;

export function createReviewPlannerV17BunPairedEvidenceAuthority(
  repoRoot: string,
  options: ReviewPlannerV17BunPairedEvidenceAuthorityOptions = {},
): PairedEvidenceAuthority {
  const root = resolve(repoRoot);
  const helperPath = resolve(root, V17_BUN_AUTHORITY_HELPER_RELATIVE_PATH);
  ...
  return createReviewPlannerV10PairedEvidenceAuthority({
    readEvidence: async (requestedRoot) => {
      if (resolve(requestedRoot) !== root) {
        return v17BunAuthorityFallback();
      }
      try {
        const result = await executeBunHelper({
          ...
          cwd: root,
          timeoutMs: V17_BUN_AUTHORITY_TIMEOUT_MS,
          maxBuffer: V17_BUN_AUTHORITY_MAX_BUFFER,
        });
        return parseReviewPlannerV17BunAuthorityRecord(result.stdout);
      } catch {
        return v17BunAuthorityFallback();
      }
    },
...
function parseReviewPlannerV17BunAuthorityRecord(stdout: string) {
  if (
    typeof stdout !== 'string' ||
    stdout.length > V17_BUN_AUTHORITY_MAX_BUFFER
  ) {
    return v17BunAuthorityFallback();
  }
  try {
...
function v17BunAuthorityFallback() {
  return Object.freeze({
...
export type ReviewPlannerV17ProductAcceptanceHost = Readonly<{
  preflight(input: {
...
export type ReviewPlannerV17ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V17-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V17 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV17ProductAcceptanceHost(
  repoRoot: string,
): ReviewPlannerV17ProductAcceptanceHost {
  const root = resolve(repoRoot);
  const hostDatabaseUrlSha256 = sha256(readRootDatabaseUrl(root));
  const pairedEvidenceAuthority =
    createReviewPlannerV17BunPairedEvidenceAuthority(root);
  let latest:
    | Awaited<
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV17ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V17_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
  ...
  > => {
    if (
      ...
      !isReviewPlannerV17DatabaseFingerprintStable(
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV17ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v17-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV17RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV17ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV17ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV17ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV17ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV17ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV17RecoveryHostOptions = {},
): ReviewPlannerV17ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
    options.readLedger ?? readReviewPlannerV17ProductAcceptanceLedger;
  const readExecutionManifest =
    options.readExecutionManifest ??
    readReviewPlannerV17ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV17ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV17ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV17ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV17ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV17ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV17ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV17ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V17_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV17DatabaseFingerprintStable(
