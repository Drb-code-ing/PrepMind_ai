...
import {
  parseReviewPlannerV13ProductAcceptanceArguments,
  runReviewPlannerV13ProductAcceptanceProductCli,
  serializeReviewPlannerV13ProductAcceptanceCliFailure,
} from './review-planner-v13-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V13 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V13_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V13 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V13_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V13 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V13 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V13 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV13ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V13_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV13ProductAcceptanceCliFailure(
      'product',
