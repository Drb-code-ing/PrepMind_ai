import { createReviewPlannerV14ProductAcceptanceDiagnosticsPort } from './review-planner-v14-product-acceptance-diagnostics';
import { createReviewPlannerV14ProductAcceptanceRunnerLedgerAdapter } from './review-planner-v14-product-acceptance-execution';
import { runReviewPlannerV14ProductAcceptanceComposition } from './review-planner-v14-product-acceptance-composition';
import { REVIEW_PLANNER_V14_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V14 execution bridge', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('emits only fixed V14 checkpoints and a nonsecret failure projection', () => {
    ...
    const diagnostics = createReviewPlannerV14ProductAcceptanceDiagnosticsPort({
      environment: 'branch',
  ...
  it('durably commits exactly four V14 runner slots without V11 records', async () => {
    const ledger = {
      attemptSha256: () => attemptSha256,
      claimSlot: jest.fn(),
    ...
    const adapter = createReviewPlannerV14ProductAcceptanceRunnerLedgerAdapter({
      environment: 'branch',
      ledger,
      manifest: {
        schemaVersion:
          'phase-6.9.5-v14-product-acceptance-manifest-v1' as const,
  ...
  it('stops at V14 preflight before owner, ledger, runner, or external work', async () => {
    const ports = {
