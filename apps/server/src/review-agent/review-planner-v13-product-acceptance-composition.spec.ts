...
describe('Review Planner V13 default product composition', () => {
  it('keeps the unconfigured host boundary default-off before any runtime capability', async () => {
    ...
    await expect(
      composition.ports.preflight({
        environment: 'branch',
        repoRoot: 'E:\\v13-default-off',
      }),
    ).resolves.toEqual({ status: 'blocked' });
  });

  it('accepts an injected V13-only ready preflight without touching a V11 boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
        repoRoot: 'E:\\v13-ready',
    ...
    const create =
      createDefaultReviewPlannerV13ProductAcceptanceComposition as unknown as (
        root: string,
        options: { boundary: { preflight: typeof preflight } },
      ) => ReturnType<
        typeof createDefaultReviewPlannerV13ProductAcceptanceComposition
      >;
    const composition = create('E:\\v13-ready', {
      boundary: { preflight },
    });
  ...
  it('obtains the default preflight from a V13 host factory instead of a test-only boundary', async () => {
    const preflight = jest.fn(() =>
      Promise.resolve({
        status: 'ready' as const,
        environment: 'branch' as const,
    ...
    const create =
      createDefaultReviewPlannerV13ProductAcceptanceComposition as unknown as (
        ...
        typeof createDefaultReviewPlannerV13ProductAcceptanceComposition
      >;
    const composition = create('E:\\v13-host', { hostFactory });
    ...
    expect(hostFactory).toHaveBeenCalledWith('E:\\v13-host');
    expect(preflight).toHaveBeenCalledTimes(1);
  });

  it('prepares the runtime only after the V13 reservation and journal exist', async () => {
    const order: string[] = [];
    const owner = {
      assertHeld: jest.fn(),
      close: jest.fn(),
  ...
  it('revalidates the ready identity after owner acquisition and before reserving V13', async () => {
    const owner = { assertHeld: jest.fn(), close: jest.fn() };
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v13-revalidate',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
  ...
  it('projects a reservation failure as default-off rather than an operation failure', async () => {
    ...
    const preflight = {
      status: 'ready' as const,
      environment: 'branch' as const,
      repoRoot: 'E:\\v13-reserve-failure',
      commitSha: 'a'.repeat(40),
      pairedEvidenceSha256: 'b'.repeat(64),
      databaseUrlSha256: 'c'.repeat(64),
    };
