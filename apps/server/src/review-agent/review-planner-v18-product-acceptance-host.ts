...
import {
  ...
  readReviewPlannerV18ProductAcceptanceExecutionManifest,
...
import type { ReviewPlannerV18ReadyPreflight } from './review-planner-v18-product-acceptance-composition';
import {
...
const V18_BUN_AUTHORITY_TIMEOUT_MS = 5_000;
const V18_BUN_AUTHORITY_MAX_BUFFER = 8 * 1024;
const V18_BUN_AUTHORITY_HELPER_RELATIVE_PATH =
  'apps/server/scripts/review-planner-v18-v10-paired-evidence-authority.ts';

type ReviewPlannerV18BunAuthorityHelper = (
  input: Readonly<{
    file: 'bun';
...
type ReviewPlannerV18BunPairedEvidenceAuthorityOptions = Readonly<{
  executeBunHelper?: ReviewPlannerV18BunAuthorityHelper;
}>;

export function createReviewPlannerV18BunPairedEvidenceAuthority(
  repoRoot: string,
  options: ReviewPlannerV18BunPairedEvidenceAuthorityOptions = {},
): PairedEvidenceAuthority {
  const root = resolve(repoRoot);
  const helperPath = resolve(root, V18_BUN_AUTHORITY_HELPER_RELATIVE_PATH);
  ...
  return createReviewPlannerV10PairedEvidenceAuthority({
    readEvidence: async (requestedRoot) => {
      if (resolve(requestedRoot) !== root) {
        return v18BunAuthorityFallback();
      }
      try {
        const result = await executeBunHelper({
          ...
          cwd: root,
          timeoutMs: V18_BUN_AUTHORITY_TIMEOUT_MS,
          maxBuffer: V18_BUN_AUTHORITY_MAX_BUFFER,
        });
        return parseReviewPlannerV18BunAuthorityRecord(result.stdout);
      } catch {
        return v18BunAuthorityFallback();
      }
    },
...
function parseReviewPlannerV18BunAuthorityRecord(stdout: string) {
  if (
    typeof stdout !== 'string' ||
    stdout.length > V18_BUN_AUTHORITY_MAX_BUFFER
  ) {
    return v18BunAuthorityFallback();
  }
  try {
...
function v18BunAuthorityFallback() {
  return Object.freeze({
...
export type ReviewPlannerV18ProductAcceptanceHost = Readonly<{
  preflight(input: {
...
export type ReviewPlannerV18ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V18-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V18 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV18ProductAcceptanceHost(
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV18ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V18_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
  ...
  > => {
    if (
      ...
      !isReviewPlannerV18DatabaseFingerprintStable(
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV18ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v18-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV18RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV18ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV18ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV18ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV18ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV18ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV18RecoveryHostOptions = {},
): ReviewPlannerV18ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
    options.readLedger ?? readReviewPlannerV18ProductAcceptanceLedger;
  const readExecutionManifest =
    options.readExecutionManifest ??
    readReviewPlannerV18ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV18ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV18ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV18ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV18ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV18ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV18ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV18ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V18_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV18DatabaseFingerprintStable(
