import {
  ...
  parseReviewPlannerV17ProductAcceptanceAggregate,
  parseReviewPlannerV17ProductAcceptanceCleanup,
  parseReviewPlannerV17ProductAcceptanceDefaultOff,
  parseReviewPlannerV17ProductAcceptanceExecutionManifest,
  parseReviewPlannerV17ProductAcceptanceFailure,
  parseReviewPlannerV17ProductAcceptanceManifest,
  parseReviewPlannerV17ProductAcceptanceOwnerIsolation,
  parseReviewPlannerV17ProductAcceptanceSlotResult,
  parseReviewPlannerV17ProductAcceptanceSuccess,
} from './review-planner-v17-product-acceptance-ledger';
import * as reviewPlannerV17Ledger from './review-planner-v17-product-acceptance-ledger';
import { parseReviewPlannerV17ProductAcceptanceCheckpoint } from './review-planner-v17-product-acceptance-recovery';
import { REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V17 product-acceptance ledger contracts', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('rejects V11 record identities from every V17 durable contract', () => {
    expect(() =>
      parseReviewPlannerV17ProductAcceptanceManifest({
        schemaVersion:
    ...
    expect(() =>
      parseReviewPlannerV17ProductAcceptanceCheckpoint({
    ...
    ).toThrow('V17_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  });

  it('accepts only the complete V17 four-slot durable success vocabulary', () => {
    const slot = parseReviewPlannerV17ProductAcceptanceSlotResult({
      schemaVersion: 'phase-6.9.5-v17-product-acceptance-slot-result-v1',
      slot: 'review-api',
      traceSha256: 'b'.repeat(64),
    });
    const defaultOff = parseReviewPlannerV17ProductAcceptanceDefaultOff({
    ...
    const isolation = parseReviewPlannerV17ProductAcceptanceOwnerIsolation({
    ...
    const cleanup = parseReviewPlannerV17ProductAcceptanceCleanup({
    ...
    const success = parseReviewPlannerV17ProductAcceptanceSuccess({
      schemaVersion: 'phase-6.9.5-v17-product-acceptance-success-v1',
      environment: 'branch',
      attemptSha256,
    });
  ...
  it.each(['https://api.deepseek.com', 'https://api.deepseek.com/v1'] as const)(
    'accepts the V17 durable default-off receipt with approved base URL %s',
    (baseUrl) => {
      const receipt = {
        schemaVersion: 'phase-6.9.5-v17-product-acceptance-default-off-v1',
        model: 'deepseek-v4-pro',
      ...
      expect(
        parseReviewPlannerV17ProductAcceptanceDefaultOff(receipt),
  ...
  ])('rejects a %s V17 durable default-off receipt base URL', (_, baseUrl) => {
    const receipt = {
      schemaVersion: 'phase-6.9.5-v17-product-acceptance-default-off-v1',
      model: 'deepseek-v4-pro',
      component: 'review',
  ...
  it('allows only Flash or Pro in the V17 default-off environment without relaxing its controls', () => {
    const assertDefaultOff = (
      reviewPlannerV17Ledger as typeof reviewPlannerV17Ledger & {
  ...
  it.each(['https://api.deepseek.com', 'https://api.deepseek.com/v1'])(
    ...
    (baseUrl) => {
      ...
      if (!assertDefaultOff) throw new Error('V17 validator unavailable');
      const entries = [
        'AI_PROVIDER_MODE=mock',
        'AI_ENABLE_LIVE_CALLS=false',
  ...
  it.each([
    ...
    'rejects a %s model from a V17 durable default-off receipt',
    (_, model) => {
      const receipt = {
        schemaVersion: 'phase-6.9.5-v17-product-acceptance-default-off-v1',
        baseUrl: 'https://api.deepseek.com',
  ...
  it('fails closed when a V17 durable record carries secret-like content', () => {
    expect(() =>
      parseReviewPlannerV17ProductAcceptanceSlotResult({
        schemaVersion: 'phase-6.9.5-v17-product-acceptance-slot-result-v1',
        slot: 'review-api',
      ...
      parseReviewPlannerV17ProductAcceptanceExecutionManifest({
        ...
        resources: {
          ...
          browser: {
            ...
            profilePath:
              '.tmp/phase-6-9-5-v17-product-acceptance/branch/profile-v17',
          },
        },
      }),
    ).toMatchObject({
  ...
  it('requires an attempt-bound hash of the selected database URL without persisting the URL', () => {
    ...
    expect(() =>
      parseReviewPlannerV17ProductAcceptanceExecutionManifest(record),
    ).toThrow('V17_PRODUCT_ACCEPTANCE_LEDGER_RECORD_INVALID');
    expect(
      parseReviewPlannerV17ProductAcceptanceExecutionManifest({
        ...record,
  ...
  it('generates distinct V17-only selectors that bind cleanup to the reserved attempt', () => {
    const execution = createReviewPlannerV17ProductAcceptanceExecutionManifest({
      environment: 'branch',
      attemptSha256,
      databaseUrlSha256: 'f'.repeat(64),
    });
    ...
    expect(execution.resources.accountId.review).toMatch(
      /^v17-synthetic-account-review-[a-f0-9]{32}$/,
    );
  ...
  it('accepts only the fixed V17 failure terminal vocabulary', () => {
    expect(
      parseReviewPlannerV17ProductAcceptanceFailure({
