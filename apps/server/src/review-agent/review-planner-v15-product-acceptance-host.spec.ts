...
import {
  createDefaultReviewPlannerV15ProductAcceptanceRecoveryHost,
  isReviewPlannerV15DatabaseFingerprintStable,
} from './review-planner-v15-product-acceptance-host';

describe('Review Planner V15 recovery host', () => {
  const databaseUrl = 'postgresql://v15-recovery.example.invalid/prepmind';
  const databaseUrlSha256 = createHash('sha256')
    .update(databaseUrl)
    .digest('hex');
  const executionManifest = {
    schemaVersion:
      'phase-6.9.5-v15-product-acceptance-execution-manifest-v1' as const,
    environment: 'branch' as const,
    attemptSha256: 'a'.repeat(64),
    databaseUrlSha256,
    resources: {
      accountId: {
        review: `v15-synthetic-account-review-${'b'.repeat(32)}`,
        planner: `v15-synthetic-account-planner-${'c'.repeat(32)}`,
      },
      fixtureId: {
        review: `v15-synthetic-fixture-review-${'d'.repeat(32)}`,
        planner: `v15-synthetic-fixture-planner-${'e'.repeat(32)}`,
      },
      browser: {
        executablePath:
  ...
  it('takes the recovery owner, revalidates its attempt, and seals only after restore plus exact cleanup', async () => {
    ...
    const host = createDefaultReviewPlannerV15ProductAcceptanceRecoveryHost(
      'E:\\v15-recovery-host',
      {
        platform: 'win32',
        readLedger,
