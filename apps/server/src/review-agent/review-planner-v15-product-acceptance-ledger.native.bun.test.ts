...
import {
  finalizeReviewPlannerV15ProductAcceptanceRecovery,
  readReviewPlannerV15ProductAcceptanceLedger,
  reserveReviewPlannerV15ProductAcceptanceLedger,
} from './review-planner-v15-product-acceptance-ledger';
import {
  acquireReviewPlannerV15ProductAcceptanceOwner,
  inspectReviewPlannerV15ProductAcceptanceRecoveryCheckpoint,
  openReviewPlannerV15ProductAcceptanceRecoveryJournal,
  prepareReviewPlannerV15ProductAcceptanceRecoveryJournal,
...
describeWindows('Review/Planner V15 durable product acceptance ledger', () => {
  let root = '';

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'prepmind-v15-ledger-'));
  ...
  it('creates one V15-only reservation whose public, binding, and private execution hashes agree', async () => {
    const state = await prepareEarliestV15State(root);
  ...
  it('permits only one V15 reservation even after the first owner releases its private lock', async () => {
    const state = await prepareEarliestV15State(root);
    const next = await acquireReviewPlannerV15ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V15_PRODUCT_ACCEPTANCE_ALREADY_RESERVED');
    } finally {
      next.owner.close();
    }
  ...
  it('rolls back a verified unstarted reservation so a fresh V15 reservation can be created', async () => {
    const acquired = await acquireReviewPlannerV15ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    const ledger = await reserveReviewPlannerV15ProductAcceptanceLedger({
      repoRoot: root,
      environment: 'branch',
    ...
    try {
      ...
      ledger.writeManifest({
        schemaVersion: 'phase-6.9.5-v15-product-acceptance-manifest-v1',
        environment: 'branch',
        attemptSha256,
      });
    ...
    const fresh = await acquireReviewPlannerV15ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      const next = await reserveReviewPlannerV15ProductAcceptanceLedger({
        repoRoot: root,
        environment: 'branch',
        owner: fresh.owner,
      });
  ...
  it('admits the strict earliest V15 state with no checkpoint for recovery', async () => {
    const state = await prepareEarliestV15State(root);
    const owner = await acquireReviewPlannerV15ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      const journal =
        await openReviewPlannerV15ProductAcceptanceRecoveryJournal({
          repoRoot: root,
          environment: 'branch',
          owner: owner.owner,
        });
  ...
  it('fails closed for a bare reservation and a malformed checkpoint', async () => {
    const publicRoot = join(root, ...v15PublicSegments('branch'));
  ...
  it('fails closed for a V11 manifest injected into a V15 reservation', async () => {
    const state = await prepareEarliestV15State(root);
    const publicRoot = join(root, ...v15PublicSegments('branch'));
    await writeFile(
  ...
  it('seals exactly four V15 slots and leaves V11/V12/V13/V14 root bytes untouched', async () => {
    ...
    try {
      ...
      for (const [slot, traceSha256] of [
        ...
        ledger.recordSlotResult({
          schemaVersion: 'phase-6.9.5-v15-product-acceptance-slot-result-v1',
          slot,
          traceSha256,
        });
      }
      ...
      ledger.recordOwnerIsolation({
        schemaVersion: 'phase-6.9.5-v15-product-acceptance-owner-isolation-v1',
      ...
      ledger.recordCleanup({
        schemaVersion: 'phase-6.9.5-v15-product-acceptance-cleanup-v1',
        syntheticAccounts: 0,
        fixtures: 0,
        traces: 0,
        browserProfiles: 0,
      ...
      await ledger.finalizeSuccess({
        schemaVersion: 'phase-6.9.5-v15-product-acceptance-aggregate-v1',
        environment: 'branch',
        attemptSha256,
        requests: 4,
        durationMs: 4_000,
  ...
  it('rejects a success terminal after a failure terminal has been written', async () => {
    ...
    try {
      ...
      ).toThrow('V15_PRODUCT_ACCEPTANCE_LEDGER_RECORD_INVALID');
    } finally {
      ledger.close();
      acquired.owner.close();
    }
  ...
  it('seals recovery exactly once and rejects a second recovery owner', async () => {
    const state = await prepareFailedV15State(root);
    const recovery = await acquireReviewPlannerV15ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      await finalizeReviewPlannerV15ProductAcceptanceRecovery({
        repoRoot: root,
        environment: 'branch',
    ...
    const second = await acquireReviewPlannerV15ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V15_PRODUCT_ACCEPTANCE_RECOVERY_NOT_AUTHORIZED');
    } finally {
      second.owner.close();
    }
    expect(state.attemptSha256).toHaveLength(64);
  ...
  it('fails closed when a failure terminal disagrees with its attempt-bound latest checkpoint', async () => {
    await prepareFailedV15State(root);
    await writeFile(
      join(root, ...v15PublicSegments('branch'), 'failure.json'),
      JSON.stringify({
        schemaVersion: 'phase-6.9.5-v15-product-acceptance-failure-v1',
        environment: 'branch',
...
async function prepareEarliestV15State(root: string) {
  const acquired = await acquireReviewPlannerV15ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
...
async function prepareFailedV15State(root: string) {
  const acquired = await acquireReviewPlannerV15ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
  ...
  let journal:
    | Awaited<
        ReturnType<
          typeof prepareReviewPlannerV15ProductAcceptanceRecoveryJournal
        >
      >
    | undefined;
  try {
    ...
    journal = await prepareReviewPlannerV15ProductAcceptanceRecoveryJournal({
      repoRoot: root,
      environment: 'branch',
      owner: acquired.owner,
    });
    journal.appendCheckpoint({
      schemaVersion: 'phase-6.9.5-v15-product-acceptance-checkpoint-v1',
      component: 'review',
      slot: 'api',
      checkpoint: 'review_api_setup',
      providerCallState: 'not_started',
...
function executionManifest(
  ...
  return {
    schemaVersion: 'phase-6.9.5-v15-product-acceptance-execution-manifest-v1',
    environment,
    attemptSha256,
    databaseUrlSha256: 'e'.repeat(64),
    resources: {
      accountId: {
        review: `v15-synthetic-account-review-${'a'.repeat(32)}`,
        planner: `v15-synthetic-account-planner-${'b'.repeat(32)}`,
      },
      fixtureId: {
        review: `v15-synthetic-fixture-review-${'c'.repeat(32)}`,
        planner: `v15-synthetic-fixture-planner-${'d'.repeat(32)}`,
      },
      browser: {
        executablePath:
...
function defaultOffReceipt(component: 'review' | 'planner') {
  ...
  return {
    schemaVersion: 'phase-6.9.5-v15-product-acceptance-default-off-v1',
