...
import * as subject from './review-planner-v21-product-acceptance-host';
...
function bridgeFactory(): BridgeFactory {
  const candidate = (
    subject as typeof subject & {
      createReviewPlannerV21BunPairedEvidenceAuthority?: unknown;
    }
  ).createReviewPlannerV21BunPairedEvidenceAuthority;
  if (typeof candidate !== 'function') {
    throw new Error('V21_BUN_AUTHORITY_BRIDGE_MISSING');
  }
  return candidate;
}

describe('Review Planner V21 Bun paired-evidence authority bridge', () => {
  beforeEach(() => {
    ...
    mockRunDefaultHostPreflight.mockImplementation(
      async (
        ...
        return Object.freeze({
          ...
          branchName: 'codex/v21-bun-authority-bridge',
  ...
  it('returns ready from the Node V21 host preflight when started in apps/server', async () => {
    const previousCwd = process.cwd();
    process.chdir(appsServerRoot);
    try {
      const host = subject.createDefaultReviewPlannerV21ProductAcceptanceHost(
        root,
        {
      ...
      if (preflight.status !== 'ready') {
        throw new Error('V21_BUN_AUTHORITY_PRELIGHT_BLOCKED');
      }
