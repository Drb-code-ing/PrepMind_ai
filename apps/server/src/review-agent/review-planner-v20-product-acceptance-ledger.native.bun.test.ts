...
import {
  ...
  openReviewPlannerV20ProductAcceptanceRecoveryJournal,
...
describeWindows('Review/Planner V20 durable product acceptance ledger', () => {
  let root = '';

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'prepmind-v20-ledger-'));
  ...
  it('creates one V20-only reservation whose public, binding, and private execution hashes agree', async () => {
    const state = await prepareEarliestV20State(root);
  ...
  it('permits only one V20 reservation even after the first owner releases its private lock', async () => {
    const state = await prepareEarliestV20State(root);
    const next = await acquireReviewPlannerV20ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V20_PRODUCT_ACCEPTANCE_ALREADY_RESERVED');
    } finally {
      next.owner.close();
    }
  ...
  it('rolls back a verified unstarted reservation so a fresh V20 reservation can be created', async () => {
    const acquired = await acquireReviewPlannerV20ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    const ledger = await reserveReviewPlannerV20ProductAcceptanceLedger({
      repoRoot: root,
      environment: 'branch',
    ...
    try {
      ...
      ledger.writeManifest({
        schemaVersion: 'phase-6.9.5-v20-product-acceptance-manifest-v1',
        environment: 'branch',
        attemptSha256,
      });
    ...
    const fresh = await acquireReviewPlannerV20ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'product',
    });
    ...
    try {
      const next = await reserveReviewPlannerV20ProductAcceptanceLedger({
        repoRoot: root,
        environment: 'branch',
        owner: fresh.owner,
      });
  ...
  it('admits the strict earliest V20 state with no checkpoint for recovery', async () => {
    const state = await prepareEarliestV20State(root);
    const owner = await acquireReviewPlannerV20ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      const journal =
        await openReviewPlannerV20ProductAcceptanceRecoveryJournal({
          repoRoot: root,
          environment: 'branch',
          owner: owner.owner,
        });
  ...
  it('fails closed for a bare reservation and a malformed checkpoint', async () => {
    const publicRoot = join(root, ...v20PublicSegments('branch'));
  ...
  it('fails closed for a V11 manifest injected into a V20 reservation', async () => {
    const state = await prepareEarliestV20State(root);
    const publicRoot = join(root, ...v20PublicSegments('branch'));
    await writeFile(
  ...
  it('seals exactly four V20 slots and leaves V11/V12/V13/V14/V15 root bytes untouched', async () => {
  ...
  it('seals recovery exactly once and rejects a second recovery owner', async () => {
    const state = await prepareFailedV20State(root);
    const recovery = await acquireReviewPlannerV20ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      await finalizeReviewPlannerV20ProductAcceptanceRecovery({
        repoRoot: root,
        environment: 'branch',
    ...
    const second = await acquireReviewPlannerV20ProductAcceptanceOwner({
      repoRoot: root,
      environment: 'branch',
      role: 'recovery',
    });
    ...
    try {
      ...
      ).rejects.toThrow('V20_PRODUCT_ACCEPTANCE_RECOVERY_NOT_AUTHORIZED');
    } finally {
      second.owner.close();
    }
    expect(state.attemptSha256).toHaveLength(64);
  ...
  it('fails closed when a failure terminal disagrees with its attempt-bound latest checkpoint', async () => {
    await prepareFailedV20State(root);
    await writeFile(
      join(root, ...v20PublicSegments('branch'), 'failure.json'),
      JSON.stringify({
        schemaVersion: 'phase-6.9.5-v20-product-acceptance-failure-v1',
        environment: 'branch',
...
async function prepareEarliestV20State(root: string) {
  const acquired = await acquireReviewPlannerV20ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
...
async function prepareFailedV20State(root: string) {
  const acquired = await acquireReviewPlannerV20ProductAcceptanceOwner({
    repoRoot: root,
    environment: 'branch',
    role: 'product',
  ...
  let journal:
    | Awaited<
        ReturnType<
          typeof prepareReviewPlannerV20ProductAcceptanceRecoveryJournal
        >
      >
    | undefined;
  try {
    ...
    journal = await prepareReviewPlannerV20ProductAcceptanceRecoveryJournal({
      repoRoot: root,
      environment: 'branch',
      owner: acquired.owner,
    });
    journal.appendCheckpoint({
      schemaVersion: 'phase-6.9.5-v20-product-acceptance-checkpoint-v1',
      component: 'review',
      slot: 'api',
      checkpoint: 'review_api_setup',
      providerCallState: 'not_started',
...
function v20PublicSegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_PROFILE.publicLedgerSegments(
    environment,
  );
}
...
function v20RecoverySegments(environment: 'branch' | 'main') {
  return REVIEW_PLANNER_V20_PRODUCT_ACCEPTANCE_PROFILE.recoverySegments(
    environment,
  );
}
