...
import * as subject from './review-planner-v18-product-acceptance-host';
...
function bridgeFactory(): BridgeFactory {
  const candidate = (
    subject as typeof subject & {
      createReviewPlannerV18BunPairedEvidenceAuthority?: unknown;
    }
  ).createReviewPlannerV18BunPairedEvidenceAuthority;
  if (typeof candidate !== 'function') {
    throw new Error('V18_BUN_AUTHORITY_BRIDGE_MISSING');
  }
  return candidate;
}

describe('Review Planner V18 Bun paired-evidence authority bridge', () => {
  beforeEach(() => {
    ...
    mockRunDefaultHostPreflight.mockImplementation(
      async (
        ...
        return Object.freeze({
          ...
          branchName: 'codex/v18-bun-authority-bridge',
  ...
  it('returns ready from the Node V18 host preflight when started in apps/server', async () => {
    const previousCwd = process.cwd();
    process.chdir(appsServerRoot);
    try {
      const host = subject.createDefaultReviewPlannerV18ProductAcceptanceHost(
        root,
        {
      ...
      if (preflight.status !== 'ready') {
        throw new Error('V18_BUN_AUTHORITY_PRELIGHT_BLOCKED');
      }
