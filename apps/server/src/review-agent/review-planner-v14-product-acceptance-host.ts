...
import {
  finalizeReviewPlannerV14ProductAcceptanceRecovery,
  readReviewPlannerV14ProductAcceptanceLedger,
  readReviewPlannerV14ProductAcceptanceExecutionManifest,
...
import type { ReviewPlannerV14ReadyPreflight } from './review-planner-v14-product-acceptance-composition';
import {
...
export type ReviewPlannerV14ProductAcceptanceHost = Readonly<{
  preflight(input: {
...
export type ReviewPlannerV14ProductAcceptanceRecoveryHost = Readonly<{
  preflight(input: {
    environment: 'branch' | 'main';
    repoRoot: string;
...
/**
 * Owns the real, V14-scoped host lifecycle. It deliberately reuses only the
 * existing V8 host mechanics (Docker activation, API/browser operations,
 * trace reads and exact cleanup); V14 owns all ledgers, confirmation and
 * recovery namespaces.
 */
export function createDefaultReviewPlannerV14ProductAcceptanceHost(
  repoRoot: string,
): ReviewPlannerV14ProductAcceptanceHost {
    ...
    root,
    {
        ...
        const result =
          await runDefaultReviewPlannerProductAcceptanceHostPreflight(input, {
            branchAcceptanceComplete: async (candidateRoot) => {
              const branch = await readReviewPlannerV14ProductAcceptanceLedger({
      ...
      resourcesFactory: () => {
        if (!executionManifest) {
          throw new Error('V14_PRODUCT_ACCEPTANCE_EXECUTION_MANIFEST_INVALID');
        }
...
function resourcesFromExecutionManifest(
  manifest: ReviewPlannerV14ProductAcceptanceExecutionManifest,
) {
  ...
  return Object.freeze({
    syntheticEmails: Object.freeze({
      ...
      probe: `v14-synthetic-probe-${manifest.attemptSha256.slice(0, 16)}@example.invalid`,
    }),
...
type ReviewPlannerV14RecoveryHostOptions = Readonly<{
  platform?: string;
  readLedger?: typeof readReviewPlannerV14ProductAcceptanceLedger;
  readExecutionManifest?: typeof readReviewPlannerV14ProductAcceptanceExecutionManifest;
  acquireOwner?: typeof acquireReviewPlannerV14ProductAcceptanceOwner;
  finalize?: typeof finalizeReviewPlannerV14ProductAcceptanceRecovery;
...
export function createDefaultReviewPlannerV14ProductAcceptanceRecoveryHost(
  repoRoot: string,
  options: ReviewPlannerV14RecoveryHostOptions = {},
): ReviewPlannerV14ProductAcceptanceRecoveryHost {
  const root = resolve(repoRoot);
  const readLedger =
    options.readLedger ?? readReviewPlannerV14ProductAcceptanceLedger;
  const readExecutionManifest =
    options.readExecutionManifest ??
    readReviewPlannerV14ProductAcceptanceExecutionManifest;
  const acquireOwner =
    options.acquireOwner ?? acquireReviewPlannerV14ProductAcceptanceOwner;
  const finalize =
    options.finalize ?? finalizeReviewPlannerV14ProductAcceptanceRecovery;
  ...
  const cleanupExact =
    ...
    ((
      manifest: ReviewPlannerV14ProductAcceptanceExecutionManifest,
      databaseUrl: string,
    ) => cleanupV14ExecutionResources(root, manifest, databaseUrl));
  let selected: ReviewPlannerV14ProductAcceptanceExecutionManifest | undefined;
...
async function cleanupV14ExecutionResources(
  repoRoot: string,
  manifest: ReviewPlannerV14ProductAcceptanceExecutionManifest,
  databaseUrl: string,
) {
  ...
  try {
    ...
    if (
      ...
      throw new Error('V14_PRODUCT_ACCEPTANCE_RECOVERY_REQUIRED');
    }
  } finally {
...
export function isReviewPlannerV14DatabaseFingerprintStable(
