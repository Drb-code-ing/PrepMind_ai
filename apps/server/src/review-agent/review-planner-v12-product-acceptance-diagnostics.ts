...
export type ReviewPlannerV12ProductAcceptanceFailure = Readonly<{
  schemaVersion: 'phase-6.9.5-v12-product-acceptance-failure-v1';
  environment: 'branch' | 'main';
  component: 'review' | 'planner';
  slot: 'api' | 'browser';
  checkpoint: ReviewPlannerV12ProductAcceptanceCheckpoint;
  terminal: 'operation_failed';
  providerCallState: 'not_started' | 'indeterminate';
}>;

export type ReviewPlannerV12ProductAcceptanceDiagnosticsPort = Readonly<{
  checkpoint(value: ReviewPlannerV12ProductAcceptanceCheckpoint): void;
  publishFailure(): void;
}>;

export function createReviewPlannerV12ProductAcceptanceDiagnosticsPort(input: {
  environment: 'branch' | 'main';
  journal: Pick<
  ...
  return Object.freeze({
    checkpoint(value) {
      if (
        !(
          REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS as readonly string[]
        ).includes(value)
      ) {
        throw new Error('V12_PRODUCT_ACCEPTANCE_DIAGNOSTIC_INVALID');
      }
...
function providerCallState(value: ReviewPlannerV12ProductAcceptanceCheckpoint) {
  const dispatch = `${value.split('_').slice(0, 2).join('_')}_dispatch`;
  return REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(value) >=
    REVIEW_PLANNER_V12_PRODUCT_ACCEPTANCE_CHECKPOINTS.indexOf(
      dispatch as ReviewPlannerV12ProductAcceptanceCheckpoint,
    )
    ? 'indeterminate'
    : 'not_started';
}
