...
import {
  parseReviewPlannerV16ProductAcceptanceArguments,
  runReviewPlannerV16ProductAcceptanceProductCli,
  serializeReviewPlannerV16ProductAcceptanceCliFailure,
} from './review-planner-v16-product-acceptance-cli';
import {
  REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE,
  REVIEW_PLANNER_V16_PRODUCT_ACCEPTANCE_PROFILE,
} from './review-planner-product-acceptance-profile';

describe('Review Planner V16 product-acceptance CLI', () => {
  const argv = [
    REVIEW_PLANNER_V16_PRODUCT_ACCEPTANCE_PROFILE.productConfirmation,
    '--environment=branch',
  ] as const;

  it('rejects an immutable V11 confirmation before V16 preflight or any external boundary', async () => {
    ...
    ).rejects.toThrow('V16_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');
  ...
  it('keeps a valid V16 invocation default-off at preflight before owner or runtime work', async () => {
    const ports = createBlockedPorts();
  ...
  it('projects a fake-ready V16 composition result instead of hard-coding default_off', async () => {
  ...
  it('keeps V11 confirmations out of the V16 parser and serializes a nonsecret rejection', () => {
    expect(() =>
      parseReviewPlannerV16ProductAcceptanceArguments(
        [
    ...
    ).toThrow('V16_PRODUCT_ACCEPTANCE_CONFIRMATION_REQUIRED');

    const value = serializeReviewPlannerV16ProductAcceptanceCliFailure(
      'product',
