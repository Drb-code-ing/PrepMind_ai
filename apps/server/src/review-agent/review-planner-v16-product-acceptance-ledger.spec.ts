import {
  ...
  parseReviewPlannerV16ProductAcceptanceAggregate,
  parseReviewPlannerV16ProductAcceptanceCleanup,
  parseReviewPlannerV16ProductAcceptanceDefaultOff,
  parseReviewPlannerV16ProductAcceptanceExecutionManifest,
  parseReviewPlannerV16ProductAcceptanceFailure,
  parseReviewPlannerV16ProductAcceptanceManifest,
  parseReviewPlannerV16ProductAcceptanceOwnerIsolation,
  parseReviewPlannerV16ProductAcceptanceSlotResult,
  parseReviewPlannerV16ProductAcceptanceSuccess,
} from './review-planner-v16-product-acceptance-ledger';
import * as reviewPlannerV16Ledger from './review-planner-v16-product-acceptance-ledger';
import { parseReviewPlannerV16ProductAcceptanceCheckpoint } from './review-planner-v16-product-acceptance-recovery';
import { REVIEW_PLANNER_V11_PRODUCT_ACCEPTANCE_PROFILE } from './review-planner-product-acceptance-profile';

describe('Review Planner V16 product-acceptance ledger contracts', () => {
  const attemptSha256 = 'a'.repeat(64);

  it('rejects V11 record identities from every V16 durable contract', () => {
    expect(() =>
      parseReviewPlannerV16ProductAcceptanceManifest({
        schemaVersion:
    ...
    expect(() =>
      parseReviewPlannerV16ProductAcceptanceCheckpoint({
    ...
    ).toThrow('V16_PRODUCT_ACCEPTANCE_RECOVERY_RECORD_INVALID');
  });

  it('accepts only the complete V16 four-slot durable success vocabulary', () => {
    const slot = parseReviewPlannerV16ProductAcceptanceSlotResult({
      schemaVersion: 'phase-6.9.5-v16-product-acceptance-slot-result-v1',
      slot: 'review-api',
      traceSha256: 'b'.repeat(64),
    });
    const defaultOff = parseReviewPlannerV16ProductAcceptanceDefaultOff({
    ...
    const isolation = parseReviewPlannerV16ProductAcceptanceOwnerIsolation({
    ...
    const cleanup = parseReviewPlannerV16ProductAcceptanceCleanup({
    ...
    const success = parseReviewPlannerV16ProductAcceptanceSuccess({
      schemaVersion: 'phase-6.9.5-v16-product-acceptance-success-v1',
      environment: 'branch',
      attemptSha256,
    });
  ...
  it.each(['https://api.deepseek.com', 'https://api.deepseek.com/v1'] as const)(
    'accepts the V16 durable default-off receipt with approved base URL %s',
    (baseUrl) => {
      const receipt = {
        schemaVersion: 'phase-6.9.5-v16-product-acceptance-default-off-v1',
        model: 'deepseek-v4-pro',
      ...
      expect(
        parseReviewPlannerV16ProductAcceptanceDefaultOff(receipt),
  ...
  ])('rejects a %s V16 durable default-off receipt base URL', (_, baseUrl) => {
    const receipt = {
      schemaVersion: 'phase-6.9.5-v16-product-acceptance-default-off-v1',
      model: 'deepseek-v4-pro',
      component: 'review',
  ...
  it('allows only Flash or Pro in the V16 default-off environment without relaxing its controls', () => {
    const assertDefaultOff = (
      reviewPlannerV16Ledger as typeof reviewPlannerV16Ledger & {
  ...
  it.each(['https://api.deepseek.com', 'https://api.deepseek.com/v1'])(
    ...
    (baseUrl) => {
      ...
      if (!assertDefaultOff) throw new Error('V16 validator unavailable');
      const entries = [
        'AI_PROVIDER_MODE=mock',
        'AI_ENABLE_LIVE_CALLS=false',
  ...
  it.each([
    ...
    'rejects a %s model from a V16 durable default-off receipt',
    (_, model) => {
      const receipt = {
        schemaVersion: 'phase-6.9.5-v16-product-acceptance-default-off-v1',
        baseUrl: 'https://api.deepseek.com',
  ...
  it('fails closed when a V16 durable record carries secret-like content', () => {
    expect(() =>
      parseReviewPlannerV16ProductAcceptanceSlotResult({
        schemaVersion: 'phase-6.9.5-v16-product-acceptance-slot-result-v1',
        slot: 'review-api',
  ...
  it('requires an attempt-bound hash of the selected database URL without persisting the URL', () => {
    ...
    expect(() =>
      parseReviewPlannerV16ProductAcceptanceExecutionManifest(record),
    ).toThrow('V16_PRODUCT_ACCEPTANCE_LEDGER_RECORD_INVALID');
    expect(
      parseReviewPlannerV16ProductAcceptanceExecutionManifest({
        ...record,
  ...
  it('generates distinct V16-only selectors that bind cleanup to the reserved attempt', () => {
    const execution = createReviewPlannerV16ProductAcceptanceExecutionManifest({
      environment: 'branch',
      attemptSha256,
      databaseUrlSha256: 'f'.repeat(64),
    });
    ...
    expect(execution.resources.accountId.review).toMatch(
      /^v16-synthetic-account-review-[a-f0-9]{32}$/,
    );
  ...
  it('accepts only the fixed V16 failure terminal vocabulary', () => {
    expect(
      parseReviewPlannerV16ProductAcceptanceFailure({
