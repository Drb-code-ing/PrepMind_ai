...
import {
  createDefaultReviewPlannerV12ProductAcceptanceRecoveryHost,
  isReviewPlannerV12DatabaseFingerprintStable,
} from './review-planner-v12-product-acceptance-host';

describe('Review Planner V12 recovery host', () => {
  const databaseUrl = 'postgresql://v12-recovery.example.invalid/prepmind';
  const databaseUrlSha256 = createHash('sha256')
    .update(databaseUrl)
    .digest('hex');
  const executionManifest = {
    schemaVersion:
      'phase-6.9.5-v12-product-acceptance-execution-manifest-v1' as const,
    environment: 'branch' as const,
    attemptSha256: 'a'.repeat(64),
    databaseUrlSha256,
    resources: {
      accountId: {
        review: `v12-synthetic-account-review-${'b'.repeat(32)}`,
        planner: `v12-synthetic-account-planner-${'c'.repeat(32)}`,
      },
      fixtureId: {
        review: `v12-synthetic-fixture-review-${'d'.repeat(32)}`,
        planner: `v12-synthetic-fixture-planner-${'e'.repeat(32)}`,
      },
      browser: {
        executablePath:
  ...
  it('takes the recovery owner, revalidates its attempt, and seals only after restore plus exact cleanup', async () => {
    ...
    const host = createDefaultReviewPlannerV12ProductAcceptanceRecoveryHost(
      'E:\\v12-recovery-host',
      {
        platform: 'win32',
        readLedger,
