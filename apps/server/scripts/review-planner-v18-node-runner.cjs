...
const entries = new Map([
  [
    ...
    resolve(__dirname, 'review-planner-v18-product-acceptance.ts'),
  ],
  [
    'review-planner-v18-product-acceptance-recovery.ts',
    resolve(__dirname, 'review-planner-v18-product-acceptance-recovery.ts'),
  ],
]);
...
function run() {
  try {
    ...
    if (realpathSync(process.cwd()) !== repositoryRoot) {
      throw new Error('V18_NODE_RUNNER_CWD_INVALID');
    }
    const approvedRoots = [
    ...
    const assertApprovedFile = (filename) => {
      if (!isApprovedFile(filename)) {
        throw new Error('V18_NODE_RUNNER_PATH_BLOCKED');
      }
    };
    const assertApprovedTypeScriptFile = (filename) => {
      if (extname(filename) !== '.ts') {
        throw new Error('V18_NODE_RUNNER_TYPESCRIPT_PATH_BLOCKED');
      }
