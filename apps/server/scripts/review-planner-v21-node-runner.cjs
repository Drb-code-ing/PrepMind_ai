...
const entries = new Map([
  [
    ...
    resolve(__dirname, 'review-planner-v21-product-acceptance.ts'),
  ],
  [
    'review-planner-v21-product-acceptance-recovery.ts',
    resolve(__dirname, 'review-planner-v21-product-acceptance-recovery.ts'),
  ],
  [
    'review-planner-v21-preflight.ts',
    resolve(__dirname, 'review-planner-v21-preflight.ts'),
  ],
]);
...
function run() {
  try {
    ...
    if (realpathSync(process.cwd()) !== repositoryRoot) {
      throw new Error('V21_NODE_RUNNER_CWD_INVALID');
    }
    const approvedRoots = [
    ...
    const assertApprovedFile = (filename) => {
      if (!isApprovedFile(filename)) {
        throw new Error('V21_NODE_RUNNER_PATH_BLOCKED');
      }
    };
    const assertApprovedTypeScriptFile = (filename) => {
      if (extname(filename) !== '.ts') {
        throw new Error('V21_NODE_RUNNER_TYPESCRIPT_PATH_BLOCKED');
      }
