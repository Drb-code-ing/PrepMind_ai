...
const entries = new Map([
  [
    'review-planner-v16-product-acceptance.ts',
    resolve(__dirname, 'review-planner-v16-product-acceptance.ts'),
  ],
  [
    'review-planner-v16-product-acceptance-recovery.ts',
    resolve(__dirname, 'review-planner-v16-product-acceptance-recovery.ts'),
  ],
]);
  ...
  try {
    ...
    const assertApprovedFile = (filename) => {
      if (!isApprovedFile(filename)) {
        throw new Error('V16_NODE_RUNNER_PATH_BLOCKED');
      }
    };
    const assertApprovedTypeScriptFile = (filename) => {
      if (extname(filename) !== '.ts') {
        throw new Error('V16_NODE_RUNNER_TYPESCRIPT_PATH_BLOCKED');
      }
