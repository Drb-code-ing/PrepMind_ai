...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | V5 R2 Tutor authority 已完成；阶段未完成 |
...
- WrongQuestionOrganizerAgent：默认 gate 关闭时，NestJS 产品继续使用本地确定性 policy，把错题组织到 `WrongQuestionSubjectGroup` / `WrongQuestionDeck` / `WrongQuestionDeckItem`；该组织层不替代 WrongQuestion / Card / ReviewLog / ReviewTask。Task 4 已实现受治理 candidate，Task 6 已把 API 迁入 owner-scoped `REPEATABLE READ + READ ONLY` snapshot、事务外双 fingerprint fence、owner advisory-lock 写事务内第三次 fence 和深冻结 model-free command；missing/cross-owner 统一 404，并发同主题、force 唯一 relation、用户 rename/move 权威和旧专题复用已通过真实 PostgreSQL E2E。Task 7 已接入 server-only default-off DeepSeek V4 Pro non-thinking runtime：single 最多一次 candidate，batch 最多 12 个低置信安全目标共享一次 `1/3500/800` 调用，其余走本地 command；模型仍只能返回受限 ordinal/enum/短标签。模型结果必须先落同一 runId 的 `command_pending` Trace，随后才可进入本地授权写入；command commit 失败会返回错误并尝试原子替换同 runId failed 终态，Trace store 同时失败则保留 pending，HTTP/provider abort 不落 admission Trace 或 command。V1 controlled-Live 的 Organizer semantic 为 `0.7`，低于固定门槛。V2 R1--R6 已完成 bounded diagnostics、association policy、robustness、独立 lineage、同题跨路由 PostgreSQL 收敛和未写题 batch 补偿；唯一 V2 R7 的 24 个 Organizer runtime 全部在结构化对象前 `fallback_runtime_error`，Organizer semantic 为 `0`、verified usage 为 `0`。V3 R5 的 Organizer 启动 `14/24`，第 14 次结构化结果命中本地 `subject_authority_violation` 并触发 breaker；Organizer semantic 为 `0.4376201923`。V4 R0--R5 已把 subject/deck/topic/evidence/confidence 收敛为单一决策矩阵，并覆盖 authority drift、question/deck reorder、locked name、ordinal/schema failure、abort/预算/no-retry 与独立 durability lineage；唯一 R6 run 启动 6 个 Organizer runtime，其中 5 个 usage verified、同 breaker pair 的 1 个 sibling aborted，后续 18 个未启动，因此仍未通过产品 API/浏览器验收前置门。同步路径不宣称跨多实例 provider exactly-once，生产 gate 默认关闭，Task 1 baseline `0.278125` 保持不变。
...
- `/api/chat` 已接入 RouterAgent、TutorAgent 与 KnowledgeVerifierAgent；Phase 6.9.4.4 的 Router/Verifier 混合路径已经完成生产验收并恢复默认 gate 关闭，Phase 6.9.7 Task 5 又完成 Tutor candidate 的 default-off Web composition 与安全 Trace。V1--V4 四条真实模型质量运行均未达门槛，因此 Tutor/Organizer 产品 gate 仍关闭，未进行 Docker/API/浏览器可用性验收。V5 R0--R2 只完成零 Provider 根因、独立数据/评测权威、baseline 与新 Tutor local-signal bounded candidate；R2 candidate 尚未接入该 Web composition，不构成新的模型或产品验收。模型只负责受限语义判断，权限、安全、canonical route、RAG 放行、预算与写业务数据仍由本地代码掌握。
...
2. 当前执行 Phase 6.9.7：V1--V4 Live 均已分别以 `quality_gate_failed` 封存且不得重跑；V5 R0--R2 已完成零 Provider 根因、独立 V2 dataset/coherence、冻结 policy/baseline 与 Tutor local-signal bounded candidate。fixture 缺陷不推翻 V4 failure，V4 前 5 对的中文 Tutor 与 Organizer 语义偏差仍是真实 evidence。
3. 下一任务仅 V5 R3：建立 WrongQuestionOrganizer owner-scoped ordinal shortlist、fingerprint 与 reorder/ABA/stale fail-closed。产品验收、Task 13/main 与 Phase 6.9.8 暂不得开始；全部 Agent 完成后才进入 Phase 6.10 分层记忆，并分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客。
