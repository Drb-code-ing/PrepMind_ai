...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | V5 R0 根因已完成；阶段未完成 |
...
- Phase 6.9.7 V5 R0 根因：V1 `tutor-runtime-06` 的中文代数 latest text 被配到英文微积分 active context，language tag 还被数组奇偶误标为 `en`。零网络差分回归同时证明合法 `step_check + submitted_step` 会在产品 candidate 应用，缺 primary/错误 evidence 才会被产品 candidate 拒绝，V4 diagnostic 只是如实投影。V5 不改写 V1；后续新建显式 language/exercise-family/coherent-context 的 V2 dataset，Tutor 改为本地 evidence authority + 模型有界 intent/depth 选择，Organizer 改为本地 topic shortlist + ordinal-only 选择。
...
- `/api/chat` 已接入 RouterAgent、TutorAgent 与 KnowledgeVerifierAgent；Phase 6.9.4.4 的 Router/Verifier 混合路径已经完成生产验收并恢复默认 gate 关闭，Phase 6.9.7 Task 5 又完成 Tutor candidate 的 default-off Web composition 与安全 Trace。V1--V4 四条真实模型质量运行均未达门槛，因此 Tutor/Organizer 产品 gate 仍关闭，未进行 Docker/API/浏览器可用性验收。V5 R0 只完成零 Provider 根因与修复设计，不构成新的模型或产品验收。模型只负责受限语义判断，权限、安全、canonical route、RAG 放行、预算与写业务数据仍由本地代码掌握。
...
2. 当前执行 Phase 6.9.7：V1--V4 Live 均已分别以 `quality_gate_failed` 封存且不得重跑；V5 R0 已完成零 Provider 根因与独立修复设计。fixture 缺陷不推翻 V4 failure，V4 前 5 对的中文 Tutor 与 Organizer 语义偏差仍是真实 evidence。
3. 下一任务仅 V5 R1：建立与 V1--V4 双向隔离的 V2 dataset/coherence validator，并冻结新 SHA/baseline/quality gate。产品验收、Task 13/main 与 Phase 6.9.8 暂不得开始；全部 Agent 完成后才进入 Phase 6.10 分层记忆，并分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客。
