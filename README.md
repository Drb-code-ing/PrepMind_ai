...
Phase 6.9.5 和 Phase 6.9.6 均已完成。Phase 6.9.7 Task 0--11 已完成，但 V1 与 V2 两条唯一 controlled-Live 均以 `quality_gate_failed` 封存且不得重跑。V1 run `39a62241...` 为 `24/24` zero-call、`27/48` strict runtime、Tutor/Organizer semantic `0.3485119048/0.7`。V2 R0--R6 已完成独立 runner/CLI/validator/evidence lineage、held-out/metamorphic/authority/leakage、marker/evidence 并发故障恢复、Chat abort、Organizer 失败终态及同题跨路由 PostgreSQL 收敛；fresh Mock 为 `24/24` zero-call、`48/48` strict runtime、semantic `1/1`。随后唯一 V2 R7 run `67ce18dd-e2ed-4a05-8507-2a98898b8ede` 保持 `24/24` zero-call，但 48 个 runtime 全部在结构化对象前进入 `fallback_runtime_error`，最终 `0/48` strict runtime、semantic `0/0`、verified usage `0`、`quality_gate_failed`。证据未保存原始异常，不能武断归因于 credential、网络、模型、endpoint 或 prompt。V3 R0 已完成零 Provider 设计；V3 R1 又完成固定 failure/stage 投影、真实 0/1 invocation recorder、outer-harness local failure、V1/V2 absent-field 兼容和 config/factory/request/response/schema/abort 零网络 compatibility harness。首个 runtime contract failure breaker、固定 48 分母和双 lane ledger 仍属于下一步 R2。

V2 marker/evidence 已封存；没有进入 R8 Docker/API/可见浏览器，生产 gate 的 tracked defaults 继续关闭。下一步是 V3 R2 strict-gate breaker、双 lane ledger 与固定分母，不是删除 marker 重跑 V2、调用真实模型、开始 Task 13/main 合并或进入 Phase 6.10。
...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | Task 0--11、V3 R0/R1 完成；V1/V2 Live 失败封存；下一步 V3 R2 breaker/ledger |
...
2. 当前执行 Phase 6.9.7：Task 0--11、V2 R0--R6 与 V3 R0/R1 均已完成；V1 run `39a62241...` 与 V2 run `67ce18dd...` 都已分别以 `quality_gate_failed` 封存。V2 保持 `24/24` guard zero-call，但为 `0/48` strict runtime、semantic `0/0`、verified usage `0`、critical `1`；48 个 runtime 都在结构化对象前回退，不能据此证明语义、价格或产品可用。V2 evidence/marker SHA 已记录并通过专用 validator，一次性名额已消费，未进入 R8 Docker/API/browser。
   V3 R1 已把受信 category/stage、安全 execution evidence、真实 invocation recorder 和零网络 adapter matrix 落为源码，同时保持 V1/V2 report 字段 absent。下一步只做 R2 breaker/双 lane ledger/固定分母；R1--R4 完成前不申请新 Live。这不授权任何网络调用。
...
V3 R1 已复核 V1/V2 四个历史 SHA、专用 validator 与 V3 Live artifact=0。禁止删除 marker 或重跑 V1/V2；任何未来网络运行都必须先完成 V3 R1--R4、使用独立 marker/journal/evidence，并取得新的精确授权。
