...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V1--V10 保持只读历史；V10 是唯一语义质量 authority。V11 已 recovery-only 封存。V12 已完成 default-off host、attempt/checkpoint 绑定、一次性 recovery、DB 指纹、owner 后重验、半激活恢复和 30 秒 headed-browser 边界的离线工程收口；未运行 V12 Docker/headed-browser/API/provider，独立 eval gate 和两条业务 gate 仍默认关闭，产品仍是 deterministic，不能称为 Phase 6.9.5 完成。详见 [V12 offline checkpoint](./docs/acceptance/phase-6-9-5-review-planner-v12-offline-checkpoint.md)。
...
1. Phase 6.9.5 的 V1--V11 保持只读历史；V10 是唯一语义质量 authority，V11 已 recovery-only 封存。V12 的产品 gate 仍默认关闭，阶段仍未完成。
2. V12 全量离线门禁与独立 contract/operations review 已无未关闭 P0/P1；只有用户再次单独授权后，才可执行唯一一次分支 Docker/headed-browser 产品验收。产品 gate 必须逐组件临时开启后恢复 `false`，并精确清理本轮合成数据；只有分支验收、main replay 和证据复核通过后，才可合并和 push。
...
下一会话可以复制：“请继续 Phase 6.9.5 V12：复核离线 checkpoint、attempt-bound recovery、DB 指纹、owner 后重验、半激活 default-off 和 30 秒 headed browser 边界。两项 review 已无未关闭 P0/P1；未获我的单独授权前，不要运行 V12 product/recovery CLI、Docker 或真实模型。”
