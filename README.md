...
Phase 6.9.5 和 Phase 6.9.6 均已完成。Phase 6.9.7 Task 0--11 已完成，但 V1、V2、V3、V4 与 V5 五条 controlled-Live 均以 `quality_gate_failed` 独立封存且不得重跑。唯一 V5 R6 run `aa637d3a-f7c4-4549-a724-9cdbefdd89c8` 使用 `deepseek_network`，保持 `24/24` guard zero-call、12 次 Provider 调用与 `11/48` strict runtime；第 6 对 Tutor 在 `3021ms > 3000ms` 后熔断，正式聚合均为 `null`。V6 R0--R4 已完成且均为 zero-provider：已落地 source contracts、package 级 bounded candidates、独立 runner/CLI/durable lineage 与 reviewed Mock checkpoint；没有调用 Provider 或接入产品路径。

V1--V5 marker/evidence 均已封存；V3--V5 journal 完整记录 dispatch、terminal、breaker、run completed 与 evidence sealed。V5 R6 因运行不完整，正式 semantic、P95、token 与总费用聚合全部为 `null`；11 条已验证 runtime 的 `9761/902 tokens`、`0.034695 CNY` 只能作为复盘 subtotal。V6 candidate 让 Tutor 模型只返回 eligible `intentIndex`，让 Organizer 模型只返回 actual shortlist fingerprint 与 subject/deck/topic ordinal；depth、策略、confidence、真实 ID、locked name 与写权限全部在本地重建。R3 又冻结 `72/24/48/24/32` 分母、guard-first、双 lane、首错 breaker、deadline、complete-only aggregate 与 V1--V5 双向 lineage。R4 fresh baseline 保持 `12/48`，fresh Mock 为 `24/24` zero-call、`48/48` strict runtime、semantic/model-owned `1/1/1`；Mock evidence 已精确删除，V6 Live artifact 保持 0。Mock 满分只证明工程合同，不证明 DeepSeek 语义、网络 P95、Provider 账单或产品可用性。下一步仅 V6 R5 controlled-Live，当前未授权；产品 Docker/API/浏览器、Task 13/main、Phase 6.10、Phase 8/9 与博客收尾均不得开始。
...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | V6 R4 已完成；阶段未完成 |
...
- Agent 成本边界：生产 Agent 的真实模型调用必须同时经过 server-only composition、全局 Live 双开关、组件独立 gate、不可变预算、超时、零自动重试、安全 eligibility 与脱敏 Trace；组件 gate 默认关闭并可独立回滚。Knowledge 两个 candidate 共享 `2 calls / 6000 input / 1200 output`，单请求硬 cap 为 `0.03 CNY`，并使用只注入 API server 的独立 `KNOWLEDGE_AGENT_DEEPSEEK_API_KEY`。Tutor 使用独立 `1/1200/300`、`0.006 CNY` 与仅注入 Web server runtime 的 `TUTOR_AGENT_DEEPSEEK_API_KEY`；WrongQuestionOrganizer 使用独立 `1/3500/800`、`0.016 CNY` 与只由 API server 读取的 `WRONG_QUESTION_ORGANIZER_AGENT_DEEPSEEK_API_KEY`，worker 强制关闭。它们不借用通用或其它 Agent 凭据，也不消耗 Router -> Verifier 共享预算。V1--V5 使用独立确认词、授权变量、marker、evidence prefix、runner identity 与 validator；V3--V5 还拥有 dispatch-before-call hash-chain journal、recovery claim、crash-only seal 与 hard-link evidence。V6 R3 已建立同等级的独立 CLI/artifact lineage，R4 已新增正式 reviewed Mock factory/checkpoint；V6 Live artifact 仍为 0。V5 R6 的 11 条 verified usage subtotal 为 `9761/902 tokens`、`0.034695 CNY`，但 timeout 使整轮 token/费用/P95/semantic 聚合保持 `null`。任何 Mock 满分、局部成功或历史 run 都不能拼接为质量通过。`tutorOrchestrationP95Ms` 只测本地 Tutor strategy + candidate，不是 Router/API/最终流式 Chat 产品 P95。Mock/CLI 只证明工程 contract，controlled-Live 才能证明语义质量，Docker/API/可见浏览器才证明产品链路。当前结果见 [V5 R6 failure seal](./docs/acceptance/2026-07-27-phase-6-9-7-tutor-organizer-v5-controlled-live-failure.md)、[V6 R3 runner/lineage](./docs/acceptance/2026-07-27-phase-6-9-7-tutor-organizer-v6-r3-runner-lineage.md) 与 [V6 R4 static/Mock checkpoint](./docs/acceptance/2026-07-27-phase-6-9-7-tutor-organizer-v6-r4-static-mock.md)。
...
2. 当前执行 Phase 6.9.7：V1--V5 Live 均已分别以 `quality_gate_failed` 封存且不得重跑。唯一 V5 R6 为 `24/24` guard zero-call、12 次 Provider invocation、`11/48` strict runtime，第 6 对 Tutor `3021ms > 3000ms` timeout 后熔断；正式聚合均为 `null`，V5 marker/journal/evidence 已 durable seal。
3. V6 R0--R4 已完成且均为 zero-provider；R4 reviewed Mock checkpoint 已通过，Mock 满分只证明工程合同。下一任务仅 V6 R5 controlled-Live，当前未授权。安全、权限、固定分母、无重试和历史证据不可变性不放宽；新的精确 Live 授权前不得读取 credential、创建 Live marker 或调用 Provider，产品验收、Task 13/main 与 Phase 6.9.8 暂不得开始。全部 Agent 完成后才进入 Phase 6.10 分层记忆，并分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客。
...
V5 R6 evidence/journal/marker 已按 run `aa637d3a...` 封存；V1--V4 evidence SHA 与四版 validator 仍不变。禁止删除任何 V1--V5 marker、重跑已消费 Live，或把 Mock/部分成功拼接成通过。
