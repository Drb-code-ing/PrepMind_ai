...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已完成前端 MVP、后端鉴权、业务数据服务端迁移、结构化 OCR、AI 讲题上下文、FSRS 复习闭环、学习统计和 ReviewTask 持久化任务流。下一步进入 Phase 4.4：离线评分队列与提醒策略。
...
- Dexie 负责本地快速恢复、离线兜底、乐观更新和旧图片预览；复习评分暂不进入通用 mutation queue。
...
3. Phase 5：RAG 知识库与 pgvector 检索。
