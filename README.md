...
The v10 zero-provider implementation now uses a new tag, authorization strings, and durability namespace. CLI v2 strictly validates the shared preflight result and exposes only bounded code/mode/count/listener diagnostics; proxy values and malformed fields are never reflected. Focused `128/128`, Agent full `1658/1658`, typecheck, lint, formatting, and diff checks pass. Merge/tag/fresh-authorization gates remain pending, so this is not Live or product authority.
