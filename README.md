...
L3 后的 Full-gate Schema Recovery SR0/SR1/SR2 已分别完成 zero-provider 设计、TDD 与 robustness checkpoint。SR1 新增 exact-schema raw-content parser capability、有界 native JSON parser、canonical integer `intentIndex` selection projection、strict projected decision 与 bounded no-raw diagnostic；contract SHA 固定为 `e2453fae...11579`。SR2 又冻结独立 Provider-like/held-out/metamorphic/schema-negative fixture（SHA `43248bfa...0d41e`），以 prompt-only anti-oracle responder 覆盖全部 24 个 Tutor runtime（含 `tutor-v2-runtime-11`）、18 个 Provider shape、Unicode/结构上限、transport/HTTP/response-audit/usage、budget/abort 以及 pair sibling/breaker。SR2 focused `9/9`、Agent `1144/1144`、AI `325/325` 通过；它没有读取 credential、调用 Provider、执行正式 Mock/Live、启动 Docker/API/browser、创建正式 artifact/tag 或修改业务数据，旧 L3 bytes/tag/validator 保持不变。该 checkpoint 不产生 semantic/P95/产品 authority；当前下一原子任务仅 SR3 zero-provider Runner、Lineage 与 Durability。
