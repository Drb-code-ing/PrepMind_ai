...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 当前是确定性只读 baseline；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。Phase 6.9.5 必须升级为 deterministic facts/constraints + 真实模型诊断与计划的混合路径。
...
- `/api/chat` 已接入 RouterAgent、TutorAgent policy 和 KnowledgeVerifierAgent；Phase 6.9.4.4 正在加入受控 Router/Verifier 真实模型候选。模型负责语义判断，权限、安全、canonical route、RAG 放行、预算与写业务数据仍由本地代码掌握。
