...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已完成前端 MVP、后端鉴权、业务数据服务端迁移、结构化 OCR、AI 讲题上下文、FSRS 复习闭环、学习统计、ReviewTask 持久化任务流、离线评分队列、复习计划预览、复习容量偏好和加权压力模型；Phase 5 RAG 知识库已进入实现阶段，当前完成数据模型、pgvector 索引预留、shared contract、文档上传与状态 API、文档解析分块与 embedding 入库，下一步进入检索 API。
...
- RAG 基础：Phase 5.3 已补齐 `Document` / `Chunk` 元数据字段、`vector(1536)` 索引预留、`@repo/types` knowledge API contract、`/knowledge/documents` 上传/列表/详情/删除 API，以及 `POST /knowledge/documents/:id/process` 文档处理 API；支持 TXT / Markdown / DOCX / PDF 基础文本解析、`@repo/rag` 段落感知分块、OpenAI-compatible `text-embedding-3-small` 默认 embedding provider 和测试/e2e fake provider；当前尚未实现检索 API、Chat RAG 注入、citations 和 `/knowledge` 前端页面。
...
- RAG 知识库已完成 Phase 5.3 文档处理地基：`Document` / `Chunk` 以 PostgreSQL + pgvector 为权威来源，`Chunk.embedding` 固定为 `vector(1536)` 并通过 raw SQL 持久化；上传资料原文件进入 MinIO，`Document` 状态流为 `PENDING -> PROCESSING -> DONE / FAILED`，空文本、零 chunk、解析或 embedding 失败进入 `FAILED`；处理写入前校验 document/user ownership，forced reprocess 会先清旧 chunks，避免 stale retrieval；Chat 仍需在未上传资料、未命中或检索失败时继续普通 AI 回答。
