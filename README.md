...
L3 后的 Full-gate Schema Recovery SR0/SR1 已分别以 `zero_provider_full_gate_schema_recovery_design` 与 `zero_provider_full_gate_schema_recovery_tdd` 完成。SR1 新增 exact-schema raw-content parser capability、有界 native JSON parser、canonical integer `intentIndex` selection projection、strict projected decision 与 bounded no-raw diagnostic；contract SHA 固定为 `e2453fae...11579`。额外字段只形成固定 enum/bucket/shape fingerprint 后丢弃，缺失、alias、字符串/浮点/null/越界、重复 key、wrapper/fence/prose/BOM/trailing data 仍 fail-closed，且不 coercion/default/clamp/retry。Schema Recovery candidate 最多调用一次 runtime，继续复用 Tutor V6 的 local signal、preferred depth、answer authority 与 merger。SR1 未读取 credential、调用 Provider、执行正式 Mock/Live、启动 Docker/API/browser、创建正式 artifact/tag 或修改业务数据；旧 L3 bytes/tag/validator 保持不变。当前下一原子任务仅 SR2 Provider-like/held-out/metamorphic/no-leak robustness。
