...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V10 是唯一语义质量 authority；V11--V18 是不可复用的封存历史。V19 已完成 default-off 离线收口，保留 repository-root CWD、只读 Bun V10-authority bridge、attempt/recovery/receipt 边界，并在产品命令前增加只读 Node preflight 来证明真实 host path；独立 eval gate 和两条业务 gate 仍默认关闭，产品仍是 deterministic，不能称为 Phase 6.9.5 完成。详见 [V18 closure / V19 plan](./docs/acceptance/phase-6-9-5-review-planner-v18-closure-v19-plan.md)。
