...
Phase 6.9.5 和 Phase 6.9.6 均已完成。Phase 6.9.7 Task 0--11 已完成，但 V1--V6 六条 controlled-Live 均以 `quality_gate_failed` 独立封存且不得重跑。唯一 V6 R5 run `b18a0a13-a2a0-4cb0-8f9c-296271c0dfa8` 使用 `deepseek_network`，`24/24` guard zero-call 通过；首个 Tutor runtime 在约 21ms 内得到 `provider_runtime / unknown`，Organizer sibling aborted，最终只有 2 次 Provider invocation 与 `0/48` strict runtime，正式聚合全部为 `null`。V7 R0/R1/R2/R3 已完成零 Provider 根因复盘、第一方 V4 Pro direct adapter、wire diagnostics、独立 runner/lineage/durable evidence、完整 fault matrix 与 reviewed static/Mock checkpoint；下一原子任务仅 R4 精确授权门，当前没有新的模型调用授权。

V1--V6 marker/evidence 均已封存；V3--V6 journal 完整记录 dispatch、terminal、breaker、run completed 与 evidence sealed。V6 candidate 让 Tutor 模型只返回 eligible `intentIndex`，让 Organizer 模型只返回 actual shortlist fingerprint 与 subject/deck/topic ordinal；depth、策略、confidence、真实 ID、locked name 与写权限全部在本地重建。R3 冻结 `72/24/48/24/32` 分母、guard-first、双 lane、首错 breaker、deadline、complete-only aggregate 与 V1--V5 双向 lineage；R4 Mock 为 `24/24` zero-call、`48/48` strict runtime、semantic/model-owned `1/1/1`，但只证明工程合同。R5 evidence/marker/journal 已 seal 且 validator `ok=true`；`unknown` 的脱敏证据不能唯一归因 credential、HTTP、网络、SDK 或 Provider 响应。V7 保持 V2 dataset 和 V6 candidate/authority bytes 不变；R1 已实现 direct transport 的 8-stage capability、executor/dispatch/response/usage 分离计数和安全 failure 投影，R2 已把它接入独立 runner、一次性 marker、hash-chain journal、hard-link evidence、crash-only recovery 与 V1--V6 双向 lineage。R3 reviewed Mock 又让全部 48 runtime 穿过真实 V6 candidate/prompt/schema/merger 与第一方 adapter，得到 `24/24` guard、`48/48` strict、semantic/model-owned `1/1/1` 和四类 `48` wire counters；它仍是 `mock_quality_not_evidence`，没有创建 V7 Live artifact 或产品接线。
...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | V7 R3 已完成；阶段未完成 |
...
- Agent 成本边界：生产 Agent 的真实模型调用必须同时经过 server-only composition、全局 Live 双开关、组件独立 gate、不可变预算、超时、零自动重试、安全 eligibility 与脱敏 Trace；组件 gate 默认关闭并可独立回滚。Knowledge 两个 candidate 共享 `2 calls / 6000 input / 1200 output`，单请求硬 cap 为 `0.03 CNY`，并使用只注入 API server 的独立 `KNOWLEDGE_AGENT_DEEPSEEK_API_KEY`。Tutor 使用独立 `1/1200/300`、`0.006 CNY` 与仅注入 Web server runtime 的 `TUTOR_AGENT_DEEPSEEK_API_KEY`；WrongQuestionOrganizer 使用独立 `1/3500/800`、`0.016 CNY` 与只由 API server 读取的 `WRONG_QUESTION_ORGANIZER_AGENT_DEEPSEEK_API_KEY`，worker 强制关闭。它们不借用通用或其它 Agent 凭据，也不消耗 Router -> Verifier 共享预算。V1--V6 使用独立确认词、授权变量、marker、evidence prefix、runner identity 与 validator；V3--V6 还拥有 dispatch-before-call hash-chain journal、recovery claim、crash-only seal 与 hard-link evidence。V6 R5 只有 2 次 Provider invocation、0 条 verified runtime usage，故 token/费用/P95/semantic 全部为 `null`，不能推导零成本或账单。任何 Mock 满分、局部成功或历史 run 都不能拼接为质量通过。`tutorOrchestrationP95Ms` 只测本地 Tutor strategy + candidate，不是 Router/API/最终流式 Chat 产品 P95。当前结果见 [V6 R5 failure seal](./docs/acceptance/2026-07-28-phase-6-9-7-tutor-organizer-v6-controlled-live-failure.md)。
...
- Phase 6.9.7 V7 R3：新增 reviewed Mock factory 与完整 zero-network fault matrix。48 条 runtime 从冻结 V2 dataset 派生，全部穿过真实 V6 Tutor/Organizer candidate、projection、prompt、strict schema、本地 merger 与第一方 direct adapter；synthetic responder 只读实际 bounded prompt，不读取 expected/oracle。Fresh baseline 保持 `12/48`，Mock run `e09baa4a...` 为 `24/24` guard、`48/48` strict、semantic/model-owned `1/1/1`、wire `48/48/48/48`，gate `mock_quality_not_evidence`。全量静态、PostgreSQL `12/12`、Compose default-off、历史 validators/SHA、V7 artifact=0 与独立终审通过；全程 zero-provider，下一步仅 R4 精确授权门。
...
2. 当前执行 Phase 6.9.7：V1--V6 Live 均已分别以 `quality_gate_failed` 封存且不得重跑。V7 R0/R1/R2/R3 已完成零 Provider transport-remediation 设计、第一方 direct adapter、8-stage wire diagnostics、独立 runner/CLI/lineage/durability、fault matrix 与 reviewed static/Mock；没有改 Agent 语义或调用模型。
3. 下一原子任务仅 V7 R4 精确授权门。Provider、产品 Docker/API/浏览器、R5--R6、Task 13/main 与 Phase 6.9.8 均未开始；普通“继续”不构成 R4 Live 授权。全部 Agent 完成后才进入 Phase 6.10 分层记忆，并分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客。

回顾时可以问：“TutorAgent 为什么不是最终回答模型？”“为什么明确教学指令和高置信错题字段保持 zero-call？”“为什么 Organizer 模型只能返回 ordinal，而不能直接写 deck？”“为什么 executor、dispatch、response、verified usage 要拆成四个计数？”“为什么 recovery 只能 seal durable prefix，不能 resume/replay/retry？”“文件 fsync、父目录 fsync、跨主机 lease 与 Provider exactly-once 有什么区别？”“为什么 synthetic provenance 永远不能通过生产 gate？”“为什么 held-out/metamorphic 满分仍不能替代 controlled-Live？”

V6 R5 evidence/journal/marker 已按 run `b18a0a13...` 封存；V1--V5 evidence SHA 与 validators 仍不变。禁止删除任何 V1--V6 marker、重跑已消费 Live，或把 Mock/部分成功拼接成通过。
