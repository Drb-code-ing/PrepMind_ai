...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已完成前端 MVP、后端鉴权、业务数据服务端迁移、本地缓存工程化和产品体验补全，下一步进入 Phase 3：AI 讲题系统。
...
| Phase 3 | AI 讲题系统：structured output、prompt、tool calling | 下一步 |
...
- 拍照识题：图片上传、OCR 流式识别、有效题目上下文注入，支持围绕当前题目继续追问。
...
- 离线补偿：WrongQuestion / OCRRecord 写操作失败时进入 Dexie mutation queue，网络恢复后自动补偿同步。
...
1. 设计 OCR structured output schema。
2. 标准化 AI 讲题 prompt 和题目字段。
3. 规划 `createWrongQuestion`、`searchKnowledge`、`createReviewTask` tool calling 边界。
