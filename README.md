...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已完成前端 MVP、后端鉴权、业务数据服务端迁移、结构化 OCR、AI 讲题上下文、FSRS 复习闭环、学习统计、ReviewTask 持久化任务流、离线评分队列、复习计划预览、复习容量偏好和加权压力模型；Phase 5 RAG 知识库已进入实现阶段，当前完成数据模型、pgvector 索引预留、shared contract、文档上传与状态 API，下一步进入解析、分块与 embedding 入库。
...
- RAG 基础：Phase 5.2 已补齐 `Document` / `Chunk` 元数据字段、`vector(1536)` 索引预留、`@repo/types` knowledge API contract，以及 `/knowledge/documents` 上传、列表、详情和删除 API；当前尚未实现解析、embedding、检索和 Chat 注入。
...
- RAG 知识库已完成 Phase 5.2 后端地基：`Document` / `Chunk` 以 PostgreSQL + pgvector 为权威来源，`Chunk.embedding` 固定为 `vector(1536)`；上传资料原文件进入 MinIO，`Document(PENDING, sourceType=UPLOAD)` 进入 PostgreSQL；Chat 命中资料时增强回答，未上传资料、未命中或检索失败时继续普通 AI 回答。
