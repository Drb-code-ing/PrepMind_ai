...
- 会话记忆地基：Phase 6.9.3.1 新增 strict prepare request/response/public state contract，以及单会话单行的 `ConversationSummary` / `ConversationState` Prisma 模型。复合外键保证 child `userId` 与会话 owner 一致，TTL 以 `expiresAt > updatedAt` 为准，summary/source hash 有 DB CHECK。当前仅完成 contract/database，尚无 prepare API、Redis、摘要生成或 Chat 注入。
...
2. Phase 6.9.3.1 已完成 contract/database；下一任务是 Phase 6.9.3.2 ConversationState、Redis 缓存与 prepare API。Phase 6.9 完成后进入 Phase 8 性能/PWA，
