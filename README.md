...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V1--V9 保持只读历史；V9 唯一 controlled-Live 已以 `quality_gate_failed` 封存（`23` provider / `22` paired，quality `30/48`、semantic `4/22`、critical `2`），无 success seal。V10 已完成离线门与最小语义 contract 修复，模型只可选择 `focusIndexes` / `blockOrder`；V10 evidence、once marker 与 success seal 仍不存在，产品仍 deterministic。独立 eval gate 和两条业务 gate 当前均默认关闭，不能称为真实模型可用或 Phase 6.9.5 已完成；精确 preflight 见 [V10 offline checkpoint](./docs/acceptance/phase-6-9-5-review-planner-v10-offline-checkpoint.md)。
...
1. Phase 6.9.5 的 V1--V9 保持只读历史；V9 唯一 Live 已质量门失败并封存，V9 committed success 不成立。V10 的最小修复和离线门已完成，但其 evidence/once/success seal 尚不存在，产品 gate 默认关闭，阶段仍未完成。
2. 下一步是独立复审后仅执行一次 V10 controlled-Live：根 `.env` 仅用于命令注入，进程内只开启 V10 eval gate，并显式关闭 V8/V9 eval 和 Review/Planner 产品 gate。未取得 V10 committed success 前，不得进入 Docker、浏览器、产品验收、main replay 或 push。
3. 全部 Agent 完成后进入 Phase 6.10 分层记忆。未来分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客，具体题目与结构由用户届时确认。

回顾时可以问：“为什么 V9 offline tests 不能代表 Live quality？”“V10 为什么只让模型返回 `focusIndexes` / `blockOrder`？”“为什么模型不能决定权限和写操作？”

下一会话可以复制：“请从 Phase 6.9.5 V10 offline checkpoint 开始独立复审；保持 Review/Planner 产品 gate 关闭，不改写 V1--V9、`.env` 或提前进入产品验收。”
