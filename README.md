...
| Phase 6.9.4.3 | 同 case paired eval；Attempt C=`structured_output`，待 headroom 修复             | 验收未完成 |
...
- Router / Verifier paired eval：Phase 6.9.4.3 已在固定 60/40 数据集上完成 deterministic、Mock 与三次受控 Live 尝试，但阶段验收未完成。2026-07-14 canonical Attempt C 在首个 eligible case 以 `PROVIDER_ERROR / structured_output` fail-closed；历史成功 Router entry 为 `61/120` 与 `108/120` output tokens，支持当前输出余量不足的高置信假设。Router / Verifier 仍为 `enabled=false / usage_unverifiable`，生产继续 deterministic；下一步先按 TDD 同步两个 candidate 的 400-token headroom 与全局成本合同，之后才重新执行完整 Live。完整证据见 [Phase 6.9.4.3 验收记录](./docs/acceptance/phase-6-9-4-3-router-verifier-paired-eval.md)。
...
1. Phase 6.9.4.3 的工程与 Mock 已完成，但 canonical Attempt C 为 `incomplete / structured_output`，阶段验收未完成；下一任务先修 structured-output token headroom，不能直接重跑或进入 enablement。
2. 从最新 main 按 TDD 把 Router/Verifier 单次输出上限统一提高到 400，同步 local/provider/global output cap、pricing 与 strict evidence contract；不增加调用次数、不自动重试、不放宽 schema。
3. 修复合并后再从新的 main 执行完整 controlled-Live。新 run 达到 28 次 strict success，且质量、安全、延迟、成本门槛全部通过后，才能标记 Phase 6.9.4.3 完成并继续 Phase 6.9 其余 Agent/记忆补强；Phase 6.9 完成后进入 Phase 8 性能/PWA，再进入 Phase 9 MCP Tool 体系。
