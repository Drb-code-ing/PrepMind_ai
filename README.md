...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已完成前端 MVP、后端鉴权、业务数据服务端迁移、结构化 OCR、AI 讲题上下文、FSRS 复习闭环、学习统计、ReviewTask 持久化任务流、离线评分队列、复习计划预览、复习容量偏好和加权压力模型；Phase 5 RAG 知识库已完成数据模型、pgvector 索引预留、文档处理、检索 API、Chat RAG 增强和 `/knowledge` 学习资料工作台；Phase 6 已完成 Agent Runtime 地基、RouterAgent 到 Chat 的轻量接入和 TutorAgent 策略层。
...
- RAG 边界：Phase 6.3 `KnowledgeVerifierAgent` 尚未实现；当前资料片段只作为回答参考，不作为绝对真理。
...
| Agent / Planned | LangGraph, MCP, BullMQ, OpenTelemetry, Sentry, Prometheus |
...
flowchart LR
  ...
  NextApi --> Agent[RouterAgent / TutorAgent policy]
...
- `/api/chat` 已接入 RouterAgent 和 TutorAgent policy：Agent 只决定路由、讲题策略和短 prompt，不直接调用模型、不写业务数据。
...
1. Phase 6.3：落地 `KnowledgeVerifierAgent`，在 RAG 命中后评估资料片段和回答初稿可信度。
2. Phase 6.4+：继续推进 `WrongQuestionOrganizerAgent`、`ReviewAgent / PlannerAgent` 和 `MemoryAgent`。
3. Phase 7：BullMQ 后台任务、事件总线和生产化工程增强。
