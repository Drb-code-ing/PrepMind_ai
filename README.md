...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V10 是唯一语义质量 authority；V11--V17 是不可复用的封存历史。V18 已完成 default-off 离线收口，保留 repository-root CWD、只读 Bun V10-authority bridge、attempt/recovery/receipt 边界，且只规范化一个 Bun 首位 separator；未运行 V18 Docker/headed-browser/API/provider，独立 eval gate 和两条业务 gate 仍默认关闭，产品仍是 deterministic，不能称为 Phase 6.9.5 完成。详见 [V17 closure / V18 plan](./docs/acceptance/phase-6-9-5-review-planner-v17-closure-v18-plan.md)。
...
1. Phase 6.9.5 的 V10 是唯一语义质量 authority，V11--V17 保持封存历史。V18 的产品 gate 仍默认关闭，阶段仍未完成。
2. V18 已完成离线门禁与独立 contract/operations review；只有用户已给出的明确授权仍有效且 preflight 全部通过时，才可执行唯一一次分支 Docker/headed-browser 产品验收。产品 gate 必须逐组件临时开启后恢复 `false`，并精确清理本轮合成数据；只有分支验收、main replay 和证据复核通过后，才可合并和 push。
...
下一会话可以复制：“请继续 Phase 6.9.5 V18：V10 是唯一语义质量 authority；V11--V17 都不可重跑。V18 仅修复 package wrapper 对一个 Bun separator 的转发，不放宽 strict confirmation，仍保持 repository-root CWD、bridge、receipt 与 recovery 边界。先做静态/Docker default-off 复验；只有明确授权和 preflight 通过后才运行唯一 V18 branch product，保留可见浏览器。”
