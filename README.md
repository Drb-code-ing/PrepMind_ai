...
| Phase 6.9.4.4 | Router/Verifier 混合生产接入、共享预算、Trace、Docker/Live/浏览器验收                  | 进行中 |
...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V10 是唯一语义质量 authority；V11--V19 是不可复用的封存历史。V20 已完成 default-off 离线收口，保留 repository-root CWD、只读 Bun V10-authority bridge、attempt/recovery/receipt 边界，并在产品命令前以同一 public execute/default ports/default host 的 `preflightOnly` 验证最后编排层；独立 eval gate 和两条业务 gate 仍默认关闭，产品仍是 deterministic，不能称为 Phase 6.9.5 完成。详见 [V19 closure / V20 plan](./docs/acceptance/phase-6-9-5-review-planner-v19-closure-v20-plan.md)。
...
- KnowledgeDedupAgent / KnowledgeOrganizerAgent：`GET /knowledge-agent/suggestions` 当前基于资料元数据和少量 chunk 摘要给出 deterministic 只读建议；Phase 6.9.6 必须增加 embedding + 真实模型语义判断。能力继续只读展示，不自动合并、删除、替换、重命名或分类资料。
- Agent 成本边界：生产 Agent 的真实模型调用必须同时经过 server-only composition、全局 Live 双开关、组件独立 gate、不可变预算、超时、零自动重试、安全 eligibility 与脱敏 Trace；组件 gate 默认关闭并可独立回滚。Mock/CLI 只证明工程 contract，controlled-Live 才能证明语义质量。
...
1. Phase 6.9.5 的 V10 是唯一语义质量 authority，V11--V18 保持封存历史。V19 的产品 gate 仍默认关闭，阶段仍未完成。
2. V19 先执行只读 Node preflight，确认 default host 在真实 runner 中 ready 且 roots 仍为空；只有其通过、用户明确授权仍有效且完整 preflight 全部通过时，才可执行唯一一次分支 Docker/headed-browser 产品验收。产品 gate 必须逐组件临时开启后恢复 `false`，并精确清理本轮合成数据；只有分支验收、main replay 和证据复核通过后，才可合并和 push。
...
下一会话可以复制：“请继续 Phase 6.9.5 V19：V10 是唯一语义质量 authority；V11--V18 都不可重跑。V19 保留 strict confirmation、repository-root CWD、bridge、receipt 与 recovery 边界，并在不创建 owner/ledger/Docker/browser/API/provider 的前提下先执行 Node read-only preflight。它 ready 后，才在明确授权下运行唯一 V19 branch product，保留可见浏览器。”
