...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V1--V8 保持只读历史；V9 Task 1--5 已在 `683a209` 前完成离线实现，但 controlled-Live 尚未运行，V9 evidence directory、once marker 与 success seal 均不存在。独立 V9 eval gate 与两条业务 gate 当前未设置、缺省关闭，不能称为真实模型可用或 Phase 6.9.5 已完成。
...
1. Phase 6.9.5 的 V1--V8 保持只读历史；V9 Task 1--5 已离线实现并把 product acceptance 绑定到 V9 committed success，但 V9 Live 尚未运行、没有 evidence/once/seal，产品 gate 默认关闭，阶段仍未完成。
2. 下一步只能先复核 V9 offline checkpoint 与单独 Live 授权条件。未获得明确授权前，不运行 V9 package script、Docker、浏览器、Trace、产品验收、main replay 或 push，也不提前进入后续 Agent 或记忆阶段。
3. 全部 Agent 完成后进入 Phase 6.10 分层记忆。未来分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客，具体题目与结构由用户届时确认。

回顾时可以问：“为什么 V9 offline tests 不能代表 Live quality？”“为什么 product authority 只接受 V9 committed success 且不回退 V8？”“Agent 之间如何通信，为什么模型不能决定权限和写操作？”

下一会话可以复制：“请复核 Phase 6.9.5 V9 offline checkpoint 与单独 Live 授权前提；保持 Review/Planner 产品 gate 关闭，不改写 V1--V8，不提前进入产品验收或记忆系统。”
