...
Phase 6.9.5 和 Phase 6.9.6 均已完成。Phase 6.9.7 Task 0--11 已完成，但 V1、V2 与 V3 三条唯一 controlled-Live 均以 `quality_gate_failed` 封存且不得重跑。V1 run `39a62241...` 为 `24/24` zero-call、`27/48` strict runtime、Tutor/Organizer semantic `0.3485119048/0.7`。V2 R0--R6 已完成独立 runner/CLI/validator/evidence lineage、held-out/metamorphic/authority/leakage、marker/evidence 并发故障恢复、Chat abort、Organizer 失败终态及同题跨路由 PostgreSQL 收敛；唯一 V2 R7 run `67ce18dd...` 保持 `24/24` zero-call，但最终 `0/48` strict runtime、semantic `0/0`、verified usage `0`。V3 R0--R4 完成安全 failure 投影、零网络 compatibility、strict-gate breaker、固定分母、双 lane ledger、crash-safe evidence 与 static/Mock checkpoint；唯一 V3 R5 run `ff2e1a54-0cbd-494c-96b7-a0f366c6c3dc` 在第 14 对的 Organizer `subject_authority_violation` 后熔断，最终 `24/24` zero-call、`27/48` strict runtime、Tutor/Organizer semantic `0.5280555556/0.4376201923`、`quality_gate_failed`。V4 R0--R5 已完成零 Provider bounded 复盘、独立 diagnostics、Tutor/Organizer 单一语义 policy、held-out/metamorphic/schema-negative robustness、独立 crash-safe evidence lineage 与 static/Mock checkpoint；Tutor 明确使用 `step > explain > concept > hint > general`，Organizer 统一 subject/deck/topic/evidence/confidence 决策矩阵，冻结 baseline 与 V1/V2/V3 artifacts 保持不变。Fresh V4 Mock 为 `24/24` zero-call、`48/48` strict runtime、semantic `1/1/1`，但 Mock 不是 Live authority。

V1/V2/V3 marker/evidence 均已封存，V3 journal 完整记录 breaker、run completed 与 evidence sealed；生产 gate 的 tracked defaults 继续关闭。V3 不得重跑；V4 R5 已验证独立 marker/journal/recovery/evidence 合同、默认关闭、历史不可变与零残留，但没有创建 Live artifact。当前停在 R6 新的精确一次性 V4 controlled-Live 授权门前；产品 Docker/API/浏览器、Task 13/main 合并与 Phase 6.10 均不得提前开始。
...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | V1/V2/V3 Live 失败封存；V4 R0--R5 完成，阶段未完成 |
...
V3 R5 已再次复核 V1/V2 四个历史 SHA 与专用 validator；V3 marker/journal/evidence 已按 run `ff2e1a54...` 封存。禁止删除 marker、重跑 V1/V2/V3 或把部分成功拼接成通过。V4 使用新的 runner/prompt/marker/journal/evidence/validator；R5 checkpoint 已完成，没有 R6 新的精确一次性授权不得调用 Provider。
