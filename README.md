...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。Phase 6.9.5 的受限模型候选已具备，但 v1--v6 Live profile 都已终态关闭；最新 V6 为 `usage_unverifiable`，两条业务 gate 继续默认 `false`，不能称为真实模型可用。
