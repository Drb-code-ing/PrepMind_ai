...
| Phase 6.9.7 | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                    | Task 0--11 完成；V1 失败封存；V2 R0--R5 完成，下一步 R6 |
...
2. 当前执行 Phase 6.9.7：Task 0--11 已完成；Task 12 唯一 V1 Live 已以 `quality_gate_failed` 封存。真实 run `39a62241...` 的 zero-call、安全、延迟、usage 与 `0.086418 CNY` 费用门通过，但 strict runtime 只有 `27/48`，Tutor/Organizer semantic 为 `0.3485119048/0.7`。因此没有进入真实 Router、HTTP、RAG、最终流式回答或产品 Docker service/API/浏览器验收，不能声称两个 Agent 已生产可用。V2 R0--R5 已完成：R4 的独立 robustness suite 覆盖 Tutor 中英文/混合语言/context 变形、Organizer subject/deck/batch ordinal authority、安全 fail-closed 与 prompt leakage；R5 新增独立 runner-v2、V2 CLI/validator、授权变量/确认词、marker/evidence prefix 和双向 lineage 校验，V1 入口与历史 SHA 不变。fresh V2 Mock run `d4fc9a3a-5825-47f2-a4d2-d0148c7ccaf4` 为 `24/24` zero-call、`48/48` strict runtime、Tutor/Organizer semantic `1/1`、P95 `246/328/328/276ms`、usage `21948/5647`、估算 `0.099726 CNY`；Mock 按 Live-only authority 保持 `quality_gate_failed`，临时 evidence 已精确删除，V2 Live marker/evidence 仍为 0。下一步执行 R6 分支静态/Mock checkpoint 与独立复审；R6 完成并取得新的精确授权前不调用 provider，V1 不重跑。
   R5 代码/合同/安全与历史不可变性两路独立复审均 `APPROVED`，无未关闭 Critical/Important；这只关闭 R5 离线工程范围，不改变 V1 失败终态或产品可用性结论。
...
下一会话可以复制：“请按 Phase 6.9.7 V2 remediation 计划执行 R6 分支静态/Mock checkpoint 与独立复审，并保持 dataset、质量门、V1 evidence 与 V2 零 Live 证据不变。”R6 完成前不读取 credential、不调用 provider、不启动产品 Docker/API/浏览器；任何 V2 controlled-Live 都需要 R6 clean commit 后的新精确授权。
