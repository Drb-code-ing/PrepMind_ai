...
Task 9C 后已完成独立 Architecture Recovery R0 设计：新 lineage 同时覆盖 DeepSeek rewrite、Qwen retrieval 与
DeepSeek FinalResponse stream，分离第一方 `providerWire` 和 runner `runnerWire`，diagnostic 只允许固定
stage/reason/type-count bucket 与 `rawDataRetained=false`，明确禁止 raw、unknown key 与 raw-derived hash。R0
未修改 TypeScript、读取 credential、调用 Provider 或创建正式 evidence，authority 仅
`zero_provider_retriever_final_response_architecture_recovery_design / qualityAuthority=none`。下一步仅 R1
