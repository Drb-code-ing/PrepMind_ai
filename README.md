...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。Phase 7 核心后台任务工程化已完成；Phase 7.8.5 RAG runtime parity 已完成真实 Docker 验收。当前先完成 Phase 6.9 全部真实模型 Agent 架构、通信、权限、可执行 LangGraph 与生产验收，再进入 Phase 6.10 分层记忆；随后进入 Phase 8 性能/PWA 和 Phase 9 MCP Tool 体系。Phase 7.23 的 production 导出与维护开关仍默认关闭。Phase 6.9.5 和 Phase 6.9.6 均已完成；KnowledgeDedup/Organizer 的唯一 V2 controlled-Live、R7 Docker/API、可见 `/knowledge` 分支验收及 main default-off 回放均已通过，失败 lineage 保持不可变。Phase 6.9.7 Task 0--11 已完成；Task 12 唯一 V1 Live run `39a62241-0f51-45be-a423-0d13b0b60ae4` 使用真实 `deepseek_network` 得到 `24/24` zero-call、`27/48` strict runtime，Tutor/Organizer semantic `0.3485119048/0.7000000000`，最终 `quality_gate_failed`。V1 marker/evidence 已封存且不得重跑；按固定质量门未进入 Docker service/API 或可见浏览器，生产 gate 的 tracked defaults 继续关闭。V2 R0--R6 已完成：Tutor/Organizer candidate 与 Web/Server config/Trace 使用 v2 prompt identity，独立 held-out/metamorphic/authority/leakage suite 已通过，runner-v2、独立 CLI/validator、确认词、授权变量、marker 和 evidence prefix 已与 V1 双向隔离；R6 又完成 marker/evidence 并发与故障恢复、Chat 最终流取消、Organizer command 失败终态、同题 normal/force 与 single/batch PostgreSQL 并发收敛。fresh V2 Mock 为 `24/24` zero-call、`48/48` strict runtime、semantic `1/1`，但按 Live-only authority 仍是 `quality_gate_failed`；当前 V2 Live marker/evidence 为 0。下一步停在 R7 新的 V2 branch controlled-Live 精确授权门前，而不是 Task 13/main 合并、产品验收或 Phase 6.10。
...
| Phase 6.9.7 | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                    | Task 0--11 完成；V1 失败封存；V2 R0--R6 完成，等待 R7 新授权 |
...
- `/api/chat` 已接入 RouterAgent、TutorAgent 与 KnowledgeVerifierAgent；Phase 6.9.4.4 的 Router/Verifier 混合路径已经完成生产验收并恢复默认 gate 关闭，Phase 6.9.7 Task 5 又完成 Tutor candidate 的 default-off Web composition 与安全 Trace。Task 12 V1 真实模型质量未达门槛，因此 Tutor/Organizer 产品 gate 仍关闭，未进行 Docker/API/浏览器可用性验收；V2 R1--R5 只收口离线 diagnostics、共享规则源、anti-overfit suite 与独立 runner/CLI/validator/evidence lineage，不改变生产 gate 或生产可用性结论。模型只负责受限语义判断，权限、安全、canonical route、RAG 放行、预算与写业务数据仍由本地代码掌握。
...
2. 当前执行 Phase 6.9.7：Task 0--11 已完成；Task 12 唯一 V1 Live 已以 `quality_gate_failed` 封存。真实 run `39a62241...` 的 zero-call、安全、延迟、usage 与 `0.086418 CNY` 费用门通过，但 strict runtime 只有 `27/48`，Tutor/Organizer semantic 为 `0.3485119048/0.7`，因此未进入产品 Docker/API/浏览器，不能声称两个 Agent 已生产可用。V2 R0--R6 已完成：R4 完成 independent robustness/authority/leakage，R5 建立独立 runner-v2/CLI/validator/one-shot lineage，R6 关闭 marker/evidence 并发与恢复、Chat 最终流 abort、Organizer failed Trace、同题 normal/force 与 single/batch PostgreSQL 收敛及未写题 batch 补偿边界。fresh V2 Mock run `593ee863-3743-4957-96e1-cb90e852a795` 为 `24/24` zero-call、`48/48` strict runtime、Tutor/Organizer semantic `1/1`、P95 `246/328/328/276ms`、usage `21948/5647`、估算 `0.099726 CNY`；Mock 按 Live-only authority 保持 `quality_gate_failed`，临时 evidence 已精确删除，V1 SHA 不变，V2 Live marker/evidence 仍为 0。下一步停在 R7 新 V2 branch controlled-Live 精确授权门前，V1 不重跑。
   R6 contract/security/concurrency/routing 与 operations/acceptance/history 两路最终复审均 `APPROVED`，无未关闭 Critical/Important；这只关闭 R6 零 Provider 工程范围，不改变 V1 失败终态或产品可用性结论。
...
下一会话先核对 R6 clean commit 与 V2 Live marker/evidence 仍为 0。继续 R7 必须由用户重新接受 DeepSeek 当前账号的数据保留/训练边界，并明确授权唯一一次 `Phase 6.9.7 Tutor/Organizer V2 branch controlled-Live`；否则不读取 credential、不调用 provider、不启动产品 Docker/API/浏览器。
