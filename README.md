...
这个仓库不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已经完成前端 MVP、后端 Auth、业务数据服务端迁移的主体链路，正在收尾 Phase 2.3。
...
| Phase 2.3 | WrongQuestion、ChatMessage、OCRRecord、图片上传链路迁移 | 进行中 |
...
- 本地缓存：Dexie 作为离线缓存与快速恢复层，不再作为登录态权威来源。
...
- 本地优先体验：Dexie 保留快速恢复、离线兜底和历史图片预览。
