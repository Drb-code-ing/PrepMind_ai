...
项目不是一次性 Demo，而是按 Phase 0 到 Phase 10 逐步推进的 AI 应用工程项目。当前已完成前端 MVP、后端鉴权、业务数据服务端迁移、结构化 OCR、AI 讲题上下文、FSRS 复习闭环、学习统计、ReviewTask 持久化任务流、离线评分队列和复习计划预览。下一步继续推进 Phase 4.5：复习提醒与长期计划策略。
...
| Phase 4.5.2 | 复习提醒策略与更长期计划设置 | 下一步 |
...
- 复习计划：`/plan` 基于只读 `/review-tasks/plan` 展示未来 7 天复习压力，不提前创建未来 ReviewTask。
...
- `/review-tasks/plan` 只读预览 `Card.nextReview`，今日任务仍由 `/review-tasks/today` 负责生成与执行。
