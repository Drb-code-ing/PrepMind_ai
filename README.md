...
| Phase 6.9.6 | KnowledgeDedup/Organizer embedding shortlist + 真实模型语义路径                       | 工程接入完成，待生产验收 |
...
- KnowledgeDedupAgent / KnowledgeOrganizerAgent：`GET /knowledge-agent/suggestions` 已接入 owner-scoped Qwen Chunk embedding shortlist、受限 DeepSeek V4 Pro candidate、本地权威 merger 与 local/hybrid/degraded 来源状态。exact hash、越权、不安全投影、abort 和预算失败保持 provider 前零调用；模型只裁决本地 ordinal，不接触真实 document ID，也不自动合并、删除、替换、重命名或分类资料。当前 API-only Docker 配置与 strict Mock paired runner 已完成，但尚未调用真实 provider 或完成 Docker/浏览器生产验收，详见 [Phase 6.9.6 设计](./docs/superpowers/specs/2026-07-21-phase-6-9-6-knowledge-agents-design.md)。
...
2. Phase 6.9.6 已完成 Task 1--11 的候选、shortlist、API/UI、Mock paired eval 和 API-only Docker 运维接线；下一步执行 Task 12 分支静态/Mock 验收并形成 evidence checkpoint，随后必须重新取得 controlled-Live 授权，才能进入真实模型、Docker/浏览器、精确清理、main replay 与推送。
...
下一会话可以复制：“请按 Phase 6.9.6 实施计划执行 Task 12 分支静态/Mock 验收与 evidence checkpoint；不要调用真实模型、启动 Docker/浏览器或提前进入 Task 13，提交后停下向我申请新的 controlled-Live 授权。”
