...
| Phase 6.9.6 | KnowledgeDedup/Organizer embedding shortlist + 真实模型语义路径                       | 设计中 |
...
- KnowledgeDedupAgent / KnowledgeOrganizerAgent：`GET /knowledge-agent/suggestions` 当前基于资料元数据和少量 chunk 摘要给出 deterministic 只读建议。Phase 6.9.6 已进入设计检查点，目标复用 owner-scoped Qwen Chunk embedding 形成候选，再由 DeepSeek V4 Pro 做受限语义裁决；exact hash 保持零调用，最终能力仍只读展示，不自动合并、删除、替换、重命名或分类资料。当前尚未实现或运行模型路径，详见 [Phase 6.9.6 设计](./docs/superpowers/specs/2026-07-21-phase-6-9-6-knowledge-agents-design.md)。
...
2. Phase 6.9.6 设计检查点已写入：先审阅 KnowledgeDedupAgent / KnowledgeOrganizerAgent 的 embedding shortlist、72-case 量化门槛、只读权限和成本预算，再按独立检查点实现、评测并完成 Docker/浏览器验收与清理。
...
下一会话可以复制：“请审阅 Phase 6.9.6 Knowledge Agent 设计，重点检查 embedding shortlist、72-case 门槛、只读权限和成本预算；不要改写 V10/V22 历史。”
