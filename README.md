...
- ReviewAgent / PlannerAgent：`@repo/agent/review` 和 `@repo/agent/planner` 的确定性只读 baseline 仍是当前产品行为；`GET /review-agent/suggestions` 基于当前用户复习事实生成建议，`/plan` 展示完整建议，`/today` 展示紧凑建议，不创建未来 ReviewTask，不写 Card / ReviewLog / ReviewPreference / WrongQuestion / deck。V1--V9 保持只读历史；V9 唯一 controlled-Live 已以 `quality_gate_failed` 封存。V10 将模型输出收窄为 `focusIndexes` / `blockOrder` 后，唯一 Live 已通过 `23/22`、`48/48` strict/quality、critical `0`、P95 `1465ms`、usage `5764/232` 和 CNY `0.018684/1.00`，其 evidence/success seal 已封存。独立 eval gate 和两条业务 gate 仍默认关闭，产品仍是 deterministic；分支 Docker/headed-browser 验收尚未开始，不能称为 Phase 6.9.5 完成。详见 [V10 controlled-Live outcome](./docs/acceptance/phase-6-9-5-review-planner-v10-offline-checkpoint.md)。
...
1. Phase 6.9.5 的 V1--V9 保持只读历史；V9 唯一 Live 已质量门失败并封存。V10 的唯一 controlled-Live 已成功封存，产品 gate 仍默认关闭，阶段仍未完成。
2. 下一步是分支 Docker/headed-browser 验收：产品 gate 逐组件临时开启后必须恢复 `false`，并精确清理本轮合成数据。只有分支验收、main replay 和证据复核通过后，才可合并和 push。
