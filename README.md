...
- 滚动会话摘要：Phase 6.9.3.3 在 prepare 中实现 12 条/70% 触发、完整 assistant 水位、输入凭据脱敏、输出凭据拒绝、事务外模型调用与事务内 source hash/CAS。PostgreSQL 继续是摘要权威源；Mock/Live 共用 strict schema 与单次预算，Docker 默认 Mock。当前摘要尚未注入 `/api/chat`，Web context assembler 留给 6.9.3.4。
...
2. Phase 6.9.3.3 已完成 rolling summary/CAS；下一任务是 Phase 6.9.3.4 Web prepare、分层 assembler 与 Dexie 恢复。Phase 6.9 完成后进入 Phase 8 性能/PWA，
