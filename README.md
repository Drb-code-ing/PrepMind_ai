...
Phase 6.9.5 和 Phase 6.9.6 均已完成。Phase 6.9.7 Task 0--11 已完成，但 V1、V2、V3 与唯一 V4 R6 controlled-Live 均以 `quality_gate_failed` 独立封存且不得重跑。唯一 V4 run `0fb47591-5ff4-4e46-bcf3-2cd267d1fb2f` 保持 `24/24` guard zero-call，在第 6 对 Tutor `dynamic_contract / invalid_evidence_association` 后熔断；Organizer sibling abort，最终 `10/48` strict runtime、后续 36 runtime 未启动。V5 R0 零 Provider 根因取证已确认：冻结 V1 dataset 存在跨题/跨语言 active context 与 language tag 错位，但产品 candidate 对非法 evidence 的拒绝并非 V4 adapter 凭空误判；中文 Tutor 及 Organizer topic/subject 仍有真实语义偏差。V5 R1--R4 已完成独立 V2 dataset/policy/baseline、两条 bounded candidate，以及原生 V5 runner/lineage/生产极端边界。V5 R5 又完成 reviewed Mock factory、fresh baseline/Mock、受影响静态门、Organizer PostgreSQL 并发 E2E、Compose default-off、历史不可变性与两路终审。V5 R0--R5 均未接产品或调用 Provider。

V1--V4 marker/evidence 均已封存；V3/V4 journal 完整记录 dispatch、terminal、breaker、run completed 与 evidence sealed。V1--V4 evidence SHA 与四版 validator 保持不变。V5 Mock 为 `24/24` zero-call、`48/48` strict runtime、semantic `1/1/1`，但合成调用不是真实 Provider evidence；仓库内 V5 Live artifact 仍为 0。当前下一原子任务仅 R6；必须重新取得 DeepSeek 数据边界确认与唯一一次 V5 branch controlled-Live 精确授权。产品 Docker/API/浏览器、Task 13/main 与 Phase 6.10 均不得开始。
...
| Phase 6.9.7   | Tutor/WrongQuestionOrganizer 混合模型、教学策略与组织层写入隔离                                | V5 R5 static/Mock 已完成；阶段未完成 |
...
- Agent 成本边界：生产 Agent 的真实模型调用必须同时经过 server-only composition、全局 Live 双开关、组件独立 gate、不可变预算、超时、零自动重试、安全 eligibility 与脱敏 Trace；组件 gate 默认关闭并可独立回滚。Knowledge 两个 candidate 共享 `2 calls / 6000 input / 1200 output`，单请求硬 cap 为 `0.03 CNY`，并使用只注入 API server 的独立 `KNOWLEDGE_AGENT_DEEPSEEK_API_KEY`。Tutor 使用独立 `1/1200/300`、`0.006 CNY` 与仅注入 Web server runtime 的 `TUTOR_AGENT_DEEPSEEK_API_KEY`；WrongQuestionOrganizer 使用独立 `1/3500/800`、`0.016 CNY` 与只由 API server 读取的 `WRONG_QUESTION_ORGANIZER_AGENT_DEEPSEEK_API_KEY`，worker 强制关闭。它们不借用通用或其它 Agent 凭据，也不消耗 Router -> Verifier 共享预算。V1/V2/V3/V4 使用独立确认词、授权变量、marker、evidence prefix、runner identity 与 validator；V3/V4 还分别拥有 dispatch-before-call hash-chain journal、recovery claim、crash-only seal 与 hard-link evidence。V4 R6 唯一 Live 已以 `quality_gate_failed` sealed；12 个 executor 中 11 个 usage verified，部分费用为 `0.032247 CNY`，但一个 sibling abort 使完整 pricing/total cost 保持 unknown。R5 Mock 和 R6 的部分成功均不能被拼接为质量通过。`tutorOrchestrationP95Ms` 只测本地 Tutor strategy + candidate，不是 Router/API/最终流式 Chat 产品 P95。Mock/CLI 只证明工程 contract，controlled-Live 才能证明语义质量，Docker/API/可见浏览器才证明产品链路。R5 checkpoint 见 [V4 R5 acceptance](./docs/acceptance/2026-07-26-phase-6-9-7-tutor-organizer-v4-r5-static-mock.md)，最终结果见 [V4 R6 failure seal](./docs/acceptance/2026-07-26-phase-6-9-7-tutor-organizer-v4-controlled-live-failure.md)。
...
- `/api/chat` 已接入 RouterAgent、TutorAgent 与 KnowledgeVerifierAgent；Phase 6.9.4.4 的 Router/Verifier 混合路径已经完成生产验收并恢复默认 gate 关闭，Phase 6.9.7 Task 5 又完成 Tutor candidate 的 default-off Web composition 与安全 Trace。V1--V4 四条真实模型质量运行均未达门槛，因此 Tutor/Organizer 产品 gate 仍关闭，未进行 Docker/API/浏览器可用性验收。V5 R0--R5 只完成 zero-provider 根因、独立数据/评测权威、baseline、两条 bounded candidate、原生 V5 runner/lineage/生产极端边界与 static/Mock checkpoint；V5 candidate 尚未接入既有 Web/NestJS composition，R5 Mock 也不构成真实模型或产品验收。模型只负责受限语义判断，权限、安全、canonical route、RAG 放行、预算与写业务数据仍由本地代码掌握。
...
2. 当前执行 Phase 6.9.7：V1--V4 Live 均已分别以 `quality_gate_failed` 封存且不得重跑；V5 R0--R5 已完成零 Provider 根因、独立 V2 dataset/policy/baseline、两条 bounded candidate、原生 V5 runner/lineage/生产极端边界与 static/Mock checkpoint。fixture 缺陷不推翻 V4 failure，V4 前 5 对的中文 Tutor 与 Organizer 语义偏差仍是真实 evidence。
3. 下一任务仅 V5 R6 controlled-Live 授权门：必须重新接受当前 DeepSeek 数据保留/训练边界，并明确授权唯一一次 V5 branch controlled-Live。此前不得读取 component credential 或调用 Provider；R6 全门通过前，产品验收、Task 13/main 与 Phase 6.9.8 暂不得开始。全部 Agent 完成后才进入 Phase 6.10 分层记忆，并分别编写《多 Agent 架构》和《记忆系统》两篇面试学习博客。
...
V4 R6 evidence/journal/marker 已按 run `0fb47591...` 封存；V1--V3 七个历史 SHA 与三版 validator 仍不变。禁止删除任何 marker、重跑 V1--V4 或把部分成功拼接成通过。
