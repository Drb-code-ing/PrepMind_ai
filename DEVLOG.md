...
> 2026-08-13 - Phase 6.9.8 SR5 v7 live-contract recovery started
>
> The v5 tag was found to contain only documentation changes while the active Live CLI still imported the old v2 tag and unversioned authorization constants. The v6 tag then exposed a PowerShell tag-message formatting defect. The v7 branch now binds the active contract to `main`, a new immutable v7 tag, and V7-bound confirmations. Focused zero-provider tests pass; no `.env`, credential, Provider, Docker, API, browser, Trace, BackgroundJob, Outbox, or business data was touched. v5/v6 tags and sealed predecessor evidence remain immutable. Next gates are feature commit, `--no-ff` merge and push, merged-main zero-provider replay, v7 annotated tag parity, then fresh V7 authorization before the one controlled-Live.
