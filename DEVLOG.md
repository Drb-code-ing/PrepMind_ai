...
> 2026-08-13 - Phase 6.9.8 SR5 v8 final parity started
>
> The v5 tag was documentation-only, v6 had stale verifier binding, and v7 had a stale manifest in its tag message. The v8 branch finalizes active runtime constants and will compute the tag message only after the merged-main source bundle and manifest are fixed. This remains zero-provider; v5-v7 tags and sealed predecessor evidence remain immutable. Next gates are feature commit, `--no-ff` merge and push, merged-main zero-provider replay, v8 annotated tag parity, then fresh V8 authorization before the one controlled-Live.
