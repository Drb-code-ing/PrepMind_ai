...
> 2026-08-13 - Phase 6.9.8 SR5 v6 live-contract recovery started
>
> The v5 tag was found to contain only documentation changes while the active Live CLI still imported the old v2 tag and unversioned authorization constants. A new ordinary git branch updates the active contract to `main`, a new immutable v6 tag, and V6-bound confirmations. Focused zero-provider tests currently pass `72/72` (`175 expect()`); no `.env`, credential, Provider, Docker, API, browser, Trace, BackgroundJob, Outbox, or business data was touched. The v5 tag and all sealed predecessor evidence remain immutable. Next gates are feature commit, `--no-ff` merge and push, merged-main zero-provider replay, v6 annotated tag parity, then fresh V6 authorization before the one controlled-Live.
