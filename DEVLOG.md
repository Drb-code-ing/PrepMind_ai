...
>
> 按 Task 12 固定顺序，只有 `quality_gate_passed` 才能进入产品验收，因此本轮没有启动/重建 Docker service、调用产品 API、打开浏览器或创建 synthetic 业务数据；也没有修改根 `.env`、删除容器/镜像/卷或清空任何 Docker 数据。V1 marker/evidence 不得重跑、覆盖、删除或拼接。Phase 6.9.7 尚未完成；下一步先做零网络 V2 remediation 设计，使用新 identity/marker/evidence 并另行取得明确授权，而不是直接进入 Task 13/main 合并。权威记录见 `docs/acceptance/phase-6-9-7-tutor-wrong-question-controlled-live.md`。
